...
const { ensureTraceContextInHeaders, withTraceContext } = require('./otelContextPropagation');
...
class S3Server {
    ...
    routeRequest(req, res) {
        ...
        // Enhanced trace context capture using OTEL utilities
        const otel = require('./otel');
        const traceHeaders = otel.setCurrentTraceHeaders(req.headers);
        
        if (Object.keys(traceHeaders).length > 0) {
            console.log('[TRACE] 📨 Incoming HTTP request with trace context:', {
                method: req.method,
                url: req.url,
                traceHeaders,
            ...
            console.log('[TRACE] 📭 No trace context in incoming request:', {
                method: req.method,
                url: req.url
            });
        }

        ensureTraceContextInHeaders(req);
        ...
        // Use the trace context wrapper for the request
        const wrappedRoutes = withTraceContext((req, res, params, logger, config) => {
            arsenal.s3routes.routes(req, res, params, logger, config);
        });
        
        wrappedRoutes(req, res, params, logger, this.config);
    }

...
class ServerAccessLogger {
    constructor(filename, highWaterMarkBytes, retryReopenDelayMS, checkFileRotationIntervalMS) {
        ...
        setInterval(() => {
            this._checkFileRotated();
        }, checkFileRotationIntervalMS);
...
function calculateTurnAroundTime(startTurnAroundTime, endTurnAroundTime) {
    if (
        startTurnAroundTime === undefined ||
        startTurnAroundTime === null ||
        endTurnAroundTime === undefined ||
        endTurnAroundTime === null
    ) {
        return null;
    }
...
function buildLogEntry(req, params, options) {
    ...
    return {
        ...
        loggingEnabled:
            params.backbeat && !params.expiration && !params.replication ? false : (params.enabled ?? undefined),
...
function logServerAccess(req, res) {
    ...
    const logEntry = buildLogEntry(req, params, {
        ...
        bodyLength:
            req.headers['content-length'] !== undefined ? parseInt(req.headers['content-length'], 10) : undefined,
    ...
    if (params.replication) {
        ...
        logEntry.requestURI = `${method} /${params.bucketName}/${encodedKey}${query} HTTP/${req.httpVersion ?? '1.1'}`;
    }

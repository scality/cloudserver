...
function validateQueryAndHeaders(request, log) {
    ...
    if (isBucketQuery) {
        const unsupportedQuery = _validateKeys(constants.unsupportedBucketQueries, reqQuery);
        if (unsupportedQuery) {
    ...
    const unsupportedQuery = _validateKeys(constants.unsupportedQueries, reqQuery);
    if (unsupportedQuery) {
        log.debug('encountered unsupported query', {
            query: unsupportedQuery,
            method: 'validateQueryAndHeaders',
    ...
    const unsupportedHeader = _validateKeys(constants.unsupportedHeaders, reqHeaders);

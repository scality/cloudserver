...
export default function retrieveData(locations, retrieveDataFn, response, log) {
    ...
    eachSeries(locations,
        (current, next) => retrieveDataFn(current, log, (err, readable) => {
            ...
            const passThrough = new PassThrough({ highWaterMark: 0 });
            // readable stream successfully consumed and passed
            // through passThrough to response
            passThrough.on('end', () => {
            ...
            return readable.pipe(passThrough)

    ...
    return backbeatMetrics.getAllMetrics(details, (err, res) => {
        ...
        const stats = {
            completions: {
                ...
                size: Math.round(parseFloat(completions.results.size) * 1000),
            },
            backlog: {
                count: backlog.results.count,
                size: Math.round(parseFloat(backlog.results.size) * 1000),
            },
            throughput: {
                count: parseFloat(throughput.results.count) * 1000,
                size: Math.round(parseFloat(throughput.results.size) * 1000),
            },
        };

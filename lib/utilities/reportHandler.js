...
function getIngestionStates(log, cb, _testConfig) {
    ...
    const { host, port } = _testConfig || config.backbeat;
    const endpoint = `http://${host}:${port}`;
    _makeRequest(endpoint, REQ_PATHS.ingestionStatus, (err, res) => {
        if (err) {
            if (err === 'responseError') {
                log.error('error response from backbeat aoi', {
                    error: res,
                    method: 'getIngestionStates',
                });
            } else {
...
function reportHandler(clientIP, req, res, log) {
    ...
    async.parallel({
        ...
        getIngestionStates: cb => getIngestionStates(log, cb),
    },
    (err, results) => {
        if (err) {
            ...
            const response = {
                ...
                ingestStatus: results.getIngestionStates,

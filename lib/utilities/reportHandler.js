...
const { errors, ipCheck, jsutil } = require('arsenal');
...
const _makeRequest = (endpoint, path, cb) => {
    ...
    const params = url.parse(endpoint);
    params.method = 'GET';
    params.path = path;
        ...
        res.on('end', () => {
            if (rawData !== '')  {
                try {
                    ...
                    return onceCB(null, parsed);
                } catch (err) {
                    // invalid json response
                    return onceCB('responseErr', err);
                }
            }
            return onceCB(null, {});
        });

...
const { errors, ipCheck } = require('@scality/arsenal');
...
function isAuthorized(clientIP, req) {
    return (
        ipCheck.ipMatchCidrList(config.healthChecks.allowFrom, clientIP) &&
...
async function _makeRequest(endpoint, path, log, cb) {
...
function _crrMetricRequest(endpoint, site, log, cb) {
    ...
    return _makeRequest(endpoint, path, log, (err, res) => {
        if (err) {

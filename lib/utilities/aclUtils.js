...
/**
 * createEl - Creates an XML element
 * @param {string} startTag - XML start tag
 * @param {object|string} content - Content to be inserted in a tag
 * @param {string} endTag - XML end tag
 * @return {undefined}
 */
aclUtils.createEl = (startTag, content, endTag) => {
    if (typeof content === 'object') {
        xml.push(startTag);
        xml.push(aclUtils.createXmlDom(content));
        xml.push(endTag);
    } else {
        // Escape any special characters in string content.
        xml.push(startTag + escapeForXML(content) + endTag);
    }
};

/**
 * createXmlDom - Calls `aclUtils.createEl()` for each object and generates
 * attributes for the `<Grantee>` tag
 * @param {object} obj - Object with a key-value pair to be converted to XML, or
 * an array of objects
 * @return {undefined}
 */
aclUtils.createXmlDom = obj => {
    if (!Array.isArray(obj)) {
        const key = Object.keys(obj)[0];
        aclUtils.createEl(`<${key}>`, obj[key], `</${key}>`);
    } else {
        obj.forEach(item => {
            const key = Object.keys(item)[0];

            if (key === 'Grantee') {
                // Handle the attribute array for the <Grantee> tag
                const attrArr = item[key][0][Object.keys(item[key][0])];
                const attr = [];

                Object.keys(attrArr).forEach(item => {
                    attr.push(` ${item}="${attrArr[item]}"`);
                });

                aclUtils.createEl(`<${key + attr.join('')}>`, item[key],
                                  `</${key}>`);
            } else if (key !== '_attr') {
                aclUtils.createEl(`<${key}>`, item[key], `</${key}>`);
            }
        });
    }
};
...
/**
 * convertToXml - Calls `aclUtils.createXmlDom()`, passing the obj to convert
 * @param {object} obj - Object with a key-value pair to be converted to XML, or
 * an array of objects
 * @return {string} xml.join('') - The XML DOM
 */
aclUtils.convertToXml = obj => {
    xml = [];

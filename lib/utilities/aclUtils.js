 ...
 */
aclUtils.handleCannedGrant = function handleCannedGrant(grantType, ownerGrant, separateBucketOwner) {
    const cannedGrants = [];
    const actions = {
        private: () => {
        ...
        'public-read-write': () => {
            ...
            cannedGrants.push(ownerGrant, publicReadGrant, publicWriteGrant);
        },
        'authenticated-read': () => {
            const authGrant = {
        ...
        'log-delivery-write': () => {
            ...
            cannedGrants.push(ownerGrant, logWriteGrant, logReadACPGrant);
        },
    ...
    return parseString(toBeParsed, (err, result) => {
        ...
        if (
            ...
            (result.AccessControlPolicy.AccessControlList[0] !== '' &&
                Object.keys(result.AccessControlPolicy.AccessControlList[0]).some(listKey => listKey !== 'Grant'))
        ) {
            log.debug('invalid acl', { acl: result });
            return next(errors.MalformedACLError);
        }
        const jsonGrants = result.AccessControlPolicy.AccessControlList[0].Grant;
        log.trace('acl grants', { aclGrants: jsonGrants });

        if (
            !Array.isArray(result.AccessControlPolicy.Owner) ||
...
aclUtils.getPermissionType = function getPermissionType(identifier, resourceACL, resourceType) {
...
aclUtils.reconstructUsersIdentifiedByEmail = function reconstruct(userInfofromVault, userGrantInfo) {
    return userGrantInfo.map(item => {
        const userEmail = item.identifier.toLowerCase();
        const user = {};
        // Find the full user grant info based on email
        const userId = userInfofromVault.find(elem => elem.email.toLowerCase() === userEmail);
...
aclUtils.sortHeaderGrants = function sortHeaderGrants(allGrantHeaders, addACLParams) {
...
aclUtils.convertToXml = grantInfo => {
    ...
    xml.push(
        ...
        '<Owner>',
        `<ID>${ownerInfo.ID}</ID>`,
        `<DisplayName>${escapeForXml(ownerInfo.displayName)}` + '</DisplayName>',
        '</Owner>',
        '<AccessControlList>',
    );
    ...
    grants.forEach(grant => {
        ...
        if (grant.displayName) {
            xml.push(`<DisplayName>${escapeForXml(grant.displayName)}` + '</DisplayName>');
        }
...
aclUtils.getCanonicalIDs = function getCanonicalIDs(acl) {
    const aclGrantees = [].concat(acl.FULL_CONTROL, acl.WRITE, acl.WRITE_ACP, acl.READ, acl.READ_ACP);
...
aclUtils.getUriGrantInfo = function getUriGrantInfo(acl) {
    ...
    const uriGrantInfo = [];
    const validGrants = Object.entries(grants).filter(
        ([permission, grantees]) => permission && Array.isArray(grantees),
    );
    validGrants.forEach(([permission, grantees]) => {
        grantees
            .filter(grantee => grantsByURI.includes(grantee))
            .forEach(grantee => {
                uriGrantInfo.push({
                    URI: grantee,
                    permission,
 ...
 * canonicalID/email
 */
aclUtils.getIndividualGrants = function getIndividualGrants(acl, canonicalIDs, emails) {
    const grants = getGrants(acl);
    ...
    validGrants.forEach(([permission, grantees]) => {
        grantees
            .filter(grantee => canonicalIDs.includes(grantee) && emails[grantee])
            .forEach(grantee => {

...
function collectResponseHeaders(objectMD, corsHeaders, versioningCfg,
    ...
    if (objectMD.archive &&
        objectMD.archive.restoreRequestedAt &&
        !objectMD.archive.restoreCompletedAt) {
        // Avoid race condition by relying on the `archive` MD of the object
        // and return the right header after a RESTORE request.
        responseMetaHeaders['x-amz-restore'] = {
            'ongoing-request': true,
        };
    } else if (objectMD['x-amz-restore']) {
        responseMetaHeaders['x-amz-restore'] = {
            'ongoing-request': objectMD['x-amz-restore']['ongoing-request'],
            'expiry-date': new Date(objectMD['x-amz-restore']['expiry-date']).toUTCString(),
        };
    }

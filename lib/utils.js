...
utils.getBucketNameFromHost = function(request) {
  var host, hostname, endpoint, region, i, hostArr;

  host = request.headers.host;
  for(i = 0, ii = this.awsRegions.length; i < ii; i++) {
    endpoint = this.awsRegions[i].endpoint;
    websiteEndpoint = 's3-website-' + this.awsRegions[i].region + '.amazonaws.com';

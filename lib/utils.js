...
//NEED TO COMPLETE THIS FUNCTION IN ORDER TO CHECK PUT AND TO STORE VALUE IN METADATA
...
utils.getMetaHeaders = function(headers) {
  ...
  for(let k in request.headers){
    if(i.substr(0, 11) === 'x-amz-meta-'){
      metastore.key.k = request.headers.k;
    }
  }
  return metaHeaders;
}
...
utils.getResourceUID = function(namespace, resource) {
  let hmacObject = crypto.createHmac(algorithm, resource);
  let hashedID = hmacObject.update(namespace + resource).digest('hex');
  return hashedID;
}

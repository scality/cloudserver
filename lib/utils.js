...
utils.encodeObjectKey = function encodeObjectKey(key) {
    ...
    for (let i = 0; i < str.length; i++) {
        // padding the hexa of each character to make its representation 8 bytes
        encoded += `000000${str.charCodeAt(i).toString(16)}`.slice(-WIDTH);
    }   
    return encoded;
};
...
utils.decodeObjectKey = function decodeObjectKey(hex) {
    ...
    for (let i = 0; i < length; i += WIDTH) {
        decoded +=
            String.fromCharCode(Number.parseInt(hex.slice(i, i + WIDTH), 16));
    }   
    return decoded;
};

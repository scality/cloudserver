...
utils.getResourceNames = function getResourceNames(request) {
    return this.getNamesFromPath(request, this.getBucketNameFromHost(request));
};
    ...
    function reconstruct(userInfofromVault, userGrantInfo) {
        return userInfofromVault.map((item) => {
            ...
            const user = userGrantInfo
                .find(elem => elem.identifier === userEmail);
...
utils.getPermissionType = function getPermissionType(identifier, bucketACL) {
    const fullControlIndex = bucketACL.FULL_CONTROL.indexOf(identifier);
    const writeIndex = bucketACL.WRITE.indexOf(identifier);
    const writeACPIndex = bucketACL.WRITE_ACP.indexOf(identifier);
    const readACPIndex = bucketACL.READ_ACP.indexOf(identifier);
    const readIndex = bucketACL.READ.indexOf(identifier);
    let permission = '';
    if (fullControlIndex > -1) {
        permission = 'FULL_CONTROL';
        bucketACL.FULL_CONTROL.splice(fullControlIndex, 1);
    } else if (writeIndex > -1) {
        permission = 'WRITE';
        bucketACL.WRITE.splice(writeIndex, 1);
    } else if (writeACPIndex > -1) {
        permission = 'WRITE_ACP';
        bucketACL.WRITE_ACP.splice(writeACPIndex, 1);
    } else if (readACPIndex > -1) {
        permission = 'READ_ACP';
        bucketACL.READ_ACP.splice(readACPIndex, 1);
    } else if (readIndex > -1) {
        permission = 'READ';
        bucketACL.READ.splice(readIndex, 1);
    }
    return permission;
};

...
export default function routes(req, res, logger) {
    ...
    const isReadableHeaders = utils.checkReadableHeaders(req.headers);
    if (!isReadableHeaders) {
        log.debug('header contains unreadable character(s)',
        { method: 'routes' });
        return routesUtils.responseXMLBody(
            errors.AccessDenied, undefined, res, log);
    }

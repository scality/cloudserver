...
const { healthcheckHandler } = require('./utilities/healthcheckHandler');
...
function checkUnsuportedRoutes(req, res, log) {
    if (utils.isUnsupportedQuery(req.query)) {
        log.trace('encountered unsupported query');
...
function routes(req, res, logger) {
    const clientInfo = {

...
const { CONFIG_OVERLAY_MESSAGE, METRICS_REQUEST_MESSAGE, CHANNEL_CLOSE_MESSAGE, CHANNEL_PAYLOAD_MESSAGE } = MessageType;
...
function startWSManagementClient(url, token, cb) {
    ...
    function receiveChannelData(channelId, payload) {
        ...
        if (!socket) {
            ...
            socket.on('data', data => {
                ws.send(ChannelMessageV0.encodeChannelDataMessage(channelId, data), err =>
                    _logError(err, 'failed to send channel data message', 'receiveChannelData')
                );
            });
            ...
            socket.on('end', () => {
                ...
                ws.send(ChannelMessageV0.encodeChannelCloseMessage(channelId), err =>
                    _logError(err, 'failed to send channel close message', 'receiveChannelData')
                );
            });
    ...
    ws.on('close', () => {
        ...
        _config.removeListener('browser-access-enabled-change', browserAccessChangeHandler);
        ...
        switch (message.getType()) {
            ...
            case CHANNEL_PAYLOAD_MESSAGE:
                ...
                if (_config.browserAccessEnabled !== false) {
                    receiveChannelData(message.getChannelNumber(), message.getPayload());
                }
                break;
            default:
                logger.error('unknown message type from push server', { messageType: message.getType() });
        }
    });
}

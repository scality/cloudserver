...
function startWSManagementClient(pushEndpoint, instanceId, token) {
    ...
    function sendPing() {
        if (ws.readyState === ws.OPEN) {
            ws.ping(err => _logError(err, 'error sending a ping'));
        }
    ...
    function pushStats(options) {
        ...
        request(fromURL, fromOptions, (err, response, body) => {
            if (err) {
                _logError(err, 'error getting metrics report');
                return;
            }
            ws.send(ChannelMessageV0.encodeMetricsReportMessage(body),
                err => _logError(err, 'error sending metrics report message'));
        }).json();
    }
        ...
        if (!socket) {
            ...
            socket.on('data', data => {
                ws.send(ChannelMessageV0.
                    ...
                    _logError(err, 'error sending channel data message'));
            });
            ...
            socket.on('end', () => {
                ...
                ws.send(ChannelMessageV0.encodeChannelCloseMessage(channelId),
                    err => _logError(err,
                      'error sending channel close message'));
            });
    ...
    ws.on('ping', () => {
        ws.pong(err => _logError(err, 'error sending a pong'));
    });

...
function remoteOverlayIsNewer(cachedOverlay, remoteOverlay) {
    return (
        overlayHasVersion(remoteOverlay) &&
        (!overlayHasVersion(cachedOverlay) || remoteOverlay.version > cachedOverlay.version)
    );
}
...
function patchConfiguration(newConf, log, cb) {
    ...
    if (_config.overlayVersion !== undefined && newConf.version <= _config.overlayVersion) {
        log.debug('configuration version already applied', { configurationVersion: newConf.version });
        return process.nextTick(cb, null, newConf);
    }
    return getStoredCredentials(log, (err, creds) => {
        ...
        if (Object.keys(locations).length !== 0) {
            try {
                ...
                log.error('could not apply configuration version location ' + 'constraints', {
            ...
            try {
                const locationsWithReplicationBackend = Object.keys(locations)
                    ...
                    .filter(key => replicationBackends[locations[key].type])
                    .reduce((obj, key) => {
                        /* eslint no-param-reassign:0 */
                        obj[key] = locations[key];
                        return obj;
                    }, {});
                _config.setReplicationEndpoints(locationsWithReplicationBackend);
            } catch (error) {
                const exceptionError = reshapeExceptionError(error);
        ...
        if (newConf.browserAccess) {
            if (Boolean(_config.browserAccessEnabled) !== Boolean(newConf.browserAccess.enabled)) {
                _config.browserAccessEnabled = Boolean(newConf.browserAccess.enabled);
        ...
        log.info('applied configuration version', { configurationVersion: _config.overlayVersion });
...
function saveConfigurationVersion(cachedOverlay, remoteOverlay, log, cb) {
    if (remoteOverlayIsNewer(cachedOverlay, remoteOverlay)) {
        ...
        metadata.putObjectMD(managementDatabaseName, objName, remoteOverlay, {}, log, error => {
            if (error) {
                const exceptionError = reshapeExceptionError(error);
            ...
            metadata.putObjectMD(
                ...
                latestOverlayVersionKey,
                remoteOverlay.version,
                {},
                log,
                error => {
                    if (error) {
...
function loadCachedOverlay(log, callback) {
    return metadata.getObjectMD(managementDatabaseName, latestOverlayVersionKey, {}, log, (err, version) => {
        if (err) {
            if (err.is.NoSuchKey) {

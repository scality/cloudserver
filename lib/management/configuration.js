        ...
        metadata.putObjectMD(managementDatabaseName, objName, remoteOverlay,
            {}, log, error => {
                ...
                metadata.putObjectMD(managementDatabaseName,
                    ...
                    error => {
                        if (!error) {
                            configurationListeners.forEach((listenerCb) => {
                                listenerCb(remoteOverlay);
                            });
                        }
...
/**
 * Register a listenet to the new remote overlay event.
 *
 * @param  {function} callback Function called with (remoteOverlay)
 *
 * @returns {undefined}
 */
function addConfigurationListener(callback) {

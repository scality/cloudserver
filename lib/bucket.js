...
Bucket.prototype.GetBucketListObjectsRefactor = function(prefix, marker, delimiter, maxKeys, callback){
	if(!callback){
	...
	//if no delimeter specified, everything goes into fetched
	if(!delimeter){


		//increment keyCount every time item goes into fetched.  
			//if hit maxKeys, set next_marker, set truncated to true and finish
	...
	//if delimeter specified:

		//put into common_prefixes based on common prefix those keys that contain the delimeter 

			//increment keycount every time a new common_prefix is created.
				//if hit maxkeys, set next_marker, set truncated to true and finish  
		//put remaining keys into fetched
				//increment keycount every time put a key into fetched.
					//if hit maxkeys, set next_marker, set truncated to true and finish
...
Bucket.prototype.GETBucketListObjects = function(prefix, marker, delimiter, maxKeys, callback) {
    ...
    //Need to fix for loop so that callback gets called in all events.  Added this as placeholder:
    callback(response);
};

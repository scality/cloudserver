...
const acl = {
        ...
        process.nextTick(function getIDs() {
            ...
            for (let i = 0; i < emails.length; i++) {
                ...
                if (!accountsKeyedbyEmail[lowercasedEmail]) {
                    log.error(`Error for user with email: ${lowercasedEmail}.` +
                        ` Returning error: UnresolvableGrantByEmailAddress`);
                    return cb('UnresolvableGrantByEmailAddress');
                }
    ...
    addACL(bucket, addACLParams, log, cb) {
        log.debug('Updating bucket ACL in Metadata');
    ...
    addObjectACL(bucket, objectKey, objectMD, addACLParams, log, cb) {
        log.debug('Updating object ACL in Metadata');

...
const metadata = {
    createBucket: (bucketName, bucketMD, log, cb) => {
        ...
        client.createBucket(bucketName, bucketMD, log, err => {
            ...
            if (config.metaIndexing)
                return index.initRowIds(bucketName, log, cb);
            return cb(err);
        });
    },
    ...
    putObjectMD: (bucketName, objName, objVal, log, cb) => {
        ...
        client.putObject(bucketName, objName, objVal, log, err => {
            ...
            if(config.metaIndexing)
                return index.updateRowIds(bucketName, objName, objVal, log, cb);
            return cb(err);
        });
    },
    ...
    listObject: (bucketName, query, prefix, marker, delimiter, maxKeys, log, cb) => {
        if (query)
            return index.evaluateQuery(query, bucketName, log, cb, {prefix, marker, maxKeys, delimiter});
        client

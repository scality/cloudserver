...
function initObjectCounter(bucketName, log, cb) {
    console.log("initObjectCounter");
    client.putObject(bucketName, "objectCounter", JSON.stringify({}), log, err => {
        if (err) {
            return cb(err);
        }
        return cb(err);
...
function updateObjectCounter(bucketName, objName, log, cb) {
    if (objName.indexOf("..|..") !== -1)
        return cb(null);
    client.getObject(bucketName, "objectCounter", log, (err, data) => {
        if (err) {
            return cb(err);
        }
        ...
        data[objName] = i;
        client.putObject(bucketName, "objectCounter", JSON.stringify(data), log, err => {
            if (err) {
...
const metadata = {
    ...
    putObjectMD: (bucketName, objName, objVal, log, cb) => {
        ...
        client.putObject(bucketName, objName, objVal, log, err => {
            ...
            log.debug('object successfully put in metadata');
            return updateObjectCounter(bucketName, objName, log, cb);
        });
    },
            ...
            .listObject(bucketName, { prefix, marker, maxKeys, delimiter },
                    log, (err, data) => {
                        ...
                        data.Contents = data.Contents.filter(function(elem) {
                            return elem.key.indexOf("objectCounter") === -1;
                        });
                        return cb(err, data);
                    });
    },

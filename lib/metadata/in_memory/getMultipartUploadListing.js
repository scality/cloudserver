...
export default function getMultipartUploadListing(bucket, params, callback) {
    ...
    const keyMap = metadata.keyMaps[bucket.getName()];
    if (prefix) {
    ...
    let uploads = Object.keys(keyMap).sort((a, b) => {
        ...
        const aInitiated = keyMap[a].initiated;
        const bInitiated = keyMap[b].initiated;
    ...
    uploads = uploads.map(stringKey => {
        ...
        const storedMD = keyMap[stringKey];
        return {

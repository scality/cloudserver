...
export default function getMultipartUploadListing(bucket, params, callback) {
    ...
    let uploads = Object.keys(bucket.keyMap).sort((a, b) => {
        const aInfo = a.split(splitter);
        const bInfo = b.split(splitter);
        const aObjectKey = aInfo[1];
        const bObjectKey = bInfo[1];
        const aInitiated = bucket.keyMap[a].initiated;
        const bInitiated = bucket.keyMap[b].initiated;
    ...
    uploads = uploads.map((stringKey) => {
        ...
        const storedMD = bucket.keyMap[stringKey];
        return {
            key: arrayKey[1],
            uploadId: arrayKey[2],

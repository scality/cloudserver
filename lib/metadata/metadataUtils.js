    ...
    async.waterfall([
        next => {
            ...
            return metadata.getBucketAndObjectMD(bucketName, objectKey, getOptions, log, (err, getResult) => {
                if (err) {
                    // if only implicit iamAuthzResults, return AccessDenied
                    // before leaking any state information
                    if (iamAuthzResults && Object.values(iamAuthzResults).every(v => v === true)) {
...
function metadataValidateBucket(params, iamAuthzResults, log, callback) {
    ...
    return metadata.getBucket(bucketName, log, (err, bucket) => {
        if (err) {
            // if only implicit iamAuthzResults, return AccessDenied before

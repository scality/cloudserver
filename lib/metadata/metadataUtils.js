...
function standardMetadataValidateBucketAndObj(params, actionImplicitDenies, log, callback) {
    ...
    async.waterfall([
        ...
        // Phase 2: Post-metadata rate limit check (skip if pre-auth already checked)
        if (!params.request.rateLimitPreAuthChecked) {
            const { checkBucketRateLimit } = require('../api/apiUtils/rateLimit');
            const { config: _config } = require('../Config');
            if (!checkBucketRateLimit(bucketName, bucket, _config, log)) {
                log.debug('request throttled in post-metadata check', { bucketName, method: 'standardMetadataValidateBucketAndObj' });
                return callback(_config.rateLimiting?.error || errors.SlowDown, bucket);
            }
        } else {
            log.trace('skipping post-metadata rate limit check (pre-auth already enforced)', { bucketName });
        }
...
function standardMetadataValidateBucket(params, actionImplicitDenies, log, callback) {
    ...
    return metadata.getBucket(bucketName, log, (err, bucket) => {
        ...
        if (!params.request.rateLimitPreAuthChecked) {
            ...
            const { config: _config } = require('../Config');
            if (!checkBucketRateLimit(bucketName, bucket, _config, log)) {
                log.debug('request throttled in post-metadata check', { bucketName, method: 'standardMetadataValidateBucket' });
                return callback(_config.rateLimiting?.error || errors.SlowDown, bucket);
            }
        } else {
            log.trace('skipping post-metadata rate limit check (pre-auth already enforced)', { bucketName });

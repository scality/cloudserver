...
const { setCachedBucketOwner, setCachedConfig } = require('../api/apiUtils/rateLimit/cache');
...
function getRateLimitsForRequest(request, bucketName, bucketMD, log) {
    const checks = {};
    ...
    if (request.accountLimits) {
        checks.account = {
            ...
            resourceId: bucketMD.getOwner(),
            measure: 'rps',
            cfg: request.accountLimits,
        };
        needsCheck = true;

        setCachedBucketOwner(bucketName, bucketMD.getOwner());
        // TODO handle cross account requests
        setCachedConfig(`account:${bucketMD.getOwner()}`, request.accountLimits);
    }

    const rateLimitConfig = extractAndCacheRateLimitConfig(bucketMD, bucketName, log);
    if (rateLimitConfig) {
        checks.bucket = {
...
function checkRateLimitIfNeeded(bucket, bucketName, request, log, callback) {
    ...
    if (request.rateLimitAccountAlreadyChecked
        && request.rateLimitBucketAlreadyChecked
        && !config.rateLimiting?.enabled
    ...
    const { needsCheck, checks } = getRateLimitsForRequest(request, bucketName, bucket, log);
    if (!needsCheck) {
    ...
    if (!request.rateLimitAccountAlreadyChecked && checks.account) {
        toCheck.push(checks.account);
    }

    if (!request.rateLimitBucketAlreadyChecked && checks.bucket) {
        toCheck.push(checks.bucket);
    }

    const allowed = checkRateLimitsForRequest(toCheck, log);
    if (!allowed) {

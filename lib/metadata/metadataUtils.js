...
function metadataValidateBucketAndObj(params, log, callback) {
    ...
    async.waterfall([
        ...
        function checkBucketAuth(bucket, objMD, next) {
            // if requester is not bucket owner, bucket policy actions should be
            // denied with MethodNotAllowed error
            const onlyOwnerAllowed = [
                'bucketDeletePolicy',
                'bucketGetPolicy',
                'bucketPutPolicy',
            ];
            if (bucket.getOwner() !== canonicalID
            && onlyOwnerAllowed.includes(requestType)) {
                return next(errors.MethodNotAllowed, bucket);
            }
            if (!isBucketAuthorized(bucket, (preciseRequestType || requestType),

...
function getRateLimitsForRequest(request, bucketName, bucketMD) {
    const checks = [];
    ...
    if (request.accountLimits) {
        ...
        setCachedBucketOwner(bucketName, bucketMD.getOwnerID());
        // TODO handle cross account requests
        setCachedConfig(`account:${bucketMD.getOwnerID()}`, request.accountLimits);
    }
...
function checkRateLimitIfNeeded(bucket, bucketName, request, log, callback) {
    ...
    const { needsCheck, checks } = getRateLimitsForRequest(request, bucketName, bucket);
    if (!needsCheck) {

...
function metadataGetObject(bucketName, objectKey, versionId, cachedDocuments, log, cb) {
    ...
    // Start timing metadata operation
    const startTime = process.hrtime.bigint();
    
    return metadata.getObjectMD(bucketName, objectKey, options, log,
        (err, objMD) => {
            // End timing and record the metric for objectmd
            const endTime = process.hrtime.bigint();
            const elapsedMicros = Number((endTime - startTime) / 1000n);
            observeElapsedTime('objectmd', elapsedMicros);
...
function standardMetadataValidateBucketAndObj(params, actionImplicitDenies, log, callback) {
    ...
    // Check if parallel auth optimization is enabled
    ...
    async.waterfall([
        next => {
            ...
            return metadata.getBucketAndObjectMD(bucketName, objectKey, getOptions, log, (err, getResult) => {
                // End timing and record the metric for bucket+objectmd
                const endTime = process.hrtime.bigint();
                const elapsedMicros = Number((endTime - startTime) / 1000n);
                observeElapsedTime('bucket+objectmd', elapsedMicros);
        ...
        (getResult, next) => {
            ...
            // Skip bucket policy and ACL validation when parallel auth is enabled
        ...
        (bucket, objMD, next) => {
            // Skip object authorization when parallel auth is enabled
            if (isParallelAuthEnabled) {
                return next(null, bucket, objMD);
            }
...
function standardMetadataValidateBucket(params, actionImplicitDenies, log, callback) {
    ...
    return metadata.getBucket(bucketName, log, (err, bucket) => {
        // End timing and record the metric for bucketmd
        const endTime = process.hrtime.bigint();
        const elapsedMicros = Number((endTime - startTime) / 1000n);
        observeElapsedTime('bucketmd', elapsedMicros);

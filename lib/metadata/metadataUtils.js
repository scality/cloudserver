...
/**
 * Optimized retention metadata update that combines validation and update in fewer MongoDB calls
 * Uses new Arsenal methods for atomic updates and partial object reads
 * @param {object} params - function parameters
 * @param {AuthInfo} params.authInfo - AuthInfo class instance
 * @param {string} params.bucketName - name of bucket
 * @param {string} params.objectKey - name of object
 * @param {string} [params.versionId] - version id if updating specific version
 * @param {string} params.requestType - type of request
 * @param {object} params.request - http request object
 * @param {boolean} params.hasGovernanceBypass - governance bypass header present
 * @param {object} params.retentionInfo - retention information to apply
 * @param {string} params.replicationAction - replication action type
 * @param {RequestLogger} log - request logger
 * @param {function} callback - callback
 * @return {undefined} - and call callback with params err, result
 */
function optimizedRetentionMetadataUpdate(params, log, callback) {
    const { authInfo, bucketName, objectKey, versionId, requestType, request,
        hasGovernanceBypass, retentionInfo, replicationAction } = params;
    
    log.trace('optimized retention metadata update', { bucketName, objectKey, versionId });
    
    // Check if new Arsenal methods are available
    if (typeof metadata.putObjectRetention === 'function' && 
        typeof metadata.getObjectMDPartial === 'function') {

        return async.waterfall([
            // Step 1: Get bucket (with caching) and partial object metadata in parallel
            next => {
                async.parallel({
                    bucket: cb => getCachedBucketMD(bucketName, log, cb),
                    objectMD: cb => {
                        // Ultra-minimal projection - only absolute essentials
                        const projection = {
                            '_id': 1,  // Required for update
                            'value.retentionMode': 1,
                            'value.retentionDate': 1,
                            'value.legalHold': 1,
                            'value.isDeleteMarker': 1
                            // Removed versionId - not needed for validation
                        };
                        metadata.getObjectMDPartial(bucketName, objectKey, projection, 
                            { versionId, getDeleteMarker: true }, log, cb);
                    }
                }, next);
            },
            
            // Step 2: Validate permissions and object lock policy
            ({ bucket, objectMD }, next) => {
                if (!bucket) {
                    log.debug('bucket not found', { bucket: bucketName });
                    return next(errors.NoSuchBucket);
                }
                
                if (!objectMD) {
                    const err = versionId ? errors.NoSuchVersion : errors.NoSuchKey;
                    log.trace('object not found', { method: 'optimizedRetentionMetadataUpdate', error: err });
                ...
                if (!isObjAuthorized(bucket, objectMD, requestType, canonicalID, authInfo, log, request)) {
                    return next(errors.AccessDenied, bucket);
                }
                
                // Validate object lock policy
                const { ObjectLockInfo } = require('../api/apiUtils/object/objectLockHelpers');
            ...
            // Step 3: Atomic retention update using new Arsenal method
            (bucket, objectMD, next) => {
                const getReplicationInfo = require('../api/apiUtils/object/getReplicationInfo');
                const { config } = require('../Config');
                
                // Prepare replication info
                const replicationInfo = getReplicationInfo(config,
                    objectKey, bucket, true, 0, replicationAction, objectMD);
                
                // Use Arsenal's atomic retention update
                const retentionUpdate = {
                    mode: retentionInfo.mode,
                    date: retentionInfo.date,
                    originOp: 's3:ObjectRetention:Put'
                };
                
                if (replicationInfo) {
                    retentionUpdate.replicationInfo = replicationInfo;
                }
                
                const { getVersionSpecificMetadataOptions } = require('../api/apiUtils/object/versioning');
                const options = getVersionSpecificMetadataOptions(objectMD, config.nullVersionCompatMode);
                
                metadata.putObjectRetention(bucketName, objectKey, retentionUpdate, options, log, 
                    (err) => {
                        if (err) {
                            return next(err);
                        }
                        // Don't need the actual updated ObjectMD - just return success
                        return next(null, { 
                            bucket, 
                            objectMD: { 
                                versionId: versionId,
                                dataStoreName: bucket.getLocationConstraint() 
                            } 
                        });
                    });
            }
    ...
    // Fallback to standard path if Arsenal methods not available
    log.debug('Arsenal optimization not available, falling back to standard path');
    return standardMetadataValidateBucketAndObj({
        authInfo, bucketName, objectKey, versionId, 
        requestType, request, getDeleteMarker: true
    }, request.actionImplicitDenies, log, (err, bucket, objectMD) => {
        if (err) {
            return callback(err, bucket);
        }
        
        // Apply retention update using standard putObjectMD
        ...
        metadata.putObjectMD(bucket.getName(), objectKey, objectMD, params,
            log, err => callback(err, { bucket, objectMD }));
    });
}
...
module.exports = {
    ...
    optimizedRetentionMetadataUpdate,
};

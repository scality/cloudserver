...
class BucketFileInterface {
    ...
    internalListObject(bucketName, params, log, cb) {
        const Ext = arsenal.algorithms.list[params.listingType || 'Delimiter'];
        const extension = new Ext(params, log);
        this.loadDBIfExists(bucketName, log, (err, db) => {
            if (err) {
                return cb(err);
            }
            let cbDone = false;
            const stream = db.createReadStream(extension.getListingParams());
            stream
                .on('data', e => {

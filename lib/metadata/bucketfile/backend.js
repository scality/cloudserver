...
class BucketFileInterface {
    ...
    constructor() {
        this.logger = logger;
        this.mdClient = new MetadataFileClient(
    ...
    putObject(bucketName, objName, objVal, params, log, cb) {
        this.loadDBIfExists(bucketName, log, (err, db) => {
            ...
            db.withRequestLogger(log)
                .put(objName, JSON.stringify(objVal), OPTIONS, err => {
                    if (err) {
                        const logObj = {
                            rawError: err,
                            error: err.message,
    ...
    getObject(bucketName, objName, params, log, cb) {
        this.loadDBIfExists(bucketName, log, (err, db) => {
            ...
            db.withRequestLogger(log).get(objName, (err, data) => {
                if (err) {
                    if (err.notFound) {
    ...
    deleteObject(bucketName, objName, params, log, cb) {
        this.loadDBIfExists(bucketName, log, (err, db) => {
            ...
            db.withRequestLogger(log).del(objName, OPTIONS, err => {
                if (err) {
                    const logObj = {
                        rawError: err,
                        error: err.message,

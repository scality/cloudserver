...
function _createBucketWithNFSEnabled(host, port, bucketName, accessKey, secretKey, verbose, ssl, locationConstraint) {
    const options = {
        host,
        port,
        method: 'PUT',
        path: `/${bucketName}/`,
        headers: { 'x-scal-nfs-enabled': 'true' },
...
/**
 * This function is used as a binary to send a request to S3 and create a
 * bucket, because most of the s3 tools don't support custom
 * headers
 *
 * @return {undefined}
 */
function createBucketWithNFSEnabled() {
    commander
        ...
        .option('-v, --verbose')
        .option('-l, --location-constraint <locationConstraint>', 'location Constraint')
        .parse(process.argv);

    const { host, port, accessKey, secretKey, bucket, verbose, ssl, locationConstraint } = commander;
    ...
    _createBucketWithNFSEnabled(host, port, bucket, accessKey, secretKey, verbose, ssl, locationConstraint);
}

...
const DataFileBackend = storage.data.file.DataFileInterface;
...
const multipleBackendGateway = {
    ...
    protectAzureBlocks: (bucketName, objectKey, location, log, cb) => {
        ...
        if (client.protectAzureBlocks) {
            return client.protectAzureBlocks(metadata, bucketName, objectKey,
            location, log, cb);
        }
        return cb();
    },
    ...
    getDiskUsage: (reqUids, callback) => {
        new DataFileBackend(config).getDiskUsage(reqUids, callback);
    },
};

...
const repairParams = {
    schedule: '00 00 02 * * 0',   // 02h00 Sunday, every week
...
class DpClient {
    ...
    put(stream, size, params, reqUids, callback) {
        ...
        const done = (key, cb) => {
            ...
            const job = new CronJob({
                cronTime: this.cronParams.schedule,
                onTick: () => {
                    this.repair(key, err => {
                        if (err) {
     ...
     * @returns {undefined}
     */
    checkAvailFrag(key, log, callback) {

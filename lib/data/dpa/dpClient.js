...
if (config.backends.data.slice(3) === 'mem') {
    client = inMemory;
    implName = 'mem';
} else if (config.backends.data.slice(3) === 'sproxyd') {
    client = new Sproxy({
...
class DpClient {
    ...
    put(stream, params, reqUids, callback) {
        ...
        this.encode.bind(this)(stream, (err, fragments) => {
            if (err) {
                return callback(err);
            }
            ...
            async.forEachOf(fragments, (frag, idx, cb) => {
                ...
                client.put(streamFrag, params, reqUids, (err, key) => {
                    if (err) {
                ...
                if (err) {
                    ...
                    return this.delete.bind(this)(storedKeys, reqUids, err1 => {
                        if (err1) {
        ...
        key.split(',').forEach(keyFrag => {
            client.get(keyFrag, null, reqUids, (err, val) => {
                ...
                val.on('end', () => {
                    ...
                    if (fragments.length === this.kmin) {
                        this.decode.bind(this)(fragments, 0, (err, obj) => {
                            if (err) {
                                log.error('error from dpClient decode',
                                    { error: err });
                                return callback(err);
                            }
                            const stream = new PassThrough(readStreamOptions);
                            stream.end(obj);

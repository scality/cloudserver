...
class AwsClient {
        ...
        const putCb = (err, data) => {
            ...
            if (!data.VersionId) {
                logHelper(log, 'error', 'missing version id for data ' +
                    'backend object', missingVerIdInternalError(this._type),
                    this._dataStoreName);
                return callback(missingVerIdInternalError(this._type));
            }
            ...
            if (err) {
                ...
                if (err.code === 'NotFound') {
                    ...
                    .customizeDescription(
                        `Unexpected error from ${this._type}: "NotFound". ` +
                        `Data on ${this._type} ` +
    ...
    get(objectGetInfo, range, reqUids, callback) {
        ...
        const request = this._client.getObject({
            ...
            log.trace(`${this._type} GET request response headers`,
              { responseHeaders: response.httpResponse.headers });
        });
        const stream = request.createReadStream().on('error', err => {
            logHelper(log, 'error', `error streaming data from ${this._type}`,
              err, this._dataStoreName);
            return callback(err);
        });
        ...
        this._client.copyObject(awsParams, (err, copyResult) => {
            if (err) {
                if (err.code === 'AccessDenied') {
                    ...
                    `${sourceAwsBucketName} ${this._type} bucket`, err,
                    this._dataStoreName);
                    return callback(errors.AccessDenied
                      .customizeDescription('Error: Unable to access ' +
                      `${sourceAwsBucketName} ${this._type} bucket`)
                    );
                }
                logHelper(log, 'error', 'error from data backend on ' +

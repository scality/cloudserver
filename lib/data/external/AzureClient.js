...
class AzureClient {
            ...
            return this._errorWrapper('put', 'createBlockBlobFromStream',
                ...
                err => {
                    ...
                    this._errorWrapper('put', 'createBlobSnapshot',
                        [this._azureContainerName, azureKey, options, (err, snapshotName) => {
                            if (err) {
                                logHelper(log, 'error', 'err from Azure PUT data ' +
                                    'backend', err, this._dataStoreName);
                                return callback(errors.ServiceUnavailable
                            ...
                            console.log('GOT AZURE VERSION ID', snapshotName);
    ...
    head(objectGetInfo, reqUids, callback) {
        const log = createLogger(reqUids);
        const { key, azureStreamingOptions, dataStoreVersionId } = objectGetInfo;
    ...
    get(objectGetInfo, range, reqUids, callback) {
        ...
        const { key, response, azureStreamingOptions, dataStoreVersionId } = objectGetInfo;
        console.log('AZURE VERSION GET', dataStoreVersionId);
        azureStreamingOptions.snapshotId = dataStoreVersionId;

...
class AzureClient {
    ...
    put(stream, size, keyContext, reqUids, callback) {
        ...
        keyContext.objectKey, this._dataStoreName, log, err => {
            ...
            return this._client.createBlockBlobFromStream(
            this._azureContainerName, azureKey, stream, size, options, err => {
                if (err) {
    ...
    partNumber, bucket, log, callback) {
        ...
        if (size <= azureMpuUtils.maxSubPartSize) {
            return azureMpuUtils.putSinglePart(this._client, stream, params,
        ...
        return azureMpuUtils.putSubParts(this._client, stream, params,
        this._dataStoreName, log, (err, dataStoreETag, numberSubParts) => {
            if (err) {
                return callback(err);
            }
    ...
    completeMPU(jsonList, mdInfo, key, uploadId, bucket, log, callback) {
        ...
        this._client.commitBlocks(this._azureContainerName, azureKey,
        commitList, null, err => {
            if (err) {
    ...
    callback) {
        ...
        this._client.startCopyBlob(`${this._azureBlobEndpoint}/` +
          `${sourceContainerName}/${sourceKey}?${this._azureBlobSAS}`,
        this._azureContainerName, destAzureKey, (err, res) => {
            if (err) {
            ...
            if (res.copy.status === 'pending') {
                ...
                destAzureKey, copyId, err => {
                    if (err) {
                        logHelper(log, 'error', 'error from data backend on ' +
                        'abortCopyBlob', err, this._dataStoreName);
                        return callback(errors.InternalError
                          .customizeDescription('Error returned from ' +

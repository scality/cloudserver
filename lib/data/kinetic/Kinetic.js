...
class Kinetic {
    ...
    getIt(callback) {
        let found = false;
        return async.eachOfSeries(this._sockets, (socket, index, next) => {
            if (socket._request === false){
                found = true;
            ...
            if (found === false) {
                ...
                sock.sock.on('connect', () => {
                    sock.sock.on('readable', err => {
                        if (sock._requestGet === false) {
                            this._parsePDU(sock, (err1, pdu) => {
    ...
    get(index, key, range, callback) {
        ...
        socket.sock.write(header, err => {
            ...
            return this._sockets[index]._events.once('getResponse', (err, chunk) => {
                ...
                this._count = 0;
                if (range) {
                    endValue = chunk.slice(range[0], range[1]);
                } else {
                    endValue = chunk;
                }
    ...
    delete(index, key, options, callback) {
        ...
        socket.sock.write(pdu.read(), err => {
            ...
            // this._sockets[index]._events.removeAllListeners('deleteResponse');
            return this._sockets[index]._events.once(

...
if (config.backends.dp) {
    // dataBE contains all data backends to store data
    const dataBE = {};
    config.backends.data.forEach(be => {
        const topoFilePath =
            `${constants.topology[be].name}.json`;
        let topo;
        try {
            topo = JSON.parse(fs.readFileSync(topoFilePath));
        } catch (e) {
            // use default topo
            topo = {
                MD: [{
        ...
        const dp = constants.topology[be].dp;
        const keyFrag = constants.topology[be].keyFrag;
        if (be === 'mem') {
            dataBE.mem = {
                main: inMemory,
                topo,
            ...
            dataBE.file = {
                main: file,
                topo,
                path: config.filePaths.dataPath,
                dp,
                keyFrag,
            };
        } else if (be === 'scality') {
    ...
    const ec = constants.ec.params || {
        bc_id: constants.ec.backendId.EC_BACKEND_JERASURE_RS_VAND, // eslint-disable-line
        k: 6,       // number of data fragments
        m: 3,       // number of parity fragments
        w: constants.ec.gfDim.RS,
        hd: 4,
        ct: constants.ec.checksumType.CHKSUM_CRC32,
    };

    const dpserver = config.dpserver;

    client = new DpClient(dataBE, { ec, dpserver });
    implName = 'dpclient';
} else {

...
function _retryDelete(key, log, backendInfo, count, cb) {
    if (count > MAX_RETRY) {
        return cb(errors.InternalError);
    }
    if (implName === 'multipleBackends') {
        return client.delete(key, backendInfo, log.getSerializedUids(), err => {
            if (err) {
...
const data = {
    ...
    delete: (objectGetInfo, backendInfo, log, cb) => {
        ...
        _retryDelete(key, backendInfo, log, 0, err => {
            if (err) {

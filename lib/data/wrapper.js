...
const { instrumentStorage } = require('../instrumentation/simple');
...
// Instrument the data wrapper for comprehensive tracing
const instrumentedData = instrumentStorage(data);
...
config.on('location-constraints-update', () => {
    if (implName === 'multipleBackends') {
        ...
        client = new MultipleBackendGateway(
            clients, metadata, locationStorageCheck);
        instrumentedData.switch(client);
    }
});

module.exports = { data: instrumentedData, client, implName };

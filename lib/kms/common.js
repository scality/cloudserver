    ...
    */
    ...
    static _deriveKey(cryptoScheme, dataKey, log, cb) {
        if (cryptoScheme <= 1) {
            ...
            const iterations = 1;
            return crypto.pbkdf2(dataKey, salt, iterations, this._keySize(), 'sha1', (err, derivedKey) => {
                if (err) {
                    log.error('pbkdf2 function failed on key derivation', { error: err });
                    cb(errors.InternalError);
                    return;
                }
                crypto.pbkdf2(derivedKey, salt, iterations, this._IVSize(), 'sha1', (err, derivedIV) => {
                    if (err) {
                        log.error('pbkdf2 function failed on IV derivation', { error: err });
    ...
    static createDecipher(cryptoScheme, dataKey, offset, log, cb) {
        this._deriveKey(cryptoScheme, dataKey, log, (err, derivedKey, derivedIV) => {
            if (err) {

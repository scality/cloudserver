...
function getScalityKms() {
    ...
    try {
        ...
        logger.warn('scality kms unavailable. ' + 'Using file kms backend unless mem specified.', { error });
        scalityKMS = file;
...
const kmsFactory = {
    ...
    kmip: () => {
        ...
        const client = Array.isArray(config.kmip.transport)
            ? new KMIPClusterClient({ kmip: config.kmip, logger, retries: config.kmip.retries })
...
if (config.sseMigration) {
    ...
    previousIdentifier = `${previousBackend.type}:${previousBackend.protocol}:${previousBackend.provider}`;
...
function getClientForKey(key, log) {
    ...
    return {
        error: new // eslint-disable-next-line new-cap
        errors.InvalidArgument.customizeDescription(`KMS unknown provider for key ${key}`),
    };
}

class KMS {
    /** Access to client for tests (to create a key without vault check and bucket) */
    static get client() {
        return client;
    }
    ...
    static createBucketKey(bucket, log, cb) {
        ...
        if (client.supportsDefaultKeyPerAccount && config.defaultEncryptionKeyPerAccount) {
            return vault.getOrCreateEncryptionKeyId(bucket.getOwner(), log, (err, data) => {
                if (err) {
                    log.debug(
                        'error retrieving or creating the default encryption key at the account level from vault',
                        { implName, error: err },
                    );
                    return cb(err);
                }
        ...
        if (algorithm === 'AES256' || algorithm === 'aws:kms') {
            ...
            if (algorithm === 'aws:kms' && configuredMasterKeyId) {
                ...
                if (isScalityKmsArn(configuredMasterKeyId)) {
                    ...
                    serverSideEncryptionInfo.configuredMasterKeyId = `${client.backend.arnPrefix}${configuredMasterKeyId}`;
                }
     ...
     */
    static createCipherBundle(serverSideEncryptionInfo, log, cb, opts) {
        ...
        return async.waterfall(
                ...
                */
                    ...
                    if (client.generateDataKey) {
                        ...
                        res = client.generateDataKey(
                            ...
                            key,
                            log,
                            (err, plainTextDataKey, cipheredDataKey) => {
                                if (err) {
                                    log.debug('error generating a new data key from KMS', { implName, error: err });
                                    return next(err);
                                }
                        ...
                        res = client.cipherDataKey(
                            ...
                            (err, cipheredDataKey) => {
                                if (err) {
                                    log.debug('error encrypting the data key using KMS', { implName, error: err });
                                    return next(err);
                                }
                ...
                function createCipher(plainTextDataKey, cipheredDataKey, next) {
                    ...
                    cipherBundle.cipheredDataKey = cipheredDataKey.toString('base64');
                    return Common.createCipher(cipherBundle.cryptoScheme, plainTextDataKey, 0, log, (err, cipher) => {
            ...
            (err, cipherBundle) => {
                if (err) {
                    log.error('error processing cipher bundle', { implName, error: err });
                }
     ...
     */
    static createDecipherBundle(serverSideEncryptionInfo, offset, log, cb) {
        if (
        ...
        const { error, client, implName: _impl, key } = getClientForKey(serverSideEncryptionInfo.masterKeyId, log);
        if (error) {
            return cb(error);
        }
        ...
        return async.waterfall(
                ...
                function decipherDataKey(next) {
                    return client.decipherDataKey(
                        ...
                        (err, plainTextDataKey) => {
                            ...
                            if (err) {
                                log.debug('error from kms', { implName: _impl, error: err });
                                return next(err);
                            }
            ...
            (err, decipherBundle) => {
                if (err) {
                    log.error('error processing decipher bundle', { implName: _impl, error: err });
                    return cb(err);
                }

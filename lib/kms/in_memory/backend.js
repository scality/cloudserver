...
const { KmsType, KmsProtocol, getKeyIdFromArn, makeBackend } = require('arsenal/build/lib/network/KMSInterface');

const kms = [];
let count = 1;

const kmsBackend = makeBackend(KmsType.internal, KmsProtocol.mem, 'scality');
...
const backend = {
    ...
    createBucketKey: function createBucketKeyMem(bucket, log, cb) {
        process.nextTick(() => {
            ...
            const keyId = (count++).toString();
            cb(null, keyId, `${kmsBackend.arnPrefix}${keyId}`);
        });
    },

    /**
     *
     * @param {string} bucketKeyIdOrArn - the Id of the bucket key
     * @param {object} log - logger object
     * @param {function} cb - callback
     * @returns {undefined}
     * @callback called with (err)
     */
    destroyBucketKey: function destroyBucketKeyMem(bucketKeyIdOrArn, log, cb) {
        process.nextTick(() => {
     ...
     * @param {string} masterKeyIdOrArn - key to retrieve master key
     ...
     */
    cipherDataKey: function cipherDataKeyMem(cryptoScheme, masterKeyIdOrArn, plainTextDataKey, log, cb) {
        process.nextTick(() => {
            const masterKeyId = getKeyIdFromArn(masterKeyIdOrArn);
            Common.createCipher(cryptoScheme, kms[masterKeyId], 0, log, (err, cipher) => {
                if (err) {
                    cb(err);
                    return;
                }
                ...
                if (final.length !== 0) {
                    cipheredDataKey = Buffer.concat([cipheredDataKey, final]);
                }
     ...
     */
    decipherDataKey: function decipherDataKeyMem(cryptoScheme, masterKeyIdOrArn, cipheredDataKey, log, cb) {
        process.nextTick(() => {
            const masterKeyId = getKeyIdFromArn(masterKeyIdOrArn);
            Common.createDecipher(cryptoScheme, kms[masterKeyId], 0, log, (err, decipher) => {
                if (err) {
                    cb(err);
                    return;
                }
                ...
                if (final.length !== 0) {
                    plainTextDataKey = Buffer.concat([plainTextDataKey, final]);
                }

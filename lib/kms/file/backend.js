...
const kmsBackend = makeBackend(KmsType.internal, KmsProtocol.file, 'scality');
...
const backend = {
    ...
    createBucketKey: function createBucketKeyMem(bucket, log, cb) {
        process.nextTick(() => {
            ...
            cb(null, newKey, `${kmsBackend.arnPrefix}${newKey}`);
        });
    },
     ...
     * @param {string} masterKeyIdOrArn - master key; for the file backend
    ...
    cipherDataKey: function cipherDataKeyMem(cryptoScheme, masterKeyIdOrArn, plainTextDataKey, log, cb) {
        process.nextTick(() => {
            ...
            Common.createCipher(cryptoScheme, masterKey, 0, log, (err, cipher) => {
                if (err) {
                    cb(err);
                    return;
                }
    ...
    decipherDataKey: function decipherDataKeyMem(cryptoScheme, masterKeyIdOrArn, cipheredDataKey, log, cb) {
        process.nextTick(() => {
            const masterKeyId = getKeyIdFromArn(masterKeyIdOrArn);
            const masterKey = Buffer.from(masterKeyId, 'hex');
            Common.createDecipher(cryptoScheme, masterKey, 0, log, (err, decipher) => {
                if (err) {
                    cb(err);
                    return;
                }

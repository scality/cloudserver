...
function nearValues(array, start, end, value, operator) {
    ...
    if (end - start < 2) {
        if (array[start] < value && array[end] > value) {
            return start;
        } else if (array[start] < value && array[start + 1] > value) {
            return start;
        } else if (array[end - 1] < value && array[end] > value) {
            return end - 1;
        }
...
function getIntRange(op, term, data) {
    ...
    if (op.indexOf('<') !== -1) {
        const nearVal = nearValues(valList, 0, valList.length - 1, value, op);
        ...
        if (value <= valList[0]) {
            ...
            if (valList.indexOf(value) !== -1) {
                ...
                nearestValue = `${attr}/${valList[nearVal + 1]}`;
            }
    ...
    if (op.indexOf('>') !== -1) {
        ...
        const nextValue = `${attr}/${valList[nearVal + 1]}`;
        if (value < valList[0]) {
            result = result.or(storeToBitmap(data[lowestValue]));
        } else if (value >= valList[valList.length - 1]) {
            result = result.or(bitmap.createObject());
        } else {
            const op1 = storeToBitmap(data[nextValue]);
...
const index = {
    ...
    updateIndex: (bucketName, objName, objVal, log, cb) => {

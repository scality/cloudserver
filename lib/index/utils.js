                ...
                client.putObject(bucketName, "I|fileSize", JSON.stringify({counter:0}), log, err => {
                    ...
                    client.putObject(bucketName, "I|modificationDate", JSON.stringify({counter:0}), log, err => {
                        ...
                        client.putObject(bucketName, "I|contentType", JSON.stringify({counter:0}), log, err => {
                            if (err)
                                return cb(err);
                            client.putObject(bucketName, "I|ACL", JSON.stringify({counter:0}), log, err => {
                                if (err)
                                    return cb(err);
                                return cb(err);
                            });
        ...
        client.getObject(bucketName, "I|objNameToRowID", log, (err, data) => {
            ...
            client.putObject(bucketName, "I|objNameToRowID", JSON.stringify(data), log, err => {
                ...
                if (config.systemMetaIndexing)
                    return updateACLIndex(bucketName, objName, objVal, rowId, Object.keys(data).length/2, log, cb)
...
function updateACLIndex(bucketName, objName, objVal, rowId, objCounter, log, cb) {
    client.getObject(bucketName, "I|ACL", log, (err, data) => {
        if (err)
            return cb(err);
        data = JSON.parse(data);
        data = deleteOldEntries(rowId, objCounter, data);
        Object.keys(objVal.acl).forEach(elem =>{
            if (typeof objVal.acl[elem] === "string") {
                    const tag = "acl-"+elem+"/"+objVal.acl[elem]
                    data[tag] = updateBitmap(storeToBitmap(data[tag]), rowId, objCounter);
            }
            else {
                objVal.acl[elem].forEach(item =>{
        ...
        if (rowId > data.counter)
            data.counter = rowId;
        client.putObject(bucketName, "I|ACL", JSON.stringify(data), log, err => {
            if (err)
                return cb(err);
            return updateContentTypeIndex(bucketName, objName, objVal, rowId, rowId, log, cb)
        });
    });
}
...
function getObjects(params, searchTerm, callback) {
    ...
    if (searchTerm.indexOf("x-amz-meta") !== -1)
        ...
        getObjectsfromACL(searchTerm, notOperator, params, callback);
}

function getObjectsfromACL(searchTerm, notOperator, params, callback) {
    ...
    client.getObject(bucketName, "I|ACL", log, (err, data) => {
        if (err)
            return cb(err);
        data = JSON.parse(data);
        const result = storeToBitmap(data[term]);
        if (notOperator) {

        ...
        if (newConf.locations) {
            ...
            Object.keys(newConf.locations || {}).forEach(k => {
                ...
                case 'location-aws-s3-v1':
                    ...
                    break;
                default:
                    console.log('unknown location type', l.locationType);
                    return;
                }
...
function loadCachedOverlay(log, callback) {
    metadata.getObjectMD(managementDatabaseName, latestOverlayVersionKey, {}, log, (err, version) => {
        if (err) {
            if (err.NoSuchKey) {
        ...
        metadata.getObjectMD(managementDatabaseName, `configuration/overlay/${version}`, {}, log, (err, conf) => {
            if (err) {
                if (err.NoSuchKey) {
                    return process.nextTick(callback, null, {});
                }

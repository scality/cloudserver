...
const managementEndpoint = process.env.MANAGEMENT_ENDPOINT ||
    'https://api-dev.private.zenko.io/api/v1/instance' ||
    'http://localhost:5000/api/v1/instance';
            ...
            Object.keys(newConf.locations || {}).forEach(k => {
                ...
                case 'location-azure-v1':
                    ...
                    location.details = {
                        ...
                        azureBlobEndpoint: l.details.accessKey,
                        azureBlobSAS: decryptSecret(creds, l.details.secretKey),
...
function applyConfigurationOverlay(instanceId, remoteToken, log, cb) {
    async.waterfall([
        ...
        if (cb) {
            cb(error, instanceId, remoteToken);
        }
    ...
    async.waterfall([
        ...
        if (error) {
            ...
            setTimeout(initManagement,
                initRemoteManagementRetryDelay, log);
        } else {

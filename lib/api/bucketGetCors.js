...
function bucketGetCors(authInfo, request, log, callback) {
    ...
    metadata.getBucket(bucketName, log, (err, bucket) => {
        if (err) {
            ...
            monitoring.promMetrics('GET', bucketName, err.code, 'getBucketCors');
            return callback(err);
        }
        if (bucketShield(bucket, requestType)) {
            monitoring.promMetrics('GET', bucketName, 404, 'getBucketCors');
        ...
        if (
            ...
            monitoring.promMetrics('GET', bucketName, 403, 'getBucketCors');

 ...
 */
let bucketDelete = function(accessKey, datastore,  metastore, request, callback) {
  ...
  async.waterfall([
      ...
      function(bucket, bucketMetadata, next){
        services.metadataChecks(bucket, bucketMetadata, metadataCheckParams, next);
      },
      function(bucket, objectMetadata, metaHeaders, next){
          services.deleteBucket(bucket, metastore, metaHeaders, objectUID, next)
      }

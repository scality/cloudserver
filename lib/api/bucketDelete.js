...
function bucketDelete(authInfo, request, log, cb) {
    ...
    if (authInfo.isRequesterPublicUser()) {
        ...
        monitoring.promMetrics('DELETE', request.bucketName, 403, 'deleteBucket');
    ...
    return standardMetadataValidateBucket(metadataValParams, request.actionImplicitDenies, log, (err, bucketMD) => {
        const corsHeaders = collectCorsHeaders(request.headers.origin, request.method, bucketMD);
        if (err) {
            log.debug('error processing request', { method: 'metadataValidateBucket', error: err });
            monitoring.promMetrics('DELETE', bucketName, err.code, 'deleteBucket');
            return cb(err, corsHeaders);
        }
        log.trace('passed checks', { method: 'metadataValidateBucket' });
        return deleteBucket(authInfo, bucketMD, bucketName, authInfo.getCanonicalID(), request, log, err => {
            if (err) {
                monitoring.promMetrics('DELETE', bucketName, err.code, 'deleteBucket');
                return cb(err, corsHeaders);
            }
            ...
            monitoring.promMetrics('DELETE', bucketName, '204', 'deleteBucket');

...
export default function completeMultipartUpload(
    ...
    const xmlParams = {
        ...
        objectKey,
        etag: fakeEtag,
        hostname: request.lowerCaseHeaders.host.split(":")[0]
    };
    ...
    async.waterfall([
        ...
        function waterfall3(bucket, multiPartMetadata, jsonList, next) {
            ...
            for (let i = 0; i < jsonList.Part.length; i ++) {
                ...
                // return an error
                if (actualPartNumber < jsonList.Part.length - 1 &&
                    multiPartMetadata
                        .partLocations[actualPartNumber].size < 5242880) {
                ...
                if (actualPartNumber <= jsonList.Part.length) {
                    ...
                    size += Number.parseInt(multiPartMetadata

            ...
            return services.metadataStoreObject(destinationBucket.getName(),
                ...
                (err, res) => {
                    ...
                    return next(null, mpuBucket, keysToDelete, aggregateETag,
                        ...
                        generatedVersionId, dataToDelete,
                        completeObjData);
                });
        },
        function cleanupPartsData(mpuBucket, keysToDelete, aggregateETag,
            extraPartLocations, destinationBucket,
            generatedVersionId, dataToDelete,
            completeObjData, next) {
            if (dataToDelete) {
                let filteredDataToDelete;
                if (completeObjData) {
                    filteredDataToDelete = dataToDelete.filter(item => {
                        if (completeObjData.key === item.key &&
                            completeObjData.dataStoreName
                            === item.dataStoreName) {
                            return false;
                        }
                        return true;
                    ...
                    filteredDataToDelete = dataToDelete;
                }
                data.batchDelete(filteredDataToDelete, request.method, null,
                                 logger.newRequestLoggerFromSerializedUids(log
                                 .getSerializedUids()));
            }
            return next(null, mpuBucket, keysToDelete, aggregateETag,
                extraPartLocations, destinationBucket, generatedVersionId);
        },

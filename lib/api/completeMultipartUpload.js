 ...
 */
export default
function completeMultipartUpload(accessKey, request, log, callback) {
        ...
        function waterfall2(mpuBucket, next) {
            if (request.post) {
                return parseXml(request.post, (err, jsonList) => {
                    return next(err, mpuBucket, jsonList);
                });
            }
            return next('MalformedXML');
        },
        ...
        function waterfall4(mpuBucket, storedParts,
            ...
            const storedPartsAsObjects = sortedStoredParts.map((item) => {
                return {
                    ...
                    location: item.value.partLocations,
                };
            });
            let dataLocations = [];

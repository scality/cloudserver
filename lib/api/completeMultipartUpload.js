...
export default function completeMultipartUpload(
    ...
    function parseXml(xmlToParse, next) {
        ...
        if (typeof xmlToParse === 'object') {
            toBeParsed = '<CompleteMultipartUpload xmlns='
    ...
    async.waterfall([
        ...
        function waterfall2(mpuBucket, mpuOverviewArray, next) {
            if (request.post) {
                return parseXml(request.post, (err, jsonList) => {
                    return next(err, mpuBucket, jsonList, mpuOverviewArray);
                });
            }
            return next('MalformedXML');
        },
        ...
        function waterfall4(mpuBucket, storedParts,
            ...
            // Decode keys
            const decodedStoredParts = storedParts.map( (item) => {
                return decodeURIComponent(item.key);
            });
            const sortedStoredParts = decodedStoredParts.sort((a, b) => {
                const aArray = a.split(splitter);
                const bArray = b.split(splitter);
                ...
                if (aPartNumber - bPartNumber === 0) {
                    // LastModified date was converted to take out "-"
                ...
                // Return parts in ascending order
            ...
            for (let i = 0; i < sortedStoredParts.length - 1; i++) {
                const currentPartNumber =
                    sortedStoredParts[i].split(splitter)[1];
                const nextPartNumber =
                    sortedStoredParts[i + 1].split(splitter)[1];
            ...
            duplicatePartsToDelete = duplicatePartsToDelete.map((item) => {
                return item.split(splitter)[5].split(',');
            });
            const storedPartsAsObjects = sortedStoredParts.map((item) => {
                const arrayItem = item.split(splitter);
                return {
                    // In order to delete the part listing in the shadow
                    // bucket, need the full key (which is item here)
                    // url encoded
            ...
            const mpuOverviewKey = encodeURIComponent(
                mpuOverviewArray.join(splitter));

            dataLocations = dataLocations.reduce((a, b) => a.concat(b));
            metadata.getObjectMD(mpuBucket.name, mpuOverviewKey, log,
                (err, storedMetadata) => {
                    if (err) {
                        return next(err);
                    }

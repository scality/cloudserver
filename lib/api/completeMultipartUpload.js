...
const { errors, errorInstances, versioning, s3middleware, storage } = require('@scality/arsenal');
...
const { isColdStorageClass } = require('./apiUtils/object/storageClass');
 ...
 *
 * Parts with no stored ChecksumValue are exempt from both rules: they are
 * uploaded by instances running with checksums disabled. Failing here would make
 * such an MPU permanently uncompletable.
 *
...
function validatePerPartChecksums(jsonList, storedParts, mpuSplitter, mpuChecksum) {
    ...
    for (const part of jsonList.Part || []) {
        ...
        if (expectedTag && presentTags.includes(expectedTag)) {
            ...
            // If storedValue is false skip the check. The part was uploaded by a cloudserver with checksums disabled.
        ...
        } else if (requireForEachPart && storedValue) {
 ...
 *
 * Returns `{ result, error, missingPartChecksums }`:
 *   - Success: `{ result: { algorithm, type, value }, error: null }`
 *   - No MPU checksum configured: `{ result: null, error: null }`
 *   - One or more parts carry no stored ChecksumValue: `{ result: null,
 *     error: null, missingPartChecksums: true }`. Those parts were uploaded by
 *     an instance running with checksums disabled, so the object completes
 *     without a checksum rather than stranding the upload. The flag lets the
 *     caller skip validating a client-asserted final checksum, which it has
 *     nothing to compare against.
 *   - "Should-not-happen" branch (unknown checksumType, compute primitive
 *     error): behavior depends on whether the client opted in to checksums —
 ...
 *   missingPartChecksums: (boolean|undefined) }>}
 */
async function computeFinalChecksum(storedParts, filteredPartList, storedMetadata, mpuSplitter, uploadId, log) {
    ...
    if (missingPartNumbers.length > 0) {
        log.error(
            'unable to calculate final object checksum, some parts have no stored checksum. ' +
                'Object will be created with no checksum',
            { uploadId, algorithm, type, missingPartNumbers },
        );
        return { result: null, error: null, missingPartChecksums: true };
    }
        ...
        [
            ...
            function validateMultipart(destBucket, objMD, next) {
                ...
                return services.metadataValidateMultipart(
                    ...
                    (err, mpuBucket, mpuOverview, storedMetadata) => {
                        ...
                        // External-backend MPUs record no checksum config (CLDSRV-964),
                        // so an absent stored type means "not tracked" rather than legacy.
                        // MPUs created while checksums were disabled record none either,
                        // so the header is ignored rather than validated.
                        if (areChecksumsEnabled()) {
                            ...
                            const typeErr = validateCompleteMPUChecksumType(
                                request.headers,
                                storedMetadata.checksumType,
                                isExternalMpu,
                            );
                            if (typeErr) {
                                return next(arsenalErrorFromChecksumError(typeErr), destBucket);
                            }
                        }
            ...
            function retrieveParts(
                ...
                return services.getMPUparts(mpuBucket.getName(), uploadId, log, (err, result) => {
                    ...
                    if (areChecksumsEnabled()) {
                        checksumErr = validatePerPartChecksums(jsonList, storedParts, splitter, mpuChecksum);
                    }
                    if (checksumErr) {
            ...
            function processParts(
                ...
                // - if checksums are disabled the parts carry no digest to compose from
                if (!filteredPartsObj || completeObjData || !areChecksumsEnabled()) {
                ...
                computeFinalChecksum(
                    ...
                    ({ result, error, missingPartChecksums }) => {
                        if (error) {
                        ...
                        try {
                            ...
                            const checksumErr = validateCompleteMultipartUploadChecksum(
                                request.headers,
                                finalChecksum,
                                missingPartChecksums,
                            );
                            if (checksumErr) {
            ...
            function prepForStoring(
                ...
                // Data stays in hot storage: the transition is triggered async from the oplog event
                if (!isPutVersion && isColdStorageClass(storedMetadata['x-amz-storage-class'])) {
                    metaStoreParams.amzStorageClass = storedMetadata['x-amz-storage-class'];
                    metaStoreParams.transitionInProgress = true;
                }

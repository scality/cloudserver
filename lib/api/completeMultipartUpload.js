...
function validatePerPartChecksums(jsonList, storedParts, mpuSplitter, mpuChecksum, isExternal) {
    ...
    const requireForEachPart = mpuChecksum.type === 'COMPOSITE'
        && !mpuChecksum.isDefault && expectedTag !== null && !isExternal;
        ...
        [
            ...
            function retrieveParts(
                ...
                return services.getMPUparts(mpuBucket.getName(), uploadId, log, (err, result) => {
                    ...
                    const isExternalMpu = !!constants.externalBackends[
                        config.getLocationConstraintType(location)];
                    const checksumErr = validatePerPartChecksums(
                        jsonList, storedParts, splitter, mpuChecksum, isExternalMpu);
                    if (checksumErr) {

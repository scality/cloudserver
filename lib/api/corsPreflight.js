...
function corsPreflight(request, log, callback) {
    ...
    if (!bucketName) {
        if (corsMethod !== 'GET' || objectKey !== undefined) {
            const errMsg = 'CORSResponse: Bucket not found';
            const err = errors.AccessForbidden.customizeDescription(errMsg);
            return callback(err);
        }
        // preflight cors request for a get service call
        const corsRule = {
            allowedMethods: ['GET'],
            allowedOrigins: ['*'],
        };

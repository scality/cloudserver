...
const { findCorsRule, generateCorsResHeaders } = require('./apiUtils/object/corsResponse');
...
function corsPreflight(request, log, callback) {
    ...
    return metadata.getBucket(bucketName, log, (err, bucket) => {
        ...
        if (!corsRules) {
            const err = errorInstances.AccessForbidden.customizeDescription(customizedErrs.corsNotEnabled);
        ...
        const corsRule = findCorsRule(corsRules, corsOrigin, corsMethod, corsHeaders);

        if (!corsRule) {
            const err = errorInstances.AccessForbidden.customizeDescription(customizedErrs.notAllowed);
        ...
        const resHeaders = generateCorsResHeaders(corsRule, corsOrigin, corsMethod, corsHeaders, true);

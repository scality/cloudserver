...
const { isColdStorageClass, validateStorageClass } = require('./apiUtils/object/storageClass');
...
function _prepMetadata(
    ...
    if (
        ...
        return {
            error: errorInstances.InvalidRequest.customizeDescription(
                'This copy request is illegal because it is trying to copy an object to itself without changing ' +
    ...
    // The storage class itself is already carried over through overrideMetadata.
    if (isColdStorageClass(headers['x-amz-storage-class'])) {
        storeMetadataParams.transitionInProgress = true;
    }
    ...
    if (areChecksumsEnabled() && sourceObjMD.checksum && !_shouldRecomputeChecksum(headers, sourceObjMD)) {
        storeMetadataParams.checksum = {
...
function objectCopy(authInfo, request, sourceBucket, sourceObject, sourceVersionId, log, callback) {
    ...
    if (storageClassError) {
        ...
        monitoring.promMetrics('PUT', destBucketName, storageClassError.code, 'copyObject');
    ...
    const { error: checksumAlgoErr, algorithm: requestedAlgo } = areChecksumsEnabled()
        ? getCopyObjectChecksumAlgorithm(request.headers)
        : { error: null, algorithm: null };
    if (checksumAlgoErr) {

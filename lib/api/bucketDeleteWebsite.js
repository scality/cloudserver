...
function bucketDeleteWebsite(authInfo, request, log, callback) {
    ...
    return metadata.getBucket(bucketName, log, (err, bucket) => {
        ...
        if (err) {
            ...
            monitoring.promMetrics('DELETE', bucketName, err.code, 'deleteBucketWebsite');
            return callback(err);
        }
        if (bucketShield(bucket, requestType)) {
            monitoring.promMetrics('DELETE', bucketName, 404, 'deleteBucketWebsite');
        ...
        if (
            !isBucketAuthorized(
                bucket,
                request.apiMethods || requestType,
                canonicalID,
                authInfo,
                log,
                request,
            ...
            monitoring.promMetrics('DELETE', bucketName, 403, 'deleteBucketWebsite');
        ...
        return metadata.updateBucket(bucketName, bucket, log, err => {
            ...
            monitoring.promMetrics('DELETE', bucketName, '200', 'deleteBucketWebsite');
            return callback(null, corsHeaders);
        });
    });
}

...
const { getReplicationConfigurationXML } = require('./apiUtils/bucket/getReplicationConfiguration');
...
function bucketGetReplication(authInfo, request, log, callback) {
    ...
    return standardMetadataValidateBucket(metadataValParams, request.actionImplicitDenies, log, (err, bucket) => {
        ...
        if (err) {
            ...
            monitoring.promMetrics('GET', bucketName, err.code, 'getBucketReplication');
        ...
        if (!replicationConfig) {
            ...
            monitoring.promMetrics('GET', bucketName, 404, 'getBucketReplication');
            return callback(errors.ReplicationConfigurationNotFoundError, null, corsHeaders);
        }
        ...
        monitoring.promMetrics('GET', bucketName, '200', 'getBucketReplication');

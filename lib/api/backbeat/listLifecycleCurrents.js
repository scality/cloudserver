...
function handleResult(listParams, requestMaxKeys, authInfo, bucketName, list, isBucketVersioned, log, callback) {
...
function listLifecycleCurrents(authInfo, locationConstraints, request, log, callback) {
    ...
    if (excludedDataStoreName && !locationConstraints[excludedDataStoreName]) {
        ...
        return callback(errorInstances.InvalidLocationConstraint.customizeDescription(errMsg));
    }
    ...
    return standardMetadataValidateBucket(metadataValParams, request.actionImplicitDenies, log, (err, bucket) => {
        if (err) {
            ...
            monitoring.promMetrics('GET', bucketName, err.code, 'listLifecycleCurrents');

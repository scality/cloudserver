...
function _constructJSON(xmlParams) {
    ...
    for (let i = 0; i < json.Uploads.length; i += 1) {
        let key = json.Uploads[i].Key;
        ...
        const contentItem = {
            "Upload": [
                {"Key": key},
                {"UploadId": json.Uploads[i].UploadId},
                {"Initiator": [
                    {"ID": json.Uploads[i].Initiator.ID},
                    {"DisplayName": json.Uploads[i].Initiator.DisplayName}
                ]},
                {"Owner": [
                    {"ID": json.Uploads[i].Owner.ID},
                    {"DisplayName": json.Uploads[i].Owner.DisplayName}
                ]},
            ]
        };
    ...
    if (contents.length > 0) {
        Array.prototype.push.apply(listMPUResultArray, contents);
    }

    const commonPrefixes = [];

    for (let i = 0; i < json.CommonPrefixes.length; i ++) {
        const prefixItem = {
...
export default function listMultipartUploads(accessKey,
    ...
    const metadataValParams = {
        ...
        // have access. They do no address ACLs at all.  It seems inconsisent
    ...
    async.waterfall([
        ...
        function watterfall2(bucket, extraArgumentFromPreviousFunction, next) {
            services.getMultipartUploadListing(bucket, listingParams, next);
        }

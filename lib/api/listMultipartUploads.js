    ...
    const contents = json.Uploads.map((upload) => {
        ...
        return {
            Upload: [
                ...
                // to replace back.
                {Initiated: upload.Initiated.replace(/::/g, '-')
                    .replace(/:::/g, '.')}
            ]
        };
    });
...
export default function listMultipartUploads(accessKey,
    metastore, request, log, callback) {
    const query = request.query;
    const bucketName = request.bucketName;
    const host = request.parsedHost;
    ...
    async.waterfall([
        ...
        function waterfall2(next) {
            services.getMultipartUploadListing(metastore, mpuBucketName,

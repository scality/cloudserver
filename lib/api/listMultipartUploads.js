    ...
    async.waterfall(
        [
            ...
            function getMPUBucket(bucket, next) {
                services.getMPUBucket(bucket, bucketName, log, (err, mpuBucket) => next(err, bucket, mpuBucket));
            },
            function waterfall2(bucket, mpuBucket, next) {
                ...
                let maxUploads = query['max-uploads'] !== undefined ? Number.parseInt(query['max-uploads'], 10) : 1000;
                if (maxUploads < 0) {
                    monitoring.promMetrics('GET', bucketName, 400, 'listMultipartUploads');
                ...
                services.getMultipartUploadListing(mpuBucketName, listingParams, log, (err, list) =>
                    next(err, bucket, list),
                );
                return undefined;
            },
        ],
        (err, bucket, list) => {
            ...
            if (err) {
                monitoring.promMetrics('GET', bucketName, err.code, 'listMultipartUploads');
            ...
            monitoring.promMetrics('GET', bucketName, '200', 'listMultipartUploads');

...
function bucketGetLifecycle(authInfo, request, log, callback) {
    ...
    return standardMetadataValidateBucket(metadataValParams, request.actionImplicitDenies, log, (err, bucket) => {
        ...
        if (err) {
            ...
            monitoring.promMetrics('GET', bucketName, err.code, 'getBucketLifecycle');
        ...
        if (!lifecycleConfig) {
            ...
            monitoring.promMetrics('GET', bucketName, 404, 'getBucketLifecycle');
            return callback(errors.NoSuchLifecycleConfiguration, null, corsHeaders);
        }

...
function generateXml(xml, owner, userBuckets, splitter) {
    ...
    userBuckets.forEach(bucket => {
        ...
        xml.push(
            '<Bucket>',
            `<Name>${key}</Name>`,
            `<CreationDate>${bucket.value.creationDate}` + '</CreationDate>',
            '</Bucket>',
        );
    });
...
function serviceGet(authInfo, request, log, callback) {
    ...
    if (authInfo.isRequesterPublicUser()) {
        ...
        monitoring.promMetrics('GET', request.bucketName, 403, 'getService');
    ...
    return services.getService(authInfo, request, log, constants.splitter, (err, userBuckets, splitter) => {
        if (err) {
            monitoring.promMetrics('GET', userBuckets, err.code, 'getService');
            return callback(err);
        }
        ...
        return callback(null, generateXml(xml, canonicalId, userBuckets, splitter));
    });
}

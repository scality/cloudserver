...
// TODO: Remove once users have removed up DBs with old splitter
const legacySplitter = constants.legacySplitter;
...
function _constructJSON(infoToConvert) {
    ...
    const buckets = userBuckets.map(userBucket => {
        // TODO: Remove once users have removed DBs with old splitter
        const functionalSplitter = userBucket.key.indexOf(splitter) > -1 ?
            splitter : legacySplitter;
        return {
            Bucket: [
                { Name: userBucket.key.split(functionalSplitter)[1] },

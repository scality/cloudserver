...
function objectPost(authInfo, request, streamingV4Params, log, callback) {
    //log.debug('processing request', { method: 'objectPost' });
    // TODO probably move parsing to before Auth check to use the accesskey in the req
    ...
    const bucketName = request.parsedFormData.bucket;
    const objectKey = request.parsedFormData.key;
    ...
    // TODO add checksum header check
    ...
    (err, bucket, objMD) => {
        // TODO NOTE where does headers origin come from?
        const responseHeaders = collectCorsHeaders(headers.origin,
            method, bucket);
        // TODO remove access denied pass
        if (err && !err.AccessDenied) {
            log.trace('error processing request', {
                error: err,
                method: 'metadataValidateBucketAndObj',
            });
            monitoring.promMetrics('POST', bucketName, err.code, 'postObject');
        ...
        if (bucket.hasDeletedFlag() && canonicalID !== bucket.getOwner()) {
            ...
            monitoring.promMetrics('POST', bucketName, 404, 'postObject');
        ...
        return async.waterfall([
            ...
            // TODO check SSE and Cipher bundle steps
            ...
            if (err) {
                monitoring.promMetrics('POST', bucketName, err.code,
                    'postObject');
                return callback(err, responseHeaders);
            }
            ...
            monitoring.promMetrics('POST', bucketName, '204',

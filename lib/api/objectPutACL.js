 ...
 * @returns {function} cb - returns function with error and result
 */
export default function objectPutACL(accessKey, metastore, request, log, cb) {
    log.debug('Processing the request in Object PUT ACL api');
    ...
    const grantReadHeader =
        utils.parseGrant(request.lowerCaseHeaders['x-amz-grant-read'], 'READ');
    const grantReadACPHeader =
        utils.parseGrant(request.lowerCaseHeaders['x-amz-grant-read-acp'],
                         'READ_ACP');
    const grantWriteACPHeader = utils.parseGrant(
        request.lowerCaseHeaders['x-amz-grant-write-acp'], 'WRITE_ACP');
    const grantFullControlHeader = utils.parseGrant(
        request.lowerCaseHeaders['x-amz-grant-full-control'], 'FULL_CONTROL');
    ...
    async.waterfall([
        ...
        function waterfall2(bucket, objectMD, next) {
            ...
            if (newCannedACL === undefined
                ...
                return parseString(xmlToParse, function parseXML(err, result) {
                    if (err) {
                        log.error(`Invalid XML: ${xmlToParse}`);
        ...
        if (err) {
            log.error(`Error processing request in Object PUT ACL api: ${err}`);
            return cb(err);
        }
        log.debug('Processed request successfully in Object PUT ACL api');
        return cb();
    });
}

...
function objectPutACL(authInfo, request, log, cb) {
    ...
    const possibleGroups = [constants.publicId, constants.allAuthedUsersId, constants.logId];
    ...
    const grantReadACPHeader = aclUtils.parseGrant(request.headers['x-amz-grant-read-acp'], 'READ_ACP');
    const grantWriteACPHeader = aclUtils.parseGrant(request.headers['x-amz-grant-write-acp'], 'WRITE_ACP');
    const grantFullControlHeader = aclUtils.parseGrant(request.headers['x-amz-grant-full-control'], 'FULL_CONTROL');
        ...
        [
                ...
                return standardMetadataValidateBucketAndObj(
                    ...
                    (err, bucket, objectMD) => {
                        ...
                        if (objectMD.isDeleteMarker) {
                            log.trace('delete marker detected', { method: 'objectPutACL' });
            ...
            function parseAclFromXml(bucket, objectMD, next) {
                ...
                if (
                    ...
                    if (request.post) {
                        ...
                        return aclUtils.parseAclXml(request.post, log, (err, jsonGrants, aclOwnerID) =>
                            next(err, bucket, objectMD, jsonGrants, aclOwnerID),
                        );
                    }
            ...
            function processAcls(bucket, objectMD, jsonGrants, aclOwnerID, next) {
                ...
                if (jsonGrants) {
                    ...
                    jsonGrants.forEach(grant => {
                        ...
                        if (!skip && granteeType === 'Group') {
                            if (possibleGroups.indexOf(grantee.URI[0]) < 0) {
                                log.trace('invalid user group', { userGroup: grantee.URI[0] });
                                hasError = true;
                    ...
                    usersIdentifiedByEmail = allGrantHeaders.filter(
                        item => item && item.userIDType.toLowerCase() === 'emailaddress',
                    );
                    usersIdentifiedByGroup = allGrantHeaders.filter(
                        itm => itm && itm.userIDType.toLowerCase() === 'uri',
                    );
                    for (let i = 0; i < usersIdentifiedByGroup.length; i++) {
                        if (possibleGroups.indexOf(usersIdentifiedByGroup[i].identifier) < 0) {
                            log.trace('invalid user group', { userGroup: usersIdentifiedByGroup[i].identifier });
                            return next(errors.InvalidArgument, bucket);
                        }
                    }
                    ...
                    usersIdentifiedByID = allGrantHeaders.filter(
                        item => item && item.userIDType.toLowerCase() === 'id',
                    );
                }
                ...
                if (justEmails.length > 0) {
                    return vault.getCanonicalIds(justEmails, log, (err, results) => {
                        if (err) {
                            log.trace('error looking up canonical ids', { error: err, method: 'getCanonicalIDs' });
                            return next(err, bucket);
                        }
            ...
            function addAclsToObjMD(bucket, objectMD, ACLParams, next) {
                ...
                acl.addObjectACL(bucket, objectKey, objectMD, ACLParams, params, log, err =>
                    next(err, bucket, objectMD),
                );
            },
        ],
        (err, bucket, objectMD) => {
            const resHeaders = collectCorsHeaders(request.headers.origin, request.method, bucket);
            if (err) {
                log.trace('error processing request', {
                    error: err,
                    method: 'objectPutACL',
                });
                monitoring.promMetrics('PUT', bucketName, err.code, 'putObjectAcl');

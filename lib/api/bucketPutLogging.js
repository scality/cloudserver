...
function bucketPutLogging(authInfo, request, log, callback) {
    ...
    return waterfall(
        [
            ...
            (bucket, next) => {
                ...
                return metadata.getBucket(loggingEnabled.TargetBucket, log, (err, targetBucket) => {
                    ...
                    if (targetBucket.getOwner() !== bucket.getOwner()) {
                        return next(
                            errorInstances.InvalidTargetBucketForLogging.customizeDescription(
                                ERROR_MSG_SOURCE_TARGET_BUCKET_OWNER_MISMATCH,
                            ),
                        );
                    }
            ...
            (bucket, next) =>
                checkExpectedBucketOwner(request.headers, bucket, log, err => {
                    if (err) {
                        return next(err);
                    }

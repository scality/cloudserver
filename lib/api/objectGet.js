...
function objectGet(authInfo, request, returnTagCount, log, callback) {
    ...
    (err, bucket, objMD) => {
        ...
        if (objMD.archive && !objMD.archive.restoreRequestedAt) {
            const error = errors.InvalidObjectState;
            monitoring.promMetrics(
                'GET', bucketName, error.code, 'getObject');
            return callback(error, null, corsHeaders);
        } else if (objMD.archive && objMD.archive.restoreRequestedAt &&
                !objMD.archive.restoreCompletedAt) {
            const error = errors.RestoreAlreadyInProgress;
            monitoring.promMetrics(
                'GET', bucketName, error.code, 'getObject');
            return callback(error, null, corsHeaders);
        }
        ...
        if (locationConstraints[bucket.getLocationConstraint()]?.cold === true) {

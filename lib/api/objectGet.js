...
// Promisified versions of functions for better performance
const standardMetadataValidateBucketAndObjAsync = promisify(
    (valParams, actionImplicitDenies, log, callback) => 
        standardMetadataValidateBucketAndObj(valParams, actionImplicitDenies, log, callback));

const dataHeadAsync = promisify((dataLocator, log, callback) => 
    data.head(dataLocator, log, callback));

const createDecipherBundleAsync = promisify((serverSideEncryption, offset, log, callback) => 
    kms.createDecipherBundle(serverSideEncryption, offset, log, callback));
...
function objectGet(authInfo, request, returnTagCount, log, callback) {
    ...
    // If parallel auth is enabled, we can directly return because auth has already been
    // processed in the callApiMethodWithParallelAuth function
    if (isParallelAuthEnabled) {
        log.debug('using parallel auth, skipping auth check in objectGet');
        return objectGetWithoutAuth(authInfo, request, returnTagCount, log, callback);
    }

...
const locationHeaderCheck = require('./apiUtils/object/locationHeaderCheck');
const getReplicationBackendDataLocator = require('./apiUtils/object/getReplicationBackendDataLocator');
...
function objectGet(authInfo, request, returnTagCount, log, callback) {
    ...
    const locCheckResult = locationHeaderCheck(request.headers, objectKey, bucketName);
    ...
    return standardMetadataValidateBucketAndObj(mdValParams, request.actionImplicitDenies, log, (err, bucket, objMD) =>
        updateEncryption(err, bucket, objMD, objectKey, log, {}, (err, bucket, objMD) => {
            ...
            if (err) {
                ...
                monitoring.promMetrics('GET', bucketName, err.code, 'getObject');
            ...
            if (coldErr) {
                monitoring.promMetrics('GET', bucketName, coldErr.code, 'getObject');
                return callback(coldErr, null, corsHeaders);
            }
            if (objMD.isDeleteMarker) {
                const responseMetaHeaders = Object.assign({}, { 'x-amz-delete-marker': true }, corsHeaders);
                if (!versionId) {
                    monitoring.promMetrics('GET', bucketName, 404, 'getObject');
                ...
                responseMetaHeaders['x-amz-version-id'] = getVersionIdResHeader(verCfg, objMD);
                monitoring.promMetrics('GET', bucketName, 405, 'getObject');
                return callback(errors.MethodNotAllowed, null, responseMetaHeaders);
            }
            const headerValResult = validateHeaders(request.headers, objMD['last-modified'], objMD['content-md5']);
            ...
            const responseMetaHeaders = collectResponseHeaders(
                ...
                corsHeaders,
                verCfg,
                returnTagCount && objMD.returnTagCount,
            ); // IAM and Bucket policy should both authorize tagging.
            ...
            const objLength = objMD.location === null ? 0 : parseInt(objMD['content-length'], 10);
            ...
            if (request.headers.range) {
                const { range, error } = parseRange(request.headers.range, objLength);
                if (error) {
                    monitoring.promMetrics('GET', bucketName, 400, 'getObject');
                ...
                if (range) {
                    ...
                    responseMetaHeaders['Content-Length'] = range[1] - range[0] + 1;
                    responseMetaHeaders['Content-Range'] = `bytes ${range[0]}-${range[1]}/${objLength}`;
                    streamingParams.rangeStart =
                        range[0] || typeof range[0] === 'number' ? range[0].toString() : undefined;
                    streamingParams.rangeEnd = range[1] ? range[1].toString() : undefined;
                }
            }
            ...
            if (objMD.location !== null) {
                ...
                dataLocator = Array.isArray(objMD.location) ? objMD.location : [{ key: objMD.location }];

                const repConf = bucket.getReplicationConfiguration();
                const prefReadLocation = repConf && repConf.preferredReadLocation;
                const prefReadDataLocator = checkReadLocation(config, prefReadLocation, objectKey, bucketName);
                const targetLocation = locCheckResult || prefReadDataLocator || null;

                if (targetLocation && targetLocation.location !== objMD.dataStoreName) {
                    const repBackendResult = getReplicationBackendDataLocator(targetLocation, objMD.replicationInfo);
                    ...
                    if (targetDataLocator) {
                        ...
                        log.debug('using source location as preferred read ' + 'is unavailable', {
                            bucketName,
                            objectKey,
                            versionId,
                ...
                if (request.query && request.query.partNumber !== undefined) {
                    if (byteRange) {
                        const error = errorInstances.InvalidRequest.customizeDescription(
                    ...
                    if (Number.isNaN(partNumber)) {
                        const error = errorInstances.InvalidArgument.customizeDescription(
                ...
                if (byteRange && dataLocator.length > 1 && dataLocator[0].start === undefined) {
                    monitoring.promMetrics('GET', bucketName, 501, 'getObject');
                ...
                if (objMD['x-amz-server-side-encryption']) {
                    for (let i = 0; i < dataLocator.length; i++) {
                        dataLocator[i].masterKeyId = objMD['x-amz-server-side-encryption-aws-kms-key-id'];
                        dataLocator[i].algorithm = objMD['x-amz-server-side-encryption'];
                    }
                }
                if (partNumber) {
                    const locations = [];
                    ...
                    for (let i = 0; i < objMD.location.length; i++) {
                        ...
                        if (dataStoreETag) {
                            locationPartNumber = Number.parseInt(dataStoreETag.split(':')[0], 10);
                        } else {
                            /**
                    ...
                    if (partsCount) {
                        responseMetaHeaders['x-amz-mp-parts-count'] = partsCount;
                    }
                } else {
            ...
            if (checksumMode === 'ENABLED' && !byteRange && !partNumber) {
                ...
                if (checksum) {
                    responseMetaHeaders[`x-amz-checksum-${checksum.checksumAlgorithm}`] = checksum.checksumValue;
            ...
            return async.each(
                ...
                (objectGetInfo, next) => {
                    ...
                    const serverSideEncryption = {
                        ...
                        cipheredDataKey: Buffer.from(objectGetInfo.cipheredDataKey, 'base64'),
                    };
                    const offset = objectGetInfo.range ? objectGetInfo.range[0] : 0;
                    return kms.createDecipherBundle(serverSideEncryption, offset, log, (err, decipherBundle) => {
                        if (err) {
                            log.error('cannot get decipher bundle from kms', { method: 'objectGet' });
                            return next(err);
                        }
                        // eslint-disable-next-line no-param-reassign
                    ...
                    return data.head(dataLocator, log, err => {
                        if (err) {
                            if (!err.is.LocationNotFound) {
                                log.error('error from external backend checking for ' + 'object existence', {
                                    error: err,
                                });
                            }
                            monitoring.promMetrics('GET', bucketName, err.code, 'getObject');
                        ...
                        pushMetric('getObject', log, {
                            ...
                            bucket: bucketName,
                            keys: [objectKey],
                            newByteLength: Number.parseInt(responseMetaHeaders['Content-Length'], 10),

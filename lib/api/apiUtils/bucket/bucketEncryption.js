...
function parseEncryptionXml(xml, log, cb) {
    return parseString(xml, (err, parsed) => {
        ...
        if (!parsed || !parsed.ServerSideEncryptionConfiguration || !parsed.ServerSideEncryptionConfiguration.Rule) {
...
function createDefaultBucketEncryptionMetadata(bucket, log, cb) {
    return kms.bucketLevelEncryption(bucket, { algorithm: 'AES256', mandatory: false }, log, (error, sseConfig) => {
        if (error) {
            return cb(error);
        }

...
const { WebsiteConfiguration } = require('arsenal').models.WebsiteConfiguration;
...
const redirectValuesToCheck = ['HostName', 'ReplaceKeyPrefixWith', 'ReplaceKeyWith'];
...
function xmlContainsElem(parent, requiredElem, options) {
    ...
    const validateParent = options && options.validateParent !== undefined ? options.validateParent : true;
    ...
    if (isList) {
        if (!Array.isArray(parent[0][requiredElem]) || parent[0][requiredElem].length === 0) {
            return false;
        }
    } else {
...
function _validateWebsiteConfigXml(parsingResult) {
    ...
    if (!parsingResult.IndexDocument && !parsingResult.RedirectAllRequestsTo) {
        errMsg = 'Value for IndexDocument Suffix must be provided if RedirectAllRequestsTo is empty';
    ...
    if (parsingResult.RedirectAllRequestsTo) {
        ...
        if (parsingResult.IndexDocument || parsingResult.ErrorDocument || parsingResult.RoutingRules) {
            errMsg = 'RedirectAllRequestsTo cannot be provided in conjunction with other Routing Rules.';
        ...
        if (xmlContainsElem(parent, 'Protocol', { validateParent: false })) {
            if (parent[0].Protocol[0] !== 'http' && parent[0].Protocol[0] !== 'https') {
                errMsg =
                    'Invalid protocol, protocol can be http or https. ' +
    ...
    if (parsingResult.IndexDocument) {
        ...
        if (!xmlContainsElem(parent, 'Suffix')) {
            ...
            return errorInstances.MalformedXML.customizeDescription(errMsg);
        } else if (!_isValidString(parent[0].Suffix[0]) || parent[0].Suffix[0].indexOf('/') !== -1) {
    ...
    if (parsingResult.RoutingRules) {
        ...
        for (let i = 0; i < parent[0].RoutingRule.length; i++) {
            ...
            if (!_isValidElem(rule.Redirect)) {
                errMsg = 'RoutingRule requires Redirect, which is missing or not well-formed';
                return errorInstances.MalformedXML.customizeDescription(errMsg);
            }
            ...
            if (
                ...
                errMsg =
                    'Redirect must contain at least one of following: Protocol, HostName, ' +
                    'ReplaceKeyPrefixWith, ReplaceKeyWith, or HttpRedirectCode element';
                return errorInstances.MalformedXML.customizeDescription(errMsg);
            }
            if (rule.Redirect[0].Protocol) {
                if (
                    !_isValidElem(rule.Redirect[0].Protocol) ||
                    (rule.Redirect[0].Protocol[0] !== 'http' && rule.Redirect[0].Protocol[0] !== 'https')
                ) {
                    errMsg =
                        'Invalid protocol, protocol can be http or https. If not ' +
                        'defined, the protocol will be selected automatically.';
                    return errorInstances.InvalidRequest.customizeDescription(errMsg);
                }
                ruleObj.redirect.protocol = rule.Redirect[0].Protocol[0];
            }
            if (rule.Redirect[0].HttpRedirectCode) {
                errMsg = 'The provided HTTP redirect code is not valid. It should be a string containing a number.';
                ...
                if (!(code > 300 && code < 400)) {
                    errMsg =
                        `The provided HTTP redirect code (${code}) is ` + 'not valid. Valid codes are 3XX except 300';
                    return errorInstances.InvalidRequest.customizeDescription(errMsg);
                }
            ...
            for (let j = 0; j < redirectValuesToCheck.length; j++) {
                ...
                if (elem) {
                    ...
                    ruleObj.redirect[`${elemName.charAt(0).toLowerCase()}` + `${elemName.slice(1)}`] = elem[0];
                }
            }
            if (
                xmlContainsElem(rule.Redirect, ['ReplaceKeyPrefixWith', 'ReplaceKeyWith'], {
                    validateParent: false,
                    checkForAll: true,
                })
            ) {
                errMsg = 'Redirect must not contain both ReplaceKeyWith and ReplaceKeyPrefixWith';
                return errorInstances.InvalidRequest.customizeDescription(errMsg);
            }
            if (Array.isArray(rule.Condition) && rule.Condition.length === 1) {
                ruleObj.condition = {};
                if (!xmlContainsElem(rule.Condition, ['KeyPrefixEquals', 'HttpErrorCodeReturnedEquals'])) {
                    errMsg =
                        'Condition is not well-formed. Condition should contain ' +
                        'valid KeyPrefixEquals or HttpErrorCodeReturnEquals element.';
                    return errorInstances.InvalidRequest.customizeDescription(errMsg);
                }
                if (rule.Condition[0].KeyPrefixEquals) {
                    const keyPrefixEquals = rule.Condition[0].KeyPrefixEquals;
                    if (!_isValidElem(keyPrefixEquals) || !_isValidString(keyPrefixEquals[0])) {
                ...
                if (rule.Condition[0].HttpErrorCodeReturnedEquals) {
                    errMsg = 'The provided HTTP error code is not valid. It should be a string containing a number.';
                    if (!_isValidElem(rule.Condition[0].HttpErrorCodeReturnedEquals)) {
                        return errorInstances.MalformedXML.customizeDescription(errMsg);
                    }
                    const code = parseInt(rule.Condition[0].HttpErrorCodeReturnedEquals[0], 10);
                    if (Number.isNaN(code)) {
                        return errorInstances.MalformedXML.customizeDescription(errMsg);
                    }
                    if (!(code > 399 && code < 600)) {
                        errMsg =
                            `The provided HTTP error code (${code}) is ` + 'not valid. Valid codes are 4XX or 5XX.';
                        return errorInstances.InvalidRequest.customizeDescription(errMsg);
                    }
...
function parseWebsiteConfigXml(xml, log, cb) {
    parseString(xml, (err, result) => {
        ...
        const validationRes = _validateWebsiteConfigXml(result.WebsiteConfiguration);
...
function convertToXml(config) {
    ...
    function _pushChildren(obj) {
        Object.keys(obj).forEach(element => {
            const xmlElem = `${element.charAt(0).toUpperCase()}` + `${element.slice(1)}`;
    ...
    if (indexDocument) {
        xml.push('<IndexDocument>', `<Suffix>${escapeForXml(indexDocument)}</Suffix>`, '</IndexDocument>');
    }
    if (errorDocument) {
        xml.push('<ErrorDocument>', `<Key>${escapeForXml(errorDocument)}</Key>`, '</ErrorDocument>');
    }
    if (redirectAllRequestsTo) {
        xml.push('<RedirectAllRequestsTo>');
        if (redirectAllRequestsTo.hostName) {
            xml.push('<HostName>', `${escapeForXml(redirectAllRequestsTo.hostName)}`, '</HostName>');
        }
        if (redirectAllRequestsTo.protocol) {
            xml.push('<Protocol>', `${redirectAllRequestsTo.protocol}`, '</Protocol>');
        }
        xml.push('</RedirectAllRequestsTo>');
    }
    if (routingRules) {

...
export function deleteBucket(bucketMD, bucketName, canonicalID, log, cb) {
    ...
    return async.waterfall([
        function checkForObjectsStep(next) {
            const params = { maxKeys: 1 };
            const versioning = bucketMD.getVersioningConfiguration();
            if (versioning) {
                params.listingType = 'allversions';
            }
            return metadata.listObject(bucketName, params, log,
                (err, objectsListRes) => {
                    ...
                    if (versioning) {
                        log.info('listing result', { objectsListRes });
                    }
                    const length = versioning ?
                        objectsListRes.Versions.length :
                        objectsListRes.Contents.length;
                    if (length) {

...
const { splitter, oldSplitter, mpuBucketPrefix } = require('../../../../constants');
...
function _deleteMPUbucket(destinationBucketName, log, cb) {
    const mpuBucketName = `${mpuBucketPrefix}${destinationBucketName}`;
...
function _deleteOngoingMPUs(authInfo, bucketName, bucketMD, mpus, request, log, cb) {
    async.mapLimit(
        ...
        1,
        (mpu, next) => {
            const splitterChar = mpu.key.includes(oldSplitter) ? oldSplitter : splitter;
    ...
    return async.waterfall(
        [
            function checkForObjectsStep(next) {
                ...
                return metadata.listObject(bucketName, params, log, (err, list) => {
                    if (err) {
                        log.error('error from metadata', { error: err });
                        return next(err);
                    }
                    const length =
                        (list.Versions ? list.Versions.length : 0) +
                        (list.DeleteMarkers ? list.DeleteMarkers.length : 0);
                    log.debug('listing result', { length });
                    if (length) {
                        log.debug('bucket delete failed', { error: errors.BucketNotEmpty });
            ...
            function deleteMPUbucketStep(next) {
                ...
                return metadata.listObject(MPUBucketName, { prefix: 'overview' }, log, (err, objectsListRes) => {
                    ...
                    if (objectsListRes.Contents.length) {
                        return _deleteOngoingMPUs(
                            authInfo,
                            bucketName,
                            bucketMD,
                            objectsListRes.Contents,
                            request,
                            log,
                            err => {
                                if (err) {
        ...
        function actualDeletionStep(err) {
            ...
            return metadata.deleteBucket(bucketName, log, err => {
                ...
                const isScalityManagedEncryptionKey =
                    serverSideEncryption && serverSideEncryption.algorithm === 'AES256';

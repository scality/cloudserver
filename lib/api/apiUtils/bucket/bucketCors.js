...
const _validator = {
    ...
    */
    ...
    validateNumberRules(length) {
        if (length > 100) {
            return errorInstances.InvalidRequest.customizeDescription(customizedErrs.numberRules);
        }
        return true;
    },
    ...
    validateOriginAndMethodExist(allowedMethods, allowedOrigins) {
        ...
        return errorInstances.MalformedXML.customizeDescription(customizedErrs.originAndMethodExist);
    },
    ...
    validateMethods(methods) {
        ...
        if (!methods.every(isValidMethod)) {
            const errMsg = `Found unsupported HTTP method in CORS config. Unsupported method is "${invalidMethod}"`;
    ...
    validateAllowedOriginsOrHeaders(elementArr, typeElement) {
        for (let i = 0; i < elementArr.length; i++) {
            ...
            if (!this.validateNumberWildcards(element)) {
                const errMsg = `${typeElement} "${element}" can not have more than one wildcard.`;
                return errorInstances.InvalidRequest.customizeDescription(errMsg);
            }
        }
        return true;
    ...
    validateAllowedHeaders(headers) {
        ...
        const result = this.validateAllowedOriginsOrHeaders(headers, 'AllowedHeader');
    ...
    validateExposeHeaders(headers) {
        ...
        for (let i = 0; i < headers.length; i++) {
            ...
            if (header.indexOf('*') !== -1) {
                const errMsg =
                    `ExposeHeader ${header} contains a wildcard. ` +
                    'Wildcards are currently not supported for ExposeHeader.';
                return errorInstances.InvalidRequest.customizeDescription(errMsg);
            }
            if (!/^[A-Za-z0-9-]*$/.test(header)) {
                const errMsg = `ExposeHeader ${header} contains invalid character.`;
                return errorInstances.InvalidRequest.customizeDescription(errMsg);
            }
        }
        return true;
...
function _validateCorsXml(rules) {
    ...
    for (let i = 0; i < rules.length; i++) {
        ...
        result = _validator.validateOriginAndMethodExist(rule.AllowedMethod, rule.AllowedOrigin);
        if (result instanceof Error) {
            return result;
        }
        ...
        result = _validator.validateAllowedOriginsOrHeaders(rule.AllowedOrigin, 'AllowedOrigin');
        if (result instanceof Error) {
            return result;
        }
...
function parseCorsXml(xml, log, cb) {
    parseString(xml, (err, result) => {
        ...
        const validationRes = _validateCorsXml(result.CORSConfiguration.CORSRule);
...
function convertToXml(arrayRules) {
    const xml = [];
    xml.push('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>', '<CORSConfiguration>');
    arrayRules.forEach(rule => {
        xml.push('<CORSRule>');
        ['allowedMethods', 'allowedOrigins', 'allowedHeaders', 'exposeHeaders'].forEach(key => {
            if (rule[key]) {
                const element = key.charAt(0).toUpperCase() + key.slice(1, -1);
                rule[key].forEach(value => {
                    xml.push(`<${element}>${escapeForXml(value)}</${element}>`);
                });
            }
        });
        if (rule.id) {

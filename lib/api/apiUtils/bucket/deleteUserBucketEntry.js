...
const { usersBucket, oldUsersBucket, splitter, oldSplitter } = require('../../../../constants');
const metadata = require('../../../metadata/wrapper');

function deleteUserBucketEntry(bucketName, canonicalID, log, cb) {
    log.trace('deleting bucket name from users bucket', { method: '_deleteUserBucketEntry' });
    const keyForUserBucket = createKeyForUserBucket(canonicalID, splitter, bucketName);
    metadata.deleteObjectMD(usersBucket, keyForUserBucket, {}, log, error => {
        ...
        if (error?.is.NoSuchKey) {
            ...
            const keyForUserBucket2 = createKeyForUserBucket(canonicalID, oldSplitter, bucketName);
            return metadata.deleteObjectMD(oldUsersBucket, keyForUserBucket2, {}, log, error => {
                ...
                log.trace('deleted bucket from user bucket', { method: '_deleteUserBucketEntry' });
                return cb(null);
            });
        } else if (error) {

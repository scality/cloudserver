...
/** decodePutVersionId - decode the version id from x-scal-s3-version-id header
 * @param {string} [versionId] - version id from x-scal-s3-version-id header
 * @return {object}
 * - {Error} err - error from decoding
 * - {string} decodedVid - decoded version ID
 */
function decodePutVersionId(versionId) {
    if (versionId) {
        if (versionId === 'null') {
            return { isPutVersion: true, decodedVid: nonVersionedObjId, isNull: true, err: null };
        }
        const decoded = versionIdUtils.decode(versionId);
        if (decoded instanceof Error) {
            return { isPutVersion: false, decodedVid: null, isNull: false, err: errors.InvalidArgument
                .customizeDescription('Invalid version id specified in x-scal-s3-version-id header') };
        }
        return { isPutVersion: true, decodedVid: decoded, isNull: false, err: null };
    }
    return { isPutVersion: false, decodedVid: undefined, isNull: false, err: null };
}
...
function versioningPreprocessing(bucketName, bucketMD, objectKey, objMD,
    ...
    if (!vCfg) {
        console.log('NON VERSIONED versioningPreprocessing => mst!!!', mst);
...
function overwrittingVersioning(putVersionId, objMD, metadataStoreParams, log) {
    const { err, decodedVid: decodedPutVersionId, isNull } = decodePutVersionId(putVersionId);
    if (err) {
        log.error('invalid x-scal-s3-version-id header value', { error: err, putVersionId });
        return { err };
    }

    // NOTE: only set creationTime when overwritting
    // - the latest version.
    // - the non-versioned object
    metadataStoreParams.creationTime = objMD['creation-time'];

    // CASE 1: overwritting the lastest null version or,
    //         overwritting a non-versioned object.
    if (isNull && (objMD.isNull || !objMD.versionId)) {
        // if masterVersionId specified, we do not generate a new version id when putting object,
        // and use masterVersionId instead.
    ...
    if (isNull) {
        ...
        // NOTE: nullVersionId is not set for a null version.
        options.isNull = true;

        metadataStoreParams.creationTime = undefined;
    } else {
        if (decodedPutVersionId !== objMD.versionId) {
            // NOTE: if overwritting a current version, we keep creation-time.
            metadataStoreParams.creationTime = undefined;
        }

...
function decodeVID(versionId) {
    ...
    let decoded;
    const invalidErr = errorInstances.InvalidArgument.customizeDescription('Invalid version id specified');
    try {
...
function checkQueryVersionId(query) {
    if (query && query.versionId !== undefined) {
        ...
        return errorInstances.InvalidArgument.customizeDescription(customMsg);
    }
    return undefined;
}
...
function versioningPreprocessing(bucketName, bucketMD, objectKey, objMD,
    ...
    if (!nullVersionId && !delOptions) {
        return callback(null, options);
    }
    
    // Build array of operations that actually need to run
    const operations = [];
    
    if (nullVersionId) {
        operations.push(next => _storeNullVersionMD(bucketName, objectKey, nullVersionId, objMD, log, next));
    }
    
    if (delOptions) {
        operations.push(next => _prepareNullVersionDeletion(
                bucketName, objectKey, delOptions, mst, log,
                (err, nullOptions) => {
                    if (err) {
                        return next(err);
                    }
                    // Directly assign properties instead of Object.assign for better performance
                    if (nullOptions.dataToDelete) {
                        options.dataToDelete = nullOptions.dataToDelete;
                    }
                    if (nullOptions.deleteNullKey) {
                        options.deleteNullKey = nullOptions.deleteNullKey;
                    }
                    return next();
                }));
        
        // Only add delete operation if needed
        if (delOptions.versionId && delOptions.versionId !== 'null') {
            operations.push(next => 
                // backward-compat: delete old null versioned key
                 _deleteNullVersionMD(
                    bucketName, objectKey, { versionId: delOptions.versionId, overheadField }, log, next)
            );
        }
    }
    
    return async.series(operations, err => {

...
function findRoutingRule(routingRules, key, errCode) {
    ...
    for (let i = 0; i < routingRules.length; i++) {
        const prefixFromRule = routingRules[i].getCondition().keyPrefixEquals;
        const errorCodeFromRule = routingRules[i].getCondition().httpErrorCodeReturnedEquals;
        if (prefixFromRule !== undefined) {
            ...
            // rule
            const redirectInfo = Object.assign({ prefixFromRule }, routingRules[i].getRedirect());
        ...
        if (errorCodeFromRule !== undefined) {
            if (errCode === errorCodeFromRule) {
                const redirectInfo = Object.assign({}, routingRules[i].getRedirect());
                return redirectInfo;
            }
            continue;
        }
...
function validateWebsiteHeader(header) {
    return !header || header.startsWith('/') || header.startsWith('http://') || header.startsWith('https://');
}

...
const { LifecycleDateTime, LifecycleUtils } = require('arsenal').s3middleware.lifecycleHelpers;
const { config } = require('../../../Config');

const { expireOneDayEarlier, transitionOneDayEarlier, timeProgressionFactor, scaledMsPerDay } = config.getTimeOptions();
...
function _generateExpHeadersObjects(rules, params, datetime) {
    const tags = {
        TagSet: params.tags ? Object.keys(params.tags).map(key => ({ Key: key, Value: params.tags[key] })) : [],
    };
...
function _generateExpHeadresMPU(rules, params, datetime) {
    ...
    if (applicable.AbortIncompleteMultipartUpload) {
        ...
        const date = calculateDate(params.date, rule.DaysAfterInitiation, datetime);

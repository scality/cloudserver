const { errors, errorInstances, auth, policies } = require('arsenal');
...
function calculateRetainUntilDate(retention) {
    ...
    const retainUntilDate = date.add(retainUntilDaysInMs, 'ms');
...
function validateHeaders(bucket, headers, log) {
    ...
    if ((objectLockDate || objectLockMode || objectLegalHold) && !bucketObjectLockEnabled) {
        log.trace('bucket is missing ObjectLockConfiguration');
        return errorInstances.InvalidRequest.customizeDescription('Bucket is missing ObjectLockConfiguration');
    }
    if ((objectLockMode || objectLockDate) && !(objectLockMode && objectLockDate)) {
    ...
    if (objectLockMode && !validModes.has(objectLockMode)) {
        return errorInstances.InvalidArgument.customizeDescription('Unknown wormMode directive');
    }
    const validLegalHolds = new Set(['ON', 'OFF']);
    if (objectLegalHold && !validLegalHolds.has(objectLegalHold)) {
        return errorInstances.InvalidArgument.customizeDescription('Legal hold status must be one of "ON", "OFF"');
    }
    const currentDate = new Date().toISOString();
    if (objectLockMode && objectLockDate <= currentDate) {
        return errorInstances.InvalidArgument.customizeDescription('The retain until date must be in the future!');
    }
    return null;
}
...
function setObjectLockInformation(headers, md, defaultRetention) {
    ...
    const finalObjectLockInfo = compareObjectLockInformation(headers, defaultRetention);
...
function checkUserGovernanceBypass(request, authInfo, bucketMD, objectKey, log, cb) {
    ...
    return vault.checkPolicies(requestContextParams, authInfo.getArn(), log, (err, authorizationResults) => {
        if (err) {
            return cb(err);
        }

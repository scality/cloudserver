...
const createAndStoreObject = require('../object/createAndStoreObject');
    ...
    async.waterfall([
        function checkDestBucketVal(next) {
            metadataValidateBucketAndObj(metadataValParams, log,
                (err, destinationBucket, existingObjectMd) => {
                    if (err) {
                    ...
                    return next(null, destinationBucket, existingObjectMd);
                });
        },
        function checkMPUval(destBucket, existingObjectMd, next) {
            ...
            services.metadataValidateMultipart(metadataValParams,
                (err, mpuBucket, mpuOverviewObj) => {
                    ...
                    return next(err, mpuBucket, mpuOverviewObj, destBucket, existingObjectMd);
                });
        },
        function ifMultipleBackend(mpuBucket, mpuOverviewObj, destBucket,
            existingObjectMd, next) {
                ...
                return multipleBackendGateway.abortMPU(objectKey, uploadId,
                    location, bucketName, log, (err, skipDataDelete) => {
                        ...
                        return next(null, mpuBucket, destBucket,
                            existingObjectMd, skipDataDelete);
                    });
            }
            return next(null, mpuBucket, destBucket, existingObjectMd, false);
        },
        function writeAbortMarker(mpuBucket, destBucket, existingObjectMd,
            skipDataDelete, next) {

            // If no previous object exists we create a new abort marker.
            // If one exists and the uploadId matches the uploadId to be aborted
            // the CompleteMPU is still in progress and we need to overwrite it
            // with an abort marker.
            const objMD = !existingObjectMd || existingObjectMd.getUploadId() === uploadId
                ? new ObjectMD({ isAborted: true })
                : existingObjectMd;

            objMD.setAbortedUploadId(uploadId);

            if (config.backends.data === 'mulitple') {
                // Handle multiplebackends
            } else {
                const stubRequest = {
                    headers: {},

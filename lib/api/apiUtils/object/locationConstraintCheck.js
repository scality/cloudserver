...
function locationConstraintCheck(request, metaHeaders, bucket, log) {
    ...
    if (metaHeaders) {
        objectLocationConstraint = metaHeaders[constants.objectLocationConstraintHeader];
    } else {
        objectLocationConstraint = request.headers[constants.objectLocationConstraintHeader];
    }
    ...
    if (!controllingBackend.isValid) {
        backendInfoObj = {
            err: errorInstances.InvalidArgument.customizeDescription(controllingBackend.description),
        };
        return backendInfoObj;
    }
    const backendInfo = new BackendInfo(
        config,
        objectLocationConstraint,
        bucketLocationConstraint,
        requestEndpoint,

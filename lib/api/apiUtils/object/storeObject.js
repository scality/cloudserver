...
function dataStore(objectContext, cipherBundle, stream, size,
    ...
    // consume all of the dataStream in a buffer
    // const dataStreamBuffer = [];
    // return dataStream.on('data', chunk => {
    //     dataStreamBuffer.push(chunk);
    // }).on('error', err => {
    //     log.error('error processing stream', { error: err });
    //     return cbOnce(err);
    // }).on('end', () => {
    //     const dataBuffer = Buffer.concat(dataStreamBuffer);
    //     console.log('dataBuffer:', dataBuffer);
    //     // save dataBuffer to a file on disk
    //     const fs = require('fs');
    //     fs.writeFile('dataBuffer.txt', dataBuffer, (err) => {
    //         if (err) {
    //             console.error(err);
    //             return;
    //         }

    ...
    return async.waterfall(
        [
            ...
            function validateBucketAndObject(next) {
                return mdUtils.standardMetadataValidateBucketAndObj(
                    mdValueParams,
                    request.actionImplicitDenies,
                    log,
                    (err, bucketMD, objectMD) => {
                        if (err) {
            ...
            function startRestore(bucketMD, objectMD, restoreInfo, next) {
                return coldStorage.startRestore(objectMD, restoreInfo, log, (err, _isObjectRestored) => {
            ...
            function updateObjectMD(bucketMD, objectMD, next) {
                ...
                metadata.putObjectMD(bucketMD.getName(), objectKey, objectMD, params, log, err =>
                    next(err, bucketMD, objectMD)
                );
            },
        ],
        (err, bucketMD) => {
            ...
            if (err) {
                ...
                monitoring.promMetrics('POST', bucketName, err.code, 'restoreObject');
            ...
            if (isObjectRestored) {
                monitoring.promMetrics('POST', bucketName, '200', 'restoreObject');
                return callback(null, 200, responseHeaders);
            }
            monitoring.promMetrics('POST', bucketName, '202', 'restoreObject');

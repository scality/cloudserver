...
function findCorsRule(rules, origin, method, headers) {
    return rules.find(rule => {
        if (rule.allowedMethods.indexOf(method) === -1) {
            return false;
        } else if (!rule.allowedOrigins.some(allowedOrigin => _matchesValue(allowedOrigin, origin))) {
            return false;
        } else if (headers && !_headersMatchRule(headers, rule.allowedHeaders)) {
            return false;
        }
        return true;
    });
 ...
 */
function generateCorsResHeaders(rule, origin, method, headers, isPreflight) {
    const resHeaders = {
    ...
    if (rule.exposeHeaders) {
        resHeaders['access-control-expose-headers'] = rule.exposeHeaders.join(', ');
    }
    if (isPreflight) {

...
const { parseRangeSpec, parseRange } = require('arsenal').network.http.utils;
...
function setUpCopyLocator(sourceObjMD, rangeHeader, log) {
    ...
    if (!sourceObjMD.location) {
        ...
        dataLocator = Array.isArray(sourceObjMD.location) ? sourceObjMD.location : [{ key: sourceObjMD.location }];
    }
    ...
    if (rangeHeader) {
        ...
        if (
            range &&
            dataLocator.length >= 1 &&
            (dataLocator[0].start === undefined || dataLocator[0].size === undefined)
        ) {
            log.trace('data model before version 2 so cannot support get range copy part');
            return {
                error: errorInstances.NotImplemented.customizeDescription(
                    'Stored object has legacy data storage model so does not support range headers on copy part',
                ),
            };
        }
        if (range) {
    ...
    if (copyObjectSize > constants.maximumAllowedPartSize) {
        log.debug('copy part size too large', { sourceSize, rangeHeader, copyObjectSize });

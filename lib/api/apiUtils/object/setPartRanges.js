 ...
 * @param {array} outerRange - range from request, where outerRange[1]
 * is the inclusive end byte boundary
...
function setPartRanges(dataLocations, outerRange) {
    ...
    if (dataLocations.length === 1) {
        ...
        if (dataLocations[0].size) {
            ...
            soleLocation.size = Math.min(partSize, end - begin + 1).toString();
        }
    ...
    for (let i = 0; i < dataLocations.length; i++) {
        ...
        if (partStart >= begin) {
            ...
            const endPart = Math.min(partSize - 1, max - total + startOffset - 1);

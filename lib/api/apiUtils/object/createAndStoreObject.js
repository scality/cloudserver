    ...
    services.metadataStoreObject(bucketName, dataGetInfo,
        cipherBundle, metadataStoreParams, (err, result) => {
            ...
            if (dataToDelete) {
                ...
                const deleteDataLatency = performance.now();
                return data.batchDelete(dataToDelete, requestMethod,
                    newDataStoreName, log, err => {
                        monitorLatency('10-deleteData', performance.now() - deleteDataLatency);
                        callback(err, result);
                    });
            }
...
function createAndStoreObject(bucketName, bucketMD, objectKey, objMD, authInfo,
    ...
    const preStoreLatency = performance.now();
    ...
    if (backendInfoObj.err) {
        return callback(backendInfoObj.err);
    }
    ...
    monitorLatency('6-preStoreLatency', performance.now() - preStoreLatency);

    const storeDataLatency = performance.now();
    let postDataStoreLatency;
    ...
    return async.waterfall([
        function storeData(next) {
            if (size === 0) {
                ...
                if (mdOnlyHeader === 'true' && mdOnlySize > 0) {
                    ...
                    // Optimized: avoid Buffer.from() + toString('hex') GC pressure
                    let md5 = null;
                    const md5Header = request.headers['x-amz-meta-md5chksum'];
                    if (md5Header) {
                        // Reuse existing buffer if possible, avoid double conversion
                        try {
                            md5 = Buffer.from(md5Header, 'base64').toString('hex');
                        } catch (err) {
                            log.debug('failed to parse md5chksum header', { error: err });
                            md5 = null;
                        }
                    }
                    
                    // Optimized: avoid template literal string creation
                    const numParts = request.headers['x-amz-meta-md5numparts'];
                    let _md5;
                    if (numParts === undefined) {
                        _md5 = md5;
                    } else {
                        // Use array join instead of template literal for efficiency
                        _md5 = md5 ? [md5, numParts].join('-') : '-' + numParts;
                    }
        ...
        function processDataResult(dataGetInfo, calculatedHash, next) {
            monitorLatency('7-processDataResult', performance.now() - storeDataLatency);
            postDataStoreLatency = performance.now();
            ...
            // Optimized: avoid destructuring to reduce variable creation
            const key = dataGetInfo.key;
            const dataStoreName = dataGetInfo.dataStoreName;
            const dataStoreType = dataGetInfo.dataStoreType;
            const dataStoreETag = dataGetInfo.dataStoreETag;
            const dataStoreVersionId = dataGetInfo.dataStoreVersionId;
            
            // Optimized: avoid template literal, use string concatenation
        ...
        function getVersioningInfo(infoArr, next) {
            ...
            return versioningPreprocessing(bucketName, bucketMD,
                metadataStoreParams.objectKey, objMD, log, (err, options) => {
                    ...
                    monitorLatency('8-getVersioningInfo', performance.now() - postDataStoreLatency);

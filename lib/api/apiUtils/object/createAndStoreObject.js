...
// overwrite version metadata if put object version.
function _managePutVersion(metadataStoreParams, isPutVersion, decodedPutVersionId, isNull) {
    if (isPutVersion) {
        metadataStoreParams.versionId = decodedPutVersionId;
        metadataStoreParams.isNull = isNull || undefined;
        if (isNull) {
            metadataStoreParams.nullVersionId = undefined;
...
function createAndStoreObject(bucketName, bucketMD, objectKey, objMD, authInfo,
    ...
    // check master request.headers['x-amz-version-id']
    ...
    return async.waterfall([
        ...
        function storeMDAndDeleteData(options, infoArr, next) {
            ...
            _managePutVersion(metadataStoreParams, isPutVersion, decodedPutVersionId, isNull);

...
const { versioningPreprocessing, decodePutVersionId, overwrittingVersioning } = require('./versioning');
...
function createAndStoreObject(bucketName, bucketMD, objectKey, objMD, authInfo,
    ...
    const isPutVersion = !!putVersionId;
    ...
    return async.waterfall([
        ...
        function getVersioningInfo(infoArr, next) {
            if (isPutVersion) {
                const { err, options } =
                    overwrittingVersioning(putVersionId, objMD, metadataStoreParams, log);
                if (err) {
                    return next(err);
                }

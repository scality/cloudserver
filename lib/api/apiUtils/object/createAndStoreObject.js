...
const { versioningPreprocessing, decodePutVersionId } = require('./versioning');
...
function createAndStoreObject(bucketName, bucketMD, objectKey, objMD, authInfo,
        ...
        log, callback) {
    console.log('createAndStoreObject objMD!!!', objMD);

    const putVersionId = request.headers['x-scal-s3-version-id'];
    const { isPutVersion, err, decodedVid: decodedPutVersionId, isNull } = decodePutVersionId(putVersionId);
    console.log('decodedPutVersionId!!!', decodedPutVersionId);
    if (err) {
    ...
    return async.waterfall([
        ...
        function getVersioningInfo(infoArr, next) {
            if (isPutVersion) {
                // creationTime is irrelevant when overwriting data.

                metadataStoreParams.creationTime = objMD['creation-time'];

                if (isNull && (objMD.isNull || !objMD.versionId)) {
                    console.log('-------------- N2B 1 --------------');
                    // CASE 1: overwritting a master version.
                    // ISSUE: master's versionId gets updated
                    const options = { versionId: '', masterVersionId: objMD.versionId || 'null' };
                    if (objMD.versionId) {

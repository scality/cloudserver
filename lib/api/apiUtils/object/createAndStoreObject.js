...
function _storeInMDandDeleteData(bucketName, dataGetInfo, cipherBundle,
    ...
    return async.waterfall([
        next => tracer.startActiveSpan('Store Metadata', undefined, activeTracerContext, currentSpan => {
...
function createAndStoreObject(bucketName, bucketMD, objectKey, objMD, authInfo,
    ...
    objectStorageParamSpan.end();
    return async.waterfall([
        next => tracer.startActiveSpan('Storing Data', undefined, activeTracerContext, currentSpan => {
            currentSpan.setAttributes({
        ...
        (options, infoArr, next) => tracer.startActiveSpan('Store Metadata and Delete old data if needed', undefined, activeTracerContext, currentSpan => {
            currentSpan.setAttributes({

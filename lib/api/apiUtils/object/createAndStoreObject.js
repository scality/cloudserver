...
const { errors, errorInstances, s3middleware } = require('@scality/arsenal');
...
function createAndStoreObject(
    ...
    if (!isDeleteMarker && !isPutVersion && isColdStorageClass(request.headers['x-amz-storage-class'])) {
        metadataStoreParams.amzStorageClass = request.headers['x-amz-storage-class'];
        metadataStoreParams.transitionInProgress = true;
    }

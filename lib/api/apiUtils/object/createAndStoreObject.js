...
function _managePutVersionOptions(isPutVersion, decodedPutVersionId, isNull) {
...
function createAndStoreObject(bucketName, bucketMD, objectKey, objMD, authInfo,
    ...
    return async.waterfall([
        ...
        function getVersioningInfo(infoArr, next) {
            if (isPutVersion) {
                const vcfg = bucketMD.getVersioningConfiguration();
                const isVersionedBucket = vcfg && vcfg.Status === 'Enabled';
                if (!isVersionedBucket) {
                    if (isNull) {
                        ...
                        console.log('-------------- N2B 2 --------------');
                        // case 3
                        // update null version on a non-version bucket
                        // null version is not master
                        const options = { versionId: decodedPutVersionId, isNull: true };
                        return next(null, options, infoArr);
                    }
                    console.log('-------------- N2B 3 --------------');
                    const options = { versionId: decodedPutVersionId };
                    return next(null, options, infoArr);
                }
                const options = {
                    versionId: decodedPutVersionId,

 ...
 */
function processBytesToWrite(apiMethod, bucket, versionId, contentLength, objMD) {
    let bytes = contentLength;
    if (!bytes && objMD?.['content-length']) {
        if (!bucket.isVersioningEnabled() || bucket.isVersioningEnabled() && versionId) {
            // object is being deleted
            bytes = -Number.parseInt(objMD['content-length'], 10);
        }
    } else if (apiMethod === 'objectRestore') {
...
/**
 * Finalizes a request by clearing quota updates for API methods and
 ...
 */
function finalizeRequestQuotas(request, callback) {
    if (request.clearQuotaUpdatesForAPI) {
        return async.forEachLimit(request.apiMethods, 5, (apiMethod, next) => {
            if (request.clearQuotaUpdatesForAPI[apiMethod]) {
                return request.clearQuotaUpdatesForAPI[apiMethod](next);
            }
            return next();
        }, callback);
    }
...
function validateQuotas(request, bucket, account, apiNames, apiMethod, inflight, log, callback) {
    ...
    return async.forEach(apiNames, (apiName, done) => {
        ...
        return async.parallel({
            ...
            const removeInflights = _done => async.parallel({
            ...
            // Remove any inflight if the quotas are exceeded
            if (!actionWithDataDeletion[apiMethod] && (bucketQuotaExceeded || accountQuotaExceeded)) {
                return removeInflights(done);
            }
            if (shouldSendInflights) {
                // eslint-disable-next-line no-param-reassign
                request.clearQuotaUpdatesForAPI = {
                    ...request.clearQuotaUpdatesForAPI,
                    [apiName]: removeInflights,
                };
            }
            return done();
        });
        ...
        if (actionWithDataDeletion[apiMethod]) {
            return callback();
        } else if (bucketQuotaExceeded || accountQuotaExceeded) {

 ...
 */
async function validateQuotas(bucket, apiNames, apiMethod, inflight, log) {
    const quota = bucket.getQuota();
    let isQuotaExceeded = false;
    if (quota <= 0 || !ScubaClientInstance?.enabled) {
        if (quota > 0) {
    ...
    try {
        ...
        for (const apiName of apiNames) {
            ...
            const result = await ScubaClientInstance.getLatestMetrics('bucket',
                `${bucket.getName()}_${creationDate}`, null, {
                action: apiName,
                inflight,
            });
            if (result.bytesTotal > quota) {
                ...
                // Only increments can trigger QuotaExceeded errors. Decrements
                // (object deletes) are allowed to proceed even if the quota is
                // exceeded.
                ...
                if (inflight > 0) {
                    ...
                    // In case of quota exceeded, revert the inflight bytes
                    // eslint-disable-next-line no-await-in-loop
        ...
        // When the scuba service is not healthy, or if the metrics are not
        // yet available, the request must be allowed as if the quota was
        // not set. This ensures no loss of service if scuba is down, or if
        // the metrics are stale. This is tracked by periodic healthchecks
        // and alerts at the platform level.

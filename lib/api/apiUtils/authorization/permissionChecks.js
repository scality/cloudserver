...
function checkBucketPolicy(policy, requestType, canonicalID, arn, bucketOwner, log, request, actionImplicitDenies) {
    ...
    let copiedStatement = [...policy.Statement];

    const requestContext = request ? new RequestContext(request.headers, request.query,
        request.bucketName, request.objectKey, request?.clientIp,
        request?.isSecure, request.resourceType, 's3', null, null,
        null, null, null, null, null, null, null, null, null,
        request.objectLockRetentionDays) : undefined;
    console.log('COPIED STATEMENT', copiedStatement);
    ...
    console.log('RETURNING BUCKET POLICY', { permission });
    return permission;
}
...
function isBucketAuthorized(bucket, requestTypesInput, canonicalID, authInfo, log, request,
    ...
    console.log('isBucketAuthorized', {
        bucket,
        requestTypesInput,
        canonicalID,
        authInfo,
    ...
    return requestTypes.every(_requestType => {
        ...
        console.log('PROCESSING BUCKET POLICY');

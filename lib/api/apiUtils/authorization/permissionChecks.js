...
const publicReadBuckets = process.env.ALLOW_PUBLIC_READ_BUCKETS ? process.env.ALLOW_PUBLIC_READ_BUCKETS.split(',') : [];
...
function checkBucketAcls(bucket, requestType, canonicalID, mainApiCall) {
    ...
    let requestTypeParsed = requestType.endsWith('Version') ? requestType.slice(0, 'Version'.length * -1) : requestType;
    requestTypeParsed = actionsToConsiderAsObjectPut.includes(requestTypeParsed) ? 'objectPut' : requestTypeParsed;
    const parsedMainApiCall = actionsToConsiderAsObjectPut.includes(mainApiCall) ? 'objectPut' : mainApiCall;
    ...
    if (requestTypeParsed === 'bucketGet' || requestTypeParsed === 'bucketHead') {
        if (
            ...
            (bucketAcl.Canned === 'authenticated-read' && canonicalID !== publicId)
        ) {
            return true;
        } else if (bucketAcl.FULL_CONTROL.indexOf(canonicalID) > -1 || bucketAcl.READ.indexOf(canonicalID) > -1) {
            return true;
        } else if (
            bucketAcl.READ.indexOf(publicId) > -1 ||
            (bucketAcl.READ.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            (bucketAcl.FULL_CONTROL.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
    ...
    if (requestTypeParsed === 'bucketGetACL') {
        if (
            (bucketAcl.Canned === 'log-delivery-write' && canonicalID === logId) ||
            bucketAcl.FULL_CONTROL.indexOf(canonicalID) > -1 ||
            bucketAcl.READ_ACP.indexOf(canonicalID) > -1
        ) {
            return true;
        } else if (
            bucketAcl.READ_ACP.indexOf(publicId) > -1 ||
            (bucketAcl.READ_ACP.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            (bucketAcl.FULL_CONTROL.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            bucketAcl.FULL_CONTROL.indexOf(publicId) > -1
        ) {
            return true;
    ...
    if (requestTypeParsed === 'bucketPutACL') {
        if (bucketAcl.FULL_CONTROL.indexOf(canonicalID) > -1 || bucketAcl.WRITE_ACP.indexOf(canonicalID) > -1) {
            return true;
        } else if (
            bucketAcl.WRITE_ACP.indexOf(publicId) > -1 ||
            (bucketAcl.WRITE_ACP.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            (bucketAcl.FULL_CONTROL.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            bucketAcl.FULL_CONTROL.indexOf(publicId) > -1
        ) {
            return true;
    ...
    if (requestTypeParsed === 'objectDelete' || requestTypeParsed === 'objectPut') {
        if (
            ...
            bucketAcl.WRITE.indexOf(publicId) > -1 ||
            (bucketAcl.WRITE.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            (bucketAcl.FULL_CONTROL.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            bucketAcl.FULL_CONTROL.indexOf(publicId) > -1
        ) {
            return true;
    ...
    return (
        requestTypeParsed === 'objectPutACL' ||
        requestTypeParsed === 'objectGetACL' ||
        requestTypeParsed === 'objectGet' ||
        requestTypeParsed === 'objectHead'
    );
}

function checkObjectAcls(
    bucket,
    objectMD,
    requestType,
    canonicalID,
    ...
    const requestTypeParsed = actionsToConsiderAsObjectPut.includes(requestType) ? 'objectPut' : requestType;
    const parsedMainApiCall = actionsToConsiderAsObjectPut.includes(mainApiCall) ? 'objectPut' : mainApiCall;
    // acls don't distinguish between users and accounts, so both should be allowed
    if (bucketOwnerActions.includes(requestTypeParsed) && bucketOwner === canonicalID) {
        return true;
    }
    ...
    if (parsedMainApiCall === 'objectGet') {
        if (
            (isUserUnauthenticated || (requesterIsNotUser && bucketOwner === objectMD['owner-id'])) &&
    ...
    if (requestTypeParsed === 'objectGet' || requestTypeParsed === 'objectHead') {
        if (
            objectMD.acl.Canned === 'public-read' ||
            objectMD.acl.Canned === 'public-read-write' ||
            (objectMD.acl.Canned === 'authenticated-read' && canonicalID !== publicId)
        ) {
            return true;
        } else if (objectMD.acl.Canned === 'bucket-owner-read' && bucketOwner === canonicalID) {
            return true;
        } else if (
            (objectMD.acl.Canned === 'bucket-owner-full-control' && bucketOwner === canonicalID) ||
            objectMD.acl.FULL_CONTROL.indexOf(canonicalID) > -1 ||
            objectMD.acl.READ.indexOf(canonicalID) > -1
        ) {
            return true;
        } else if (
            objectMD.acl.READ.indexOf(publicId) > -1 ||
            (objectMD.acl.READ.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            (objectMD.acl.FULL_CONTROL.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
    ...
    if (requestTypeParsed === 'objectPutACL') {
        if (
            ...
            objectMD.acl.WRITE_ACP.indexOf(publicId) > -1 ||
            (objectMD.acl.WRITE_ACP.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            (objectMD.acl.FULL_CONTROL.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            objectMD.acl.FULL_CONTROL.indexOf(publicId) > -1
        ) {
            return true;
    ...
    if (requestTypeParsed === 'objectGetACL') {
        if (
            ...
            objectMD.acl.READ_ACP.indexOf(publicId) > -1 ||
            (objectMD.acl.READ_ACP.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            (objectMD.acl.FULL_CONTROL.indexOf(allAuthedUsersId) > -1 && canonicalID !== publicId) ||
            objectMD.acl.FULL_CONTROL.indexOf(publicId) > -1
        ) {
            return true;
    ...
    const bucketAcl = bucket.getAcl();
    const allowPublicReads =
        publicReadBuckets.includes(bucket.getName()) &&
        bucketAcl.Canned === 'public-read' &&
...
function _checkPrincipals(canonicalID, arn, principal, bucketOwnerCanonicalID) {
    ...
    if (principal.CanonicalUser) {
        return _findBestPrincipalMatch(principal.CanonicalUser, p =>
            _checkPrincipalCanonicalUser(p, arn, canonicalID, bucketOwnerCanonicalID),
        );
    }

    if (principal.AWS) {
        return _findBestPrincipalMatch(principal.AWS, p =>
            _checkPrincipalAWS(p, arn, canonicalID, bucketOwnerCanonicalID),
        );
    }
...
function checkBucketPolicy(policy, requestType, canonicalID, arn, bucketOwner, log, request, actionImplicitDenies) {
    ...
    while (copiedStatement.length > 0) {
        ...
        const okCross =
            principalMatch === checkPrincipalResult.CROSS_ACCOUNT_OK && actionMatch && resourceMatch && conditionsMatch;
        switch (permission) {
...
function processBucketPolicy(
    ...
    if (!bucketPolicy || request?.bypassUserBucketPolicies) {
        ...
        const bucketPolicyPermission = checkBucketPolicy(
            bucketPolicy,
            requestType,
            canonicalID,
            arn,
        ...
        if (bucketPolicyPermission === checkBucketPolicyResult.EXPLICIT_DENY) {
            ...
            bucketPolicyPermission === checkBucketPolicyResult.CROSS_ACCOUNT_ALLOW &&
...
function isBucketAuthorized(
    ...
    return requestTypes.every(_requestType => {
        ...
        const { allowed, aclRequired } = processBucketPolicy(
            _requestType,
            bucket,
            canonicalID,
            arn,
...
function evaluateBucketPolicyWithIAM(
    bucket,
    requestTypesInput,
    canonicalID,
    authInfo,
    ...
    return requestTypes.every(_requestType => {
        ...
        const { allowed } = processBucketPolicy(
            _requestType,
            bucket,
            canonicalID,
            arn,
...
function isObjAuthorized(
    ...
    return requestTypes.every(_requestType => {
        ...
        if (!objectMD) {
            ...
            results[_requestType] = isBucketAuthorized(
                bucket,
                permission,
                canonicalID,
                authInfo,
            ...
            // bucket has canned ACL public-read-write
            if (
                (parsedMethodName === 'objectPut' || parsedMethodName === 'objectDelete') &&
        ...
        if (bucketOwnerActions.includes(parsedMethodName) && bucketOwner === canonicalID && requesterIsNotUser) {
            results[_requestType] = actionImplicitDenies[_requestType] === false;
            return results[_requestType];
        }
        const aclPermission = checkObjectAcls(
            bucket,
            objectMD,
            parsedMethodName,
            canonicalID,
    ...
    for (let i = 0; i < policy.Statement.length; i++) {
        ...
        if (s.Condition) {
            ...
            for (const conditionOperator of conditionOperators) {
                ...
                const validCondition = validConditions.find(
                    validCondition => validCondition.conditionKey === conditionKey,
                );
...
function isLifecycleSession(arn) {
    ...
    return (
        service === 'sts' && resourceType === assumedRoleArnResourceType && sessionName === backbeatLifecycleSessionName
    );
}

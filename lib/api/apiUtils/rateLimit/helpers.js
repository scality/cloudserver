 ...
 */
function getRateLimitFromCache(resourceClass, resourceId) {
    const cacheKey = `${resourceClass}:${resourceId}`;
 ...
 * @returns {undefined}
 */
function checkRateLimitWithConfig(className, resourceId, measure, limitConfig, log, callback) {
    ...
    const tokenBucket = getTokenBucket(className, resourceId, measure, limitConfig, log);
...
function checkRateLimitCapacity(className, resourceId, measure, limitConfig, log) {
    // Get or create token bucket for this bucket
    const tokenBucket = getTokenBucket(className, resourceId, measure, limitConfig, log);
    return tokenBucket.hasCapacity();
}

function checkRateLimitsForRequest(checks, log) {
    const buckets = checks.map(c => getTokenBucket(c.resourceClass, c.resourceId, c.measure, config, log));
    const hasCapacity = buckets.every(bucket =>  {
        const hasCap = bucket.hasCapacity();
        if (hasCap) {
            log.trace('Rate limit check: allowed (token consumed)', {
                className: bucket.className, resourceId: bucket.resourceId, measure: bucket.measure,

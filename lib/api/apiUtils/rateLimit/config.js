...
function parseRateLimitConfig(rateLimitingConfig, clusters) {
    ...
    if (rateLimitingConfig.bucket) {
        parsed.bucket = parseConfigForClass('bucket', rateLimitingConfig.bucket, clusters, parsed.nodes);
    }

    // Override with user-provided account configuration
    if (rateLimitingConfig.account) {
        parsed.account = parseConfigForClass('account', rateLimitingConfig.account, clusters, parsed.nodes);
    }
...
function parseConfigForClass(className, rawCfg, clusters, nodes) {
    const parsed = {
        defaultConfig: undefined,  // No global default unless specified
        configCacheTTL: rateLimitDefaultConfigCacheTTL,  // Default cache TTL
        defaultBurstCapacity: rateLimitDefaultBurstCapacity,  // Default burst capacity for per-bucket configs
    ...
    assert.strictEqual(
        ...
        `rateLimiting.${className} must be an object`
    );
    ...
    if (rawCfg.defaultConfig) {
        const { defaultConfig } = rawCfg;

        // Validate config structure
        assert.strictEqual(
            typeof defaultConfig, 'object',
            `rateLimiting.${className}.defaultConfig must be an object`
        );

        const limitConfig = {};

        if (defaultConfig.requestsPerSecond) {
            assert.strictEqual(
                typeof defaultConfig.requestsPerSecond, 'object',
                `rateLimiting.${className}.defaultConfig.requestsPerSecond must be an object`
            );

            const { limit } = defaultConfig.requestsPerSecond;

            // Validate limit
            assert(
                typeof limit === 'number' && Number.isInteger(limit) && limit >= 0,
                `rateLimiting.${className}.defaultConfig.requestsPerSecond.limit must be a non-negative integer`
            );
            ...
            if (limit > 0 && limit < minLimit) {
                throw new Error(
                    ...
                    `(nodes × workers = ${nodes} × ${clusters} = ${minLimit}) ` +
            ...
            let burstCapacity = parsed.defaultBurstCapacity;

            // Override if provided in config
            if (defaultConfig.requestsPerSecond.burstCapacity !== undefined) {
                burstCapacity = defaultConfig.requestsPerSecond.burstCapacity;
                assert(
                    typeof burstCapacity === 'number' && Number.isInteger(burstCapacity) && burstCapacity > 0,
                    `rateLimiting.${className}.defaultConfig.requestsPerSecond.burstCapacity must be a positive integer`
                );
            }
        ...
        parsed.defaultConfig = limitConfig;
    }

    // Parse config cache TTL
    // If configCacheTTL is specified: Override the default cache TTL
    if (rawCfg.configCacheTTL !== undefined) {
        const configCacheTTL = rawCfg.configCacheTTL;
        assert(
            typeof configCacheTTL === 'number' &&
            Number.isInteger(configCacheTTL) &&
            configCacheTTL > 0,
            `rateLimiting.${className}.configCacheTTL must be a positive integer`
        );
        parsed.configCacheTTL = configCacheTTL;
    }

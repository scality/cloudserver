...
const MAX_PIPELINE_SIZE = 100; // Max counters per Redis pipeline (POC pattern)
...
function evalGCRA(key, arrivedAt, interval, burstCapacityMs = 0) {
    ...
    // Step 1: Get current TAT from in-memory cache (simple number, not object)
    // Returns Date.now() if not exists (POC behavior for cold start)
    let emptyAt = getCounter(key);
    ...
    // Production feature: allows brief traffic spikes beyond strict rate
    // POC equivalent: burstCapacityMs=0 (strict enforcement)
    // Example: arrivedAt=1000ms, burstCapacityMs=1000ms => maxAllowedEmptyAt=2000ms
    ...
    // Step 6: Update counter with simple number (POC pattern)
    setCounter(key, newEmptyAt);
...
function syncCounters(config, logger) {
    try {
        ...
        // Step 1: Snapshot dirty counters and clear for next cycle (POC pattern)
        // This ensures each counter is synced once even if modified during sync
        let toSync = Array.from(dirtyCounters.keys());
        dirtyCounters.clear();
        ...
        // Helper function to sync a single batch
        const syncBatch = (batch, callback) => {
            // CRITICAL: Calculate now once per batch (POC pattern)
            const now = Date.now();

            // Build batch with TAT values
            const items = batch.map(key => {
                const tat = getCounter(key);  // Returns simple number
                return { key, tat };
            });

            // CRITICAL: Pass now to Redis for delay merging (POC pattern)
            rateLimitClient.syncCounters(items, now, (err, results) => {
                if (err) {
                ...
                // CRITICAL: Update in-memory TAT from Redis results (POC pattern)
                // Workers MUST learn from each other via Redis!
                results.forEach(({ key, tat }) => {
                    setCounter(key, tat);  // Update with merged TAT from Redis
                });

                logger.debug('batch synced', { count: batch.length });
                callback(null);
            });
        };

        // Step 2: Process in batches of 100 (POC pattern)
        // Prevents Redis overload with thousands of pipeline commands
        const batches = [];
        while (toSync.length > 0) {
            batches.push(toSync.slice(0, MAX_PIPELINE_SIZE));
            toSync = toSync.slice(MAX_PIPELINE_SIZE);
        }

        // Step 3: Sync all batches sequentially (callback pattern)
        let batchIndex = 0;
        const syncNextBatch = () => {
            if (batchIndex >= batches.length) {
                logger.debug('all batches synced', { totalBatches: batches.length });
                return;
            }

            syncBatch(batches[batchIndex], (err) => {
                if (err) {
                ...
                // Continue to next batch even if this one failed (fail-open)
                batchIndex++;
                syncNextBatch();
            });
        };

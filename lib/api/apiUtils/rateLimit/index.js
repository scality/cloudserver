...
// Constants
const SYNC_INTERVAL = 7500;           // 7.5 seconds (binary search for optimal)
...
function checkBucketRateLimit(bucketName, bucket, config, log) {
    try {
        ...
        // Workers coordinate through Redis bidirectional sync:
        // - Each worker enforces R/N locally using in-memory counter
        // - Every 10s: sync local counter to Redis (add cost)
        // - Redis returns shared counter value
        // - Update local counter with Redis value (merge back)
        // - All workers on node share the same quota through Redis
...
function syncCounters(config, logger) {
    try {
        ...
        // Build batch with cost calculation per design pseudocode:
        // amount = ceil((counter - now) / workers / replicas)
        const workers = config.clusters || 1;
        const replicas = config.rateLimiting.nodeCount || 1;
        ...
        const batch = dirtyKeys
            .map(key => {
                ...
                // Calculate delay (how far behind we are)
                const delay = Math.max(0, currentValue - now);

                // Skip if no delay (counter already drained)
                if (delay === 0) {
                    return null;
                }

                // Calculate proportional share per pseudocode
                // For LOCAL Redis (single node): divide by workers only
                // replicas division is for cross-node coordination with shared Redis
                const amount = Math.ceil(delay / workers);

                return { key, cost: amount };
            })
        ...
        rateLimitClient.updateLocalCounters(batch, (err, results) => {
            if (err) {
                ...
                // BIDIRECTIONAL SYNC: Merge using MAX to prevent time travel
                // Lua script returns value BEFORE this worker's contribution
                // Use MAX since local counter may have grown during sync
                if (results && Array.isArray(results)) {
                    results.forEach((result) => {
                        if (result && result.value !== undefined) {
                            const key = result.key;
                            const redisValue = result.value;
                            const currentLocal = cache.getCounter(key) || 0;

                            // Use MAX to prevent going backwards
                            const mergedValue = Math.max(currentLocal, redisValue);
                            cache.setCounter(key, mergedValue);
                        }
                    });
                    logger.debug('merged Redis counters to local cache', { count: results.length });
                }
            }

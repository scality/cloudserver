 ...
 * - bucketSize: Maximum bucket size in milliseconds
 *
 ...
 * @param {number} bucketSize - Maximum bucket size (ms)
 * @returns {Object} { allowed: boolean, newEmptyAt: number }
 */
function evaluate(emptyAt, arrivedAt, interval, bucketSize) {
    ...
    // If emptyAt > arrivedAt + bucketSize, the bucket is full
    if (adjustedEmptyAt > arrivedAt + bucketSize) {
        return {
            allowed: false,

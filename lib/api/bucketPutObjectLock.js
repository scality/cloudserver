...
const { errorInstances } = arsenal;
const {
    models: { ObjectLockConfiguration },
} = arsenal;
    ...
    return waterfall(
        [
            ...
            (parsedXml, next) => {
                ...
                process.nextTick(() => {
                    const configObj = lockConfigClass.getValidatedObjectLockConfiguration();
                    return next(configObj.error || null, configObj);
                });
            },
            (objectLockConfig, next) =>
            ...
            (bucket, objectLockConfig, next) => {
                ...
                process.nextTick(() => {
                    if (!isObjectLockEnabled) {
                        return next(
                            errorInstances.InvalidBucketState.customizeDescription(
        ...
        (err, bucket) => {
            ...
            if (err) {
                log.trace('error processing request', { error: err, method: 'bucketPutObjectLock' });

...
function bucketGetVersioning(authInfo, request, log, callback) {
    ...
    standardMetadataValidateBucket(metadataValParams, request.actionImplicitDenies, log, (err, bucket) => {
        ...
        if (err) {
            log.debug('error processing request', { method: 'bucketGetVersioning', error: err });
            monitoring.promMetrics('GET', bucketName, err.code, 'getBucketVersioning');
        ...
        monitoring.promMetrics('GET', bucketName, '200', 'getBucketVersioning');

 ...
 */
export default
function multipartDelete(accessKey, request, log, callback) {
    ...
    async.waterfall([
        ...
        function waterfall4(mpuBucket, storedParts, next) {
            ...
            let locations = storedParts.map((item) => {
                return item.value.partLocations;
            });
            // If no actual parts to delete, move on
            if (locations.length === 0) {
                return next(null, mpuBucket, storedParts);
            }
            // If have locations, flatten the array
            locations = locations.reduce( (a, b) => a.concat(b));

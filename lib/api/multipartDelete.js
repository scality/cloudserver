...
function multipartDelete(authInfo, request, log, callback) {
    ...
    async.waterfall([
        ...
        function ifMultipleBackend(mpuBucket, mpuOverviewArray, destBucket,
        next) {
            ...
            if (config.backends.data === 'multiple') {
                ...
                {}, log, (err, storedMetadata) => {
                    ...
                    const location = storedMetadata.
                        controllingLocationConstraint;
                    return multipleBackendGateway.abortMPU(objectKey, uploadId,
                    location, destBucket, log, err => {
                        if (err) {
                            return next(err, destBucket);
                        }
                        return next(null, mpuBucket, mpuOverviewArray,
                        destBucket, mpuOverviewKey);
                    });
                });
            }
            return next(null, mpuBucket, mpuOverviewArray, destBucket,
            mpuOverviewKey);
        },
        ...
        function deleteData(mpuBucket, storedParts, destBucket, mpuOverviewKey,
        next) {
        ...
        function deleteMetadata(mpuBucket, storedParts, destBucket,
        mpuOverviewKey, next) {

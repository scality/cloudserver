...
const { isObjAuthorized, isBucketAuthorized } = require('./apiUtils/authorization/permissionChecks');
 ...
 * @return {undefined}
 */
function _errorActions(err, errorDocument, routingRules, bucket, objectKey, corsHeaders, request, log, callback) {
    ...
    if (errRoutingRule) {
        ...
        monitoring.promMetrics(request.method, bucketName, err.code, action);
        return callback(err, false, null, corsHeaders, errRoutingRule, objectKey);
    }
    if (request.method === 'HEAD') {
        monitoring.promMetrics('HEAD', bucketName, err.code, 'headObject');
        return callback(err, false, null, corsHeaders);
    }
    if (errorDocument) {
        return metadata.getObjectMD(bucketName, errorDocument, {}, log, (errObjErr, errObjMD) => {
            if (errObjErr) {
            ...
            if (errObjMD['x-amz-server-side-encryption']) {
                for (let i = 0; i < dataLocator.length; i++) {
                    dataLocator[i].masterKeyId = errObjMD['x-amz-server-side-encryption-aws-' + 'kms-key-id'];
                    dataLocator[i].algorithm = errObjMD['x-amz-server-side-encryption'];
                }
            }

            if (errObjMD['x-amz-website-redirect-location']) {
                const redirectLocation = errObjMD['x-amz-website-redirect-location'];
                const redirectInfo = { withError: true, location: redirectLocation };
                log.trace('redirecting to x-amz-website-redirect-location', { location: redirectLocation });
                return callback(err, false, dataLocator, corsHeaders, redirectInfo, '');
            }

            const responseMetaHeaders = collectResponseHeaders(errObjMD, corsHeaders);
...
function callbackGetToHead(callback) {
    return (err, userErrorPageFailure, dataGetInfo, resMetaHeaders, redirectInfo, key) =>
...
function website(request, log, callback) {
    ...
    return metadata.getBucket(bucketName, log, (err, bucket) => {
        ...
        if (bucketShield(bucket, `object${methodCapitalized}`)) {
            ...
            monitoring.promMetrics(request.method, bucketName, 404, action);
        ...
        if (!websiteConfig) {
            monitoring.promMetrics(request.method, bucketName, 404, action);
            return callback(errors.NoSuchWebsiteConfiguration, false, null, corsHeaders);
        }
        ...
        if (websiteConfig.getRedirectAllRequestsTo()) {
            return callback(null, false, null, corsHeaders, websiteConfig.getRedirectAllRequestsTo(), reqObjectKey);
        }
        ...
        if (keyRoutingRule) {
            ...
            return callback(null, false, null, corsHeaders, keyRoutingRule, reqObjectKey);
        }
        ...
        function runWebsite(originalError) {
            ...
            // validation
            return metadata.getObjectMD(bucketName, request.objectKey, {}, log, (err, objMD) => {
                ...
                if (err) {
                    log.trace('error retrieving object metadata', { error: err });
                    let returnErr = err;
                    const bucketAuthorized = isBucketAuthorized(
                        bucket,
                        request.apiMethods || 'bucketGet',
                        constants.publicId,
                        null,
                        log,
                        request,
                    ...
                    return _errorActions(
                        ...
                        websiteConfig.getErrorDocument(),
                        routingRules,
                        bucket,
                        reqObjectKey,
                        corsHeaders,
                ...
                if (originalError) {
                    const redirectInfo = { withError: true, location: `/${reqObjectKey}/` };
                    return callback(errors.Found, false, null, corsHeaders, redirectInfo, '');
                }
                ...
                if (objMD['x-amz-website-redirect-location']) {
                    const redirectLocation = objMD['x-amz-website-redirect-location'];
                    const redirectInfo = extractRedirectInfo(redirectLocation);
                    log.trace('redirecting to x-amz-website-redirect-location', { location: redirectLocation });
                    return callback(null, false, null, corsHeaders, redirectInfo, '');
                }
                ...
                monitoring.promMetrics('GET', bucketName, '200', 'getObject', responseMetaHeaders['Content-Length']);

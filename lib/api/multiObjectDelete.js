...
function multiObjectDelete(authInfo, request, log, callback, oTel) {
    ...
    return tracer.startActiveSpan('Validating and Deleting Objects from S3', undefined, cloudserverApiSpanContext, multiObjectDeleteSpan => {
        ...
        return async.waterfall([
            next => tracer.startActiveSpan('Bucket check for Server side configuration - SSE', undefined, multiObjectDeleteSpanContext, currentSpan => {
            ...
            function parseXML(currentSpan, next) {
            ...
            (quietSetting, objects, next) => tracer.startActiveSpan('Checking Bucket Metadata and check if user is authorized', undefined, multiObjectDeleteSpanContext, currentSpan => {
                currentSpan.setAttributes({
                    'code.function': 'multiObjectDelete()',
                    'code.filename': 'lib/api/multiObjectDelete.js',
                    'code.lineno': 529,
                });
                return next(null, quietSetting, objects, currentSpan);
            }),
            function checkBucketMetadata(quietSetting, objects, currentSpan, next) {
            ...
            (quietSetting, errorResults, objects, bucketMD, next) => tracer.startActiveSpan('Starting Vault Authorization for each object', undefined, multiObjectDeleteSpanContext, currentSpan => {
                currentSpan.setAttributes({
                    'code.function': 'multiObjectDelete()',
                    'code.filename': 'lib/api/multiObjectDelete.js',
                    'code.lineno': 591,
                ...
                return next(null, quietSetting, errorResults, objects, bucketMD, currentSpan);
            }),
            function checkPolicies(quietSetting, errorResults, objects, bucketMD, currentSpan, next) {
            ...
            (quietSetting, errorResults, inPlay, bucket, next) => tracer.startActiveSpan('Getting Object Metadata and Deleting Objects', undefined, multiObjectDeleteSpanContext, currentSpan => {
                currentSpan.setAttributes({
                    'code.function': 'multiObjectDelete()',
                    'code.filename': 'lib/api/multiObjectDelete.js',
                    'code.lineno': 744,
                });
                return next(null, quietSetting, errorResults, inPlay, bucket, currentSpan);
            }),
            function getObjMetadataAndDeleteStep(quietSetting, errorResults, inPlay,
                bucket, currentSpan, next) {

...
const { auth, errors, versioning, s3middleware, policies } = require('@scality/arsenal');
...
const { isBucketAuthorized, evaluateBucketPolicyWithIAM } = require('./apiUtils/authorization/permissionChecks');
const { preprocessingVersioningDelete } = require('./apiUtils/object/versioning');
...
function _formatXML(quietSetting, errorResults, deleted) {
    ...
    errorResults.forEach(errorObj => {
        errorXML.push(
            '<Error>',
            '<Key>',
            escapeForXml(errorObj.entry.key),
            '</Key>',
            '<Code>',
            escapeForXml(errorObj.error.message),
            '</Code>',
        );
        if (errorObj.entry.versionId) {
            const version = errorObj.entry.versionId === 'null' ? 'null' : escapeForXml(errorObj.entry.versionId);
            errorXML.push('<VersionId>', version, '</VersionId>');
        }
        errorXML.push('<Message>', escapeForXml(errorObj.error.description), '</Message>', '</Error>');
    });
    ...
    deleted.forEach(version => {
        ...
        deletedXML.push('<Deleted>', '<Key>', escapeForXml(version.entry.key), '</Key>');
        if (version.entry.versionId) {
            deletedXML.push('<VersionId>', escapeForXml(version.entry.versionId), '</VersionId>');
        }
        if (isDeleteMarker) {
            deletedXML.push(
                '<DeleteMarker>',
...
function decodeObjectVersion(entry) {
    ...
    if (entry.versionId) {
        decodedVersionId = entry.versionId === 'null' ? 'null' : versionIdUtils.decode(entry.versionId);
    }
 ...
 */
function getObjMetadataAndDelete(
    authInfo,
    canonicalID,
    request,
    bucketName,
    ...
    return async.waterfall(
                                        ...
                                        return checkUserGovernanceBypass(
                                            ...
                                            error => {
                                                if (error && error.is.AccessDenied) {
                                                    log.debug(
                                                        'user does not have BypassGovernanceRetention ' +
                                                            'and object is locked',
                                                        { error },
                                                    );
        ...
        (err, ...results) => {
            ...
            if (err) {
                monitoring.promMetrics('DELETE', bucketName, err.code, 'multiObjectDelete');
                return next(err);
            }
...
function multiObjectDelete(authInfo, request, log, callback) {
    ...
    if (!request.post) {
        monitoring.promMetrics('DELETE', request.bucketName, 400, 'multiObjectDelete');
    ...
    return async.waterfall(
        ...
        (
            ...
            const xml = _formatXML(quietSetting, errorResults, successfullyDeleted);
            const deletedKeys = successfullyDeleted.map(item => item.key);
            const removedDeleteMarkers = successfullyDeleted.filter(
                item => item.isDeleteMarker && item.entry && item.entry.versionId,
            ).length;
            ...
            monitoring.promMetrics(
                ...
                '200',
                'multiObjectDelete',
                Number.parseInt(totalContentLengthDeleted, 10),
                null,
                null,
                numOfObjectsRemoved,
            );

...
function bucketPutWebsite(authInfo, request, log, callback) {
    ...
    if (!request.post) {
        monitoring.promMetrics('PUT', bucketName, 400, 'putBucketWebsite');
    ...
    return async.waterfall(
        ...
        (err, bucket) => {
            ...
            if (err) {
                log.trace('error processing request', { error: err, method: 'bucketPutWebsite' });
                monitoring.promMetrics('PUT', bucketName, err.code, 'putBucketWebsite');
            } else {
                pushMetric('putBucketWebsite', log, {
                    authInfo,
                    bucket: bucketName,
                });
                monitoring.promMetrics('PUT', bucketName, '200', 'putBucketWebsite');
            }

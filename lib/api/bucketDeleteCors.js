...
function bucketDeleteCors(authInfo, request, log, callback) {
    ...
    return metadata.getBucket(bucketName, log, (err, bucket) => {
        ...
        if (err) {
            ...
            monitoring.promMetrics('DELETE', bucketName, 400, 'deleteBucketCors');
            return callback(err);
        }
        ...
        if (
            ...
            monitoring.promMetrics('DELETE', bucketName, 403, 'deleteBucketCors');
        ...
        return metadata.updateBucket(bucketName, bucket, log, err => {
            ...
            monitoring.promMetrics('DELETE', bucketName, '204', 'deleteBucketCors');
            return callback(err, corsHeaders);
        });
    });
}

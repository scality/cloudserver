...
export default function bucketPut(authInfo, request, locationConstraint, log,
    ...
    if (locationConstraint && config.locationConstraints &&
      Object.keys(config.locationConstraints).indexOf(locationConstraint) < 0) {
        log.trace('locationConstraint is invalid',
          { locationConstraint });
        return callback(errors.InvalidLocationConstraint);
    }

    if (!locationConstraint && config.restEndpoints && request.parsedHost &&

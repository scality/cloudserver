...
const { logAPIOperation } = require('../utilities/apiLogHelper');
    ...
    return async.waterfall([
        ...
        if (deleteInfo.newDeleteMarker) {
            ...
            */
            ...
            // Log successful API operation (delete marker creation)
            const deleteMarkerObjectSize = (objectMD && objectMD['content-length']) || 0;
            logAPIOperation('deleteObject', request, log, bucketMD, authInfo, 204, 0, deleteMarkerObjectSize, null);
        } else {
            log.end().addDefaultFields({
            ...
            // Log successful API operation (actual object deletion)
            const deletedObjectSize = Number.parseInt(objectMD['content-length'], 10);
            logAPIOperation('deleteObject', request, log, bucketMD, authInfo, 204, 0, deletedObjectSize, null);
        }
        return cb(err, resHeaders);
    });
}

...
function objectDeleteInternal(authInfo, request, log, isExpiration, cb) {
    ...
    return async.waterfall([
        ...
        function raceDelay(bucketMD, objectMD, next) {
            if (isExpiration) {
                log.info('DEBUG: delaying expiration delete by 30s to widen race window');
                return setTimeout(() => next(null, bucketMD, objectMD), 30000);
            }
            return next(null, bucketMD, objectMD);
        },

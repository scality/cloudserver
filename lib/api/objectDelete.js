...
const { decodeVersionId, preprocessingVersioningDelete } = require('./apiUtils/object/versioning');
...
function objectDeleteInternal(authInfo, request, log, isExpiration, cb) {
    ...
    if (authInfo.isRequesterPublicUser()) {
        ...
        monitoring.promMetrics('DELETE', request.bucketName, 403, 'deleteObject');
    ...
    return async.waterfall(
        [
            function validateBucketAndObj(next) {
                return standardMetadataValidateBucketAndObj(
                    ...
                    (err, bucketMD, objMD) => {
                        ...
                        if (
                            versioningCfg &&
                            versioningCfg.Status === 'Enabled' &&
                            objMD.versionId === reqVersionId &&
                            isExpiration &&
                            !objMD.isDeleteMarker
                        ) {
                            log.warn(
                                'expiration is trying to delete a master version ' +
                        ...
                        if (reqVersionId && objMD.location && Array.isArray(objMD.location) && objMD.location[0]) {
            ...
            function validateHeaders(bucketMD, objectMD, next) {
                if (objectMD) {
                    ...
                    const { modifiedSinceRes, unmodifiedSinceRes } = checkDateModifiedHeaders(
                        request.headers,
                        lastModified,
                    );
                ...
                return process.nextTick(() => next(null, bucketMD, objectMD));
            },
            function deleteOperation(bucketMD, objectMD, next) {
                ...
                if (delOptions && delOptions.deleteData && bucketMD.isNFS() && bucketMD.getReplicationConfiguration()) {
                    ...
                    return createAndStoreObject(
                        ...
                        err => {
                            ...
                            return services.deleteObject(
                                ...
                                false,
                                log,
                                isExpiration ? 's3:LifecycleExpiration:Delete' : 's3:ObjectRemoved:Delete',
                                (err, delResult) => next(err, bucketMD, objectMD, delResult, deleteInfo),
                            );
                        },
                    );
                }
        ...
        (err, bucketMD, objectMD, result, deleteInfo) => {
            const resHeaders = collectCorsHeaders(request.headers.origin, request.method, bucketMD);
            ...
            if (reqVersionId) {
                resHeaders['x-amz-version-id'] =
                    reqVersionId === 'null' ? reqVersionId : versionIdUtils.encode(reqVersionId);
            ...
            if (err) {
                log.debug('error processing request', { error: err, method: 'objectDelete' });
                monitoring.promMetrics('DELETE', bucketName, err.code, 'deleteObject');
            ...
            if (deleteInfo.newDeleteMarker) {
                ...
                if (result.versionId) {
                    resHeaders['x-amz-delete-marker'] = true;
                    resHeaders['x-amz-version-id'] =
                        result.versionId === 'null' ? result.versionId : versionIdUtils.encode(result.versionId);
                }
                ...
                const versioningSuspended =
                    bucketMD.getVersioningConfiguration() &&

...
function parseRequestBody(requestBody, callback) {
    // Parse JSON request body (async.waterfall requires async callbacks)
    return process.nextTick(() => {
        try {
            const jsonData = JSON.parse(requestBody);
            if (typeof jsonData !== 'object' || jsonData === null) {
                return callback(errorInstances.InvalidArgument
                    .customizeDescription('Request body must be a valid JSON object'));
            }
            return callback(null, jsonData);
        } catch (jsonError) {
            return callback(errorInstances.InvalidArgument
                .customizeDescription('Request body must be valid JSON: ' + jsonError.message));
        }
    });
}

    ...
    if (limit > 0 && limit < minLimit) {
        return callback(errorInstances.InvalidArgument
            .customizeDescription(
                `RequestsPerSecond (${limit}) must be >= (nodes × workers = ${nodes} × ${workers} = ${minLimit}) or 0 (unlimited). ` +
                `Each worker enforces limit/nodes/workers locally. With limit < ${minLimit}, per-worker rate would be < 1 req/s, effectively blocking traffic.`
            ));
    }

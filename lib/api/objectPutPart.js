...
// expect parts of at least 5MB until last part.
...
function objectPutPart(accessKey, datastore, metastore, request, callback) {
    ...
    async.waterfall([
        ...
        function waterfall3(bucket, multiPartMetadata, newLocation, next) {
            services.metadataStorePart(bucket,
            newLocation, partNumber, contentMD5, multiPartMetadata, next);
        }
    ], function watefallFinal(err) {
        console.log("metastore at end of waterfall", JSON.stringify(metastore));

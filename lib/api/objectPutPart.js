...
function objectPutPart(authInfo, request, streamingV4Params, log,
    ...
    return async.waterfall([
        ...
        (destinationBucket, objectLocationConstraint, cipherBundle,
         splitter, next) => {
            const res = data.uploadPart(
                request, streamingV4Params, null, size,
                objectLocationConstraint, objectKey, uploadId, partNumber,
                bucketName, log, (err, partInfo) => {
                    if (err) {
                    ...
                    assert.strictEqual(typeof partInfo, 'object');
                    return next(null, destinationBucket,
                    objectLocationConstraint, cipherBundle, splitter, partInfo);
                });
            return res;
        },

...
function objectPutPart(authInfo, request, streamingV4Params, log,
    ...
    const originalIdentityAuthzResults = request.isImplicitIdentityDeny;
    ...
    return async.waterfall([
        ...
        (destinationBucket, next) => {
            ...
            if (!isBucketAuthorized(destinationBucket, requestType, canonicalID, authInfo,
                request.isImplicitIdentityDeny, log, request)) {
        ...
        splitter, next) => {
            ...
            delete request.isImplicitIdentityDeny;
        ...
        // eslint-disable-next-line no-param-reassign
        request.isImplicitIdentityDeny = originalIdentityAuthzResults;
        if (err) {
            if (err === skipError) {

...
export default function objectPutPart(authInfo, request, log, cb) {
    ...
    log.debug(`Owner canonicalID to send to data: ${authInfo.getCanonicalID}`);
    ...
    services.metadataValidateMultipart(metadataValParams, (err, mpuBucket) => {
        ...
        return services.dataStore(null, objectKeyContext, request, log,
            (err, extraArg, keys) => {
                if (err) {
                    return cb(err);
                }
                const mdParams = {
                    partNumber,
                    contentMD5: request.calculatedMD5,
                    size,
                    uploadId,
                };

 ...
 */
export default function objectPutPart(accessKey, metastore, request, cb) {
    ...
    services.metadataValidateMultipart(metadataValParams, (err, mpuBucket) => {
        ...
        return services.dataStore(null, request.post, (err, extraArg, keys) => {
            if (err) {
                return cb(err);
            }
            services.metadataStorePart(mpuBucket.name, keys, mdParams, cb);
        });
    });
}

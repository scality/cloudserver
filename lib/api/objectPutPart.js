...
function checksumTypeMismatchErr(expected, actual) {
    return errors.InvalidRequest.customizeDescription(
        `Checksum Type mismatch occurred, expected checksum Type: ${expected}, ` + `actual checksum Type: ${actual}`,
    );
}
 ...
 * @return {undefined}
 */
function objectPutPart(authInfo, request, streamingV4Params, log, cb) {
    ...
    if (Number.parseInt(size, 10) > constants.maximumAllowedPartSize) {
        ...
        monitoring.promMetrics('PUT', request.bucketName, 400, 'putObjectPart');
    ...
    return async.waterfall(
        [
            ...
            next =>
                metadata.getBucket(bucketName, log, (err, destinationBucket, raftSessionId) => {
                    ...
                    storeServerAccessLogInfo(request, destinationBucket, raftSessionId);
            ...
            (destinationBucket, next) =>
                validateQuotas(
                    ...
                    isPutVersion,
                    log,
                    err => next(err, destinationBucket),
                ),
            // Validate that no object SSE is provided for part.
            // Part must use SSE from initiateMPU (overview in metadata)
            (destinationBucket, next) => {
                const { error, objectSSE } = parseObjectEncryptionHeaders(request.headers);
                if (error) {
                    return next(error, destinationBucket);
                }
            ...
            (destinationBucket, next) =>
                metadata.getBucket(mpuBucketName, log, (err, mpuBucket) => {
                    ...
                    return next(null, destinationBucket, splitter);
                }),
            // Check authorization of the MPU shadow bucket.
            (destinationBucket, splitter, next) => {
                const mpuOverviewKey = _getOverviewKey(splitter, objectKey, uploadId);
                return metadata.getObjectMD(mpuBucketName, mpuOverviewKey, {}, log, (err, res) => {
                    ...
                    const objectLocationConstraint = res.controllingLocationConstraint;
                    ...
                    const sse = sseAlgo
                        ...
                        : null;
                    return next(null, destinationBucket, objectLocationConstraint, sse, splitter);
                });
            },
            // Use MPU overview SSE config
            (destinationBucket, objectLocationConstraint, encryption, splitter, next) => {
                // If MPU has server-side encryption, pass the `res` value
                if (encryption) {
                    return kms.createCipherBundle(
                        ...
                        (err, res) => {
                            if (err) {
                                log.error('error processing the cipher bundle for ' + 'the destination bucket', {
                                    error: err,
                                });
                                return next(err, destinationBucket);
                            }
            ...
            (destinationBucket, objectLocationConstraint, cipherBundle, splitter, next) => {
                const mpuInfo = {
                    destinationBucket,
                    size,
                    objectKey,
                ...
                return data.putPart(
                    ...
                    (err, partInfo, updatedObjectLC) => {
                        ...
                        // TODO CLDSRV-640 (artesca) data backend should return SSE to include in response headers
                        if (partInfo && partInfo.dataStoreType === 'aws_s3') {
                            return next(skipError, destinationBucket, partInfo.dataStoreETag);
                        }
                        ...
                        return next(null, destinationBucket, updatedObjectLC, cipherBundle, splitter, partInfo);
                    },
                );
            },
            // Get any pre-existing part.
            (destinationBucket, objectLocationConstraint, cipherBundle, splitter, partInfo, next) => {
                const paddedPartNumber = _getPaddedPartNumber(partNumber);
                const partKey = _getPartKey(uploadId, splitter, paddedPartNumber);
                return metadata.getObjectMD(mpuBucketName, partKey, {}, log, (err, res) => {
                    ...
                    if (res) {
                        ...
                        oldLocations = Array.isArray(res.partLocations) ? res.partLocations : [res.partLocations];
                    }
                    return next(
                        null,
                        destinationBucket,
            ...
            (
                ...
                if (partInfo && constants.skipBatchDeleteBackends[partInfo.dataStoreType]) {
                ...
                // The MPU recorded a checksum algorithm, so this part was meant to
                // get a digest and the final object a checksum. Storing none means
                // CompleteMultipartUpload will not be able to compose one. Logged
                // for implicit-default MPUs too: the object still loses a checksum
                // it would otherwise have had.
                if (!checksumsEnabled && mpuChecksumAlgo) {
                    log.warn(
                        "checksums are disabled; MPU required part checksum won't be stored; " +
                            "final object won't get a checksum",
                        {
                            uploadId,
                            partNumber,
                            checksumAlgorithm: mpuChecksumAlgo,
                            checksumType: mpuChecksumType,
                            checksumIsDefault: !!mpuChecksumIsDefault,
                        },
                    );
                }
                const headerChecksum = checksumsEnabled ? getChecksumDataFromHeaders(request.headers) : null;
                ...
                if (checksumsEnabled && !headerChecksum && mpuChecksumType === 'COMPOSITE') {
            ...
            (
                ...
                const sseHeaders = {};
                if (cipherBundle) {
                    const { algorithm, masterKeyId, cryptoScheme, cipheredDataKey } = cipherBundle;
                    ...
                    sseHeaders.algo = algorithm;
                    sseHeaders.kmsKey = masterKeyId;
                }
                const omVal = {
                ...
                return metadata.putObjectMD(mpuBucketName, partKey, omVal, mdParams, log, err => {
                    if (err) {
                ...
                if (oldLocationsToDelete) {
                    ...
                    return data.batchDelete(
                        ...
                        err => {
                            ...
                            return next(null, destinationBucket, hexDigest, sseHeaders, prevObjectSize);
                        },
                    );
                }
                return next(null, destinationBucket, hexDigest, sseHeaders, prevObjectSize);
        ...
        (err, destinationBucket, hexDigest, sseHeaders, prevObjectSize) => {
            ...
            if (sseHeaders) {
                setSSEHeaders(corsHeaders, sseHeaders.algo, sseHeaders.kmsKey);
            }
            if (err) {
                if (err === skipError) {
                ...
                monitoring.promMetrics('PUT', bucketName, err.code, 'putObjectPart');
            ...
            monitoring.promMetrics('PUT', bucketName, '200', 'putObjectPart', size, prevObjectSize);
            return cb(null, hexDigest, corsHeaders);
        },
    );
}

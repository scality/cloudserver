 ...
 */
export default function objectPutPart(accessKey, metastore, request, callback) {
    ...
    async.waterfall([
        ...
        function waterfall2(mpuBucket, next) {
            services.dataStore(mpuBucket, null, Buffer.concat(request.post), next);
        },
        function waterfall3(mpuBucket, extraArg, keys, next) {
            services.metadataStorePart(mpuBucket, keys, metaStoreParams, next);
        }

...
const memory = require('../metadata/in_memory/metadata').metadata;
...
function deleteData(objectGetInfo, log, cb) {
    data.delete(objectGetInfo, log, err => {
        if (err) {
            log.error('error deleting object data in mpu bucket', {
                error: err,
                method: 'objectPutPart::data.delete',
            });
            return cb(err);
        }
        return cb();
...
function safePutObjectMD(mpuBucketName, partKey, omVal, log, cb) {
    metadata.putObjectMD(mpuBucketName, partKey, omVal, {}, log, err => {
        if (err) {
            ...
            const objectGetInfo = omVal.partLocations[0];
            // Make a best effort single attempt to cleanup the orphaned data
            // while returning the original error from the metadata layer.
            return deleteData(objectGetInfo, log, () => cb(err));
        }
        return cb();
    });
}
...
function objectPutPart(authInfo, request, streamingV4Params, log,
    ...
    return async.waterfall([
        ...
        (destinationBucket, dataGetInfo, hexDigest, cipherBundle, partKey,
            ...
            if (process.env.TEST_API_DATA === 'true') {
                // Simulates a metadata failure for testing purposes.
                memory.buckets.delete(mpuBucketName);
            }
            return safePutObjectMD(mpuBucketName, partKey, omVal, log, err => {
                if (err) {
                    return next(err, destinationBucket);
                }

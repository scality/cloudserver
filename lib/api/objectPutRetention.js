...
const enableOPTIM1 = process.env.enableOPTIM1 === 'true';
const enableOPTIM2 = process.env.enableOPTIM1 === 'true';
/**
...
function objectPutRetention(authInfo, request, log, callback) {
    ...
    return async.waterfall([
        ...
        (bucket, retentionInfo, objectMD, next) => {
            ...
            if (enableOPTIM2) {
                metadata.updateObjectMD(bucket.getName(), objectKey, objectMD.versionId, {
                    'value.retentionMode': objectMD.retentionMode,
                    'value.retentionDate': objectMD.retentionDate,
                    'value.legalHold': objectMD.legalHold,
                }, params, log, err => next(err, bucket, objectMD));
            } else {

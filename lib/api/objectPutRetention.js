...
const { optimizedRetentionMetadataUpdate } = require('../metadata/metadataUtils');
...
// Enable optimized retention path via env var
const ENABLE_OPTIMIZED_RETENTION = process.env.ENABLE_OPTIMIZED_RETENTION === 'true';
...
function objectPutRetention(authInfo, request, log, callback) {
            ...
            next => {
                ...
                parseRetentionXml(request.post, log,
                    (err, retentionInfo) => {
                        ...
                        return next(null, retentionInfo, remainingDays);
                    });
            },
            (retentionInfo, remainingDays, next) => {
                const optimizedParams = {
                    ...
                    request: {
                        ...request,
                        objectLockRetentionDays: remainingDays
                    },
                    hasGovernanceBypass,
                    retentionInfo,
                    replicationAction: REPLICATION_ACTION
                };

                optimizedRetentionMetadataUpdate(optimizedParams, log, next);
            }
        ], (err, result) => {
            const additionalResHeaders = collectCorsHeaders(request.headers.origin,
                request.method, result?.bucket);
            if (err) {
                ...
                pushMetric('putObjectRetention', log, {
                    ...
                    bucket: bucketName,
                    keys: [objectKey],
                    versionId: result.objectMD ? result.objectMD.versionId : undefined,
                    location: result.objectMD ? result.objectMD.dataStoreName : undefined,
                });
                const verCfg = result.bucket.getVersioningConfiguration();
                additionalResHeaders['x-amz-version-id'] =
                    getVersionIdResHeader(verCfg, result.objectMD);
            }
    ...
    return async.waterfall([
        ...
        (retentionInfo, next) => {
            // Import standardMetadataValidateBucketAndObj here for fallback
            const { standardMetadataValidateBucketAndObj } = require('../metadata/metadataUtils');
            standardMetadataValidateBucketAndObj(metadataValParams,

...
function _storeIt(bucket, objectKey, objMD, authInfo, canonicalID, cipherBundle,
    ...
    const backendInfoObj = locationConstraintCheck(request, null, bucket, log);
    if (backendInfoObj.err) {
        return process.nextTick(() => {
            callback(backendInfoObj.err.customizeDescription('Location ' +
             'Constraint Info is invalid.'));
        });
    }

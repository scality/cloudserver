...
function _getVersioningParams(objectMD, versioningConfig, requestVid) {
    ...
    if (versioningConfig) {
        // if versionId is specified in request query, Metadata should
        // overwrite that version (if it exists)
        if (requestVid) {
            versioningParams = { versionId: requestVid };
        // TODO: Verify value for versioningConfiguration.Status is validated
        } else if (versioningConfig.Status === 'Enabled') {
            versioningParams = { versioning: true };
            // if latest version is null version, need to send separate request
            // to store null version in version history
            // NOTE: null versions can be created in two ways:
            // a) version of an object before versioning configuration existed
            // b) version of object when versioning was suspended
            // NOTE: should confirm want to store null version in either case
            // when versioning is enabled (test with AWS)
            if (objectMD && objectMD.isNull) {
                nullVersionParams = {
                    objectMD,
                    nullVersionId: objectMD.getattr('nullVersionId'),
                };
            }
        } else if (versioningConfig.Status === 'Suspended') {
            // this will overwrite the master version without generating
    ...
    return { versioningParams, isNull, nullVersionParams };
}
...
function _storeIt(bucketName, objectKey, objMD, authInfo, canonicalID,
    ...
    const { versioningParams, isNull, nullVersionParams } =
    _getVersioningParams(new versioning.Version(objMD), versioningConfiguration,

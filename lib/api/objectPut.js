...
const { errors, errorInstances, versioning, s3routes } = require('@scality/arsenal');
...
function objectPut(authInfo, request, streamingV4Params, log, callback) {
    ...
    const { bucketName, headers, method, objectKey, parsedContentLength, query } = request;
    const storageClassError = validateStorageClass(headers);
    if (storageClassError) {
        log.trace('invalid storage-class header');
        monitoring.promMetrics('PUT', request.bucketName, storageClassError.code, 'putObject');
        return callback(storageClassError);
    }
    if (!aclUtils.checkGrantHeaderValidity(headers)) {
        log.trace('invalid acl header');
        monitoring.promMetrics('PUT', request.bucketName, 400, 'putObject');
    ...
    if (Number.parseInt(size, 10) > constants.maximumAllowedUploadSize && !config.bypassMaxPutObjectSize) {
        log.debug('Upload size exceeds maximum allowed for a single PUT', { size });
        return callback(errors.EntityTooLarge);
    }

    const requestType = request.apiMethods || 'objectPut';
    const valParams = { authInfo, bucketName, objectKey, versionId, requestType, request, withVersionId: isPutVersion };
    ...
    return standardMetadataValidateBucketAndObj(valParams, request.actionImplicitDenies, log, (err, bucket, objMD) =>
        updateEncryption(err, bucket, objMD, objectKey, log, { skipObject: true }, (err, bucket, objMD) => {
            ...
            return async.waterfall(
                ...
                (err, storingResult) => {
                    ...
                    // metrics.
                    const ingestSize =
                        request.headers['x-amz-meta-mdonly'] && !Number.isNaN(request.headers['x-amz-meta-size'])
                            ? Number.parseInt(request.headers['x-amz-meta-size'], 10)
                            : null;
                    ...
                    const oldByteLength =
                        objMD && objMD['content-length'] !== undefined ? objMD['content-length'] : null;
                    if (storingResult) {

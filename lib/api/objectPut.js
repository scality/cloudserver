...
function objectPut(authInfo, request, streamingV4Params, log, callback) {
    ...
    (err, bucket, objMD) => {
        ...
        // Check rate limiting AFTER bucket metadata load
        // Uses fresh bucket config and populates cache for Phase 1
        if (config.rateLimiting && config.rateLimiting.enabled) {
            const { checkBucketRateLimit } = require('./apiUtils/rateLimit');
            const rateLimitResult = checkBucketRateLimit(bucketName, bucket, config, log);
            ...
            if (!rateLimitResult.allowed) {
                ...
                return callback(rateLimitResult.error, responseHeaders);
            }
        }

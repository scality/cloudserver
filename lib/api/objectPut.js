...
function objectPut(authInfo, request, streamingV4Params, log, callback) {
    ...
    // Create auth function - either real auth or dummy function
    const doAuthFunction = isParallelAuthEnabled && request.authParallel ?
        cb => {
            log.debug('running auth in parallel');
            request.authParallel(err => {
                if (err) {
                    log.debug('parallel auth failed but ignoring', { error: err });
                }
                cb();
            });
        } :
        cb => cb(); // Dummy function that just calls callback right away

    // Run auth (real or dummy) and validation in parallel
    return async.parallel([
        doAuthFunction,
        cb => standardMetadataValidateBucketAndObj(valParams, request.actionImplicitDenies, log,
            (err, bucket, objMD) => cb(err, { bucket, objMD }))
    ], (err, results) => {
        if (err) {

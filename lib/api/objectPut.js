...
function objectPut(authInfo, request, streamingV4Params, log, callback) {
    ...
    (err, bucket, objMD) => {
        ...
        // Check rate limiting (skip if pre-auth already checked to avoid double-counting)
        if (!request.rateLimitPreAuthChecked) {
            ...
            if (!checkBucketRateLimit(bucketName, bucket, _config, log)) {
                log.debug('request throttled', { bucketName, method: 'objectPut' });
                monitoring.promMetrics('PUT', bucketName, 429, 'putObject');
                return callback(_config.rateLimiting.error || errors.SlowDown, responseHeaders);
            }
        } else {
            log.debug('skipping post-metadata check (pre-auth already checked)', { bucketName, method: 'objectPut' });
        }

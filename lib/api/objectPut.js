...
// Import backend mocks for optimization
const { mockStandardMetadataValidateBucketAndObj, mockCreateAndStoreObject, 
    ENABLE_METADATA_MOCK, ENABLE_STORE_OBJECT_MOCK } = require('./apiUtils/mock/backendMocks');
...
const OPTIM_CLOUDSERVER_PUT_OBJECT = process.env.OPTIM_CLOUDSERVER_PUT_OBJECT === 'true';
...
function objectPut(authInfo, request, streamingV4Params, log, callback) {
    const setupLatency = performance.now();
    ...
    if (headers['x-amz-storage-class'] &&
        ...
        monitoring.promMetrics('PUT', request.bucketName,
            errorInstances.InvalidStorageClass.code, 'putObject');
    ...
    if (Number.parseInt(parsedContentLength, 10) > constants.maximumAllowedUploadSize) {
    ...
    const valParams = { authInfo, bucketName, objectKey, versionId,
        requestType, request, withVersionId: isPutVersion, isForPutObject: !!OPTIM_CLOUDSERVER_PUT_OBJECT };
    ...
    monitoring.monitorLatency('3-setup', performance.now() - setupLatency);

    const getMDLatency = performance.now();

    const metadataValidateFn = ENABLE_METADATA_MOCK ? 
        (params, actionImplicitDenies, log, callback) => mockStandardMetadataValidateBucketAndObj(standardMetadataValidateBucketAndObj, params, actionImplicitDenies, log, callback) :
        standardMetadataValidateBucketAndObj;

    return metadataValidateFn(valParams, request.actionImplicitDenies, log,
    (err, bucket, objMD) => updateEncryption(err, bucket, objMD, objectKey, log, { skipObject: true },
    (err, bucket, objMD) => {
        monitoring.monitorLatency('4-getMD', performance.now() - getMDLatency);
        const logicLatency = performance.now();
        ...
        return async.waterfall([
                ...
                return getObjectSSEConfiguration(headers, bucket, log,
                    (err, sseConfig) => {
                        if (err) {
                            ...
                            return next(errorInstances.InvalidArgument.customizeDescription(
            ...
            function objectCreateAndStore(cipherBundle, next) {
                ...
                monitoring.monitorLatency('5-logicLatency', performance.now() - logicLatency);
                
                const createAndStoreObjectFn = ENABLE_STORE_OBJECT_MOCK ?
                    (bucketName, bucket, objectKey, objMD, authInfo, canonicalID, cipherBundle,
                     request, isDeleteMarker, streamingV4Params, overheadField, log, originOp, callback) => 
                        mockCreateAndStoreObject(createAndStoreObject, bucketName, bucket, objectKey, objMD, authInfo,
                            canonicalID, cipherBundle, request, isDeleteMarker, streamingV4Params, overheadField, log, originOp, callback) :
                    createAndStoreObject;
                return createAndStoreObjectFn(bucketName,

...
function bucketPutACL(authInfo, request, log, callback) {
    ...
    const possibleGroups = [constants.allAuthedUsersId, constants.publicId, constants.logId];
    ...
    const possibleGrants = ['FULL_CONTROL', 'WRITE', 'WRITE_ACP', 'READ', 'READ_ACP'];
    const addACLParams = {
        Canned: '',
        FULL_CONTROL: [],
        WRITE: [],
    ...
    return async.waterfall(
            ...
            function waterfall2(bucket, next) {
                ...
                if (
                    ...
                    if (request.post) {
                        ...
                        return aclUtils.parseAclXml(request.post, log, (err, jsonGrants) =>
                            next(err, bucket, jsonGrants)
                        );
                    }
                ...
                if (jsonGrants) {
                    ...
                    for (let i = 0; i < usersIdentifiedByGroup.length; i++) {
                        ...
                        if (possibleGroups.indexOf(userGroup) < 0) {
                            log.trace('invalid user group', { userGroup, method: 'bucketPutACL' });
                            return next(errors.InvalidArgument, bucket);
                        }
                    }
        ...
        (err, bucket) => {
            ...
            if (err) {
                log.trace('error processing request', { error: err, method: 'bucketPutACL' });

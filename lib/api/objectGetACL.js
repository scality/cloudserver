    ...
    services.metadataValidateAuthorization(metadataValParams,
        (err, bucket, objectMD) => {
            ...
            if (objectACL.Canned !== '') {
                ...
                */
                let cannedGrants;
                if (bucket.owner !== objectMD['owner-id']) {
            ...
            */
            const canonicalIDs = allSpecificGrants.map(item => {
                if (grantsByURI.indexOf(item) < 0) {
                    return item;
                }
            }).filter(item => {
            ...
            * as display names
            */
            vault.getEmailAddresses(canonicalIDs, log, (err, emails) => {
                if (err) {
                    log.warn('error processing request',
                        { method: 'objectGetACL', error: err });
                    return callback(err);

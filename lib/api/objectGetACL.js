...
function objectGetACL(authInfo, request, log, callback) {
    ...
    return async.waterfall(
        [
            function validateBucketAndObj(next) {
                return standardMetadataValidateBucketAndObj(
                    ...
                    (err, bucket, objectMD) => {
                        if (err) {
                            log.trace('request authorization failed', { method: 'objectGetACL', error: err });
                            return next(err);
                        }
                        if (!objectMD) {
                            const err = versionId ? errors.NoSuchVersion : errors.NoSuchKey;
                            log.trace('error processing request', { method: 'objectGetACL', error: err });
                            return next(err, bucket);
                        }
                        if (objectMD.isDeleteMarker) {
                            if (versionId) {
                                log.trace('requested version is delete marker', { method: 'objectGetACL' });
                                // FIXME we should return a `x-amz-delete-marker: true` header,
                                // see S3C-7592
                                return next(errors.MethodNotAllowed);
                            }
                            log.trace('most recent version is delete marker', { method: 'objectGetACL' });
                            // FIXME we should return a `x-amz-delete-marker: true` header,
                            // see S3C-7592
            ...
            function gatherACLs(bucket, objectMD, next) {
                ...
                if (objectACL.Canned !== '') {
                    ...
                    if (bucket.getOwner() !== objectMD['owner-id']) {
                        cannedGrants = aclUtils.handleCannedGrant(objectACL.Canned, ownerGrant, bucket);
                    } else {
                        cannedGrants = aclUtils.handleCannedGrant(objectACL.Canned, ownerGrant);
                    }
                ...
                return vault.getEmailAddresses(canonicalIDs, log, (err, emails) => {
                    ...
                    const individualGrants = aclUtils.getIndividualGrants(objectACL, canonicalIDs, emails);
                    // Add to grantInfo any individual grants and grants by uri
                    grantInfo.grants = grantInfo.grants.concat(individualGrants).concat(uriGrantInfo);
        ...
        (err, bucket, xml, resVersionId) => {
            ...
            if (err) {
                monitoring.promMetrics('GET', bucketName, err.code, 'getObjectAcl');

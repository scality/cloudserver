...
let _putObjectData = function(dataRequest, body, dataConnectCallback) {
 //Assume bucket exists since vault confirmed bucket exists
 //Should there be a separate check to make sure data has the bucket?
...
module.exports = {
	...
	dataConnect: function(dataRequest, request, dataConnectCallback) {
		if(dataRequest.resource === 'objectPut'){
			return _putObjectData(dataRequest, request.body, dataConnectCallback);
		}
	...
	metadataConnect: function(metadataRequest, metadataRequestCallback) {

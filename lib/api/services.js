...
services.metadataValidate = function(request, cb) {
	let self = this;
	async.parallel([
	   self.bucketExists(request.bucketname, function(exists) {
	   	if(!exists){
	   		return cb("Bucket does not exist.", null);
	   	}
	   	return;
	   }),
	   self.isUserAuthorized(request.accessKey, request.bucketname, function(userAuthorized){
	   	if(!userAuthorized){
	   		return cb("User is not authorized.", null);
	   	}
	   	return;
	   })
	  ],

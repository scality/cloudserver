...
services.metadataValidateAuthorization = function(params, cb) {
	...
	if(bucket.owner === params.accessKey){
		if(bucket.keyMap[objectUID]){
			let objectDatastoreLocation = bucket.keyMap[objectUID]['location'];
			return cb(null, bucket, objectDatastoreLocation);
		}
 ...
 * @param {function} cb - callback containing result for the next task
 */
services.dataStore = function(bucket, objectDatastoreLocation, ds, params, cb) {
	...
	//If existing object, use or pass on current location or just put in new location and pass on new location?
	...
	this.putDataStore(ds, key, params.value, function(err, newLocation){
		...
		if(newLocation) {
			return cb(null, bucket, objectDatastoreLocation, newLocation);
		}
...
services.metadataStoreObject = function(bucket, objectExistsDatastoreLocation, newLocation, metastore, params, cb) {
	let omVal;
	let objectUID = params.objectUID;
	if(objectExistsDatastoreLocation){
		//Save objectExistsDatastoreLocation to have prior location?
...
services.metadataStoreBucket = function(accessKey, bucketUID, headers, locationConstraint, metastore, callback) {
	...
	services.getFromDatastore = function(bucket, objectDatastoreLocation, ds, params, cb) {

		//TODO: Handle range requests
		let result = ds[objectDatastoreLocation];
		if(!result) {
			return cb("Object not found in datastore", null);
		}
		cb(null, result);
	};

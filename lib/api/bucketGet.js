...
function bucketGet(authInfo, request, log, callback) {
    ...
    metadataValidateBucket(metadataValParams, log, (err, bucket) => {
        ...
        if (params.search !== undefined) {
            ...
            // TODO: update this so fallback is 'null'!!!
            const start = listParams.marker ? `"${listParams.marker}"` : 0;
            const searchCodeToExecute = 'new MetadataQuery' +
                '(spark, config,' +
                ...
                'SparkUtils.getQueryResults(spark,query)';
            ...
            return livyClient.getSessionOrBatchState('session', defaultSession,
                (err, res) => {
                    ...
                    const searchCode = 'query = ' + searchCodeToExecute;
                    return handleStatement(defaultSession, searchCode,

...
const setUpSessionCode = 'import com.scality.clueso._\n' +
    ...
    'loadCluesoConfig("/apps/spark-modules/application.conf"); config \n';
...
function handleStatement(sessionId, codeToExecute, corsHeaders, bucketName,
    ...
    log.debug('about to postStatement with codeToExecute', { codeToExecute });
    return livyClient.postStatement(sessionId,
        codeToExecute, (err, res) => {
            ...
            return livyClient.getStatement(sessionId, res.id, (err, res) => {
                ...
                // (3) handle nextMarkers, common prefixes, etc.
...
function bucketGet(authInfo, request, log, callback) {
    ...
    metadataValidateBucket(metadataValParams, log, (err, bucket) => {
        ...
        if (params.search !== undefined) {
            ...
            // params.search should just be sql
            // where clause (e.g. userMd['x-amz-meta-color']=blue)
            // spark should return keys starting AFTER marker alphabetically
            // spark should return up to maxKeys
            const start = listParams.marker || 'null';
            const searchCodeToExecute = 'new MetadataQuery(spark, config,' +
                ` ${bucketName}, ${params.search}, ${start}, ` +
                `${listParams.maxKeys});`;
            ...
            return livyClient.getSessionOrBatchState('session', defaultSession,
                err => {
                    if (err && err.message === 'Not Found') {
                        log.info('livy session not found. creating');
                        return livyClient.postSession({ kind: 'spark' },
                        (err, res) => {
                            if (err) {
                    ...
                    if (err) {
                        log.info('error from livy getting session', { error:
                            err.message });
                        return callback(errors.InternalError
                            .customizeDescription('Error performing search'),
                            null, corsHeaders);
                    }

    ...
    return livyClient.postStatement(sessionId,
        codeToExecute, (err, res) => {
            ...
            return livyClient.getStatement(sessionId, res.id, (err, res) => {
                ...
                // TODO: UPDATE THIS.  JUST RETURNING WHOLE LIVY RESPONSE FOR NOW
                // SO CAN SEE RESULT
...
function bucketGet(authInfo, request, log, callback) {
    ...
    metadataValidateBucket(metadataValParams, log, (err, bucket) => {
        ...
        if (params.search !== undefined || params.search === 'test') {
            ...
            // If at least 5 active and busy/starting, return SlowDown error
            // (don't want to create too many since holding dataframes in mem within a session)
            // If idle sessions, use random available one
            return livyClient.getSessions(null, null,
                (err, res) => {
                    if (err || !res) {
                        log.info('err from livy listing sessions'), { error: err };
                        return callback(errors.InternalError
                            .customizeDescription('Error contacting spark for search'),
                            null, corsHeaders);
                    }
                    ...
                    return handleStatement(availableSession.sessionId, searchCodeToExecute,

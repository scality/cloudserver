...
let _constructJSON = function(json, xmlParams){
	...
	if(commonPrefixes.length > 0){
		let commonPrefixesObject = {"CommonPrefixes": commonPrefixes};
	...
	for(let i=0; i< json.Contents.length; i+=1){
		let contentItem = {
			"Contents": [
				...
				{"Owner": [{"ID": json.Contents[i].Owner.ID}, {"DisplayName": json.Contents[i].Owner.DisplayName}]},
 ...
 * @param  {function} callback - callback to respond to http request with either error code or xml response body
 */
...
let bucketGet = function(accessKey, metastore, request, callback) {
	...
	let xmlParams = {
		...
		host: request.lowerCaseHeaders.host.split(":")[0]
	};

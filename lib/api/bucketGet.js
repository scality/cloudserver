    ...
    standardMetadataValidateBucket(metadataValParams, request.actionImplicitDenies, log, (err, bucket) => {
        ...
        if (!request.rateLimitPreAuthChecked) {
            ...
            if (!checkBucketRateLimit(bucketName, bucket, _config, log)) {
                log.debug('request throttled', { bucketName, method: 'bucketGet' });
                monitoring.promMetrics('GET', bucketName, 429, 'listBucket');
                return callback(_config.rateLimiting.error || errors.SlowDown, null, corsHeaders);
            }
        } else {
            log.debug('skipping post-metadata check (pre-auth already checked)', { bucketName, method: 'bucketGet' });
        }

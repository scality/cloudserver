...
const skipUrlEncoding = new Set(['ContinuationToken', 'NextContinuationToken']);
...
function processVersions(bucketName, listParams, list) {
    ...
    xmlParams.forEach(p => {
        if (p.value) {
            const val =
                p.tag !== 'NextVersionIdMarker' || p.value === 'null' ? p.value : versionIdUtils.encode(p.value);
    ...
    list.Versions.forEach(item => {
        ...
        xml.push(
            ...
            `<Key>${objectKey}</Key>`,
            '<VersionId>',
            v.IsNull || v.VersionId === undefined ? 'null' : versionIdUtils.encode(v.VersionId),
            '</VersionId>',
...
function processMasterVersions(bucketName, listParams, list) {
    ...
    list.Contents.forEach(item => {
        ...
        return xml.push(`<StorageClass>${v.StorageClass}</StorageClass>`, '</Contents>');
    });
    list.CommonPrefixes.forEach(item => {
        const val = escapeXmlFn(item);
        xml.push(`<CommonPrefixes><Prefix>${val}</Prefix></CommonPrefixes>`);
...
function bucketGet(authInfo, request, log, callback) {
    ...
    if (v2 !== undefined && Number.parseInt(v2, 10) !== 2) {
        return callback(
            errorInstances.InvalidArgument.customizeDescription('Invalid ' + 'List Type specified in Request')
        );
    }
    if (v2) {

...
function handleResult(listParams, requestMaxKeys, encoding, authInfo,
    ...
    const handleResultSpan = tracer.startSpan('Cloudserver:: Preparing result', undefined, traceContext);
...
function bucketGet(authInfo, request, log, callback, parentSpan) {
    ...
    const bucketMetadataSpan = tracer.startSpan('Cloudserver:: Validating Bucket Metadata', undefined, ctx);
    standardMetadataValidateBucket(metadataValParams, request.actionImplicitDenies, log, (err, bucket) => {
        ...
        const objectListSpan = tracer.startSpan('Cloudserver:: Listing object in the bucket', undefined, ctx);

...
function constructTreefromQuery(query, prefix) {
    if (!query)
        return prefix;
    const tree = [];
    const queryList = query.split("&");
    for (let i=0; i<queryList.length; i++) {
        if (queryList[i] !== "x-amz-meta-op:NOT")
            tree.push("T|"+queryList[i].split(":")[0]+"/"+queryList[i].split(":")[1])
        else {
            tree.push("T|"+queryList[i].split(":")[0]+"/"+queryList[i].split(":")[1]+":T|"+queryList[i+1].split(":")[0]+"/"+queryList[i+1].split(":")[1])
            i+=1
        }
    }
    return tree;
...
export default function bucketGet(authInfo, request, log, callback) {
    ...
    const listParams = {
        ...
        prefix: constructTreefromQuery(request.headers.meta, params.prefix),
    };

...
function initiateMultipartUpload(authInfo, request, log, callback) {
    ...
    if (
        ...
        monitoring.promMetrics('PUT', bucketName, errorInstances.InvalidStorageClass.code, 'initiateMultipartUpload');
    ...
    if (!validateWebsiteHeader(websiteRedirectHeader)) {
        ...
        log.debug(`invalid x-amz-website-redirect-locationvalue ${websiteRedirectHeader}`, { error: err });
        return callback(err);
    }
    ...
    const storageClassOptions = ['standard', 'standard_ia', 'reduced_redundancy'];
    let storageClass = 'STANDARD';
    if (storageClassOptions.indexOf(request.headers['x-amz-storage-class']) > -1) {
        storageClass = request.headers['x-amz-storage-class'].toUpperCase();
    }

...
const { errors, errorInstances, s3middleware, s3routes } = require('@scality/arsenal');
...
const { validateStorageClass } = require('./apiUtils/object/storageClass');
const { validateHeaders, compareObjectLockInformation } = require('./apiUtils/object/objectLockHelpers');
...
function initiateMultipartUpload(authInfo, request, log, callback) {
    ...
    if (hasNonPrintables(objectKey)) {
        return callback(
            errorInstances.InvalidInput.customizeDescription('object keys cannot contain non-printable characters'),
        );
    }
    ...
    const storageClassError = validateStorageClass(request.headers);
    if (storageClassError) {
        log.trace('invalid storage-class header');
        monitoring.promMetrics('PUT', bucketName, storageClassError.code, 'initiateMultipartUpload');
        return callback(storageClassError);
    }
    ...
    const checksumConfig = areChecksumsEnabled() ? getChecksumDataFromMPUHeaders(request.headers) : null;
    if (checksumConfig && checksumConfig.error) {
    ...
    const storageClass = request.headers['x-amz-storage-class'] || 'STANDARD';
    ...
    function _getMPUBucket(destinationBucket, log, corsHeaders, uploadId, cipherBundle, locConstraint, callback) {
        // External backends don't support MPU checksums so delete them from the metadata.
        const isExternalLocation = !!constants.externalBackends[config.getLocationConstraintType(locConstraint)];
        if (isExternalLocation) {
            delete metadataStoreParams.checksumAlgorithm;
            delete metadataStoreParams.checksumType;
            delete metadataStoreParams.checksumIsDefault;
        }
        const xmlParams = {
            bucketName,
            objectKey,
        ...
        services.getMPUBucket(destinationBucket, bucketName, log, (err, MPUbucket) => {
            if (err) {
            ...
            return services.metadataStoreMPObject(
                MPUbucket.getName(),
                cipherBundle,
                metadataStoreParams,
                log,
                (err, mpuMD) => {
                    ...
                    // Only respond the headers if the user sent them and
                    // the MPU can honor them (not an external backend).
                    if (checksumConfig && !checksumConfig.isDefault && !isExternalLocation) {
                    ...
                    monitoring.promMetrics('PUT', bucketName, '200', 'initiateMultipartUpload');
    ...
    function _storetheMPObject(destinationBucket, corsHeaders, serverSideEncryption) {
        ...
        const backendInfoObj = locationConstraintCheck(request, null, destinationBucket, log);
        ...
        if (request.headers) {
            const objectLockValError = validateHeaders(destinationBucket, request.headers, log);
        ...
        const finalObjectLockInfo = compareObjectLockInformation(request.headers, defaultRetention);

        if (finalObjectLockInfo.retentionInfo) {
            metadataStoreParams.retentionMode = finalObjectLockInfo.retentionInfo.mode;
            metadataStoreParams.retentionDate = finalObjectLockInfo.retentionInfo.date;
        }
        ...
        return data.initiateMPU(mpuInfo, websiteRedirectHeader, log, (err, dataBackendResObj, isVersionedObj) => {
            // will return as true and a custom error if external backend does
            // not support versioned objects
            if (isVersionedObj) {
                monitoring.promMetrics('PUT', bucketName, 501, 'initiateMultipartUpload');
                return callback(err);
            }
            if (err) {
                monitoring.promMetrics('PUT', bucketName, err.code, 'initiateMultipartUpload');
            ...
            return _getMPUBucket(destinationBucket, log, corsHeaders, uploadId, cipherBundle, locConstraint, callback);
        });
    }
        ...
        [
            ...
            (corsHeaders, destinationBucket, next) => {
                ...
                if (destinationBucket.hasTransientFlag() || destinationBucket.hasDeletedFlag()) {
                    ...
                    return cleanUpBucket(destinationBucket, accountCanonicalID, log, error => {
                        if (error) {
            ...
            (corsHeaders, destinationBucket, next) =>
                getObjectSSEConfiguration(request.headers, destinationBucket, log, (error, objectSSEConfig) => {
                    if (error) {
            ...
            (corsHeaders, destinationBucket, objectSSEConfig, next) => {
                if (objectSSEConfig) {
                    return kms.createCipherBundle(objectSSEConfig, log, err =>
                        next(err, corsHeaders, destinationBucket, objectSSEConfig),
                    );
                }
                return next(null, corsHeaders, destinationBucket, objectSSEConfig);
            },

...
const checkObjectLock = require('./apiUtils/object/checkObjectLock');
...
function initiateMultipartUpload(authInfo, request, log, callback) {
    ...
    function _storetheMPObject(destinationBucket, corsHeaders) {
        ...
        const retentionObj = checkObjectLock(destinationBucket, request.headers);
        if (retentionObj.error) {
            return callback(retentionObj.error);
        }
        if (retentionObj.retentionInfo) {
            metadataStoreParams.retentionInfo = retentionObj.retentionInfo;
        }
        if (retentionObj.legalHold) {
            metadataStoreParams.legalHold = retentionObj.legalHold;
        }

...
const { errors, errorInstances, versioning, s3middleware, models, jsutil } = require('arsenal');
...
const { algorithms, areChecksumsEnabled } = require('./apiUtils/integrity/validateChecksums');
const { buildSourcePartsStream, computeChecksumFromDataLocator } = require('./apiUtils/object/sourceChecksum');
...
function _shouldRecomputeChecksum(request, sourceChecksum, algo) {
    if (!areChecksumsEnabled()) {
        return false;
    }
    if (request.headers['x-amz-copy-source-range']) {
        return true;
    }
    return !(
        sourceChecksum &&
        sourceChecksum.checksumType === 'FULL_OBJECT' &&
        sourceChecksum.checksumAlgorithm === algo
    );
}

/**
 * Copy a part and calculate its checksum in a single streaming pass.
 *
 * @param {Array} dataLocator - ordered (range-adjusted) source parts
 * @param {number} size - copied part size in bytes
 * @param {object|null} sse - server-side encryption config for the dest MPU
 * @param {string} destLocationConstraint - destination MPU location constraint
 * @param {object} dataStoreContext - destination object data-store context
 * @param {string} algo - checksum algorithm to compute
 * @param {object} log - request logger
 * @param {function} cb - cb(err, { locations, totalHash, checksum })
 * @return {undefined}
 */
function _copyPartStreamingWithChecksum(
    dataLocator,
    size,
    sse,
    destLocationConstraint,
    dataStoreContext,
    algo,
    log,
    cb,
 ...
 * @return {undefined}
 */
function objectPutCopyPart(authInfo, request, sourceBucket, sourceObject, reqVersionId, log, callback) {
    ...
    if (partNumber > 10000 || !Number.isInteger(partNumber) || partNumber < 1) {
        monitoring.promMetrics('PUT', destBucketName, 400, 'putObjectCopyPart');
    ...
    return async.waterfall(
        [
            function checkDestAuth(next) {
                return standardMetadataValidateBucketAndObj(
                    ...
                    (err, destBucketMD) => {
                        if (err) {
                            log.debug('error validating authorization for destination bucket', { error: err });
            ...
            function checkSourceAuthorization(destBucketMD, next) {
                return standardMetadataValidateBucketAndObj(
                    ...
                    (err, sourceBucketMD, sourceObjMD) => {
                        ...
                        if (sourceObjMD.isDeleteMarker) {
                            ...
                            if (reqVersionId) {
                                err = errorInstances.InvalidRequest.customizeDescription(
                                    'The source of a copy request may not specifically ' +
                                        'refer to a delete marker by version id.',
                                );
                            } else {
                        ...
                        const copyLocator = setUpCopyLocator(
                            sourceObjMD,
                            request.headers['x-amz-copy-source-range'],
                            log,
                        );
                        if (copyLocator.error) {
                            // eslint-disable-next-line no-param-reassign
                            request.sourceServerAccessLog && (request.sourceServerAccessLog.error = copyLocator.error);
                        ...
                        if (sourceBucketMD.getVersioningConfiguration() || reqVersionId) {
                            if (sourceObjMD.isNull || !sourceObjMD.versionId) {
                                sourceVerId = 'null';
                            } else {
                                sourceVerId = versionIdUtils.encode(sourceObjMD.versionId);
                            }
                        }
                        return next(
                            null,
                            copyLocator.dataLocator,
                            destBucketMD,
            ...
            function _validateQuotas(
                ...
                return validateQuotas(
                    ...
                    valPutParams.requestType,
                    request.apiMethod,
                    sourceObjMD?.['content-length'] || 0,
                    false,
                    log,
                    err =>
                        next(
            ...
            function getMpuShadowBucket(
                ...
                return metadata.getBucket(mpuBucketName, log, (err, mpuBucket) => {
            ...
            function getMpuOverviewObject(
                ...
                return metadata.getObjectMD(mpuBucketName, mpuOverviewKey, null, log, (err, mpuOverviewMD) => {
                    if (err) {
                        // TODO: move to `.is` once BKTCLT-9 is done and bumped in Cloudserver
                        if (err.NoSuchKey) {
                            return next(errors.NoSuchUpload);
                        }
                        log.error('error getting overview object from mpu bucket', {
                            error: err,
                            method: 'objectPutCopyPart::metadata.getObjectMD',
                        });
                        return next(err);
                    }
                    const initiatorID = mpuOverviewMD.initiator.ID;
                    const requesterID = authInfo.isRequesterAnIAMUser() ? authInfo.getArn() : authInfo.getCanonicalID();
                    if (initiatorID !== requesterID) {
                        return next(errors.AccessDenied);
                    }
                    const destObjLocationConstraint = mpuOverviewMD.controllingLocationConstraint;
                    const sseAlgo = mpuOverviewMD['x-amz-server-side-encryption'];
                    const sse = sseAlgo
                        ? {
                              algorithm: sseAlgo,
                              masterKeyId: mpuOverviewMD['x-amz-server-side-encryption-aws-kms-key-id'],
                          }
                        : null;
                    return next(
                        null,
            ...
            function goGetData(
                ...
                const destIsExternal =
                    constants.externalBackends[config.getLocationConstraintType(destObjLocationConstraint)];
                if (recompute && dataLocator.length > 0 && !destIsExternal) {
                    return _copyPartStreamingWithChecksum(
                        dataLocator,
                        copyObjectSize,
                        sse,
                        destObjLocationConstraint,
                ...
                return data.uploadPartCopy(
                    ...
                    (error, eTag, lastModified, serverSideEncryption, locations) => {
                        // eslint-disable-next-line no-param-reassign
                        request.actionImplicitDenies = originalIdentityAuthzResults;
                        const isSkip = error && error.message === 'skip';
                        if (error && !isSkip) {
                            // eslint-disable-next-line no-param-reassign
                            request.sourceServerAccessLog && (request.sourceServerAccessLog.error = error);
                            return next(error, destBucketMD);
                        }

                        // Skip checksum compute for external backends matching UploadPart.
                        if (recompute && !destIsExternal) {
                            return computeChecksumFromDataLocator(dataLocator, algo, log, (cksErr, partChecksum) => {
                                if (cksErr) {
                                    // eslint-disable-next-line no-param-reassign
                                    request.sourceServerAccessLog && (request.sourceServerAccessLog.error = cksErr);
                                    return next(cksErr, destBucketMD);
                                }
                                if (isSkip) {
                                    return next(
                                        skipError,
                        ...
                        // Reuse the source's stored checksum, or none for a legacy or
                        // external-backend MPU.
                        const partChecksum =
                            algo && !destIsExternal && areChecksumsEnabled()
                                ? { algorithm: algo, value: sourceObjMD.checksum.checksumValue }
                                : undefined;
                        if (isSkip) {
                            return next(
                                skipError,
            ...
            function getExistingPartInfo(
                ...
                metadata.getObjectMD(mpuBucketName, partKey, {}, log, (err, result) => {
                    // If there is nothing being overwritten just move on
                    // TODO: move to `.is` once BKTCLT-9 is done and bumped in Cloudserver
                    if (err && !err.NoSuchKey) {
                        log.debug('error getting current part (if any)', { error: err });
                        return next(err);
                    }
                    let oldLocations;
                    ...
                    if (result) {
                        ...
                        oldLocations = Array.isArray(oldLocations) ? oldLocations : [oldLocations];
                    }
                    return next(
                        null,
                        destBucketMD,
            ...
            function storeNewPartMetadata(
                ...
                if (partChecksum) {
                    metaStoreParams.checksumValue = partChecksum.value;
                    metaStoreParams.checksumAlgorithm = partChecksum.algorithm;
                }
                return services.metadataStorePart(mpuBucketName, locations, metaStoreParams, log, err => {
                    if (err) {
                        log.debug('error storing new metadata', { error: err, method: 'storeNewPartMetadata' });
                        return next(err);
                    }
                    return next(
                        null,
            ...
            function cleanupExistingData(
                ...
                if (oldLocationsToDelete) {
                    return data.batchDelete(oldLocationsToDelete, request.method, null, log, err => {
                        if (err) {
        ...
        (
            ...
            if (request.serverAccessLog) {
                // eslint-disable-next-line no-param-reassign
                request.serverAccessLog.objectSize = copyObjectSize;
            }

            // Initialize the queue for internal log request logging
            initializeInternalLogRequestQueue(request);
            // Queue the source-side access log (REST.COPY.PART_GET)
            queueInternalLogRequest(request, {
                operation: 'REST.COPY.PART_GET',
                sourceBucket,
                sourceObject,
                objectSize: copyObjectSize || null,
            });

            if (err && err !== skipError) {
                log.trace('error from copy part waterfall', { error: err });
                monitoring.promMetrics('PUT', destBucketName, err.code, 'putObjectCopyPart');
            ...
            // Surface the part checksum only for non-default MPUs like AWS.
            if (partChecksum && !mpuOverviewMD.checksumIsDefault) {
                const xmlTag = algorithms[partChecksum.algorithm] && algorithms[partChecksum.algorithm].xmlTag;
                if (xmlTag) {
                    xml.push(`<${xmlTag}>`, partChecksum.value, `</${xmlTag}>`);
                }
            }
            xml.push('</CopyPartResult>');
            const xmlStr = xml.join('');

            const additionalHeaders = corsHeaders || {};
            if (serverSideEncryption) {
                setSSEHeaders(additionalHeaders, serverSideEncryption.algorithm, serverSideEncryption.masterKeyId);
            }
            ...
            monitoring.promMetrics('PUT', destBucketName, '200', 'putObjectCopyPart');
            return callback(null, xmlStr, additionalHeaders);
        },
    );
}
...
module.exports._shouldRecomputeChecksum = _shouldRecomputeChecksum;
module.exports._copyPartStreamingWithChecksum = _copyPartStreamingWithChecksum;

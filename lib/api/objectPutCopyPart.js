    ...
    return async.waterfall([
        ...
        function _validateQuotas(dataLocator, destBucketMD,
            ...
            return validateQuotas(request, destBucketMD, request.accountQuotas, valPutParams.requestType,
                ...
                next(err, dataLocator, destBucketMD, copyObjectSize, sourceVerId,
                    sourceLocationConstraintName, sourceObjMD));
        },
            ...
            return metadata.getObjectMD(mpuBucketName, mpuOverviewKey,
                    null, log, (err, mpuOverviewMD) => {
                        ...
                        const destObjLocationConstraint =
                            mpuOverviewMD.controllingLocationConstraint;
                ...
                (error, eTag, lastModified, serverSideEncryption, locations) => {
                    ...
                    if (recompute && !destIsExternal) {
                        return computeChecksumFromDataLocator(dataLocator, algo, log, (cksErr, partChecksum) => {
                            ...
                            if (isSkip) {
                                return next(skipError, destBucketMD, eTag, lastModified, sourceVerId,
                                    serverSideEncryption, undefined, undefined, mpuOverviewMD, partChecksum);
                            }
                            return next(null, destBucketMD, locations, eTag, copyObjectSize, sourceVerId,
                                serverSideEncryption, lastModified, splitter, mpuOverviewMD, partChecksum);
                        });
                    }

                    // Reuse the source's stored checksum, or none for a legacy or
                    // external-backend MPU.
                    const partChecksum = (algo && !destIsExternal)
        ...
        function getExistingPartInfo(destBucketMD, locations, totalHash,
            ...
            splitter, mpuOverviewMD, partChecksum, next) {
            const partKey =
        ...
        function storeNewPartMetadata(destBucketMD, locations, totalHash,
            ...
            lastModified, oldLocations, splitter, mpuOverviewMD, partChecksum, next) {
        ...
        function checkCanDeleteOldLocations(partLocations, oldLocations, destBucketMD,
            ...
            prevObjectSize, copyObjectSize, splitter, mpuOverviewMD, partChecksum, next) {
            if (!oldLocations) {
                return next(null, oldLocations, destBucketMD, totalHash,
                    lastModified, sourceVerId, serverSideEncryption,
                    prevObjectSize, copyObjectSize, mpuOverviewMD, partChecksum);
            }
        ...
        function cleanupExistingData(oldLocationsToDelete, destBucketMD, totalHash,
            lastModified, sourceVerId, serverSideEncryption,
            prevObjectSize, copyObjectSize, mpuOverviewMD, partChecksum, next) {
        ...
        if (partChecksum && !mpuOverviewMD.checksumIsDefault) {
            const xmlTag = algorithms[partChecksum.algorithm]
                && algorithms[partChecksum.algorithm].xmlTag;
            if (xmlTag) {

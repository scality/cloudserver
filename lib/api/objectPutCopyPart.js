...
function objectPutCopyPart(authInfo, request, sourceBucket,
    ...
    return async.waterfall([
        ...
        function checkSourceAuthorization(destBucketMD, next) {
            return metadataValidateBucketAndObj(valGetParams, log,
                (err, sourceBucketMD, sourceObjMD) => {
                    ...
                    const sourceLocationConstraintName =
        ...
        function goGetData(dataLocator, destBucketMD,
            ...
            // NOTE: using uploadPartCopy only if copying object from one
            // external backend to the same external backend
            ...
            if (locationTypeMatch && dataLocator.length === 1) {
                ...
                destObjLocationConstraint, awsSourceKey,
                sourceLocationConstraintName, log, (error, eTag) => {
                    if (error) {
                        return next(error, destBucketMD);
                    }
                    return next(null, destBucketMD, dataLocator, eTag,
                                copyObjectSize, sourceVerId,
                                serverSideEncryption, lastModified);
                });
            }

            const backendInfo = new BackendInfo(destObjLocationConstraint);
            ...
            return async.forEachOfSeries(dataLocator,
                ...
                function copyPart(part, index, cb) {
                    return data.get(part, null, log, (err, stream) => {
                        ...
                        // destObjLocationConstraint is location of the
                        // destination MPU object
                        if (externalBackends[config
                            .locationConstraints[destObjLocationConstraint]
                            .type]) {
                            return multipleBackendGateway.uploadPart(null, null,
                            hashedStream, numberPartSize,
                            destObjLocationConstraint, destObjectKey, uploadId,
                            partNumber, destBucketName, log, err => {
                                ...
                                // uploadPart did the job of storing the data
                                return cb();
                            });
                        }

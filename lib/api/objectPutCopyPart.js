...
function objectPutCopyPart(authInfo, request, sourceBucket,
    ...
    return async.waterfall([
        ...
        function validateQuotas(dataLocator, destBucketMD,
            copyObjectSize, sourceVerId,
            sourceLocationConstraintName, sourceObjMD, next) {
                return validateQuotas(destBucketMD, valPutParams.requestType, request.apiMethod,
                    sourceObjMD?.['content-length'] || 0, log)
                    .then(err => next(err, dataLocator, destBucketMD,
                        copyObjectSize, sourceVerId,
                        sourceLocationConstraintName))
                    .catch(err => next(err, dataLocator, destBucketMD,
                        copyObjectSize, sourceVerId,
                        sourceLocationConstraintName,));
            },

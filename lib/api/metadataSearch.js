...
const { decryptToken } = require('../api/apiUtils/object/continueToken');
const { processVersions, processMasterVersions } = require('./bucketGet');

function handleResult(listParams, requestMaxKeys, encoding, authInfo, bucketName, list, corsHeaders, log, callback) {
...
function metadataSearch(authInfo, request, log, callback) {
    ...
    if (v2 !== undefined && Number.parseInt(v2, 10) !== 2) {
        return callback(errorInstances.InvalidArgument.customizeDescription('Invalid List Type specified in Request'));
    }
    log.debug('processing request', { method: 'metadataSearch' });
    const encoding = params['encoding-type'];
    if (encoding !== undefined && encoding !== 'url') {
        monitoring.promMetrics('GET', bucketName, 400, 'metadataSearch');
        return callback(
            errorInstances.InvalidArgument.customizeDescription('Invalid Encoding Method specified in Request'),
        );
    }
    ...
    try {
        ...
        monitoring.promMetrics('GET', bucketName, 400, 'metadataSearch');
        return callback(
            errorInstances.InvalidArgument.customizeDescription('Invalid sql where clause sent as search query'),
        );
    }
    if (v2) {
        listParams.v2 = true;
    ...
    standardMetadataValidateBucket(metadataValParams, request.actionImplicitDenies, log, (err, bucket) => {
        ...
        if (err) {
            ...
            monitoring.promMetrics('GET', bucketName, err.code, 'metadataSearch');

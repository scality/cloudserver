    ...
    async.waterfall([
        ...
        function waterfall2(bucket, extraArgumentFromPreviousFunction, next) {
            ...
            if (allSpecificGrants.length > 0) {
                return services.getManyDisplayNames(allSpecificGrants, next);
            }
            return next(null, null);
        },
        function waterfall3(accountIdentifiers, next) {
            if (accountIdentifiers) {
                accountIdentifiers.forEach((item) => {
                    let permission = 'READ';
                    if (bucketACL.FULL_CONTROL.indexOf(item.canonicalID) > -1) {
                        permission = 'FULL_CONTROL';
                    } else if (bucketACL.WRITE.indexOf(item.canonicalID) > -1) {
                        permission = 'WRITE';
                    } else if (bucketACL.WRITE_ACP
                            .indexOf(item.canonicalID) > -1) {
                        permission = 'WRITE_ACP';
                    } else if (bucketACL.READ_ACP
                            .indexOf(item.canonicalID) > -1) {
                        permission = 'READ_ACP';
                    }
                    const grant = {
                ...
                grantsByURI.forEach((uri) => {
                    let permission = '';
                    if (bucketACL.FULL_CONTROL.indexOf(uri) > -1) {
                        permission = 'FULL_CONTROL';
                    } else if (bucketACL.WRITE.indexOf(uri) > -1) {
                        permission = 'WRITE';
                    } else if (bucketACL.WRITE_ACP
                            .indexOf(uri) > -1) {
                        permission = 'WRITE_ACP';
        ...
        console.log('xml from get bucket acl', xml);

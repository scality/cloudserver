...
function bucketGetACL(authInfo, request, log, callback) {
    ...
    standardMetadataValidateBucket(metadataValParams, request.actionImplicitDenies, log, (err, bucket) => {
        ...
        if (err) {
            log.debug('error processing request', { method: 'bucketGetACL', error: err });
            monitoring.promMetrics('GET', bucketName, err.code, 'getBucketAcl');
        ...
        if (bucketACL.Canned !== '') {
            const cannedGrants = aclUtils.handleCannedGrant(bucketACL.Canned, ownerGrant);
        ...
        return vault.getEmailAddresses(canonicalIDs, log, (err, emails) => {
            if (err) {
                log.debug('error processing request', { method: 'vault.getEmailAddresses', error: err });
                monitoring.promMetrics('GET', bucketName, err.code, 'getBucketAcl');
                return callback(err, null, corsHeaders);
            }
            const individualGrants = aclUtils.getIndividualGrants(bucketACL, canonicalIDs, emails);

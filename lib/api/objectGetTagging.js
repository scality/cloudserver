...
function objectGetTagging(authInfo, request, log, callback) {
    ...
    return async.waterfall(
        [
            next =>
                standardMetadataValidateBucketAndObj(
                    ...
                    (err, bucket, objectMD) => {
                        if (err) {
                            log.trace('request authorization failed', { method: 'objectGetTagging', error: err });
                            return next(err);
                        }
                        if (!objectMD) {
                            const err = reqVersionId ? errors.NoSuchVersion : errors.NoSuchKey;
                            log.trace('error no object metadata found', { method: 'objectGetTagging', error: err });
                            return next(err, bucket);
                        }
                        if (objectMD.isDeleteMarker) {
                            if (reqVersionId) {
                                log.trace('requested version is delete marker', { method: 'objectGetTagging' });
                                return next(errors.MethodNotAllowed);
                                // FIXME we should return a `x-amz-delete-marker: true` header,
                                // see S3C-7592
                            }
                            log.trace('most recent version is delete marker', { method: 'objectGetTagging' });
                            // FIXME we should return a `x-amz-delete-marker: true` header,
                            // see S3C-7592
        ...
        (err, bucket, xml, objectMD) => {
            ...
            if (err) {
                log.trace('error processing request', { error: err, method: 'objectGetTagging' });
                monitoring.promMetrics('GET', bucketName, err.code, 'getObjectTagging');
            } else {
                ...
                monitoring.promMetrics('GET', bucketName, '200', 'getObjectTagging');

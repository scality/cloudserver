...
const { validatePolicyResource, validatePolicyConditions } = require('./apiUtils/authorization/permissionChecks');
...
function _checkNotImplementedPolicy(policyString) {
    ...
    return policyString.includes('"Service"') || policyString.includes('"Federated"');
}
...
function bucketPutPolicy(authInfo, request, log, callback) {
    ...
    return async.waterfall(
            ...
            next => {
                ...
                process.nextTick(() => {
                    ...
                    if (_checkNotImplementedPolicy(request.post)) {
                        const err = errorInstances.NotImplemented.customizeDescription(
                            'Bucket policy contains element not yet implemented',
                        );
                        return next(err);
                    }
                    if (policyObj.error) {
                        const err = errorInstances.MalformedPolicy.customizeDescription(policyObj.error.description);
                        return next(err);
                    }
            ...
            (bucketPolicy, next) => {
                process.nextTick(() => {
                    if (!validatePolicyResource(bucketName, bucketPolicy)) {
                        return next(errorInstances.MalformedPolicy.customizeDescription('Policy has invalid resource'));
                    }
        ...
        (err, bucket) => {
            ...
            if (err) {
                log.trace('error processing request', { error: err, method: 'bucketPutPolicy' });

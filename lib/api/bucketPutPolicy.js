...
const { errorInstances, models } = require('@scality/arsenal');
    ...
    return async.waterfall([
        next => {
            ...
            process.nextTick(() => {
                ...
                if (policyObj.error) {
                    const err = errorInstances.MalformedPolicy.customizeDescription(
        ...
        (bucketPolicy, next) => {
            process.nextTick(() => {
                if (!validatePolicyResource(bucketName, bucketPolicy)) {
                    return next(errorInstances.MalformedPolicy.customizeDescription(

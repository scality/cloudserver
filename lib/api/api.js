...
const api = {
    callApiMethod(apiMethod, request, response, log, callback, oTel) {
        ...
        activeSpan.addEvent('Using cloudserver to call API method');
        cloudserverApiSpan.updateName(`Performing operations to satisfy ${action} API request. Cloudserver API method: ${apiMethod}()`);
        // cloudserverApiSpan.updateName(`Performing operations to satisfy ${action} API related operations.`);
        ...
        return tracer.startActiveSpan('Preparing request object for authentication', undefined, cloudserverApiSpanContext, callApiMethod => {
            ...
            apiMethod === 'corsPreflight') {
                ...
                callApiMethod.end();
                activeSpan.addEvent(`Routing request to API method ${apiMethod}`);
            ...
            callApiMethod.end();
            activeSpan.addEvent('Authenticating and authorizing user');
            return async.waterfall([
                next => tracer.startActiveSpan('Authentication and authorization of the request', undefined, cloudserverApiSpanContext, iamAuthSpan => {
                    activeSpan.addEvent('Authentication and authorization against Vault started');
                    iamAuthSpan.setAttributes({
                        'code.function': 'callApiMethod()',
                        'code.filename': 'lib/api/api.js',
                        'code.lineno': 220,
                        'rpc.method': 'callApiMethod()',
                    });
                    return next(null, iamAuthSpan);
                }),
                (iamAuthSpan, next) => auth.server.doAuth(
                    request, log, (err, userInfo, authorizationResults, streamingV4Params) => {
                        activeSpan.addEvent('Got response from Vault');
                        if (err) {
                    ...
                    }, 's3', requestContexts, activeSpan),
                (userInfo, authorizationResults, streamingV4Params, iamAuthSpan, next) => {
                    activeSpan.addEvent('Processing results from Vault');
                ...
                (userInfo, authorizationResults, streamingV4Params, iamAuthSpan, next) => {
                    activeSpan.addEvent('Processed results from Vault');
                    activeSpan.addEvent('Checking Tag Conditions in Authorization Results');
                    return next(null, userInfo, authorizationResults, streamingV4Params, iamAuthSpan);
                },
                ...
                iamAuthSpan.end();
                activeSpan.addEvent('Auth with Vault completed');
                if (err) {
                    activeSpan.recordException(err);
                    return callback(err);
                }
                if (authorizationResults) {
                    activeSpan.addEvent('Checking authorization results');
                    ...
                    activeSpan.addEvent('Authorization results checked, no errors found');

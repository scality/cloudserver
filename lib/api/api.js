...
const bucketDeleteRateLimit = require('./bucketDeleteRateLimit');
...
const bucketGetRateLimit = require('./bucketGetRateLimit');
...
const bucketPutRateLimit = require('./bucketPutRateLimit');
...
const api = {
    callApiMethod(apiMethod, request, response, log, callback) {
        ...
        // Pre-authentication rate limit check (using cache)
        // Provides early rejection before Vault to protect IAM service from load
        if (request.bucketName) {
            const { rateLimitPreCheck } = require('./apiUtils/rateLimit');
            const preAuthResult = rateLimitPreCheck(request.bucketName, config, log);

            if (preAuthResult.checked && !preAuthResult.allowed) {
                // Pre-auth checked and rejected
                log.debug('request throttled (pre-auth)', {
                    method: apiMethod,
                    bucketName: request.bucketName,
                });
                return callback(config.rateLimiting?.error || errors.SlowDown);
            }

            // Only set flag if pre-auth actually performed GCRA check
            // If checked=false (no cache), post-metadata will check instead
            if (preAuthResult.checked) {
                request.rateLimitPreAuthChecked = true;
            }
        }

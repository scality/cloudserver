...
function parseMultipartFormData(request, callback) {
    ...
    request.on('data', (chunk) => {
        ...
        while ((boundaryIndex = buffer.indexOf(boundaryBuffer)) !== -1) {
            ...
            const translatedPart = part.toString();
            const translatedbuffer = buffer.toString();
            if (part.length === 0) continue; // skip empty parts

            if (part.indexOf(newlineBuffer) === 0) {
                buffer = buffer.slice(newlineBuffer.length);
                continue; // skip initial CRLF
            }

            const headersEndIndex = part.indexOf(newlineBuffer);
            const headers = part.slice(0, headersEndIndex).toString().split('\r\n');
            const content = part.slice(headersEndIndex + newlineBuffer.length);
            ...
            if (contentDisposition) {
                ...
                if (nameMatch) {
                    ...
                    if (filenameMatch) {
                        ...
                        fileStream = fs.createWriteStream(path.join(os.tmpdir(), filename));

                        // Write the initial content to the file stream
                        fileStream.write(content);

                        // Pipe the remaining data
                        //request.pipe(fileStream, { end: false });
                        //request.pipe(fileStream);

                        if (algoOK && credOK && dateOK && sigOK && policyOK) {
                            //callback(null, { fieldname, filename, encoding });
                        }

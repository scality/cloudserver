...
const api = {
    callApiMethod(apiMethod, request, response, log, callback, tracer) {
        return tracer.startActiveSpan('Cloudserver:: Processing API related operations', methodSpan => {
            methodSpan.setAttribute('code.function', 'callApiMethod');
            methodSpan.setAttribute('code.filepath', 'lib/api/api.js');
            methodSpan.setAttribute('code.lineno', 92);
            ...
            return async.waterfall([
                next => tracer.startActiveSpan('Cloudserver:: Authentication of user', authUserSpan => {
                ...
                methodSpan.addEvent('Checking and generating auth results')
                ...
                const apiSpan = tracer.startSpan(`Cloudserver:: API operation for ${apiMethod}`, undefined, ctx);
                if (apiMethod === 'objectPut' || apiMethod === 'objectPutPart') {
                ...
                if (apiMethod === 'objectGet') {
                    return this[apiMethod](userInfo, request, returnTagCount, log, callback, apiSpan);
                }
                return this[apiMethod](userInfo, request, log, (err, res, cosrsHeaders) => {
                    methodSpan.addEvent('Cloudserver:: API operation completed, preparing and sending response to client');
                    apiSpan.end();
                    methodSpan.end();
                    if (err) {
                        return callback(err);

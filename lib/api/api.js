...
function callApiHandler(apiMethod, apiHandler, request, response, log, callback) {
    ...
    // Vault as a hint so it can correlate the request with the owning
    // account (e.g. for cross-account policy decisions). Only included when
    // the cache has a hit — Vault treats the field as optional.
    const authOptions = {};

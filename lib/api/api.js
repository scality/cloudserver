...
const { observeElapsedTime } = require('../utilities/monitoringHandler');
...
// Check if parallel auth is enabled
const isParallelAuthEnabled = process.env.OPTIM_PARALLEL_AUTH === 'true';
...
const api = {
    callApiMethod(apiMethod, request, response, log, callback) {
        // If parallel auth is enabled and the API method is objectGet or objectPut
        if (isParallelAuthEnabled && (apiMethod === 'objectGet' || apiMethod === 'objectPut')) {
            return this.callApiMethodWithParallelAuth(apiMethod, request, response, log, callback);
        }
        ...
        // Measure request context preparation time
        const startPrqTime = process.hrtime.bigint();
        ...
        const endPrqTime = process.hrtime.bigint();
        const prqElapsedMicros = Number((endPrqTime - startPrqTime) / 1000n);
        observeElapsedTime('prq', prqElapsedMicros);
        ...
        const checkAuthResults = authResults => {
        ...
        // Define the auth function that we'll either call directly or store for parallel execution
        const doAuthFunction = callback => {
            const startAuthTime = process.hrtime.bigint();
            auth.server.doAuth(
                request, log, (err, userInfo, authorizationResults, streamingV4Params, infos) => {
                    const endAuthTime = process.hrtime.bigint();
                    const authElapsedMicros = Number((endAuthTime - startAuthTime) / 1000n);
                    observeElapsedTime('auth', authElapsedMicros);
                    ...
                    if (err) {
                        ...
                        return callback(arsenalError);
                    }
                    return callback(null, userInfo, authorizationResults, streamingV4Params, infos);
                }, 's3', requestContexts);
        };

        // If parallel auth is enabled, store the auth function on the request object
        // but don't call it yet - mock the values instead
        if (isParallelAuthEnabled) {
            request.authParallel = doAuthFunction;

            // Mock values for allowed access
            const mockUserInfo = {
                getAccountDisplayName: () => 'mockAccount',
                getIAMdisplayName: () => 'mockUser',
                getShortid: () => 'account:session',
                isRequesterAnIAMUser: () => true,
                getCanonicalID: () => '79a59df900b949e55d96a1e698fbacedfd6e09d98eacf8f8d5218e7cd47ef2be',
                getArn: () => 'arn:aws:iam::123456789012:user/mock-user',
                isRequesterNonAccountUser: () => false,
            };

            // Mock authorization results with everything allowed
            const mockAuthResults = apiMethods.map(method => ({
                isAllowed: true,
                isImplicit: false,
                action: method,
            }));

            // Continue with the mocked values
            return async.waterfall([
                next => next(null, mockUserInfo, mockAuthResults, {}, { accountQuota: {} }),
                ...
                (userInfo, authorizationResults, streamingV4Params, infos, next) =>
                    next(null, userInfo, authorizationResults, streamingV4Params, infos),
            ], (err, userInfo, authorizationResults, streamingV4Params, infos) => {
                if (err) {
                    return callback(err);
                }
                // Continue with processing as if auth succeeded
                return this._continueAfterAuth(
                    request,
                    response,
                    userInfo,
        ...
        // If parallel auth is not enabled, proceed with the original flow
        return async.waterfall([
            next => {
                doAuthFunction(next);
            },
    ...
    // Helper method to continue processing after auth is complete
    _continueAfterAuth(
        request,
        response,
        userInfo,
        ...
        const startApiTime = process.hrtime.bigint();
        let apiCalled = false;

        const apiCallbackWrapper = (err, ...results) => {
            if (!apiCalled) {
                const endApiTime = process.hrtime.bigint();
                const apiElapsedMicros = Number((endApiTime - startApiTime) / 1000n);
                observeElapsedTime('apilogic', apiElapsedMicros);
                apiCalled = true;
            }
            return methodCallback(err, ...results);
        };

        if (apiMethod === 'objectPut' || apiMethod === 'objectPutPart') {
            request._response = response;
            return this[apiMethod](userInfo, request, streamingV4Params || {},
                log, apiCallbackWrapper, authorizationResults);
        }
        if (apiMethod === 'objectCopy' || apiMethod === 'objectPutCopyPart') {
            return this[apiMethod](userInfo, request, sourceBucket,
                sourceObject, sourceVersionId, log, apiCallbackWrapper);
        }
        if (apiMethod === 'objectGet') {
            return this[apiMethod](userInfo, request, returnTagCount, log, apiCallbackWrapper);
        }
        return this[apiMethod](userInfo, request, log, apiCallbackWrapper);
    },

    // Add a new method to handle parallel auth
    callApiMethodWithParallelAuth(apiMethod, request, response, log, callback) {
        // Start tracking the total request time
        const startTotalTime = process.hrtime.bigint();
        // Start tracking API execution time immediately, including parallel operations
        const startApiTime = process.hrtime.bigint();
        let apiCalled = false;

        // Create a wrapper that will measure both apilogic and total request time
        const wrappedCallback = (err, ...args) => {
            ...
            const endTotalTime = process.hrtime.bigint();
            const totalRequestMicros = Number((endTotalTime - startTotalTime) / 1000n);
            observeElapsedTime('totalrequest', totalRequestMicros);
        ...
        if (!actionLog &&
            ...
            apiMethod !== 'websiteHead' &&
            apiMethod !== 'corsPreflight') {
            log.error('callApiMethodWithParallelAuth(): No actionLog for this api method', {
                apiMethod,
            });
        }
        log.addDefaultFields({
        ...
        if (validationRes.error) {
            log.debug('request query / header validation failed', {
                error: validationRes.error,
                method: 'api.callApiMethodWithParallelAuth',
            });
        ...
        // Define the auth function for parallel execution
        const doAuthFunction = callback => {
            const startAuthTime = process.hrtime.bigint();
            auth.server.doAuth(
                request, log, (err, userInfo, authorizationResults, streamingV4Params, infos) => {
        ...
        // Store the auth function on the request object for later execution
        request.authParallel = doAuthFunction;

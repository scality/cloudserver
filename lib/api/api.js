...
const api = {
    callApiMethod(apiMethod, request, response, log, callback, tracer) {
        return tracer.startActiveSpan('Cloudserver:: Processing API related operations', apiSpan => {
            apiSpan.setAttribute('code.function', 'callApiMethod');
            apiSpan.setAttribute('code.filepath', 'lib/api/api.js');
            apiSpan.setAttribute('code.lineno', 92);
            ...
            return async.waterfall([
                next => tracer.startActiveSpan('Cloudserver:: Authentication', authSpan => {
                    authSpan.setAttribute('code.function', 'auth.server.doAuth');
                    authSpan.setAttribute('code.filepath', 'lib/api/api.js');
                    authSpan.setAttribute('code.lineno', 197);
                    return next(null, authSpan);
                }),

...
const api = {
    callApiMethod(apiMethod, request, response, log, callback, oTel) {
        ...
        return tracer.startActiveSpan(`Initiating Processing for ${action} API Method in CloudServer`, undefined, cloudserverApiSpanContext, callApiMethod => {
            ...
            return async.waterfall([
                ...
                activeSpan.addEvent('Authentication of user completed');
                if (err) {
                    activeSpan.recordException(err);
                    callApiMethod.end();
                    return callback(err);

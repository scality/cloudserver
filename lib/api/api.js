...
const { releaseRequestContexts } = require('./apiUtils/authorization/prepareRequestContexts');
...
const { mockDoAuth, ENABLE_DOAUTH_MOCK } = require('./apiUtils/mock/backendMocks');
...
const api = {
    callApiMethod(apiMethod, request, response, log, callback) {
        ...
        const wrappedCallback = function (err, ...results) {
            // GC Optimization: Release pooled request contexts 
            if (requestContexts) {
                releaseRequestContexts(requestContexts);
            }
        ...
        // Optimized: Extract all the _apiMethods avoiding .map() GC pressure
        const apiMethods = [];
        if (requestContexts) {
            // Manual iteration is more GC-friendly than .map() for small arrays
            for (let i = 0; i < requestContexts.length; i++) {
                if (requestContexts[i] && requestContexts[i]._apiMethod) {
                    apiMethods.push(requestContexts[i]._apiMethod);
                }
            }
        }
        ...
        return async.waterfall([
            next => {
                if (ENABLE_DOAUTH_MOCK) {
                    return mockDoAuth(auth.server.doAuth.bind(auth.server), request, log,
                ...
                return auth.server.doAuth(request, log, (err, userInfo, authorizationResults,
                    streamingV4Params, infos) => {
                    monitorLatency('1-doAuth', performance.now() - startTime);
                    if (err) {
                        // VaultClient returns standard errors, but the route requires
                        // Arsenal errors

...
const { getCachedBucketOwner } = require('./apiUtils/rateLimit/cache.js');
    ...
    return async.waterfall([
        ...
        (userInfo, authorizationResults, streamingV4Params, infos, next) => handleAuthorizationResults(
            request, authorizationResults, apiMethod, returnTagCount, log, (err, res) => {
                ...
                const accLimit = infos?.limits?.RequestsPerSecond?.Limit;
                if (accLimit !== undefined) {
                    const burstCapacitySeconds =
                        config.rateLimiting.account?.defaultConfig?.requestsPerSecond?.burstCapacity || 1;
                    const burstCapacity = burstCapacitySeconds * 1000;
                    request.accountLimits = { limit: accLimit, source: 'account', burstCapacity };
                }
                if (err) {
                    return next(err);
                }
...
function getRateLimitsForRequest(request) {
    ...
    if (request.accountLimits) {
        checks.account = request.accountLimits;
        needsCheck = true;
    }

    if (request.bucketName) {
        const cachedConfig = getRateLimitFromCache('bucket', request.bucketName);
        if (cachedConfig) {
            checks.bucket = cachedConfig;
            needsCheck = true;
        }

        if (!request.accountLimits) {
            const cachedOwner = getCachedBucketOwner(request.bucketName);
            if (cachedOwner !== undefined) {
                const cachedConfig = getRateLimitFromCache('account', cachedOwner);
                if (cachedConfig) {
                    checks.account = cachedConfig;
                    checks.bucketOwner = cachedOwner;
                    needsCheck = true;
                }
            }
        }
...
const api = {
    callApiMethod(apiMethod, request, response, log, callback) {
        ...
        const rateLimitConfig = getRateLimitsForRequest(request);
        if (!rateLimitConfig.needsCheck) {
            return process.nextTick(callApiHandler, apiMethod, apiHandler, request, response, log, callback);
        }

        const checks = [];
        if (rateLimitConfig.checks.account) {
            checks.push({
                resourceClass: 'account',
                resourceId: rateLimitConfig.checks.bucketOwner,
                measure: 'rps',
                cfg: rateLimitConfig.checks.account,
            });
        }

        if (rateLimitConfig.checks.bucket) {
            checks.push({
                resourceClass: 'bucket',
                resourceId: request.bucketName,
                measure: 'rps',
                cfg: rateLimitConfig.checks.bucket,
            });
        }

        const allowed = checkRateLimitsForRequest(checks, log);
        if (!allowed) {
            log.addDefaultFields({
                rateLimited: true,

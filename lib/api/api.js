...
const api = {
    callApiMethod(apiMethod, request, response, log, callback) {
        ...
        // Check rate limiting BEFORE Vault authentication to protect IAM layer
        // Uses cached bucket config for fast rejection (90%+ cache hit rate)
        // Falls back to Phase 2 (post-metadata check) if no cached config
        if (request.bucketName && config.rateLimiting && config.rateLimiting.enabled) {
            const { rateLimitPreCheck } = require('./apiUtils/rateLimit');
            const { errors } = require('arsenal');
            const result = rateLimitPreCheck(request.bucketName, config, log);

            // Only mark as checked if cache hit (not 'cache-miss')
            // Cache miss means post-metadata check must run to populate cache
            if (result !== 'cache-miss') {
                ...
                if (result === false) {
                    ...
                    return process.nextTick(callback, config.rateLimiting.error || errors.SlowDown);
                }
            }
        }

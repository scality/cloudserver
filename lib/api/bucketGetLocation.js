...
function bucketGetLocation(authInfo, request, log, callback) {
    ...
    return metadata.getBucket(bucketName, log, (err, bucket) => {
        if (err) {
            ...
            monitoring.promMetrics('GET', bucketName, err.code, 'getBucketLocation');
            return callback(err);
        }
        if (bucketShield(bucket, requestType)) {
            monitoring.promMetrics('GET', bucketName, 404, 'getBucketLocation');
        ...
        if (
            ...
            monitoring.promMetrics('GET', bucketName, 403, 'getBucketLocation');
        ...
        monitoring.promMetrics('GET', bucketName, '200', 'getBucketLocation');

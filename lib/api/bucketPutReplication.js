...
const { getReplicationConfiguration } = require('./apiUtils/bucket/getReplicationConfiguration');
const validateConfiguration = require('./apiUtils/bucket/validateReplicationConfig');
const collectCorsHeaders = require('../utilities/collectCorsHeaders');
const monitoring = require('../utilities/monitoringHandler');

// The error response when a bucket does not have versioning 'Enabled'.
const versioningNotEnabledError = errorInstances.InvalidRequest.customizeDescription(
        ...
        [
            ...
            (config, bucket, next) => {
                ...
                if (!validateConfiguration(config, bucket)) {
                    const msg = 'Replication configuration lacks a preferred ' + 'read location';
                    log.error(msg, { bucketName: bucket.getName() });
                    return next(errorInstances.ValidationError.customizeDescription(msg));
                }
                bucket.setReplicationConfiguration(config);
                return metadata.updateBucket(bucket.getName(), bucket, log, err => next(err, bucket));
            },
        ],
        (err, bucket) => {
            ...
            if (err) {
                ...
                monitoring.promMetrics('PUT', bucketName, err.code, 'putBucketReplication');
            ...
            monitoring.promMetrics('PUT', bucketName, '200', 'putBucketReplication');

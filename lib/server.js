...
// Extract and set trace context from Beyla for the entire request
...
// Manual GC configuration
const GC_INTERVAL_MS = parseInt(process.env.GC_INTERVAL_MS, 10) || 100;
...
class S3Server {
    ...
    constructor(config, worker) {
        ...
        // Setup manual GC if exposed
        if (global.gc) {
            this.gcInterval = setInterval(() => {
                global.gc();
            }, GC_INTERVAL_MS);
            logger.info('Manual GC enabled', { intervalMs: GC_INTERVAL_MS });
        } else {
            logger.warn('GC not exposed. Start Node.js with --expose-gc to enable manual GC');
        }
    ...
    routeRequest(req, res) {
        ...
        if (req.headers.traceparent) {
            traceHeaders.traceparent = req.headers.traceparent;
        }
        if (req.headers.tracestate) {
            traceHeaders.tracestate = req.headers.tracestate;
        }
        
        // Extract Beyla trace context and run entire request within it
        if (Object.keys(traceHeaders).length > 0) {
            const remoteContext = propagation.extract(context.active(), traceHeaders);
            
            // Run the entire request processing within the Beyla trace context
            return context.with(remoteContext, () => {
                arsenal.s3routes.routes(req, res, {
                    api,
                    internalHandlers,
                    statsClient,
                    allEndpoints,
        ...
        // Fallback: normal request processing without trace context
        const wrappedRoutes = (req, res, params, logger, config) => {
            arsenal.s3routes.routes(req, res, params, logger, config);
        };
        
        return wrappedRoutes(req, res, {
            api,
            internalHandlers,
            statsClient,
            allEndpoints,
    ...
    cleanUp() {
        ...
        // Clear manual GC interval if it exists
        if (this.gcInterval) {
            clearInterval(this.gcInterval);
            logger.info('Manual GC interval cleared');
        }

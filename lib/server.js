...
class S3Server {
    ...
    constructor(worker) {
        ...
        this.cluster = true;
        this.servers = [];
        const optimHttpAgent = process.env.OPTIM_HTTP_AGENT === 'true' ? {
            keepAliveMsecs: 1000, // Milliseconds between keep-alive probes
            maxSockets: process.env.OPTIM_MAX_SOCKETS || 256,
            maxFreeSockets: 256, // Maximum idle sockets to keep alive
            freeSocketTimeout: 30000, // 30 seconds timeout for free sockets
            timeout: 60000, // Overall socket timeout
            scheduling: 'lifo', // Last in first out - improves connection reuse
        } : {};

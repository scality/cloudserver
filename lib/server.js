...
if (CONFIG.CLUSTERING && cluster.isMaster) {
  ...
  (function () {
      ...
      let datastore = {}; // could use environment/config var to toggle between production/development    
      ...
      router.put('/:resource', function(request, response) {
         checkAuth(request, function(err, accessKey){
           ...
           if(bucketname === undefined) {
             bucketPut(accessKey, request, function(err, result) {
               if(err) {
           ...
           if(bucketname){
            objectPut(accessKey, datastore,  request, function(responseCode){

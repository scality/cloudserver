...
const serverLog = logger.newRequestLogger('S3-Server');
...
class S3Server {
    ...
    startup() {
        ...
        this.server.on('listening', () => {
            ...
            serverLog.info(`Serving web content at ${addr.address}${addr.port}`
                    + ` - PID: ${process.pid}`);
        });
...
export default function main() {
    if (_config.clustering && cluster.isMaster && !process.env.S3BACKEND) {
        ...
        cluster.on('disconnect', worker => {
            serverLog.error(`worker ${worker.id} disconnected. Restarting.`);
            cluster.fork();
        });

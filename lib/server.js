...
class S3Server {
    ...
    initiateStartup(log) {
        series([next => metadata.setup(next), next => clientCheck(true, log, next)], (err, results) => {
            ...
            logger.info('integrityChecks config', { config: _config.integrityChecks });
            if (!_config.integrityChecks.enabled) {
                logger.warn('x-amz-checksum-* digests are disabled: not computed, not validated, and not stored');
            }

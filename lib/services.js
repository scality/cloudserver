...
export default {
    ...
    metadataStoreMPObject(bucketName, params, log, cb) {
        assert.strictEqual(typeof bucketName, 'string');
        // Replace "-" in date with ::  and "."
        // with ":::" to prevent routing error
        const initiated = new Date().toJSON().replace(/-/g, '::')
            .replace(/\./g, ':::');
        ...
        // 3) Bucket names are subject to limited AWS restrictions mostly
        // based on DNS.
        ...
        // 7) Initiated will be an ISO date string
        let longMPUIdentifier =
            ...
            `${splitter}${initiated}`;
        longMPUIdentifier = encodeURIComponent(longMPUIdentifier);
        ...
        metadata.getBucket(mpuBucketName, log, (err, mpuBucket) => {
            ...
            metadata.listObject(mpuBucketName, searchArgs.prefix,
                ...
                log, (err, response) => {
                    ...
                    let mpuOverview =
                        decodeURIComponent(response.Contents[0].key);
                    mpuOverview = mpuOverview.split(splitter);
    ...
    metadataStorePart(mpuBucketName, dataLocations, metaStoreParams, log, cb) {
        ...
        const partKey = encodeURIComponent(
            `${uploadId}${splitter}` +
            `${partNumber}${splitter}` +
            ...
            `${dataLocations}`);
        const value = {};
        metadata.putObjectMD(mpuBucketName, partKey, value, log, (err) => {
            if (err) {
                return cb(err);
            }
            return cb(null);

...
export default {
    ...
    dataStore(bucket, objectMetadata, ds, params, cb) {
        if (params.headers['content-md5']) {
            const bufferMD5 =
                new Buffer(params.headers['content-md5'], 'base64');
            const sentMD5Hexed = bufferMD5.toString('hex');
            if (sentMD5Hexed !== params.contentMD5) {
                return cb('InvalidDigest');
            }
        }
    ...
    metadataValidateMultipart(params, cb) {
        const { metastore, bucketUID, uploadId, accessKey, objectKey } = params;

        const bucket = metastore.buckets[bucketUID];
        if (bucket === undefined) {
            return cb('NoSuchBucket');
        }
        const multiPartUpload = bucket.multiPartObjectKeyMap[uploadId];
        if (multiPartUpload === undefined ||
            multiPartUpload.key !== objectKey) {
            return cb('NoSuchUpload');
        }

        // In order for account to be authorized must both have putObject
        // authorization AND be the initiator of the multipart upload
        ...
        // Only the initiator of the multipart upload can upload a part
        // TODO: initiator.id is the initiator's canonical or IAM user's
        // ARN.  So, accessKey here should be canonicalID or ARN (if applicable)
        const isInitiator =
    ...
    metadataStorePart(
        bucket, newLocation, partNumber, contentMD5, multiPartMetadata, cb) {
        bucket.putPartLocation(partNumber, contentMD5,

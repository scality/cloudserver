...
export default {
    ...
    metadataStoreObject(bucketName, dataGetInfo, cipherBundle, params, cb) {
        const { objectKey, authInfo, size, contentMD5, metaHeaders,
            ...
            lastModifiedDate, versioningConfiguration, versionId } = params;
        ...
        if (!multipart) {
            ...
            acl.parseAclFromHeaders(parseAclParams, (err, parsedACL) => {
                ...
                }, { versioningConfiguration, versionId });
                return undefined;
            });
        } else {
            metadata.putObjectMD(bucketName, objectKey, omVal, log, err => {
     ...
     * @param {object} params - extra versioning parameters
     * @return {undefined}
     */
    deleteObject(bucketName, objectMD, objectKey, log, cb, params) {
        ...
        const { versioningConfiguration, versionId } = params;
        const versioningEnabled = versioningConfiguration &&
            versioningConfiguration.Status === 'Enabled';
        const versioningSuspended = versioningConfiguration &&
            versioningConfiguration.Status === 'Suspended';
        const versionSpecific = !!versionId;
        // this is equivalent to:
        // - versioningEnabled && versionId: deleting a specific version
        // - not versioningEnabled && not versionId: deleting an object
        // - versioningSuspended: deleting either an object or a version
        if (versioningEnabled === versionSpecific || versioningSuspended) {
            log.trace('deleteObject: physical deleting object/version');
            ...
            log.trace('deleteObject: creating a delete marker');
            return metadata.deleteObjectMD(bucketName, objectKey, log, cb,
                    params);
        }
        return undefined;

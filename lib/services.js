...
function _deleteUserBucket(bucketName, accessKey, log, cb) {
    ...
    metadata.deleteObjectMD(usersBucket, keyForUserBucket, (err) => {
        return cb(err);
    });
}
...
export default {
    getService(accessKey, request, cb) {
        ...
        metadata.listObject(usersBucket, prefix, null, null, null,
            (err, listResponse) => {
        ...
        metadata.getBucket(bucketName, log, (err, bucket) => {
            ...
            metadata.getObjectMD(bucketName, objectKey, log, (err, objMD) => {
                ...
                if (err && (err === 'NoSuchKey' || err === 'ObjNotFound')) {
                    ...
                    log.debug(`Object key not found`);
                    return cb(null, bucket, null);
                }
     ...
     */
    dataStore(objectMetadata, objectContext, value, cb) {
        ...
        data.put(value, objectContext, (err, keys) => {
            if (err) {
                return cb(err);
            }
            if (keys) {
    ...
    createBucket(accessKey, bucketName, headers, locationConstraint,
        ...
        async.waterfall([
            ...
            function addToUsersBucket(bucketName, next) {
                ...
                metadata.putObjectMD(usersBucket, key, omVal, err => {
                    if (err === 'NoSuchBucket') {
        ...
        metadata.listObject(bucketName, null, null, null, null, log,
            (err, objectsListRes) => {
                ...
                metadata.deleteBucket(bucketName, log, (err) => {
                    ...
                    if (err) {
                        log.error(`Bucket DELETE failed: ${err}`);
                        return cb(err);
                    }
                    _deleteUserBucket(bucketName, accessKey, log, cb);
                    log.debug('Bucket deleted from metadata');
                });
            });
    },
     ...
     */
    metadataStorePart(mpuBucketName, newLocation, metaStoreParams, log, cb) {

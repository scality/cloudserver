...
const { instrumentServices } = require('./instrumentation/simple');
...
const services = {
    ...
    */
    ...
    metadataStoreObject(bucketName, dataGetInfo, cipherBundle, params, cb) {
        ...
        // Optimized: Reduce chained method calls by batching property setting
        // This avoids potential intermediate object creation in some JS engines
        const canonicalID = authInfo.getCanonicalID();
        const ownerDisplayName = authInfo.getAccountDisplayName();
        
        // Set core properties first
        md.setOwnerId(canonicalID);
        md.setKey(objectKey);
        md.setContentLength(size);
        md.setContentType(contentType);
        md.setContentMd5(contentMD5);
        md.setLocation(dataGetInfo);
        
        // Set optional properties if they exist
        if (cacheControl !== undefined) md.setCacheControl(cacheControl);
        if (contentDisposition !== undefined) md.setContentDisposition(contentDisposition);
        if (contentEncoding !== undefined) md.setContentEncoding(contentEncoding);
        if (expires !== undefined) md.setExpires(expires);
        if (dataStoreName !== undefined) md.setDataStoreName(dataStoreName);
        if (creationTime !== undefined) md.setCreationTime(creationTime);
        if (originOp !== undefined) md.setOriginOp(originOp);
        ...
        md.setOwnerDisplayName(ownerDisplayName);
        ...
        // Optimize: Break chain to avoid intermediate objects
        md.setIsNull(isNull);
        md.setNullVersionId(nullVersionId);
        md.setNullUploadId(nullUploadId);
        md.setIsDeleteMarker(isDeleteMarker);
    ...
    metadataValidateMultipart(params, cb) {
        const { authInfo, bucketName, objectKey, uploadId, log,
                requestType = 'ResumeableUpload' } = params;
...
// Export instrumented services
module.exports = instrumentServices(services);

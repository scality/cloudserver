...
export default {
    getService(authInfo, request, log, splitter, cb, overrideUserbucket) {
        ...
        metadata.listObject(bucketUsers, null, prefix, null, null, null, log,
            (err, listResponse) => {
    ...
    getObjectListing(bucketName, listingParams, log, cb) {
        assert.strictEqual(typeof bucketName, 'string');
        const { delimiter, marker, prefix, query } = listingParams;
        const maxKeys = Number(listingParams.maxKeys);
        log.trace('performing metadata get object listing',
            { listingParams, maxKeys });
        metadata.listObject(bucketName, query, prefix, marker, delimiter, maxKeys, log,
            (err, listResponse) => {
                if (err) {
    ...
    metadataValidateMultipart(params, cb) {
        ...
        metadata.getBucket(mpuBucketName, log, (err, mpuBucket) => {
            ...
            metadata.listObject(mpuBucketName, null, searchArgs.prefix,
    ...
    */
    ...
    getMPUparts(mpuBucketName, uploadId, log, cb) {
        ...
        metadata.listObject(mpuBucketName, null, searchArgs.prefix, searchArgs.marker,

...
const { errors, s3middleware } = require('@scality/arsenal');
...
const ObjectMDChecksum = require('@scality/arsenal').models.ObjectMDChecksum;
...
const services = {
    ...
    metadataStoreObject(bucketName, dataGetInfo, cipherBundle, params, cb) {
        ...
        if (transitionInProgress) {
            md.setTransitionInProgress(true, Date.now());
        }
        // update restore
        if (archive) {
            md.setArchive(
    ...
    metadataStorePart(mpuBucketName, partLocations, metaStoreParams, log, cb) {
        ...
        if (checksumValue && checksumAlgorithm) {
            omVal.checksumValue = checksumValue;
            omVal.checksumAlgorithm = checksumAlgorithm;
        }

...
import ObjectMD from './metadata/models/object/object';
...
export default {
    ...
    */
    ...
    metadataStoreObject(bucketName, dataGetInfo, cipherBundle, params, cb) {
        ...
        md.setOwnerDisplayName(authInfo.getAccountDisplayName());
        ...
        md.setHeaders(metaHeaders);
        if (overrideMetadata) {
            md.setHeaders(overrideMetadata);
        }
        ...
        if (!multipart) {
            ...
            acl.parseAclFromHeaders(parseAclParams, (err, parsedACL) => {
                ...
                metadata.putObjectMD(bucketName, objectKey, md, log, err => {
                    if (err) {
                        log.error('error from metadata', { error: err });
                        return cb(err);
                    }

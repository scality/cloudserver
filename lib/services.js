...
function _deleteUserBucket(bucketName, canonicalID, log, cb) {
    ...
    metadata.deleteObjectMD(usersBucket, keyForUserBucket, log, error => {
        if (error && !error.NoSuchKey && !error.NoSuchBucket) {
    ...
    createBucket(authInfo, bucketName, headers, locationConstraint, log, cb) {
        ...
        acl.parseAclFromHeaders(parseAclParams, (error, parsedACL) => {
            ...
            return metadata.createBucket(bucketName, bucket, log, error => {
                ...
                // Getting new UserBucket
                return metadata.getBucket(usersBucket, log, (err, bUsers) => {
                    if (err && !err.NoSuchBucket && !err.BucketAlreadyExists) {
                        return cb(error);
                    }
                    // BACKWARD: Remove to remove the old splitter
                    const splitter = bUsers ?
                        constants.splitter : constants.oldSplitter;
                    let key = `${owner}${splitter}${bucketName}`;
                    const omVal = { creationDate: new Date().toJSON() };
                    // If the new UserBucket does not exist, try to put in the
                    // old user bucket
                    const usersBucketBeingCalled = bUsers ?
                    ...
                    return metadata.putObjectMD(usersBucketBeingCalled, key,
                        omVal, log, err => {
                            if (err && err.NoSuchBucket) {
                                ...
                                key = `${owner}${constants.splitter}` +
                                    `${bucketName}`;
                                const creationDate = new Date().toJSON();
                                const freshBucket = new BucketInfo(usersBucket,
                                    'admin', 'admin', creationDate, 2);
                                // No one user bucket are created, create a new
                                // UserBucket

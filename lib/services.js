...
export default {
    ...
    metadataStoreObject(
            bucketOrBucketUID, objectMetadata, dataLocations, params, cb) {
        const { objectUID, objectKey, accessKey, size,
            contentMD5, metaHeaders, contentType,
            multipart, headers, metastore } = params;
        // If bucket is actually a string rather than an object, pull the
        // bucket from the metastore
        const bucket = (typeof bucketOrBucketUID === 'object') ?
            bucketOrBucketUID : metastore.buckets[bucketOrBucketUID];
    ...
    metadataStoreMPObject(bucket, params, cb) {
        ...
        acl.parseAclFromHeaders(parseAclParams, (err, parsedACL) => {
            ...
            bucket.putMPobjectMD(longMPUIdentifier,
    ...
    metadataValidateMultipart(params, cb) {
        ...
        const mpuBucket = metastore.buckets[`mpu...${bucketUID}`];
        if (mpuBucket === undefined) {
            return cb('NoSuchBucket');
        }
        ...
        mpuBucket.getBucketListObjects(searchArgs.prefix, searchArgs.marker,
            ...
            function handleSearchResponse(err, response) {
                ...
                const mpuOverview = response.Contents[0].Key.split(splitter);

                // Having first comma ignores the overview string at array
                // position 0
                // Disable eslint since the linter does not like pulling these
                // items and not using them in this file.  But the array is used
                // in other files.
                /*eslint-disable */
                const [ , objectKey,
                        uploadId,
     ...
     */
    metadataStorePart(
        mpuBucket, newLocation, metaStoreParams, cb) {
        ...
        const partKey =
            ...
            `${newLocation}`;
        const value = '';
        mpuBucket.putObjectMD(partKey, value, (err) => {
            if (err) {
                return cb(err);
            }
            return cb(null);
    ...
    */
    getMultipartUploadListing(metastore, MPUbucketName, listingParams, cb) {
        const mpuBucket = metastore.buckets[MPUbucketName];
        if (mpuBucket === undefined) {
            return cb(null, {
                IsTruncated: false,
        ...
        mpuBucket.getMultipartUploadListing(listingParams,
               (err, listResponse) => {
                   if (err) {
                       return cb(err);
                   }
     ...
     * @param {string} canonicalId - canonicalId of user or user's parent
    ...
    getMPUBucket(destinationBucket, metastore, bucketUID, cb) {
        ...
        process.nextTick(() => {
            if (metastore.buckets[MPUbucketName] !== undefined) {
                return cb(null, metastore.buckets[MPUbucketName]);
            }
            const bucket = new Bucket();
            bucket.name = MPUbucketName;
            bucket.owner = destinationBucket.owner;
            ...
            // a hidden bucket.
            metastore.buckets[MPUbucketName] = bucket;
            return cb(null, bucket);
        });
    },
    ...
    getMPUparts(mpuBucket, uploadId, cb) {
        ...
        async.doWhilst(
            function searching(moveOn) {
                ...
                mpuBucket.getBucketListObjects(searchArgs.prefix,
    ...
    getObjectMD(bucket, key, cb) {
        bucket.getObjectMD(key, (err, metadata) => {
            if (err) {
                return cb("InternalError");
            }
    ...
    batchDeleteObjectMetadata(mpuBucket, keysToDelete, cb) {
        ...
        async.each(keysToDelete, function action(key, callback) {
            mpuBucket.deleteObjectMD(key, callback);
        },

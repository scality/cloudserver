...
export default {
                 ...
                 (err, dataRetrievalInfo, hashedStream) => {
                     ...
                     hashedStream.on('hashed', () => {
                         ...
                         const checkHashMatchMD5Res = this.checkHashMatchMD5(
                           request, hashedStream, dataRetrievalInfo, log);
                         if (checkHashMatchMD5Res instanceof Error) {
                             return cb(checkHashMatchMD5Res);
                         }
    ...
    * @param {object} request - request linked to the stream containine the data
    * @param {string} hashedStream - hased stream once completed
    * @param {object} dataRetrievalInfo - dataStore: backend stored key
    * {key, dataStoreName (mem/file)}
    * @param {RequestLogger} log - the current request logger
    * @return {object} errors
    */
    checkHashMatchMD5(request, hashedStream, dataRetrievalInfo, log) {
        if (request.contentMD5 && hashedStream.completedHash &&
        request.contentMD5 !== hashedStream.completedHash) {
            // delete data if does not match.
            data.batchDelete(dataRetrievalInfo, log);
            log.debug('contentMD5 and completedHash does not match',
            { method: 'checkHashMatchMD5',
            completedHash: hashedStream.completedHash,
            contentMD5: request.contentMD5 });
            return errors.BadDigest;
        }
        return {};
    },

...
const services = {
    getService(authInfo, request, log, splitter, cb, overrideUserbucket, oTel, serviceGetContext) {
        ...
        return tracer.startActiveSpan(`${overrideUserbucket ? 'Old buckets' : ''} Metadata request`, undefined, serviceGetContext, serviceGetService => {
            ...
            metadata.listObject(bucketUsers, { prefix, maxKeys: 10000 }, log,
                (err, listResponse) => {
                    ...
                    serviceGetService.end();
                    activeSpan.addEvent('Exiting serviceGet()');
                    return cb(null, listResponse.Contents, splitter);
                }, { activeSpan, activeTracerContext, tracer });
        });
    },

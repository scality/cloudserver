...
services.getService = function(accessKey, metastore, cb) {
  let userBuckets = metastore.users[accessKey];
...
services.deleteBucket = function(bucket, responseMetaHeaders, metastore, bucketUID, accessKey, cb) {
  var bucketMetadata = metastore.buckets[bucketUID];
  ...
  for(let i=0, ii = userBuckets.length; i < ii; i++) {
    if(userBuckets[i].bucketUID === bucketUID) {
      userBuckets[i] = undefined;
      break;
    }
  }

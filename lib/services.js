...
function _deleteUserBucket(metastore, bucketUID, accessKey, cb) {
...
export default {
    ...
    metadataValidateAuthorization(params, cb) {
        const { accessKey, bucketUID, objectKey, requestType } = params;
        metadata.getBucket(bucketUID, function bucketCheck(err, bucket) {
            if (err) {
                return cb(err);
            }
            ...
            metadata.getObjectMD(bucket.uid, objectKey, (err, objMD) => {
                ...
                if (err && (err === 'Not Found' || err === 'NoSuchKey')) {
     ...
     */
    dataStore(bucket, objectMetadata, value, cb) {
        ...
        data.put(value, (err, keys) => {
            ...
            // actually the multipart upload metadata
                return cb(null, bucket, objectMetadata, keys);
            }
     ...
     */
    metadataStoreObject(bucket, objectMetadata, keys, params, cb) {
        const { objectUID, objectKey, accessKey, size,
            contentMD5, metaHeaders, contentType,
            multipart, headers } = params;
        ...
        if (!multipart) {
            ...
            acl.parseAclFromHeaders(parseAclParams, (err, parsedACL) => {
                ...
                omVal.acl = parsedACL;
                metadata.putObjectMD(bucket.uid, objectKey,
                    JSON.stringify(omVal), (err) => {
                        return cb(err, contentMD5);
                    });
            });
        } else {
            metadata.putObjectMD(bucket.uid, objectKey, JSON.stringify(omVal),
                (err) => {
                    return cb(err, contentMD5);
                });
        }
    ...
    createBucket(accessKey, bucketName, bucketUID, headers, locationConstraint,
        ...
        async.waterfall([
            ...
            function waterfall3(bucket, next) {
                bucket.uid = bucketUID;
                metadata.createBucket(bucketUID, bucket, (err) => {
                    if (err) {
                        return next(err);
                    }
     ...
     */
    deleteObject(bucket, objectMD, responseMDHeaders, objectKey, cb) {
        ...
        if (objectMD['x-amz-version-id'] === 'null') {
            ...
            data.delete(objectMD.location, err => {
                ...
                metadata.deleteObjectMD(bucket.uid, objectKey, cb);
            });
        } else {
            // versioning
            cb('NotImplemented');
    ...
    deleteBucket(bucket, metastore, bucketUID, accessKey, cb) {
        metadata.listObject(bucket.uid, null, null, null, null,
            function buckDeleter(err, objectsListRes) {
                if (objectsListRes.Contents.length) {
                    return cb('BucketNotEmpty');
                }
                metadata.deleteBucket(bucketUID, (err) => {
                    if (err) {
                        return cb(err);
                    }
                    _deleteUserBucket(metastore, bucketUID, accessKey, cb);
                });
            });
    },
    ...
    getObjectListing(bucket, listingParams, cb) {
        ...
        metadata.listObject(bucket.uid, prefix, marker, delimiter, maxKeys,
            (err, listResponse) => {
                if (err) {
                    return cb(err);
                }
    ...
    metadataStoreMPObject(bucket, params, cb) {
        ...
        acl.parseAclFromHeaders(parseAclParams, (err, parsedACL) => {
            ...
            metadata.putObjectMD(bucket.uid, longMPUIdentifier,
    ...
    metadataValidateMultipart(params, cb) {
        const { bucketUID, uploadId, accessKey,
        ...
        const mpuBucketUID = `mpu...${bucketUID}`;
        metadata.getBucket(mpuBucketUID, (err, mpuBucket) => {
            if (err) {
            ...
            metadata.listObject(mpuBucket.uid, searchArgs.prefix,
     ...
     */
    metadataStorePart(mpuBucket, newLocation, metaStoreParams, cb) {
        ...
        const partKey =
            ...
            `${newLocation}`;
        const value = '{}';
        metadata.putObjectMD(mpuBucket.uid, partKey, value, (err) => {
            if (err) {
                return cb(err);
            }
            return cb(null);
    ...
    */
     ...
     */
    getMPUBucket(destinationBucket, metastore, bucketUID, cb) {
    ...
    getMPUparts(mpuBucket, uploadId, cb) {
        let results = [];
        let weAreNotDone;
        let marker;
        async.doWhilst(
    ...
    getSomeMPUparts(params, cb) {
        const { uploadId, mpuBucket, maxParts, partNumberMarker} = params;
        ...
        metadata.listObject(mpuBucket.uid, searchArgs.prefix, searchArgs.marker,
    ...
    batchDeleteObjectMetadata(mpuBucket, keysToDelete, cb) {
        ...
        async.each(keysToDelete, function action(key, callback) {
            metadata.deleteObjectMD(mpuBucket.uid, key, callback);
        },

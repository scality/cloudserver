...
const services = {
    getService(authInfo, request, log, splitter, cb, overrideUserbucket, oTel, serviceGetContext) {
        const {
            activeSpan,
            tracer,
        } = oTel;
        return tracer.startActiveSpan(`Metadata request for ${overrideUserbucket ? `${overrideUserbucket}` : `${usersBucket}`}`, undefined, serviceGetContext, serviceGetServiceSpan => {
    ...
    */
    ...
    metadataStoreObject(bucketName, dataGetInfo, cipherBundle, params, cb, oTel) {
        ...
        return async.waterfall([
            next => tracer.startActiveSpan('Parse ACLs from headers', undefined, activeTracerContext, currentSpan => {
            ...
            next => tracer.startActiveSpan('Store object in metadata', undefined, activeTracerContext, currentSpan => {
                currentSpan.setAttributes({
                    'code.function': 'metadataStoreObject()',
                    'code.filename': 'lib/services.js',
                    'code.lineno': 329,

...
import {accounts} from './testdata/vault.json';
...
const services = {
    ...
    metadataStoreObject(
        ...
        bucket.putObjectMD(params.objectKey, omVal, function (err) {
            if (err) {
                return cb(err);
            }
        ...
        async.waterfall([
            ...
            function waterfall2(next) {
                ...
                if (headers['x-amz-grant-read'] &&
                    bucket.acl.canned !== undefined) {
                        // TODO -- use utils.parseGrant instead.
                    // const grantees = headers['x-amz-grant-read'].split('');
                    // grantees.forEach(function (item) {
    ...
    deleteObjectFromBucket(
        ...
        if (objectMetadata['x-amz-delete-marker']) {
            ...
            bucket.deleteObjectMD(objectUID, function (err) {
                if (err) {
    ...
    deleteBucket(
        ...
        bucket.getBucketListObjects(
            null, null, null, null, function (err, objectsListRes) {
                ...
                bucket.deleteBucketMD(function (err) {
                    if (err) {
                        return cb(err);
                    }
    ...
    getCanonicalID(email, cb) {
        ...
        process.nextTick(()=> {
            if (accounts[lowercasedEmail] === undefined) {
            ...
            const canonicalID = accounts[lowercasedEmail].canonicalID;
            return cb(null, canonicalID);
        });
    },
    ...
    getManyCanonicalIDs(emails, cb) {
        process.nextTick(() => {
            ...
            for (let i = 0; i < emails.length; i++) {
                ...
                canonicalID = accounts[lowercasedEmail];
                if (accounts[lowercasedEmail] === undefined) {
                    return cb('UnresolvableGrantByEmailAddress');
                }
                canonicalID = accounts[lowercasedEmail].canonicalID;
                account = {

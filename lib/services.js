...
function _deleteUserBucket(bucketName, canonicalID, log, cb) {
    ...
    metadata.deleteObjectMD(usersBucket, keyForUserBucket, log, error => {
        if (error && error !== errors.NoSuchKey) {
            ...
            return metadata.deleteObjectMD(usersBucket, keyForUserBucket2,
                log, error => {
                    if (error && error !== errors.NoSuchKey) {
...
export default {
    getService(authInfo, request, log, splitter, cb) {
        ...
        acl.parseAclFromHeaders(parseAclParams, (error, parsedACL) => {
            ...
            return metadata.createBucket(bucketName, bucket, log, error => {
                ...
                return metadata.getBucket(usersBucket, log, (err, bUsers) => {
                    ...
                    if (bUsers && bUsers.getMdBucketModelVersion() < 2) {
                        ...
                        if (err && err.NoSuchBucket) {
                            ...
                            return metadata.createBucket(usersBucket,
                                freshBucket, log, err => {
                                    ...
                                    // to the usersBucket.
                                    if (err &&
                                        err !== errors.BucketAlreadyExists) {

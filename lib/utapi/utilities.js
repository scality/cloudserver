 ...
 * @param {string} metricObj.bucket - bucket name
 * @param {string} metricObj.byteLength - [optional] current object size (used,
 * for example, for pushing 'deleteObject' metrics)
 * @param {string} metricObj.newByteLength - [optional] new object size
 * @param {string} metricObj.oldByteLength - [optional] old object size
 * (obj. overwrites)
 * @param {string} metricObj.numberOfObjects - [optional] number of obects
 * added/deleted
 * @return {function} - `utapi.pushMetric`
 */
export function pushMetric(action, log, metricObj) {
    return utapi.pushMetric(action, log.getSerializedUids(), metricObj);
}

...
function _listMetrics(host, port, metric, metricType, timeRange, accessKey, secretKey, verbose, recent, ssl) {
    const listAction = recent ? 'ListRecentMetrics' : 'ListMetrics';
    // If recent listing, we do not provide `timeRange` in the request
    const requestObj = recent ? { [metric]: metricType } : { timeRange, [metric]: metricType };
    ...
    request.path = `/${metric}`;
    auth.client.generateV4Headers(request, { Action: listAction }, accessKey, secretKey, 's3');
...
function listMetrics(metricType) {
    ...
    if (metricType === 'buckets') {
        commander.option('-b, --buckets <buckets>', 'Name of bucket(s) with ' + 'a comma separator if more than one');
    } else {
        commander
            .option('-m, --metric <metric>', 'Metric type')
            .option('--buckets <buckets>', 'Name of bucket(s) with a comma ' + 'separator if more than one')
            .option('--accounts <accounts>', 'Account ID(s) with a comma ' + 'separator if more than one')
            .option('--users <users>', 'User ID(s) with a comma separator if ' + 'more than one')
    ...
    const { host, port, accessKey, secretKey, start, end, verbose, recent, ssl } = commander;
    ...
    if (metricType !== 'buckets') {
        ...
        if (validMetrics.indexOf(commander.metric) < 0) {
            logger.error("metric must be 'buckets', 'accounts', " + "'users', or 'service'");
    ...
    _listMetrics(host, port, metric, resources, timeRange, accessKey, secretKey, verbose, recent, ssl);
}
...
function pushMetric(action, log, metricObj) {
    ...
    if (utapiVersion === 2) {
        ...
        if (
            (action === 'putDeleteMarkerObject' && byteLength === null) ||
        ...
        const isLifecycle =
            _config.lifecycleRoleName &&
            authInfo &&
            authInfo.arn.endsWith(`:assumed-role/${_config.lifecycleRoleName}/backbeat-lifecycle`);
...
function pushLocationMetric(location, byteLength, log, cb) {
    const locationId = _getLocationId(location);
    return utapi.pushLocationMetric(locationId, byteLength, log.getSerializedUids(), cb);
}

...
export default function routePUT(request, response, metastore) {
            ...
            if (request.query.acl === undefined) {
                ...
                request.on('end', function combineChunks() {
                    ...
                    if (request.lowerCaseHeaders['content-md5']) {
                        ...
                        if (request.calculatedMD5 !== request
                    ...
                    if (request.query.partNumber) {
                        objectPutPart(accessKey, metastore, request,
                            (err) => {
                                if (err) {
                                    return utils
                        ...
                        objectPut(accessKey, metastore, request,
                            (err) => {
                                if (err) {
                                    return utils
                                        .errorXmlResponse(response, err);

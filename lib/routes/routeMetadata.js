...
function routeMetadata(clientIP, request, response, log) {
    ...
    if (resourceType === 'admin' && !['bucket', 'log', 'id'].includes(subResource)) {
        ...
        [
            ...
            (userInfo, next) => {
                ...
                return metadataProxy.web(request, response, { target }, err => {
                    if (err) {
                        log.error('error proxying request to metadata admin server', { error: err.message });

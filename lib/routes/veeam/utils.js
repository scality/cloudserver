...
async function receiveData(request, log) {
    ...
    return await new Promise((resolve, reject) => {
        ...
        const prepared = prepareStream(request, request.streamingV4Params, checksums, log, settle);
        if (prepared.error) {
            settle(prepared.error);
            return;
        }
        pipeline(prepared.stream, collector).then(
            () => {
                // Checksum transforms only compute digests while streaming:
                // validation against the expected values (header or trailer)
                // must be done once the stream is fully consumed.
                const checksumErr =
                ...
                if (checksumErr) {
                    ...
                    return settle(arsenalErrorFromChecksumError(checksumErr));
                }
                return settle(null, Buffer.concat(chunks).toString());
            },
            err => settle(err),
        );
    });

...
async function receiveData(request, log) {
    ...
    // `checksums.primary` is always set above, so primaryChecksumStream is the
    // end of the pipeline here; validate it explicitly rather than relying on
    // `prepared.stream` happening to be that transform.
    const checksumErr =
        (prepared.contentSHA256Stream && prepared.contentSHA256Stream.validateChecksum()) ||
        prepared.primaryChecksumStream.validateChecksum();
    if (checksumErr) {

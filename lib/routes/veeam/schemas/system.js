        ...
        SystemInfo: joi
            .object({
                ...
                SystemRecommendations: joi.object({
                    ...
                    KbBlockSize: joi.number().valid(256, 512, 1024, 2048, 4096, 8192).default(1024),
                }),
            })
            .required(),
    }),
...
function validateSystemSchema(parsedXML) {
    ...
    if (!protocolVersion) {
        throw new Error(
            errorInstances.MalformedXML.customizeDescription('ProtocolVersion must be set for the system.xml file')
        );
    }
    ...
    if (validatedData.error) {
        ...
        switch (protocolVersion) {
            case '"1.0"':
                ...
                if (
                    validatedData.value.SystemInfo.ProtocolCapabilities.IAMSTS &&

...
async function listVeeamFiles(request, response, bucketMd, log) {
    ...
    if (!('list-type' in request.query) && !('versions' in request.query)) {
        return responseXMLBody(
            errorInstances.InvalidRequest.customizeDescription('The Veeam folder does not support this action.'),
            null,
            response,
            log,
        );
    ...
    fieldsToGenerate.forEach(file => {
        ...
        const lastModified = isCapacity ? bucketMetrics.date : file.LastModified;
        // eslint-disable-next-line no-param-reassign
        delete file.LastModified;
        const dataBuffer = Buffer.from(buildXML(file));
        filesToBuild.push({
            ...getResponseHeader(request, data, dataBuffer, lastModified, log),
            name: file.name,
        });
    });

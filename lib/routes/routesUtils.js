...
function okXMLResponse(xml, response, log, additionalHeaders) {
    console.log('Reached okXMLresponse', Date.now(), new Date().toUTCString());
    // console.log('xml', xml);
    ...
    return response.end(xml, 'utf8', () => {
        console.log('Request ended in okXMLresponse', Date.now(), new Date().toUTCString())
...
const routesUtils = {
    ...
    responseXMLBody(errCode, xml, response, log, additionalHeaders) {
        console.log('Reached responseXML body');
        if (errCode && !response.headersSent) {
            console.log('About to send error XML response');
            return errorXMLResponse(errCode, response, log, additionalHeaders);
        }
        if (!response.headersSent) {
            console.log('About to send okXMLResponse');

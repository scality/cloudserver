...
function okHeaderResponse(headers, response, httpCode, log) {
    ...
    log.info(`Response HTTP Code: ${httpCode}`);
...
function okXMLResponse(xml, response, log) {
    ...
    log.info(`Response HTTP Code: 200`);
...
function errorXMLResponse(errCode, response, log) {
    log.debug(`Sending error XMl response for error: ${errCode}`);
    ...
    log.info(`Response HTTP Code: ${errObj.httpCode}`);
...
const routesUtils = {
    ...
    responseStreamData(errCode, overrideHeaders,
        ...
        if (Array.isArray(readStream)) {
            return this.responseStreamDataArray(readStream, response, log);
        }

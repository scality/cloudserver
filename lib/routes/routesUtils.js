...
function errorXMLResponse(errCode, response, log) {
    ...
    log.debug('response http code', { httpCode: errCode.code });
...
const routesUtils = {
     ...
     * headers. These are extracted from the request object
     * @param {string} resHeaders - headers to be set for the response
     * @param {stream.Readable, array or null} data --
     *   - stream.Readable from data if only one part,
     *   - array of locations to get streams from sproxyd if object
     *   was put as a multipart upload
    ...
    responseStreamData(errCode, overrideHeaders,
        ...
        if (Array.isArray(data)) {
            return this.responseStreamDataArray(data, response,
                                                log);
        }
        data.pipe(response, { end: false });
        data.on('end', function readStreamRes() {
            return response.end(() => {

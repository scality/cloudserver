  ...
  router.get("/:resource/(.*)", function(request, response){
    ...
    checkAuth(request, function(err, accessKey){
      ...
      objectGet(accessKey, datastore,  metastore, request, function(err, result){
          ...
          let etag = utils.getContentMD5(result);
          let additionalResponseHeaders = {};
          additionalResponseHeaders['Etag'] = etag;
          additionalResponseHeaders['Content-Length'] = result.length;
          if(request.lowerCaseHeaders['response-content-type']) {
            additionalResponseHeaders['Content-Type'] = request.lowerCaseHeaders['response-content-type']
          }
          if(request.lowerCaseHeaders['response-content-language']) {
            additionalResponseHeaders['Content-Language'] = request.lowerCaseHeaders['response-content-language']
          }
          if(request.lowerCaseHeaders['response-expires']) {
            additionalResponseHeaders['Expires'] = request.lowerCaseHeaders['response-expires']
          }
          if(request.lowerCaseHeaders['response-cache-control']) {
            additionalResponseHeaders['Cache-Control'] = request.lowerCaseHeaders['response-cache-control']
          }
          if(request.lowerCaseHeaders['response-content-disposition']) {
            additionalResponseHeaders['Content-Disposition'] = request.lowerCaseHeaders['response-content-disposition']
          }
          if(request.lowerCaseHeaders['response-content-encoding']) {
            additionalResponseHeaders['Content-Encoding'] = request.lowerCaseHeaders['response-content-encoding']
          }

...
function putMetadata(request, response, bucketInfo, objMd, log, callback) {
    return _getRequestPayload(request, (err, payload) => {
        ...
        let isNull = false;
        const backendType = config.backends.metadata;
        ...
        if (omVal.replicationInfo && omVal.replicationInfo.isNFS) {
        ...
        // MongoDB-specific logic for versioning edge cases
        if (backendType === 'mongodb' && versioning && versionId !== 'null') {
            log.debug('MongoDB backend: fetching versioned metadata', { bucketName, objectKey, versionId });
            return async.timeout(
                cb => metadata.getObjectMD(bucketName, objectKey, { versionId }, log, cb),
                5000
            )((err, versionedMD) => {
                if (err && err.TimeoutError) {
                    log.error('Timeout fetching versioned metadata', { bucketName, objectKey, versionId });
                    return callback(errors.Timeout);
                }
                log.debug('MongoDB backend: versioned metadata result', { err, versionedMD });
                if (!err && versionedMD) {
                    log.debug('MongoDB backend: updating versioned key directly', { versionId });
                    return async.timeout(
                        cb => metadata.putObjectMD(bucketName, objectKey, omVal, { versionId, versioning: true },
                            log, cb),
                        5000
                    )((err, result) => {
                        if (err && err.TimeoutError) {
                            log.error('Timeout updating versioned metadata', { bucketName, objectKey, versionId });
                            return callback(errors.Timeout);
                        }
                        return callback(err, result);
                    });
                }
                log.debug('MongoDB backend: fetching master metadata');
                return async.timeout(
                    cb => metadata.getObjectMD(bucketName, objectKey, {}, log, cb),
                    5000
                )((err2, masterMD) => {
                    if (err2 && err2.TimeoutError) {
                        log.error('Timeout fetching master metadata', { bucketName, objectKey });
                        return callback(errors.Timeout);
                    }
                    if (err2 && !err2.NoSuchKey) {
                        return callback(err2);
                    }
                    const masterIsNull = masterMD && (masterMD.isNull || !masterMD.versionId);
                    log.debug('MongoDB backend: masterIsNull check', { masterIsNull });
                    if (masterIsNull) {
                        const nullVersionId = masterMD.versionId || 'null';
                        log.debug('MongoDB backend: preserving null version', { nullVersionId });
                        const nullVersionMD = { ...masterMD };
                        delete nullVersionMD.versionId;
                        delete nullVersionMD.isNull;
                        return async.timeout(
                            cb => metadata.putObjectMD(bucketName, objectKey, nullVersionMD, 
                                { versionId: nullVersionId, versioning: true }, log, cb),
                            5000
                        )(err3 => {
                            if (err3 && err3.TimeoutError) {
                                log.error('Timeout preserving null version', { bucketName, objectKey, nullVersionId });
                                return callback(errors.Timeout);
                            }
                            if (err3) {
                                return callback(err3);
                            }
                            log.debug('MongoDB backend: writing new version as current');
                            return async.timeout(
                                cb => metadata.putObjectMD(bucketName, objectKey, omVal, options, log, cb),
                                5000
                            )((err4, result) => {
                                if (err4 && err4.TimeoutError) {
                                    log.error('Timeout writing new version', { bucketName, objectKey });
                                    return callback(errors.Timeout);
                                }
                                return callback(err4, result);
                            });
                        });
                    }
                    log.debug('MongoDB backend: writing new version as current (no null version preservation)');
                    return async.timeout(
                        cb => metadata.putObjectMD(bucketName, objectKey, omVal, options, log, cb),
                        5000
                    )((err, result) => {

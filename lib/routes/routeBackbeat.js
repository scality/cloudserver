...
function putMetadata(request, response, bucketInfo, objMd, log, callback) {
    return _getRequestPayload(request, (err, payload) => {
        ...
        if (backendType === 'mongodb' && versionId && versionId !== 'null') {
            // Try to update the versioned key directly first
            return metadata.getObjectMD(bucketName, objectKey, { versionId }, log, (err, versionedMD) => {
                if (!err && versionedMD) {
                    // Update the versioned key directly (non-current null version update)
                    return metadata.putObjectMD(bucketName, objectKey, omVal, { versionId, versioning: true }, log, callback);
                }
                // If not found, check the master for null version preservation
                return metadata.getObjectMD(bucketName, objectKey, {}, log, (err2, masterMD) => {
                    if (err2 && !err2.NoSuchKey) {
                        return callback(err2);
                    }
                    const masterIsNull = masterMD && (masterMD.isNull || !masterMD.versionId);
                    if (masterIsNull) {
                        // Preserve the null version as a non-current version
                        ...
                        return metadata.putObjectMD(
                            ...
                            objectKey,
                            nullVersionMD,
                            { versionId: nullVersionId, versioning: true },
                            log,
                            err3 => {
                                if (err3) return callback(err3);
                                // Write the new version as current
                                return metadata.putObjectMD(bucketName, objectKey, omVal, options, log, callback);
                            }
                        );
                    }
                    // Write the new version as current

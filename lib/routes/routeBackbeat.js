...
function handleTaggingOperation(request, response, type, dataStoreVersionId,
    ...
    return data.objectTagging(type, request.objectKey,
...
function deleteObject(request, response, log, callback) {
    ...
    const storageLocation = request.headers['x-scal-storage-class'];
    const objectGetInfo = data.toObjectGetInfo(
    ...
    return data.delete(objectGetInfo, reqUids, err => {
        if (err) {
            log.error('error deleting object in multiple backend', {
                error: err,
                method: 'deleteObject',
...
function initiateMultipartUpload(request, response, log, callback) {
    ...
    const mpuInfo = {
        ...
        metaHeaders,
        bucketName: request.bucketName,
        locConstraint: storageLocation,
        contentType,
        cacheControl,
        contentDisposition,
        contentEncoding,
    };
    return data.initiateMPU(mpuInfo, undefined, log,
        (err, data) => {
            if (err) {
...
function abortMultipartUpload(request, response, log, callback) {
    ...
    return data.abortMPU(request.objectKey, uploadId,
...
function putPart(request, response, log, callback) {
    ...
    return data.putPart(undefined, mpuInfo, {}, storageLocation,
        locationConstraintCheck, log, (err, data) => {
            if (err) {
...
function completeMultipartUpload(request, response, log, callback) {
    ...
    request.on('end', () => {
        ...
        return data.completeMPU(request, mpuInfo, undefined, storageLocation,
            metaHeaders, contentSettings, locationConstraintCheck,
            log, (err, retrievalInfo) => {
                if (err) {

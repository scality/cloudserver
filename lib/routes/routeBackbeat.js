...
function putMetadata(request, response, bucketInfo, objMd, log, callback) {
    return _getRequestPayload(request, (err, payload) => {
        ...
        let isNull = false;
        const nullVersionCompatMode = request.query.nullVersionCompatMode === 'true';
        ...
        if (backendType === 'mongodb' && versionId && versionId !== 'null') {
            return metadata.getObjectMD(bucketName, objectKey, {}, log, (err, masterMD) => {
                if (err && !err.NoSuchKey) return callback(err);
        
                // Handle both: (a) master is a null version (no versionId or isNull)
                // and (b) versioning is suspended and master is a null version
                const versioningConf = bucketInfo.getVersioningConfiguration();
                const isSuspended = versioningConf && versioningConf.Status === 'Suspended';
                const masterIsNull = masterMD && (masterMD.isNull || !masterMD.versionId);
        
                if (masterIsNull) {
                    // Always preserve the null version as a non-current version
                    ...
                    return metadata.putObjectMD(
                        ...
                        err2 => {
                            ...
                            // Writing the new version as current

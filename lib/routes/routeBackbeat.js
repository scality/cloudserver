...
function routeBackbeat(clientIP, request, response, log) {
    ...
    // DEBUG: bypass auth for expiration requests
    if (request.resourceType === 'expiration') {
        const fakeUserInfo = new AuthInfo({
        ...
        log.info('DEBUG: bypassing auth for expiration request');
        return deleteObjectFromExpiration(request, response, fakeUserInfo, log, err => {
            if (err) {
                return responseJSONBody(err, null, response, log);
            }

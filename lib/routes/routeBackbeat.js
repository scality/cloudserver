...
const {
    auth, errors, errorInstances, s3middleware, s3routes, models, storage, versioning,
} = require('@scality/arsenal');
...
function putObject(request, response, log, callback) {
    ...
    return dataStore(context, CIPHER, request, payloadLen, {}, backendInfo, null, log, (err, retrievalInfo, md5) => {
        if (err) {
            log.error('error putting data', {
                error: err,
                method: 'putObject',

...
function routeBackbeatAPIProxy(request, response, requestContexts, apiMethods, log, callback) {
    const path = request.url.replace('/_/backbeat/api', '/_/');
    const { host, port } = config.backbeat;
    const target = `http://${host}:${port}${path}`;

    return async.waterfall([
        next => auth.server.doAuth(request, log, (err, userInfo, authorizationResults) => {
            if (err) {
        ...
        (userInfo, authorizationResults, next) => handleAuthorizationResults(
            authorizationResults, apiMethods[0], log, err => next(err, userInfo)),
    ], (err, userInfo) => {
        if (err) {
            return respondToRequest(err, response, log, callback);
        }
        ...
        if (userInfo.getCanonicalID() === constants.publicId) {
            ...
            return respondToRequest(errors.AccessDenied, response, log, callback);
        }
        return backbeatProxy.web(request, response, { target }, err => {
            log.error('error proxying request to api server',
                { error: err.message });
            return respondToRequest(errors.ServiceUnavailable, response, log, callback);
        });
    });
}
...
function routeBackbeat(clientIP, request, response, log, callback) {
    ...
    if (request.resourceType === 'api') {
        ...
        return routeBackbeatAPIProxy(request, response, requestContexts, apiMethods, log, callback);
    }
    ...
    return async.waterfall([
        ...
        (userInfo, authorizationResults, next) =>
            handleAuthorizationResults(request, authorizationResults, apiMethods[0], {}, log, err =>
                next(err, userInfo)),
        (userInfo, next) => {
            // TODO: understand why non-object requests (batchdelete) were not authenticated
            // eslint-disable-next-line no-console
            console.log('is object request', _isObjectRequest(request), request.resourceType)
            if (!_isObjectRequest(request)) {
                ...
                return route(request, response, userInfo, log, next);
            }
        ...
        (bucketInfo, objMd, next) => {
            // eslint-disable-next-line no-console
            console.log('resource', request.method, request.resourceType);
            if (!useMultipleBackend) {
            ...
            if (err) {
                return async.forEachLimit(
                    ...
                    () => {
                        ...
                        return respondToRequest(null, response, log, callback);
                    },
                );
            }
            return respondToRequest(null, response, log, callback);

...
const { auth, s3routes, errors, errorInstances } = require('arsenal');
...
const allowedSdkQueryKeys = new Set(['x-id', 'x-amz-user-agent']);
        ...
        for (const [key, value] of Object.entries(originalQuery)) {
            ...
            if (normalizedKey === 'x-id' && value !== apiToAction[method]) {
                return errorInstances.InvalidRequest.customizeDescription(
                    'The Veeam SOSAPI folder does not support this action.',
                );
            }

            const isAllowedSdkKey = allowedSdkQueryKeys.has(normalizedKey) || normalizedKey.startsWith('x-amz-sdk-');
...
function _normalizeVeeamRequest(req) {
    ...
    req.parsedContentLength = Number.parseInt(contentLength?.toString() ?? '', 10);
    /* eslint-enable no-param-reassign */
}
...
function routeVeeam(clientIP, request, response, log) {
    ...
    if (bucketOrKeyError) {
        log.error('error with bucket or key value', { error: bucketOrKeyError });

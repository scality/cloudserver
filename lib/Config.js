...
function gcpLocationConstraintAssert(location, locationObj) {
    ...
    const keyFilename = serviceCredentials && serviceCredentials.keyFilename ||
        serviceKeyFileFromEnv || defaultKeyFileFromEnv;
    const serviceEmail = serviceCredentials &&
        serviceCredentials.serviceEmail || serviceEmailFromEnv;
    const serviceKey = serviceCredentials &&
        serviceCredentials.serviceKey || serviceKeyFromEnv;
    const stringFields = [
        gcpEndpoint,
        bucketName,
        mpuBucketName,
...
class Config extends EventEmitter {
    ...
    getGcpServiceParams(locationConstraint) {
        ...
        return {
            keyFilename: serviceCredentials && serviceCredentials.keyFilename ||
                serviceKeyFileFromEnv || defaultKeyFileFromEnv,
            /* eslint-disable camelcase */
            credentials: {
                client_email: serviceCredentials &&
                    serviceCredentials.serviceEmail || serviceEmailFromEnv,
                private_key: serviceCredentials &&
                    serviceCredentials.serviceKey || serviceKeyFromEnv,
            },

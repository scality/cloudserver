...
class Config extends EventEmitter {
    ...
    _getConfig() {
        ...
        if (config.rateLimiting?.enabled) {
            ...
            // Default rate limit (simple maxRps number)
            ...
            // Default burst capacity (in seconds)
            if (config.rateLimiting.default?.burstCapacity !== undefined) {
                assert(
                    typeof config.rateLimiting.default.burstCapacity === 'number' &&
                    Number.isInteger(config.rateLimiting.default.burstCapacity) &&
                    config.rateLimiting.default.burstCapacity >= 0,
                    'rateLimiting.default.burstCapacity must be a non-negative integer (0 = no burst)'
                );
                this.rateLimiting.default.burstCapacity = config.rateLimiting.default.burstCapacity;
            }
            ...
            // Custom error response (429 with configurable message)
            if (config.rateLimiting.error) {
                ...
                assert(config.rateLimiting.error.code, 'rateLimiting.error.code required');
                assert(config.rateLimiting.error.description, 'rateLimiting.error.description required');

                this.rateLimiting.error = new ArsenalError('SlowDown',
                    config.rateLimiting.error.code,
                    config.rateLimiting.error.description);
            } else {
                // Default to standard SlowDown error (503)

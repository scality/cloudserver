    ...
    Object.keys(locationConstraints).forEach(l => {
        ...
        if (details.supportsVersioning !== undefined) {
            assert(
                ...
                'bad config: locationConstraints[region].supportsVersioningmust be a boolean',
            );
        } else {
            // default to true
            // eslint-disable-next-line no-param-reassign
        ...
        if (config.localCache) {
            ...
            if (config.localCache.sentinels) {
                ...
                assert(
                    ...
                    'bad config: localCache sentinelsmust be an array or string',
                );
                ...
                if (typeof config.localCache.sentinels === 'string') {
                    ...
                    config.localCache.sentinels.forEach(item => {
                        ...
                        assert(typeof host === 'string', 'bad config: localCachesentinel host must be a string');
                        assert(typeof port === 'number', 'bad config: localCachesentinel port must be a number');
            ...
            if (utapiVersion === 1) {
                ...
                if (config.utapi.replaySchedule) {
                    assert(
                        ...
                        'badconfig: utapi.replaySchedule must be a string',
                    );
                ...
                if (config.utapi.batchSize) {
                    assert(typeof config.utapi.batchSize === 'number', 'badconfig: utapi.batchSize must be a number');
                ...
                if (config.utapi.expireMetrics !== undefined) {
                    assert(
                        ...
                        'badconfig: utapi.expireMetrics must be a boolean',
                    );

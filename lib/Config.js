...
function bucketNotifAssert(bucketNotifConfig) {
    const { resource, type, host, port, auth } = bucketNotifConfig;
...
class Config extends EventEmitter {
    ...
    _getConfig() {
        ...
        if (config.bucketNotificationTargets) {
            bucketNotifAssert(config.bucketNotificationTargets);
            this.bucketNotificationTargets = config.bucketNotificationTargets;
            const cert = this.bucketNotificationTargets.auth.cert;
            if (cert) {
                this.bucketNotificationTargets.auth.cert = (cert[0] === '/') ? cert : `${this._basePath}/${cert}`;
            }
        }
    }

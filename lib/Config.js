        ...
        this.kmip = {
            ...
            transport: {
                ...
                tls: {
                    ...
                    key: process.env.S3KMIP_KEY || undefined,
                    cert: process.env.S3KMIP_CERT || undefined,
                ...
                if (config.kmip.transport.tls) {
                    ...
                    if (key) {
                        assert(typeof key === 'string',
                               'bad config: KMIP TLS key must be a string');
                        const keypath = (key[0] === '/')
                              ? key
                              : path.join(this._basePath, key);
                        this.kmip.transport.tls.key = fs.readFileSync(keypath);
                    }
                    if (cert) {
                        assert(typeof cert === 'string',
                               'bad config: KMIP TLS cert must be a string');
                        const certpath = (cert[0] === '/')
                              ? cert
                              : path.join(this._basePath, cert);
                        this.kmip.transport.tls.cert =
                            fs.readFileSync(certpath);
                    }
                    if (Array.isArray(ca)) {
                        this.kmip.transport.tls.ca = ca.map(cacert => {
                            assert(typeof cacert === 'string',
                                   'bad config: KMIP TLS ca must be a string ' +
                                   'or an array of strings');
                            const cacertpath = (cacert[0] === '/')
                                  ? cacert
                                  : path.join(this._basePath, cacert);
                        ...
                        const capath = (ca[0] === '/')
                              ? ca
                              : path.join(this._basePath, ca);
                        this.kmip.transport.tls.ca = fs.readFileSync(capath);
                    }
                }
            }
        }

...
const { ValidLifecycleRules: supportedLifecycleRules, NotificationConfiguration } = require('arsenal').models;
...
// config paths
const configSearchPaths = [path.join(__dirname, '../conf'), path.join(__dirname, '..')];
...
function findConfigFile(fileName) {
    ...
    if (!containingPath) {
        throw new Error(
            `Unable to find the configuration file "${fileName}" ` +
...
function assertCertPaths(key, cert, ca, basePath) {
    ...
    if (ca) {
        ...
        assert.doesNotThrow(() => fs.accessSync(capath, fs.F_OK | fs.R_OK), `File not found or unreachable: ${capath}`);
...
function parseSupportedLifecycleRules(supportedLifecycleRulesConfig) {
    const supportedLifecycleRulesSchema = joi
        .array()
...
function restEndpointsAssert(restEndpoints, locationConstraints) {
    assert(typeof restEndpoints === 'object', 'bad config: restEndpoints must be an object of endpoints');
    assert(
        Object.keys(restEndpoints).every(r => typeof restEndpoints[r] === 'string'),
        'bad config: each endpoint must be a string',
    );
    assert(
        Object.keys(restEndpoints).every(r => typeof locationConstraints[restEndpoints[r]] === 'object'),
        'bad config: rest endpoint target not in locationConstraints',
    );
}

function gcpLocationConstraintAssert(location, locationObj) {
    const { gcpEndpoint, bucketName, mpuBucketName } = locationObj.details;
    const stringFields = [gcpEndpoint, bucketName, mpuBucketName];
    stringFields.forEach(field => {
        if (field !== undefined) {
            assert(typeof field === 'string', `bad config: ${field} must be a string`);
        }
    });
}
...
function azureGetStorageAccountName(location, locationDetails) {
    ...
    const storageAccountNameFromEnv = process.env[`${location}_AZURE_STORAGE_ACCOUNT_NAME`];
...
function azureGetLocationCredentials(location, locationDetails) {
    const storageAccessKey =
        process.env[`${location}_AZURE_STORAGE_ACCESS_KEY`] || locationDetails.azureStorageAccessKey;
    const sasToken = process.env[`${location}_AZURE_SAS_TOKEN`] || locationDetails.sasToken;
    const clientKey = process.env[`${location}_AZURE_CLIENT_KEY`] || locationDetails.clientKey;
    ...
    switch (authMethod) {
        ...
        default:
            return {
                authMethod,
                storageAccountName: azureGetStorageAccountName(location, locationDetails),
                storageAccessKey,
            };
        ...
        case 'client-secret':
            return {
                authMethod,
                tenantId: process.env[`${location}_AZURE_TENANT_ID`] || locationDetails.tenantId,
                clientId: process.env[`${location}_AZURE_CLIENT_ID`] || locationDetails.clientId,
                clientKey,
            };
    }
}
...
function azureLocationConstraintAssert(location, locationObj) {
    const locationParams = {
        ...
        azureStorageEndpoint:
            process.env[`${location}_AZURE_STORAGE_ENDPOINT`] || locationObj.details.azureStorageEndpoint,
    ...
    Object.keys(locationParams).forEach(param => {
        ...
        assert.strictEqual(
            typeof value,
            'string',
            `bad location constraint: "${location}" ${param} ` + `"${value}" must be a string`,
        );
    });

    if (locationParams.authMethod === 'shared-key') {
        assert(
            azureAccountNameRegex.test(locationParams.storageAccountName),
            `bad location constraint: "${location}" azureStorageAccountName ` +
                `"${locationParams.storageAccountName}" is an invalid value`,
        );
        assert(
            base64Regex.test(locationParams.storageAccessKey),
            `bad location constraint: "${location}" ` + 'azureStorageAccessKey is not a valid base64 string',
        );
    }
    assert(
        isValidBucketName(locationParams.azureContainerName, []),
        `bad location constraint: "${location}" ` + 'azureContainerName is an invalid container name',
    );
}
...
function hdClientLocationConstraintAssert(configHd) {
    ...
    if (configHd.bootstrap !== undefined) {
        assert(
            Array.isArray(configHd.bootstrap) && configHd.bootstrap.every(e => typeof e === 'string'),
            'bad config: hdclient.bootstrap must be an array of strings',
        );
        assert(configHd.bootstrap.length > 0, 'bad config: hdclient bootstrap list is empty');
...
function locationConstraintAssert(locationConstraints) {
    const supportedBackends = ['mem', 'file', 'scality', 'mongodb', 'tlp', 'crr'].concat(
        Object.keys(validExternalBackends),
    );
    assert(typeof locationConstraints === 'object', 'bad config: locationConstraints must be an object');
    Object.keys(locationConstraints).forEach(l => {
        assert(typeof locationConstraints[l] === 'object', 'bad config: locationConstraints[region] must be an object');
        assert(
            typeof locationConstraints[l].type === 'string',
            'bad config: locationConstraints[region].type is mandatory and must be a string',
        );
        assert(
            supportedBackends.indexOf(locationConstraints[l].type) > -1,
            'bad config: locationConstraints[region].type must ' + `be one of ${supportedBackends}`,
        );
        assert(
            typeof locationConstraints[l].objectId === 'string',
            'bad config: locationConstraints[region].objectId is ' +
                'mandatory and must be a unique string across locations',
        );
        assert(
            Object.keys(locationConstraints).filter(
                loc => locationConstraints[loc].objectId === locationConstraints[l].objectId,
            ).length === 1,
        ...
        assert(
            typeof locationConstraints[l].legacyAwsBehavior === 'boolean',
            'bad config: locationConstraints[region].legacyAwsBehavior is mandatory and must be a boolean',
        );
        assert(
            ['undefined', 'boolean'].includes(typeof locationConstraints[l].isTransient),
            'bad config: locationConstraints[region].isTransient must be a boolean',
        );
        if (locationConstraints[l].sizeLimitGB !== undefined) {
            assert(
                typeof locationConstraints[l].sizeLimitGB === 'number' || locationConstraints[l].sizeLimitGB === null,
        ...
        if (details.serverSideEncryption !== undefined) {
            assert(
                typeof details.serverSideEncryption === 'boolean',
                'bad config: locationConstraints[region].details.serverSideEncryption must be a boolean',
            );
        }
        const stringFields = ['awsEndpoint', 'bucketName', 'credentialsProfile', 'region'];
        stringFields.forEach(field => {
            if (details[field] !== undefined) {
                assert(typeof details[field] === 'string', `bad config: ${field} must be a string`);
            }
        });
        if (details.bucketMatch !== undefined) {
            assert(typeof details.bucketMatch === 'boolean', 'bad config: details.bucketMatch must be a boolean');
        }
        if (details.credentials !== undefined) {
            assert(typeof details.credentials === 'object', 'bad config: details.credentials must be an object');
            assert(
                typeof details.credentials.accessKey === 'string',
                'bad config: credentials must include accessKey as string',
            );
            assert(
                typeof details.credentials.secretKey === 'string',
        ...
        if (details.https !== undefined) {
            assert(
                typeof details.https === 'boolean',
                'bad config: locationConstraints[region].details https must be a boolean',
            );
        } else {
        ...
        if (details.pathStyle !== undefined) {
            assert(
                typeof details.pathStyle === 'boolean',
                'bad config: locationConstraints[region].pathStyle must be a boolean',
            );
        } else {
            // eslint-disable-next-line no-param-reassign
        ...
        if (details.supportsVersioning !== undefined) {
            assert(
                typeof details.supportsVersioning === 'boolean',
                'bad config: locationConstraints[region].supportsVersioning must be a boolean',
            );
        } else {
            // default to true
            // eslint-disable-next-line no-param-reassign
        ...
        if (locationConstraints[l].type === 'pfs') {
            assert(
                typeof details.pfsDaemonEndpoint === 'object',
                'bad config: pfsDaemonEndpoint is mandatory and must be an object',
            );
        }
        if (
            locationConstraints[l].type === 'scality' &&
    ...
    assert(
        Object.keys(locationConstraints).includes('us-east-1'),
        'bad locationConfig: must include us-east-1 as a locationConstraint',
    );
}

function parseUtapiReindex(config) {
    const { enabled, schedule, redis, bucketd, onlyCountLatestWhenObjectLocked } = config;
    assert(typeof enabled === 'boolean', 'bad config: utapi.reindex.enabled must be a boolean');

    const parsedRedis = parseRedisConfig(redis);
    assert(Array.isArray(parsedRedis.sentinels), 'bad config: utapi reindex redis config requires a list of sentinels');

    assert(typeof bucketd === 'object', 'bad config: utapi.reindex.bucketd must be an object');
    assert(typeof bucketd.port === 'number', 'bad config: utapi.reindex.bucketd.port must be a number');
    assert(typeof schedule === 'string', 'bad config: utapi.reindex.schedule must be a string');
    if (onlyCountLatestWhenObjectLocked !== undefined) {
        assert(
            typeof onlyCountLatestWhenObjectLocked === 'boolean',
    ...
    try {
        ...
        assert(false, 'bad config: utapi.reindex.schedule must be a valid ' + `cron schedule. ${e.message}.`);
    }
    return {
        enabled,
        schedule,
...
function requestsConfigAssert(requestsConfig) {
    if (requestsConfig.viaProxy !== undefined) {
        assert(
            typeof requestsConfig.viaProxy === 'boolean',
            'config: invalid requests configuration. viaProxy must be a boolean',
        );

        if (requestsConfig.viaProxy) {
            assert(
                Array.isArray(requestsConfig.trustedProxyCIDRs) &&
                    requestsConfig.trustedProxyCIDRs.length > 0 &&
                    requestsConfig.trustedProxyCIDRs.every(ip => typeof ip === 'string'),
                'config: invalid requests configuration. trustedProxyCIDRs ' +
                    'must be set if viaProxy is set to true and must be an array',
            );

            assert(
                typeof requestsConfig.extractClientIPFromHeader === 'string' &&
                    requestsConfig.extractClientIPFromHeader.length > 0,
                'config: invalid requests configuration. extractClientIPFromHeader ' +
                    'must be set if viaProxy is set to true and must be a string',
            );

            assert(
                typeof requestsConfig.extractProtocolFromHeader === 'string' &&
                    requestsConfig.extractProtocolFromHeader.length > 0,
                'config: invalid requests configuration. extractProtocolFromHeader ' +
                    'must be set if viaProxy is set to true and must be a string',
            );
        }

        // All headers in NodeJS are lowercase: to be exploitable
        // we need to lowercase the value.
        // eslint-disable-next-line no-param-reassign
        requestsConfig.extractClientIPFromHeader = requestsConfig.extractClientIPFromHeader?.toLowerCase();
        // eslint-disable-next-line no-param-reassign
        requestsConfig.extractProtocolFromHeader = requestsConfig.extractProtocolFromHeader?.toLowerCase();
    }
}

function bucketNotifAssert(bucketNotifConfig) {
    assert(Array.isArray(bucketNotifConfig), 'bad config: bucket notification configuration must be an array');
    bucketNotifConfig.forEach(c => {
        const { resource, type, host, port, auth } = c;
        assert(typeof resource === 'string', 'bad config: bucket notification configuration resource must be a string');
        assert(typeof type === 'string', 'bad config: bucket notification configuration type must be a string');
        assert(typeof host === 'string' && host !== '', 'bad config: hostname must be a non-empty string');
        if (port) {
            assert(Number.isInteger(port, 10) && port > 0, 'bad config: port must be a positive integer');
        }
        if (auth) {
            assert(typeof auth === 'object', 'bad config: bucket notification auth must be an object');
        }
    });
...
function parseServerAccessLogs(config) {
    ...
    if (config && config.serverAccessLogs) {
        const settings = [
            ...
            { key: 'checkFileRotationIntervalMS', type: 'number' },
        ];

        settings.forEach(setting => {
            if (setting.key in config.serverAccessLogs) {
                assert(
                    typeof config.serverAccessLogs[setting.key] === setting.type,
                    `bad config: serverAccessLogs.${setting.key} is not a ${setting.type}`,
                );
        ...
        if ('mode' in config.serverAccessLogs) {
            assert(
                validModes.includes(config.serverAccessLogs.mode),
    ...
    if (process.env.S3_SERVER_ACCESS_LOGS_MODE) {
        assert(
            validModes.includes(process.env.S3_SERVER_ACCESS_LOGS_MODE),
 ...
 *
 * Safe to toggle part-way through a multipart upload, and safe to roll out
 * progressively across a fleet. Parts uploaded while disabled carry no digest,
...
class Config extends EventEmitter {
    constructor() {
        ...
        this.configPath = findConfigFile(process.env.S3_CONFIG_FILE || 'config.json');

        let locationConfigFileName = 'locationConfig.json';
        if (process.env.CI === 'true' && !process.env.S3_END_TO_END) {
            locationConfigFileName = 'tests/locationConfig/locationConfigTests.json';
        }
        this.locationConfigPath = findConfigFile(process.env.S3_LOCATION_FILE || locationConfigFileName);
    ...
    _parseKmsAWS(config) {
        ...
        assert(
            ['undefined', 'boolean'].some(type => type === typeof noAwsArn),
        ...
        if (tls) {
            ...
            if (tls.minVersion !== undefined) {
                assert(typeof tls.minVersion === 'string', 'bad config: KMS AWS TLS minVersion must be a string');
                kmsAWS.tls.minVersion = tls.minVersion;
            }
            if (tls.maxVersion !== undefined) {
                assert(typeof tls.maxVersion === 'string', 'bad config: KMS AWS TLS maxVersion must be a string');
    ...
    _parseKmipTransport(transportKmip) {
        const transport = {
            ...
            tls: {
                ...
                cert: this._loadTlsFile(process.env.S3KMIP_CERT || undefined),
                ca: (process.env.S3KMIP_CA ? process.env.S3KMIP_CA.split(',') : []).map(ca => this._loadTlsFile(ca)),
            },
        };
        ...
        if (transportKmip.tls) {
            ...
            if (!!key !== !!cert) {
                throw new Error('bad config: KMIP TLS certificate and key must come along');
            }
            if (port) {
                assert(typeof port === 'number', 'bad config: KMIP TLS Port must be a number');
                transport.tls.port = port;
            }
            if (host) {
                assert(typeof host === 'string', 'bad config: KMIP TLS Host must be a string');
    ...
    _parseKmsKmip(config) {
        this.kmip = {
            client: {
                 ...
                 */
                compoundCreateActivate: process.env.S3KMIP_COMPOUND_CREATE === 'true' || false,
                /** Set the bucket name attribute name here if the KMIP
                 * server supports storing custom attributes along
                 * with the keys.
                 */
                bucketNameAttributeName: process.env.S3KMIP_BUCKET_ATTRIBUTE_NAME || '',
            },
        ...
        if (config.kmip) {
            ...
            assert(
                isValidProvider(config.kmip.providerName),
            ...
            if (config.kmip.client) {
                if (config.kmip.client.compoundCreateActivate) {
                    assert(typeof config.kmip.client.compoundCreateActivate === 'boolean');
                    this.kmip.client.compoundCreateActivate = config.kmip.client.compoundCreateActivate;
                }
                if (config.kmip.client.bucketNameAttributeName) {
                    assert(typeof config.kmip.client.bucketNameAttributeName === 'string');
                    this.kmip.client.bucketNameAttributeName = config.kmip.client.bucketNameAttributeName;
                }
            }
            if (config.kmip.transport) {
                if (Array.isArray(config.kmip.transport)) {
                    this.kmip.transport = config.kmip.transport.map(t => this._parseKmipTransport(t));
                    if (config.kmip.retries) {
                        assert(
                            typeof config.kmip.retries === 'number',
                            'bad config: KMIP Cluster retries must be a number',
                        );
                        assert(
                            config.kmip.retries <= this.kmip.transport.length - 1,
                            'bad config: KMIP Cluster retries must be lower or equal to the number of hosts - 1',
                        );
                    } else {
                        this.kmip.retries = this.kmip.transport.length - 1;
                    }
                } else {
                    this.kmip.transport = this._parseKmipTransport(config.kmip.transport);
                }
            }
        }
    }
    ...
    _getLocationConfig() {
        let locationConfig;
        try {
            const data = fs.readFileSync(this.locationConfigPath, { encoding: 'utf-8' });
        ...
        Object.keys(locationConfig).forEach(l => {
            ...
            if (locationConfig[l].details.connector !== undefined) {
                assert(
                    typeof locationConfig[l].details.connector === 'object',
                    'bad config: connector must be an object',
                );
                if (locationConfig[l].details.connector.sproxyd !== undefined) {
                    details.connector.sproxyd = parseSproxydConfig(locationConfig[l].details.connector.sproxyd);
                }
            }
        });
    }
    ...
    _loadTlsFile(tlsFileName) {
        ...
        if (typeof tlsFileName !== 'string') {
            throw new Error('bad config: TLS file specification must be a string');
        }
        const tlsFilePath = tlsFileName[0] === '/' ? tlsFileName : path.join(this._basePath, tlsFileName);
        let tlsFileContent;
        try {
            tlsFileContent = fs.readFileSync(tlsFilePath);
        } catch (err) {
            throw new Error(`Could not load tls file '${tlsFileName}':` + ` ${err.message}`);
        }
    ...
    _parseEndpoints(listenOn, fieldName) {
        ...
        if (listenOn !== undefined) {
            assert(
                Array.isArray(listenOn) && listenOn.every(e => typeof e === 'string'),
            ...
            result = listenOn.map(item => {
                ...
                const ipAddress = item.indexOf(']') > 0 ? item.substr(1, lastColon - 2) : item.substr(0, lastColon);
                // the port should not include the colon
                const port = item.substr(lastColon + 1);
                assert(Number.parseInt(port, 10), `bad config: ${fieldName} port must be a positive integer`);
    ...
    _getConfig() {
        ...
        if (this.replicationConfigPath) {
            try {
                const repData = fs.readFileSync(this.replicationConfigPath, { encoding: 'utf-8' });
                const replicationEndpoints = JSON.parse(repData);
                config.replicationEndpoints.push(...replicationEndpoints);
            } catch (err) {
                throw new Error(`could not parse replication file: ${err.message}`);
            }
        }

        if (config.port !== undefined) {
            assert(Number.isInteger(config.port) && config.port > 0, 'bad config: port must be a positive integer');
        }

        if (config.internalPort !== undefined) {
            assert(
                Number.isInteger(config.internalPort) && config.internalPort > 0,
        ...
        if (config.metricsPort !== undefined) {
            assert(
                Number.isInteger(config.metricsPort) && config.metricsPort > 0,
        ...
        if (config.replicationGroupId) {
            assert(typeof config.replicationGroupId === 'string', 'bad config: replicationGroupId must be a string');
        ...
        if (instanceId) {
            assert(typeof instanceId === 'string', 'bad config: instanceId must be a string');
        ...
        if (config.replicationEndpoints) {
            ...
            assert(
                replicationEndpoints instanceof Array,
                'bad config: `replicationEndpoints` property must be an array',
            );
            replicationEndpoints.forEach(replicationEndpoint => {
                assert.strictEqual(
                    typeof replicationEndpoint,
                    'object',
                    'bad config: `replicationEndpoints` property must be an array of objects',
                );
                const { site, servers, type } = replicationEndpoint;
                assert.notStrictEqual(
                    site,
                    undefined,
                    'bad config: each object of `replicationEndpoints` array must have a `site` property',
                );
                assert.strictEqual(
                    typeof site,
                    'string',
                    'bad config: `site` property of object in `replicationEndpoints` must be a string',
                );
                assert.notStrictEqual(
                    site,
                    '',
                    "bad config: `site` property of object in `replicationEndpoints` must not be ''",
                );
                if (type !== undefined) {
                    assert(
                        validExternalBackends[type],
                        'bad config: `type` property of `replicationEndpoints` ' +
                            'object must be a valid external backend (one of: "' +
                    ...
                    assert.notStrictEqual(
                        servers,
                        undefined,
                        'bad config: each object of `replicationEndpoints` array ' +
                            'that is not an external backend must have `servers` property',
                    );
                    assert(
                        servers instanceof Array,
                        'bad config: `servers` property of object in `replicationEndpoints` must be an array',
                    );
                    servers.forEach(item => {
                        assert(
                            typeof item === 'string' && item !== '',
                            'bad config: each item of `replicationEndpoints:servers` must be a non-empty string',
                        );
                    });
                }
            });
        ...
        if (config.backbeat) {
            ...
            assert.strictEqual(typeof backbeat.host, 'string', 'bad config: backbeat host must be a string');
            assert(
                Number.isInteger(backbeat.port) && backbeat.port > 0,
        ...
        if (config.workflowEngineOperator) {
            ...
            assert.strictEqual(
                typeof workflowEngineOperator.host,
                'string',
                'bad config: workflowEngineOperator host must be a string',
            );
            assert(
                Number.isInteger(workflowEngineOperator.port) && workflowEngineOperator.port > 0,
        ...
        if (config.websiteEndpoints !== undefined) {
            assert(
                Array.isArray(config.websiteEndpoints) && config.websiteEndpoints.every(e => typeof e === 'string'),
        ...
        if (config.clusters !== undefined) {
            assert(
                Number.isInteger(config.clusters) && config.clusters > 0,
        ...
        if (config.usEastBehavior !== undefined) {
            throw new Error(
                'bad config: usEastBehavior key is deprecated. Please use restEndpoints and locationConfig',
            );
        }
        // legacy
        ...
        if (config.cdmi !== undefined) {
            if (config.cdmi.host !== undefined) {
                assert.strictEqual(typeof config.cdmi.host, 'string', 'bad config: cdmi host must be a string');
                this.cdmi.host = config.cdmi.host;
            }
            if (config.cdmi.port !== undefined) {
                assert(
                    Number.isInteger(config.cdmi.port) && config.cdmi.port > 0,
                    'bad config: cdmi port must be a positive integer',
                );
                this.cdmi.port = config.cdmi.port;
            }
            if (config.cdmi.path !== undefined) {
                assert(typeof config.cdmi.path === 'string', 'bad config: cdmi.path must be a string');
                assert(config.cdmi.path.length > 0, 'bad config: cdmi.path is empty');
                assert(config.cdmi.path.charAt(0) === '/', 'bad config: cdmi.path should start with a "/"');
                this.cdmi.path = config.cdmi.path;
            }
            if (config.cdmi.readonly !== undefined) {
                assert(typeof config.cdmi.readonly === 'boolean', 'bad config: cdmi.readonly must be a boolean');
        ...
        if (config.bucketd !== undefined && config.bucketd.bootstrap !== undefined) {
            assert(
                config.bucketd.bootstrap instanceof Array && config.bucketd.bootstrap.every(e => typeof e === 'string'),
        ...
        if (config.dataClient) {
            this.dataClient = {};
            assert.strictEqual(
                typeof config.dataClient.host,
                'string',
                'bad config: data client host must be a string',
            );
            this.dataClient.host = config.dataClient.host;

            assert(
                Number.isInteger(config.dataClient.port) && config.dataClient.port > 0,
                'bad config: dataClient port must be a positive integer',
            );
        ...
        if (config.metadataClient) {
            this.metadataClient = {};
            assert.strictEqual(
                typeof config.metadataClient.host,
                'string',
            ...
            assert(
                Number.isInteger(config.metadataClient.port) && config.metadataClient.port > 0,
                'bad config: metadata client port must be a positive integer',
            );
        ...
        if (config.pfsClient) {
            this.pfsClient = {};
            assert.strictEqual(typeof config.pfsClient.host, 'string', 'bad config: pfsClient host must be a string');
            this.pfsClient.host = config.pfsClient.host;

            assert(
                Number.isInteger(config.pfsClient.port) && config.pfsClient.port > 0,
                'bad config: pfsClient port must be a positive integer',
            );
        ...
        if (config.dataDaemon) {
            this.dataDaemon = {};
            assert.strictEqual(
                typeof config.dataDaemon.bindAddress,
                'string',
            ...
            assert(
                Number.isInteger(config.dataDaemon.port) && config.dataDaemon.port > 0,
                'bad config: data daemon port must be a positive integer',
            );
             ...
             */
            this.dataDaemon.dataPath = process.env.S3DATAPATH ? process.env.S3DATAPATH : `${__dirname}/../localData`;
        ...
        if (config.pfsDaemon) {
            this.pfsDaemon = {};
            assert.strictEqual(
                typeof config.pfsDaemon.bindAddress,
                'string',
                'bad config: data daemon bind address must be a string',
            );
            this.pfsDaemon.bindAddress = config.pfsDaemon.bindAddress;
            assert(
                Number.isInteger(config.pfsDaemon.port) && config.pfsDaemon.port > 0,
                'bad config: data daemon port must be a positive integer',
            );
            this.pfsDaemon.port = config.pfsDaemon.port;
            this.pfsDaemon.dataPath = process.env.PFSD_MOUNT_PATH
            ...
            this.pfsDaemon.isReadOnly = process.env.PFSD_READONLY === 'true';
        }

        if (config.metadataDaemon) {
            this.metadataDaemon = {};
            assert.strictEqual(
                typeof config.metadataDaemon.bindAddress,
                'string',
                'bad config: metadata daemon bind address must be a string',
            );
            this.metadataDaemon.bindAddress = config.metadataDaemon.bindAddress;

            assert(
                Number.isInteger(config.metadataDaemon.port) && config.metadataDaemon.port > 0,
                'bad config: metadata daemon port must be a positive integer',
            );
             ...
             */
            this.metadataDaemon.metadataPath = process.env.S3METADATAPATH
                ? process.env.S3METADATAPATH
                : `${__dirname}/../localMetadata`;

            this.metadataDaemon.restEnabled = config.metadataDaemon.restEnabled;
        ...
        if (config.localCache) {
            assert(
                typeof config.localCache === 'object',
                'config: invalid local cache configuration. localCache must be an object',
            );
            if (config.localCache.sentinels) {
                this.localCache = { sentinels: [], name: null };

                assert(
                    typeof config.localCache.name === 'string',
                    'bad config: localCache sentinel name must be a string',
                );
                this.localCache.name = config.localCache.name;

                assert(
                    Array.isArray(config.localCache.sentinels) || typeof config.localCache.sentinels === 'string',
                    'bad config: localCache sentinels must be an array or string',
                );

                if (typeof config.localCache.sentinels === 'string') {
                    config.localCache.sentinels.split(',').forEach(item => {
                        const [host, port] = item.split(':');
                        this.localCache.sentinels.push({ host, port: Number.parseInt(port, 10) });
                    });
                } else if (Array.isArray(config.localCache.sentinels)) {
                    config.localCache.sentinels.forEach(item => {
                        const { host, port } = item;
                        assert(typeof host === 'string', 'bad config: localCache sentinel host must be a string');
                        assert(typeof port === 'number', 'bad config: localCache sentinel port must be a number');
                        this.localCache.sentinels.push({ host, port });
                    });
                }
            } else {
                assert(
                    typeof config.localCache.host === 'string',
                    'config: bad host for localCache. host must be a string',
                );
                assert(
                    typeof config.localCache.port === 'number',
                    'config: bad port for localCache. port must be a number',
                );
                if (config.localCache.password !== undefined) {
                    assert(
                        typeof config.localCache.password === 'string',
                        'config: vad password for localCache. password must be a string',
                    );
                }
                this.localCache = {
        ...
        if (config.scuba) {
            ...
            if (config.scuba.host) {
                assert(typeof config.scuba.host === 'string', 'bad config: scuba host must be a string');
                this.scuba.host = config.scuba.host;
            }
            if (config.scuba.port) {
                assert(
                    Number.isInteger(config.scuba.port) && config.scuba.port > 0,
        ...
        if (process.env.SCUBA_HOST && process.env.SCUBA_PORT) {
            assert(typeof process.env.SCUBA_HOST === 'string', 'bad config: scuba host must be a string');
            assert(
                Number.isInteger(Number(process.env.SCUBA_PORT)) && Number(process.env.SCUBA_PORT) > 0,
                'bad config: scuba port must be a positive integer',
            );
            this.scuba = {
        ...
        if (config.utapi) {
            ...
            if (config.utapi.host) {
                assert(typeof config.utapi.host === 'string', 'bad config: utapi host must be a string');
                this.utapi.host = config.utapi.host;
            }
            if (config.utapi.port) {
                assert(
                    Number.isInteger(config.utapi.port) && config.utapi.port > 0,
            ...
            if (utapiVersion === 1) {
                if (config.utapi.workers !== undefined) {
                    assert(
                        Number.isInteger(config.utapi.workers) && config.utapi.workers > 0,
                ...
                // config duplication.
                assert(config.localCache, 'missing required property of utapi configuration: localCache');
                this.utapi.localCache = this.localCache;
                assert(config.utapi.redis, 'missing required property of utapi configuration: redis');
                ...
                if (config.utapi.enabledOperationCounters !== undefined) {
                    ...
                    assert(
                        Array.isArray(enabledOperationCounters),
                        'bad config: utapi.enabledOperationCounters must be an array',
                    );
                    assert(
                        enabledOperationCounters.length > 0,
                        'bad config: utapi.enabledOperationCounters cannot be empty',
                    );
                ...
                if (config.utapi.disableOperationCounters !== undefined) {
                    ...
                    assert(
                        typeof disableOperationCounters === 'boolean',
                        'bad config: utapi.disableOperationCounters must be a boolean',
                    );
                    this.utapi.disableOperationCounters = disableOperationCounters;
                }
                if (
                    config.utapi.disableOperationCounters !== undefined &&
                    config.utapi.enabledOperationCounters !== undefined
                ) {
                    assert(
                        config.utapi.disableOperationCounters === false,
                        'bad config: conflicting rules: utapi.disableOperationCounters ' +
                            'and utapi.enabledOperationCounters cannot both be specified',
                    );
                }
                ...
                if (config.utapi.replaySchedule) {
                    assert(
                        typeof config.utapi.replaySchedule === 'string',
                        'bad config: utapi.replaySchedule must be a string',
                    );
                ...
                if (config.utapi.batchSize) {
                    assert(typeof config.utapi.batchSize === 'number', 'bad config: utapi.batchSize must be a number');
                    assert(config.utapi.batchSize > 0, 'bad config: utapi.batchSize must be a number greater than 0');
                ...
                if (config.utapi.expireMetrics !== undefined) {
                    assert(
                        typeof config.utapi.expireMetrics === 'boolean',
                        'bad config: utapi.expireMetrics must be a boolean',
                    );
                ...
                if (config.utapi.expireMetricsTTL !== undefined) {
                    assert(
                        typeof config.utapi.expireMetricsTTL === 'number',
            ...
            if (utapiVersion === 2 && config.utapi.filter) {
                ...
                allowedUtapiEventFilterFields.forEach(field =>
                    allowedUtapiEventFilterStates.forEach(state => {
                        const resources = (filterConfig[state] && filterConfig[state][field]) || null;
                        if (resources) {
                            assert.strictEqual(
                                utapiResourceFilters[field],
                                undefined,
                                `bad config: utapi.filter.${state}.${field} can't define an allow and a deny list`,
                            );
                            assert(
                                resources.every(r => typeof r === 'string'),
        ...
        if (Object.keys(this.locationConstraints).some(loc => this.locationConstraints[loc].sizeLimitGB)) {
            assert(
                this.utapi && this.utapi.metrics && this.utapi.metrics.includes('location'),
        ...
        if (config.log !== undefined) {
            if (config.log.logLevel !== undefined) {
                assert(typeof config.log.logLevel === 'string', 'bad config: log.logLevel must be a string');
                this.log.logLevel = config.log.logLevel;
            }
            if (config.log.dumpLevel !== undefined) {
                assert(typeof config.log.dumpLevel === 'string', 'bad config: log.dumpLevel must be a string');
        ...
        if (config.kms) {
            ...
            assert(
                isValidProvider(config.kms.providerName),
        ...
        assert(typeof this.globalEncryptionEnabled === 'boolean', 'config.globalEncryptionEnabled must be a boolean');
        ...
        assert(
            typeof this.defaultEncryptionKeyPerAccount === 'boolean',
        ...
        if (config.healthChecks && config.healthChecks.allowFrom) {
            assert(
                config.healthChecks.allowFrom instanceof Array,
                'config: invalid healthcheck configuration. allowFrom must be an array',
            );
            config.healthChecks.allowFrom.forEach(item => {
                assert(
                    typeof item === 'string',
                    'config: invalid healthcheck configuration. allowFrom IP address must be a string',
                );
            });
            this.healthChecks.allowFrom = defaultHealthChecks.allowFrom.concat(config.healthChecks.allowFrom);
        }
        /**
        ...
        if (config.certFilePaths) {
            assert(
                typeof config.certFilePaths === 'object' &&
                    typeof config.certFilePaths.key === 'string' &&
                    typeof config.certFilePaths.cert === 'string' &&
                    ((config.certFilePaths.ca && typeof config.certFilePaths.ca === 'string') ||
        ...
        if (proxyUrl) {
            assert(typeof proxyUrl === 'string', 'bad proxy config: url must be a string');
            const { protocol, hostname, port, auth } = url.parse(proxyUrl);
            assert(protocol === 'http:' || protocol === 'https:', 'bad proxy config: protocol must be http or https');
            assert(
            ...
            if (port) {
                ...
                assert(!Number.isNaN(portInt) && portInt > 0, 'bad proxy config: port must be a number greater than 0');
            }
            if (auth) {
                assert(typeof auth === 'string', 'bad proxy config: auth must be string');
                const authArray = auth.split(':');
                assert(
                    authArray.length === 2 && authArray[0].length > 0 && authArray[1].length > 0,
                    'bad proxy config: auth must be of format username:password',
                );
            }
            ...
            if (p) {
                assert(typeof p === 'object', 'bad config: "proxy" should be an object');
            }
            if (key) {
                assert(typeof key === 'string', 'bad config: proxy.key should be a string');
            }
            if (cert) {
                assert(typeof cert === 'string', 'bad config: proxy.cert should be a string');
            }
            if (caBundle) {
                assert(typeof caBundle === 'string', 'bad config: proxy.caBundle should be a string');
            }
        ...
        if (config.managementAgent !== undefined) {
            if (config.managementAgent.port !== undefined) {
                assert(
                    Number.isInteger(config.managementAgent.port) && config.managementAgent.port > 0,
                    'bad config: managementAgent port must be a positive integer',
                );
                this.managementAgent.port = config.managementAgent.port;
            }
            if (config.managementAgent.host !== undefined) {
                assert.strictEqual(
                    typeof config.managementAgent.host,
                    'string',
                    'bad config: management agent host must be a string',
                );
        ...
        this.reportToken = process.env.REPORT_TOKEN || config.reportToken || uuidv4();
        ...
        if (config.externalBackends) {
            ...
            extBackendsConfig.forEach(b => {
                ...
                assert(
                    validExternalBackends[b] !== undefined,
                    `bad config: ${b} is not one of valid external backends: ` +
                        `${Object.keys(validExternalBackends).join(', ')}`,
                );

                const { httpAgent } = config.externalBackends[b];
                assert(typeof httpAgent === 'object', `bad config: ${b} must have httpAgent object defined`);
                const { keepAlive, keepAliveMsecs, maxFreeSockets, maxSockets } = httpAgent;
                assert(typeof keepAlive === 'boolean', `bad config: ${b}.httpAgent.keepAlive must be a boolean`);
                assert(
                    typeof keepAliveMsecs === 'number' && httpAgent.keepAliveMsecs > 0,
                    `bad config: ${b}.httpAgent.keepAliveMsecs must be` + ' a number > 0',
                );
                assert(
                    typeof maxFreeSockets === 'number' && httpAgent.maxFreeSockets >= 0,
                    `bad config: ${b}.httpAgent.maxFreeSockets must be ` + 'a number >= 0',
                );
                assert(
                    (typeof maxSockets === 'number' && maxSockets >= 0) || maxSockets === null,
                    `bad config: ${b}.httpAgent.maxFreeSockets must be ` + 'null or a number >= 0',
                );
        ...
        if (config.maxScannedLifecycleListingEntries !== undefined) {
            ...
            assert(
                Number.isInteger(config.maxScannedLifecycleListingEntries) &&
        ...
        // Restrict events to supported product features to be explicit about what can be notified to end user
        NotificationConfiguration.restrictSupportedNotificationBasedOnLifecycle(this.supportedLifecycleRules);

        this.apiBodySizeLimits = { ...constants.defaultApiBodySizeLimits };
        if (config.apiBodySizeLimits) {
            assert(
                typeof config.apiBodySizeLimits === 'object' && !Array.isArray(config.apiBodySizeLimits),
            ...
            for (const [apiKey, limit] of Object.entries(config.apiBodySizeLimits)) {
                ...
                assert(
                    Object.hasOwn(constants.defaultApiBodySizeLimits, apiKey),
         ...
         */
        this.bypassMaxPutObjectSize =
            process.env.BYPASS_MAX_PUT_OBJECT_SIZE === 'true' || config.bypassMaxPutObjectSize || false;
        ...
        if (overrideObjectKeyByteLimit !== null && overrideObjectKeyByteLimit !== undefined) {
            ...
            assert(
                Number.isInteger(this.objectKeyByteLimit) && this.objectKeyByteLimit >= 0,
        ...
        // Allow object write APIs to name a cold location in x-amz-storage-class: data stays
        // in hot storage, and the transition is triggered async from the oplog event
        this.enableDirectToCold =
            process.env.ENABLE_DIRECT_TO_COLD !== undefined
                ? process.env.ENABLE_DIRECT_TO_COLD === 'true'
                : (config.enableDirectToCold ?? false);
        assert(typeof this.enableDirectToCold === 'boolean', 'bad config: enableDirectToCold must be a boolean');
    ...
    _setTimeOptions() {
        ...
        assert(
            !isIncompatible,
            'The environment variables "EXPIRE_ONE_DAY_EARLIER" or ' +
    ...
    _configureBackends() {
        ...
        if (process.env.S3BACKEND) {
            ...
            assert(
                validBackends.indexOf(process.env.S3BACKEND) > -1,
                'bad environment variable: S3BACKEND environment variable should be one of mem/file/scality/cdmi',
            );
        ...
        if (auth === 'file' || auth === 'mem' || auth === 'cdmi') {
            ...
            auth = 'mem';
            let authData;
            if (process.env.SCALITY_ACCESS_KEY_ID && process.env.SCALITY_SECRET_ACCESS_KEY) {
        ...
        if (process.env.S3DATA) {
            ...
            assert(
                validData.indexOf(process.env.S3DATA) > -1,
                'bad environment variable: S3DATA environment variable should be one of mem/file/scality/multiple',
            );
        ...
        assert(
            this.locationConstraints !== undefined && this.restEndpoints !== undefined,
        ...
        // bucket format.
        this.nullVersionCompatMode = metadata === 'mongodb' || process.env.ENABLE_NULL_VERSION_COMPAT_MODE === 'true';
    ...
    _sseMigration(config) {
        if (config.sseMigration) {
            ...
            if (!previousKeyType) {
                assert.fail('NotImplemented: No dynamic KMS key migration. Set sseMigration.previousKeyType');
            }
            ...
            assert(isValidType(previousKeyType), 'ssenMigration.previousKeyType must be "internal" or "external"');
            ...
            if (previousKeyType === KmsType.internal) {
                ...
                assert(previousKeyProtocol, 'sseMigration.previousKeyProtocol must be defined for external provider');
                this.sseMigration.previousKeyProtocol = previousKeyProtocol;
                assert(previousKeyProvider, 'sseMigration.previousKeyProvider must be defined for external provider');
                this.sseMigration.previousKeyProvider = previousKeyProvider;
                expectedProtocol = [KmsProtocol.kmip, KmsProtocol.aws_kms];
            }

            assert(
                isValidProtocol(previousKeyType, this.sseMigration.previousKeyProtocol),
                `sseMigration.previousKeyProtocol must be one of ${expectedProtocol}`,
            );
            assert(
                isValidProvider(previousKeyProvider),
    ...
    getGcpBucketNames(locationConstraint) {
        const { bucketName, mpuBucketName } = this.locationConstraints[locationConstraint].details;
    ...
    setReplicationEndpoints(locationConstraints) {
        this.replicationEndpoints = Object.keys(locationConstraints).map(key => ({
            site: key,
    ...
    getAzureStorageCredentials(locationConstraint) {
        return azureGetLocationCredentials(locationConstraint, this.locationConstraints[locationConstraint].details);
    }

    getPfsDaemonEndpoint(locationConstraint) {
        return (
            process.env[`${locationConstraint}_PFSD_ENDPOINT`] ||
    ...
    isSameAzureAccount(locationConstraintSrc, locationConstraintDest) {
        ...
        const azureSrcAccount = this.getAzureStorageAccountName(locationConstraintSrc);
        const azureDestAccount = this.getAzureStorageAccountName(locationConstraintDest);
        return azureSrcAccount === azureDestAccount;
    }

    isAWSServerSideEncryption(locationConstraint) {
        return this.locationConstraints[locationConstraint].details.serverSideEncryption === true;
    }
    ...
    setPublicInstanceId(instanceId) {
        this.publicInstanceId = crypto.createHash('sha256').update(instanceId).digest('hex');
    }

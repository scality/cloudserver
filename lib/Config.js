...
class Config extends EventEmitter {
    ...
    _getConfig() {
        ...
        if (config.fsDataDaemon) {
            this.fsDataDaemon = {};
            assert.strictEqual(
                typeof config.fsDataDaemon.bindAddress, 'string',
                'bad config: data daemon bind address must be a string');
            this.fsDataDaemon.bindAddress = config.fsDataDaemon.bindAddress;
            assert(Number.isInteger(config.fsDataDaemon.port)
                   && config.fsDataDaemon.port > 0,
                   'bad config: data daemon port must be a positive ' +
                   'integer');
            this.fsDataDaemon.port = config.fsDataDaemon.port;
            this.fsDataDaemon.dataPath =
                process.env.FSDATA_PATH ?
                process.env.FSDATA_PATH : `${__dirname}/../localFsData`;
        }

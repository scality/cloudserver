...
class Config {
    ...
    _getConfig() {
        ...
        if (config.kinetic && process.env.S3SPROXYD === 'kinetic') {
            assert(Array.isArray(config.kinetic),
                   'The kinetic config should be an array - ["host:port"]');
            this.kinetic = {};
            this.kinetic.drives = [];
            console.log('enter in config kinetic')
            config.kinetic.forEach(drive => {
                for (var i = 0; i < 30; i++) {
                    console.log('enter in config kinetic Loop : ', i);
                    console.log(this.kinetic.drives.push(drive));
                    // Initialize the objects
                    this.kinetic.drives.push(drive);
                }
            });
            //  = config.kinetic.map(drive => {
            //     const [host, port] = drive.split(':');
            //     return { host, port };
            // });

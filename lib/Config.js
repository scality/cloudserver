...
class Config extends EventEmitter {
    ...
    _getConfig() {
        ...
        this.apiLog = {
            ...
            logLevel: 'info',
            dumpLevel: 'error',
            outputFile: './logs/api-operations.log',
            enabledBuckets: [] // List of buckets for which logging is enabled
        };
        if (config.apiLog !== undefined) {
            if (config.apiLog.enabled !== undefined) {
                assert(typeof config.apiLog.enabled === 'boolean',
                       'bad config: apiLog.enabled must be a boolean');
                this.apiLog.enabled = config.apiLog.enabled;
            }
            if (config.apiLog.logLevel !== undefined) {
                assert(typeof config.apiLog.logLevel === 'string',
                       'bad config: apiLog.logLevel must be a string');
                this.apiLog.logLevel = config.apiLog.logLevel;
            }
            if (config.apiLog.dumpLevel !== undefined) {
                assert(typeof config.apiLog.dumpLevel === 'string',
                       'bad config: apiLog.dumpLevel must be a string');
                this.apiLog.dumpLevel = config.apiLog.dumpLevel;
            }
            if (config.apiLog.outputFile !== undefined) {
                assert(typeof config.apiLog.outputFile === 'string',
                       'bad config: apiLog.outputFile must be a string');
                this.apiLog.outputFile = config.apiLog.outputFile;
            }
            if (config.apiLog.enabledBuckets !== undefined) {
                assert(Array.isArray(config.apiLog.enabledBuckets) &&
                       config.apiLog.enabledBuckets.every(bucket => typeof bucket === 'string'),
                       'bad config: apiLog.enabledBuckets must be an array of strings');
                this.apiLog.enabledBuckets = config.apiLog.enabledBuckets;
            }
        }

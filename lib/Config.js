...
export default class Config {
    ...
    _getConfig() {
        ...
        this.vault = {};
        if (config.vault) {
            if (config.vault.port !== undefined) {
                assert(Number.isInteger(config.vault.port)
                       && config.vault.port > 0,
                       'bad config: vault port must be a positive integer');
                this.vault.port = config.vault.port;
            }
            if (config.vault.host !== undefined) {
                assert.strictEqual(typeof config.vault.host, 'string',
                                   'bad config: vault host must be a string');
                this.vault.host = config.vault.host;
            }
        }

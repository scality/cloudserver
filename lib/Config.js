...
class Config extends EventEmitter {
    ...
    _getConfig() {
        ...
        // We use the same env variables as the AWS CLI does.
        ...
        this.kms_aws = {
            ...
            endpoint: process.env.AWS_ENDPOINT_URL_KMS || process.env.AWS_ENDPOINT_URL,
            ak: process.env.AWS_ACCESS_KEY_ID,
            sk: process.env.AWS_SECRET_ACCESS_KEY
        };
        if (config.kms_aws) {
            const {region, endpoint, ak, sk} = config.kms_aws;
            if (region) {
                this.kms_aws.region = region;
            }
            if (endpoint) {
                this.kms_aws.endpoint = endpoint;
            }

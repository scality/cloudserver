...
function findConfigFile(fileName) {
    ...
    if (!containingPath) {
        throw new Error(
            `Unable to find the configuration file "${fileName}" under the paths: ${JSON.stringify(configSearchPaths)}`,
        );
    }
    ...
    Object.keys(locationConstraints).forEach(l => {
        ...
        assert(
            ...
            'bad config: location constraint objectId ' +
                `"${locationConstraints[l].objectId}" is not unique across configured locations`,
        );
        assert(

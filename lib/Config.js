...
function azureLocationConstraintAssert(location, locationObj) {
    ...
    Object.keys(locationParams).forEach(param => {
        ...
        assert.notEqual(
            value,
            undefined,
            `bad location constraint: "${location}" ${param} ` + 'must be set in locationConfig or environment variable'
        );
        assert.strictEqual(
            typeof value,
            'string',
...
function locationConstraintAssert(locationConstraints) {
    const supportedBackends = ['mem', 'file', 'scality', 'mongodb', 'tlp'].concat(Object.keys(validExternalBackends));

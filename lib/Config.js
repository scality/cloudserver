...
function requestsConfigAssert(requestsConfig) {
    if (requestsConfig.viaProxy !== undefined) {
        ...
        if (requestsConfig.viaProxy) {
            ...
            assert(typeof requestsConfig.extractProtocolFromHeader === 'string'
        ...
        // eslint-disable-next-line no-param-reassign
        requestsConfig.extractClientIPFromHeader =
            requestsConfig.extractClientIPFromHeader?.toLowerCase();
        // eslint-disable-next-line no-param-reassign
        requestsConfig.extractProtocolFromHeader =
            requestsConfig.extractProtocolFromHeader?.toLowerCase();
    }
}
...
class Config extends EventEmitter {
            ...
            if (config.kmip.transport) {
                if (Array.isArray(config.kmip.transport)) {
                    ...
                    if (config.kmip.retries) {
                        assert(typeof config.kmip.retries === 'number',
                            'bad config: KMIP Cluster retries must be a number');
                        assert(config.kmip.retries <= this.kmip.transport.length - 1,
    ...
    _getConfig() {
        ...
        if (config.rateLimiting?.enabled) {
            ...
            console.log('this.rateLimiting.serviceUserArn', this.rateLimiting.serviceUserArn);

            // Node count for R/N distribution (required for multi-node deployments)
            if (config.rateLimiting.nodeCount !== undefined) {
                assert(
                    typeof config.rateLimiting.nodeCount === 'number' &&
                    Number.isInteger(config.rateLimiting.nodeCount) &&
                    config.rateLimiting.nodeCount > 0,
                    'rateLimiting.nodeCount must be a positive integer'
                );
                this.rateLimiting.nodeCount = config.rateLimiting.nodeCount;
            }

            // Initialize default object if needed
            if (!this.rateLimiting.default) {
                this.rateLimiting.default = {};
            }

            // Default rate limit (fallback maxRps for buckets without explicit config)
            if (config.rateLimiting.default?.maxRps !== undefined) {
                assert(
                    typeof config.rateLimiting.default.maxRps === 'number' &&
                    Number.isInteger(config.rateLimiting.default.maxRps) &&
                    config.rateLimiting.default.maxRps >= 0,
                    'rateLimiting.default.maxRps must be a non-negative integer'
                );
                this.rateLimiting.default.maxRps = config.rateLimiting.default.maxRps;
            }
            ...
            if (config.rateLimiting.bucket) {
                ...
                let burstCapacity = 1; // Default 1 second
                if (config.rateLimiting.bucket.burstCapacity !== undefined) {
                    assert(
                        typeof config.rateLimiting.bucket.burstCapacity === 'number' &&
                        Number.isInteger(config.rateLimiting.bucket.burstCapacity) &&
                        config.rateLimiting.bucket.burstCapacity >= 0,
                        'rateLimiting.bucket.burstCapacity must be a non-negative integer (seconds)'
                    );
                    burstCapacity = config.rateLimiting.bucket.burstCapacity;
                }
            ...
            if (config.rateLimiting.error) {
                ...
                this.rateLimiting.error = new ArsenalError('SlowDown',
                    config.rateLimiting.error.code, config.rateLimiting.error.description);
            } else {

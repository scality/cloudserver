...
// HTTP server options (Node.js http/https.createServer options)
const defaultHttpServerOptions = { requireHostHeader: true };
    ...
    if (config && config.serverAccessLogs) {
        ...
        settings.forEach(setting => {
            if (setting.key in config.serverAccessLogs) {
                assert(typeof config.serverAccessLogs[setting.key] === setting.type,
...
class Config extends EventEmitter {
    ...
    _getConfig() {
        ...
        // HTTP server options (Node.js http/https.createServer options)
        this.httpServerOptions = { ...defaultHttpServerOptions };
        if (config.httpServerOptions) {
            if (config.httpServerOptions.requireHostHeader !== undefined) {
                assert(typeof config.httpServerOptions.requireHostHeader === 'boolean',
                    'config: httpServerOptions.requireHostHeader must be a boolean');
                this.httpServerOptions.requireHostHeader = config.httpServerOptions.requireHostHeader;
            }
        }

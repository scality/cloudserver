...
class Config {
    ...
    _getConfig(_cfg) {
        ...
        // Add user-supplied predicates
        if (config.predicates !== undefined) {
            assert(Array.isArray(config.predicates));
            config.predicates.forEach(v => {
                assert.doesNotThrow(() =>
                registry.put(v.eventInfo, v.path),
                `Error loading user-supplied predicate: ${v.path}`);
            });
        }

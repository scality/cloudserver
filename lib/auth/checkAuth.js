...
var getBucketNameFromHost = require("../utils.js").getBucketNameFromHost;
...
Auth._v2HeaderAuthCheck = function (request, callback) {
    ...
    if (!timestamp || isNaN(timestamp)) {
        // No specific S3 error code for this.  Use S3's Missing Security Header or more specific message?
...
Auth._v2QueryAuthCheck = function (request, callback) {
    ...
    if (request.method === "POST") {
        // No specific S3 error code for this.  Use this or S3's MissingSecurityHeader?
        return callback("Query string authentication is not supported for POST");
    }

    // Check whether request has expired or if expires parameter is more than 15 minutes in the future.
    // AWS docs are not clear on whether having expires header too far in the future would cause request to be rejected.
    // Expires time is provided in seconds so need to multiply by 1000 to obtain
    // milliseconds to compare to Date.now()
    var expirationTime = parseInt(request.query["Expires"]);
    if (isNaN(expirationTime)) {
        // No specific S3 error code for this.  Use this or S3's InvalidArgumet?
        return callback("Missing or invalid Expires query parameter");
    }
    var expirationTime = parseInt(request.query["Expires"]) * 1000;
    var currentTime = Date.now();
    var fifteenMinutes = (15 * 60 * 1000);
    if (currentTime > expirationTime || currentTime + fifteenMinutes < expirationTime) {
        return callback('RequestTimeTooSkewed');
    }

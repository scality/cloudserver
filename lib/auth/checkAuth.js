...
Auth._getCanonicalizedResource = function(request){
	...
	var host = request.lowerCaseHeaders.host;
	var bucket = '';
  if(host && host.indexOf('s3') !== -1) {
   	var hostArray = host.split(".");
		var tmp = hostArray.shift();
		var bucketArray = [];
		while(tmp !== undefined && !tmp.match(/s3.*/)) {
			bucketArray.push(tmp);
    	tmp = hostArray.shift()
		}
		bucket = bucketArray.join('.');
  }
...
Auth._checkSignatureMatch = function(secretKey, signature, request, callback){
...
Auth._getSecretKey = function(accessKey, signature, request, callback){
  ...
  process.nextTick(function() {
		...
		if(!secretKey){
			...
			Auth._checkSignatureMatch(secretKey, signature, request, callback);
		}
...
Auth.checkAuth = function(request, response, callback){

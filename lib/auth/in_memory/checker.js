...
function checkAccount(data, accountObj) {
    ...
    if (checkExists(data, 'Account', 'arn', 'string', accountObj)) {
        // eslint-disable-next-line no-param-reassign
        data.emails[accountObj.arn] = incr(data.emails[accountObj.arn]);
    }
    if (checkExists(data, 'Account', 'canonicalID', 'string', accountObj)) {
        // eslint-disable-next-line no-param-reassign
        data.emails[accountObj.canonicalID] =
            incr(data.emails[accountObj.canonicalID]);
    }

    if (accountObj.users) {
        if (checkType(data, 'Account', 'users', 'object', accountObj)) {
            accountObj.users.forEach(userObj => checkUser(data, userObj));
        }
    }

    if (accountObj.keys) {
        if (checkType(data, 'Account', 'keys', 'object', accountObj)) {
...
function dumpErrors(checkData) {
    ...
    dumpCountError('CanonicalID', checkData.canonicalIds);
    dumpCountError('Email', checkData.emails);
    dumpCountError('ARN', checkData.arns);
    dumpCountError('AccessKey', checkData.keys);

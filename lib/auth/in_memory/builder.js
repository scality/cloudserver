const serviceAccountPrefix = require('arsenal').constants.zenkoServiceAccount;
 ...
 */
function buildAuthDataAccount(accessKey, secretKey, canonicalId, serviceName, userName) {
    // TODO: remove specific check for clueso and generate unique
    // canonical id's for accounts
    const finalCanonicalId =
        canonicalId ||
        (serviceName
            ? `${serviceAccountPrefix}/${serviceName}`

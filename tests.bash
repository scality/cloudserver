...
then
  ...
  S3BACKEND=mem MPU_TESTING=yes S3DATA=multiple npm start > $CIRCLE_ARTIFACTS/server_multiple_kms_awssdk.txt &
  bash wait_for_local_port.bash 8000 40
  ...
  S3BACKEND=mem npm start > $CIRCLE_ARTIFACTS/server_mem_java.txt &
  bash wait_for_local_port.bash 8000 40
  pushd ./tests/functional/jaws
  mvn test
  ...
  S3BACKEND=mem npm start > $CIRCLE_ARTIFACTS/server_mem_fog.txt &
  bash wait_for_local_port.bash 8000 40
...
then
  ...
  S3BACKEND=mem MPU_TESTING=yes npm start > $CIRCLE_ARTIFACTS/server_mem_awssdk.txt &
  bash wait_for_local_port.bash 8000 40
  S3DATA=file npm run ft_awssdk

  killandsleep 8000

  S3BACKEND=mem npm start > $CIRCLE_ARTIFACTS/server_mem_s3cmd.txt &
  bash wait_for_local_port.bash 8000 40
  S3DATA=file npm run ft_s3cmd

  killandsleep 8000

  S3BACKEND=mem npm start > $CIRCLE_ARTIFACTS/server_mem_s3curl.txt &
  bash wait_for_local_port.bash 8000 40
  S3DATA=file npm run ft_s3curl

  killandsleep 8000

  S3BACKEND=mem npm start > $CIRCLE_ARTIFACTS/server_mem_rawnode.txt &
  bash wait_for_local_port.bash 8000 40
  ...
  S3BACKEND=mem MPU_TESTING=yes npm start > $CIRCLE_ARTIFACTS/server_mem_kms_awssdk.txt &
  bash wait_for_local_port.bash 8000 40
  ENABLE_KMS_ENCRYPTION=true S3DATA=file npm run ft_awssdk

  killandsleep 8000

  S3BACKEND=mem npm start > $CIRCLE_ARTIFACTS/server_mem_kms_s3cmd.txt &
  bash wait_for_local_port.bash 8000 40
  ENABLE_KMS_ENCRYPTION=true S3DATA=file npm run ft_s3cmd

  killandsleep 8000

  S3BACKEND=mem npm start > $CIRCLE_ARTIFACTS/server_mem_kms_s3curl.txt &
  bash wait_for_local_port.bash 8000 40
  ENABLE_KMS_ENCRYPTION=true S3DATA=file npm run ft_s3curl

  killandsleep 8000

  S3BACKEND=mem npm start > $CIRCLE_ARTIFACTS/server_mem_kms_rawnode.txt &
  bash wait_for_local_port.bash 8000 40
...
then
  ...
  S3BACKEND=file S3VAULT=mem MPU_TESTING=yes npm start > $CIRCLE_ARTIFACTS/server_file_awssdk.txt &
  bash wait_for_local_port.bash 8000 40
  S3DATA=file npm run ft_awssdk

  killandsleep 8000

  S3BACKEND=file S3VAULT=mem npm start > $CIRCLE_ARTIFACTS/server_file_management.txt &
  bash wait_for_local_port.bash 8000 40
  npm run ft_management

  killandsleep 8000

  S3BACKEND=file S3VAULT=mem npm start > $CIRCLE_ARTIFACTS/server_file_s3cmd.txt &
  bash wait_for_local_port.bash 8000 40
  S3DATA=file npm run ft_s3cmd

  killandsleep 8000

  S3BACKEND=file S3VAULT=mem npm start > $CIRCLE_ARTIFACTS/server_file_s3curl.txt &
  bash wait_for_local_port.bash 8000 40
  S3DATA=file npm run ft_s3curl

  killandsleep 8000

  S3BACKEND=file S3VAULT=mem npm start > $CIRCLE_ARTIFACTS/server_file_rawnode.txt &
  bash wait_for_local_port.bash 8000 40
  S3DATA=file npm run ft_node
  ...
  S3BACKEND=file S3VAULT=mem MPU_TESTING=yes npm start > $CIRCLE_ARTIFACTS/server_file_kms_awssdk.txt &
  bash wait_for_local_port.bash 8000 40
  ENABLE_KMS_ENCRYPTION=true S3DATA=file npm run ft_awssdk

  killandsleep 8000

  S3BACKEND=file S3VAULT=mem npm start > $CIRCLE_ARTIFACTS/server_file_kms_s3cmd.txt &
  bash wait_for_local_port.bash 8000 40
  ENABLE_KMS_ENCRYPTION=true S3DATA=file npm run ft_s3cmd

  killandsleep 8000

  S3BACKEND=file S3VAULT=mem npm start > $CIRCLE_ARTIFACTS/server_file_kms_s3curl.txt &
  bash wait_for_local_port.bash 8000 40
  ENABLE_KMS_ENCRYPTION=true S3DATA=file npm run ft_s3curl

  killandsleep 8000

  S3BACKEND=file S3VAULT=mem npm start > $CIRCLE_ARTIFACTS/server_file_kms_rawnode.txt &
  bash wait_for_local_port.bash 8000 40
  ENABLE_KMS_ENCRYPTION=true S3DATA=file npm run ft_node
  ...
  bash wait_for_local_port.bash 8000 40
  ENABLE_LOCAL_CACHE=true S3DATA=file npm run ft_healthchecks

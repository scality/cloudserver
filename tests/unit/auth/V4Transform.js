...
const streamingV4Params = { accessKey: 'accessKey1',
  signatureFromRequest: '2b8637632a997e06ee7b6c85d7147d2025e8f04d4374f4d7d7320de1618c7509',
  region: 'us-east-1',
  scopeDate: '20170516',
...
describe.only('V4Transform class', () => {
    ...
    it.only('should error due to my bug', done => {
        const v4Transform = new V4Transform(streamingV4Params, log, err => {
            assert.strictEqual(err, errors.SignatureDoesNotMatch);
            done();
        });
        const chunks = [new Buffer('8;chunk-signature=' +
        '51d2511f7c6887907dff20474d8db67d557e5f515a6fa6a8466bb12f8833bcca\r\n' +
        ...
        authMe.pipe(v4Transform);
        v4Transform.on('finish', () => {
            const err = new Error('should not have completed');
            done(err);
        });
    });
});

...
describe('TrailingChecksumTransform class', () => {
    ...
    describe('TrailingChecksumTransform trailer parsing and emitting', () => {
        describe('happy path', () => {
            ...
            it('should accept chunk size field with leading and trailing whitespace', async () => {
                // AWS accepts whitespace around the hex digits
                const input = '  5  \r\nhello\r\n0\r\nx-amz-checksum-crc32:AAAAAA==\r\n';
                const { data, trailers } = await runTransform([input]);
                assert.strictEqual(data.toString(), 'hello');
                assert.strictEqual(trailers.length, 1);
        ...
        describe('_flush error cases', () => {
            ...
            it('should return IncompleteBody when stream ends in READ_CHUNK_LEN_LF (CR but no LF)', async () => {
                const err = await expectError(['5\r']);
                assert.deepStrictEqual(err, errors.IncompleteBody);
            });
        ...
        describe('_transform error cases', () => {
            ...
            it('should return InvalidArgument when chunk size CR is not followed by LF', async () => {
                // '5\r\r' — second byte in READ_CHUNK_LEN_LF is CR not LF
                const err = await expectError(['5\r\r']);
                assert.deepStrictEqual(err, errors.InvalidArgument);
            });

            it('should return InvalidArgument when chunk data is not followed by CR', async () => {
                // 1 byte of data 'A', then 'x' instead of CR
                const err = await expectError(['1\r\nAx']);
                assert.deepStrictEqual(err, errors.InvalidArgument);
            });

            it('should return InvalidArgument for negative hex chunk size (-1)', async () => {
                // parseInt('-1', 16) = -1, which would cause an infinite loop without the regex guard
                const err = await expectError(['-1\r\n']);
                assert.deepStrictEqual(err, errors.InvalidArgument);
            });

            it('should return InvalidArgument for negative hex chunk size (-a)', async () => {
                const err = await expectError(['-a\r\n']);
                assert.deepStrictEqual(err, errors.InvalidArgument);
            });

            it('should accept chunk size with leading zeros (05 = 5 bytes)', async () => {
                // "05" round-trips as "5" via toString(16), which the old check wrongly rejected
                const input = '05\r\nhello\r\n0\r\nx-amz-checksum-crc32:AAAAAA==\r\n';
                const { data, trailers } = await runTransform([input]);
                assert.strictEqual(data.toString(), 'hello');
                assert.strictEqual(trailers.length, 1);
            ...
            it('should accept zero-terminator with leading zeros (00)', async () => {
                // "00" round-trips as "0", which the old check wrongly rejected
                const input = '5\r\nhello\r\n00\r\nx-amz-checksum-crc32:AAAAAA==\r\n';
                const { data, trailers } = await runTransform([input]);
                assert.strictEqual(data.toString(), 'hello');
                assert.strictEqual(trailers.length, 1);
            ...
            it('should return InvalidArgument when chunk data CR is not followed by LF', async () => {
                // 1 byte of data 'A', then '\r' then 'x' instead of LF
                const err = await expectError(['1\r\nA\rx']);
                assert.deepStrictEqual(err, errors.InvalidArgument);
            });

            it('should return InvalidArgument when trailing CR is not followed by LF', async () => {
                // valid trailer up to \r, then 'x' instead of LF
                const err = await expectError(['0\r\nx-amz-checksum-crc32:AAAAAA==\rx']);
                assert.deepStrictEqual(err, errors.InvalidArgument);
            });
        });
    });

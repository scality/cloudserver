...
describe('objectGet API', () => {
    ...
    it('should get the object metadata', done => {
        bucketPut(authInfo, testPutBucketRequest, log, () => {
            objectPut(authInfo, testPutObjectRequest, undefined, log, (err, resHeaders) => {
                ...
                objectGet(authInfo, testGetRequest, false, log, (err, result, responseMetaHeaders) => {
                    assert.strictEqual(responseMetaHeaders[userMetadataKey], userMetadataValue);
                    assert.strictEqual(responseMetaHeaders.ETag, `"${correctMD5}"`);
                    done();
                });
            });
        });
    ...
    const createPutDummyRetention = (date, mode) =>
        new DummyRequest(
            {
                bucketName,
                namespace,
        ...
        bucketPut(authInfo, testPutBucketRequestObjectLock, log, () => {
            ...
            objectPut(authInfo, request, undefined, log, (err, headers) => {
                ...
                objectGet(authInfo, req, false, log, (err, r, headers) => {
                    assert.ifError(err);
                    assert.strictEqual(headers['x-amz-object-lock-retain-until-date'], testDate);
                    assert.strictEqual(headers['x-amz-object-lock-mode'], 'GOVERNANCE');
                    assert.strictEqual(headers.ETag, `"${correctMD5}"`);
                    changeObjectLock(
                        [
                            {
    ...
    const createPutDummyLegalHold = legalHold =>
        new DummyRequest(
            {
                bucketName,
                namespace,
            ...
            bucketPut(authInfo, testPutBucketRequestObjectLock, log, () => {
                ...
                objectPut(authInfo, request, undefined, log, (err, resHeaders) => {
                    ...
                    objectGet(authInfo, testGetRequest, false, log, (err, res, headers) => {
                        assert.ifError(err);
                        assert.strictEqual(headers['x-amz-object-lock-legal-hold'], status);
                        assert.strictEqual(headers.ETag, `"${correctMD5}"`);
                        changeObjectLock(
                            [
                                {
    ...
    it('should get the object metadata with both retention and legal hold', done => {
        bucketPut(authInfo, testPutBucketRequestObjectLock, log, () => {
            const request = createPutDummyRetentionAndLegalHold(testDate, 'COMPLIANCE', 'ON');
            objectPut(authInfo, request, undefined, log, (err, resHeaders) => {
                assert.ifError(err);
                assert.strictEqual(resHeaders.ETag, `"${correctMD5}"`);
                const auth = authInfo;
                ...
                objectGet(auth, req, false, log, (err, r, headers) => {
                    assert.ifError(err);
                    assert.strictEqual(headers['x-amz-object-lock-legal-hold'], 'ON');
                    assert.strictEqual(headers['x-amz-object-lock-retain-until-date'], testDate);
                    assert.strictEqual(headers['x-amz-object-lock-mode'], 'COMPLIANCE');
                    assert.strictEqual(headers.ETag, `"${correctMD5}"`);
                    done();
                });
            });
    ...
    it('should get the object data retrieval info', done => {
        bucketPut(authInfo, testPutBucketRequest, log, () => {
            objectPut(authInfo, testPutObjectRequest, undefined, log, (err, resHeaders) => {
                assert.strictEqual(resHeaders.ETag, `"${correctMD5}"`);
                objectGet(authInfo, testGetRequest, false, log, (err, dataGetInfo) => {
    ...
    it('should get the object data retrieval info for an object put by MPU', done => {
                ...
                mdColdHelper.putObjectMock(bucketName, objectName, objectCustomMDFields, () => {
                    objectGet(authInfo, testGetRequest, false, log, (err, res, headers) => {
                        ...
                        assert.strictEqual(
                            ...
                            `ongoing-request="false", expiry-date="${utcDate}"`,
                        );
                        done();
                    });
                });
    ...
    it('should not return checksum headers when mode is ENABLED but object has no checksum', done => {
        mdColdHelper.putBucketMock(bucketName, null, () =>
            mdColdHelper.putObjectMock(bucketName, objectName, undefined, () => {
                ...
                objectGet(authInfo, req, false, log, (err, _locator, headers) => {
                    assert.ifError(err);
                    checksumAlgorithms.forEach(({ header }) => assert.strictEqual(headers[header], undefined));

...
const { isObjAuthorized } = require('../../../lib/api/apiUtils/authorization/permissionChecks');
...
const bucket = new BucketInfo('niftyBucket', bucketOwnerCanonicalId, 'iAmTheOwnerDisplayName', creationDate);
...
describe('object acl authorization for objectGet and objectHead', () => {
    ...
    it('should allow access to object owner', () => {
        const results = requestTypes.map(type =>
            isObjAuthorized(bucket, object, type, objectOwnerCanonicalId, authInfo, log)
        );
    ...
    it('should allow access to user in object owner account', () => {
        const results = requestTypes.map(type =>
            isObjAuthorized(bucket, object, type, objectOwnerCanonicalId, userAuthInfo, log)
        );
        assert.deepStrictEqual(results, [true, true]);
    });
    ...
    it('should allow access to bucket owner if same account as object owner', () => {
        const results = requestTypes.map(type =>
            isObjAuthorized(bucket, object, type, bucketOwnerCanonicalId, authInfo, log)
        );
        assert.deepStrictEqual(results, [true, true]);
    });
    ...
    it('should allow access to bucker owner when object owner is alt account if ' + 'bucket-owner-read ACL', () => {
        const altAcctObj = {
        ...
        const noAuthResults = requestTypes.map(type =>
            isObjAuthorized(bucket, altAcctObj, type, bucketOwnerCanonicalId, authInfo, log)
        );
    ...
    it(
        'should allow access to bucker owner when object owner is alt account if ' + 'bucket-owner-full-control ACL',
        () => {
            const altAcctObj = {
                'owner-id': accountToVet,
                acl: {
    ...
    it('should allow access to account if ' + 'account was granted FULL_CONTROL', () => {
        const noAuthResults = requestTypes.map(type =>
            isObjAuthorized(bucket, object, type, accountToVet, altAcctAuthInfo, log)
        );
        assert.deepStrictEqual(noAuthResults, [false, false]);
    ...
    it('should allow access to account if ' + 'account was granted READ right', () => {
        const noAuthResults = requestTypes.map(type =>
            isObjAuthorized(bucket, object, type, accountToVet, altAcctAuthInfo, log)
        );
        assert.deepStrictEqual(noAuthResults, [false, false]);
...
describe('object authorization for objectPutACL and objectGetACL', () => {
    ...
    it(
        'should allow access to bucket owner when object owner is alt account if ' +
        ...
        () => {
            ...
            const authorizedResults = requestTypes.map(type =>
                isObjAuthorized(bucket, altAcctObj, type, bucketOwnerCanonicalId, authInfo, null, log)
            );
    ...
    it('should allow objectPutACL access to account if ' + 'account was granted WRITE_ACP right', () => {
        const noAuthResult = isObjAuthorized(bucket, object, 'objectPutACL', accountToVet, altAcctAuthInfo, log);
        assert.strictEqual(noAuthResult, false);
        object.acl.WRITE_ACP = [accountToVet];
        const authorizedResult = isObjAuthorized(bucket, object, 'objectPutACL', accountToVet, altAcctAuthInfo, log);
        assert.strictEqual(authorizedResult, true);
    });

    it('should allow objectGetACL access to account if ' + 'account was granted READ_ACP right', () => {
        const noAuthResult = isObjAuthorized(bucket, object, 'objectGetACL', accountToVet, altAcctAuthInfo, log);
        assert.strictEqual(noAuthResult, false);
        object.acl.READ_ACP = [accountToVet];
        const authorizedResult = isObjAuthorized(bucket, object, 'objectGetACL', accountToVet, altAcctAuthInfo, log);
        assert.strictEqual(authorizedResult, true);
    });
});
...
describe('without object metadata', () => {
    ...
    const tests = [
        ...
        {
            it: 'should not allow public if bucket is canned ' + 'authenticated-read',
        ...
        {
            it: 'should allow authenticated users if bucket is canned ' + 'authenticated-read',
            canned: 'authenticated-read',
            id: accountToVet,
            aclParam: null,
            response: allowedAccess,
    ...
    tests.forEach(value => {
        it(value.it, done => {
            ...
            const results = requestTypes.map(type => isObjAuthorized(bucket, null, type, value.id, authInfoUser, log));
    ...
    it('should treat initiateMultipartUpload as objectPut', () => {
        ...
        const results = isObjAuthorized(bucket, null, 'initiateMultipartUpload', accountToVet, altAcctAuthInfo, log);
    ...
    it('should treat objectPutPart as objectPut', () => {
        ...
        const results = isObjAuthorized(bucket, null, 'objectPutPart', accountToVet, altAcctAuthInfo, log);
        assert.strictEqual(results, true);
    });
    ...
    it('should treat completeMultipartUpload as objectPut', () => {
        ...
        const results = isObjAuthorized(bucket, null, 'completeMultipartUpload', accountToVet, altAcctAuthInfo, log);
        assert.strictEqual(results, true);
    });
});

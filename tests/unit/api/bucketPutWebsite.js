...
const { xmlContainsElem } = require('../../../lib/api/apiUtils/bucket/bucketWebsite');
...
describe('putBucketWebsite API', () => {
    ...
    it("should update a bucket's metadata with website config obj", done => {
        ...
        const testBucketPutWebsiteRequest = _getPutWebsiteRequest(config.getXml());
        bucketPutWebsite(authInfo, testBucketPutWebsiteRequest, log, err => {
            ...
            return metadata.getBucket(bucketName, log, (err, bucket) => {
                ...
                assert.strictEqual(bucketWebsiteConfig._indexDocument, config.IndexDocument.Suffix);
                assert.strictEqual(bucketWebsiteConfig._errorDocument, config.ErrorDocument.Key);
                assert.strictEqual(
                    bucketWebsiteConfig._routingRules[0]._condition.keyPrefixEquals,
                    config.RoutingRules[0].Condition.KeyPrefixEquals,
                );
                assert.strictEqual(
                    bucketWebsiteConfig._routingRules[0]._redirect.replaceKeyPrefixWith,
    ...
    describe('helper functions', () => {
        it('xmlContainsElem should return true if xml contains specified element', done => {
            ...
            parseString(xml, (err, result) => {
                ...
                const containsRes = xmlContainsElem(result.Toplevel.Parent, 'Element');
        ...
        it(
            'xmlContainsElem should return true if parent contains list of ' +
                'elements and isList is specified in options',
            done => {
                const xml =
                    '<Toplevel><Parent>' +
                ...
                parseString(xml, (err, result) => {
                    ...
                    const containsRes = xmlContainsElem(result.Toplevel.Parent, 'Element', { isList: true });
                    assert.strictEqual(containsRes, true);
                    return done();
                });
            },
        ...
        it(
            'xmlContainsElem should return true if parent contains at least ' +
                ...
                parseString(xml, (err, result) => {
                    ...
                    const containsRes = xmlContainsElem(result.Toplevel.Parent, ['ElementA', 'ElementB']);
                    assert.strictEqual(containsRes, true);
                    return done();
                });
            },
        ...
        it(
            'xmlContainsElem should return true if parent contains all ' +
                'of multiple elements specified and checkForAll specified in options',
            done => {
                const xml =
                    '<Toplevel><Parent><ElementA>value</ElementA><ElementB>value</ElementB></Parent></Toplevel>';
                parseString(xml, (err, result) => {
                    if (err) {
                        process.stdout.write(`Unexpected err ${err} parsing xml`);
                        return done(err);

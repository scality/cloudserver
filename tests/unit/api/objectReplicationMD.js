...
// Put the object key and check the replication information.
function putObjectAsLifecycleAccountAndCheckMD(key, expected, cb) {
    return objectPut(authInfoLifecycleService, getObjectPutReq(key, true),
        undefined, log, err => {
            if (err) {
                return cb(err);
            }
...
[true, false].forEach(hasStorageClass => {
    ...
    `${hasStorageClass ? 'with' : 'without'} storage class`, () => {
        ...
        'Lifecycle service account', done =>
            async.series([
                next => putObjectAsLifecycleAccountAndCheckMD(
                    keyA, newReplicationMD, next),
                next => objectDelete(authInfo, deleteReq, log, next),
            ], err => {
                if (err) {
                    return done(err);

...
describe('objectGetACL API', () => {
    ...
    it('should get a canned private ACL', done => {
        ...
        async.waterfall(
            [
                ...
                (corsHeaders, next) => objectPut(authInfo, testPutObjectRequest, undefined, log, next),
                (resHeaders, next) => {
    ...
    it('should get a canned bucket-owner-read ACL', done => {
        ...
        async.waterfall(
            [
                next => bucketPut(otherAccountAuthInfo, testBucketPutRequest, log, next),
                (corsHeaders, next) => objectPut(authInfo, testPutObjectRequest, undefined, log, next),
                (resHeaders, next) => {
                    assert.strictEqual(resHeaders.ETag, `"${correctMD5}"`);
                    objectGetACL(authInfo, testGetACLRequest, log, next);
    ...
    it('should get specifically set ACLs', done => {
        ...
        async.waterfall(
            ...
            (err, result) => {
                ...
                assert.strictEqual(result.AccessControlPolicy.AccessControlList[0].Grant[4].Permission[0], 'READ');
                assert.strictEqual(result.AccessControlPolicy.AccessControlList[0].Grant[5], undefined);
                done();
            }
        );
    });

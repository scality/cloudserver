...
describe('bucketPutVersioning API', () => {
    ...
    describe('with version enabled location constraint', () => {
        ...
        const tests = [
            ...
            {
                msg:
                    'should successfully suspend versioning on location ' +
                    'constraint with supportsVersioning set to true',
            ...
            it(test.msg, done => {
                ...
                bucketPutVersioning(authInfo, request, log, err => {
                    ...
                    metadata.getBucket(bucketName, log, (err, bucket) => {
                        assert.ifError(err, `Expected success, but got err: ${err}`);
                        assert.deepStrictEqual(bucket._versioningConfiguration, test.output);
                        done();
                    });
                });
            })
        ...
        it('should not suspend versioning on bucket with replication', done => {
            async.series(
                [
                    ...
                    next => {
                        const request = _putReplicationRequest(xmlReplicationConfiguration);
    ...
    describe('with version disabled location constraint', () => {
        ...
        tests.forEach(test =>
            it(test.msg, done => {
                const putBucketVersioningRequest = _putVersioningRequest(test.input);
                bucketPutVersioning(authInfo, putBucketVersioningRequest, log, err => {

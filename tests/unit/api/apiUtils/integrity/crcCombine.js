...
const { combineCrcPair, combinePartCrcs } = require('../../../../../lib/api/apiUtils/integrity/crcCombine');
    ...
    SPECS.forEach(({ algo, polyReversed, dim }) => {
        ...
        describe(`${label} (dim=${dim})`, () => {
            it('should combine(crc1, crc2, len2) to crc(chunk1 ‖ chunk2) for random data', async () => {
                ...
                const got = combineCrcPair(crc1, crc2, BigInt(b.length), polyReversed, dim);
            ...
            it('should return crc1 unchanged when len2 = 0 (identity)', async () => {
                ...
                const crc1 = await crcOf(algo, a);
                const got = combineCrcPair(crc1, 0n, 0n, polyReversed, dim);
            ...
            it('should equal the original CRC when combined with the CRC of empty', async () => {
                ...
                const got = combineCrcPair(crc1, crcEmpty, 0n, polyReversed, dim);
                assert.strictEqual(got, crc1 & mask);
            });
            ...
            it('should be associative across three chunks', async () => {
                ...
                const ab = combineCrcPair(crcA, crcB, BigInt(b.length), polyReversed, dim);
                const left = combineCrcPair(ab, crcC, BigInt(c.length), polyReversed, dim);

                // Right-fold: combine(A, combine(B, C), len(B)+len(C))
                const bc = combineCrcPair(crcB, crcC, BigInt(c.length), polyReversed, dim);
                const right = combineCrcPair(crcA, bc, BigInt(b.length + c.length), polyReversed, dim);
            ...
            it('should handle single-byte chunks', async () => {
                ...
                const got = combineCrcPair(await crcOf(algo, a), await crcOf(algo, b), 1n, polyReversed, dim);
                const expected = await crcOf(algo, Buffer.concat([a, b]));
                assert.strictEqual(got, expected);
            });
            ...
            it('should handle odd-length chunk2 sizes (not a multiple of 8 bytes)', async () => {
                ...
                for (const size of sizes) {
                    ...
                    const got = combineCrcPair(crcA, await crcOf(algo, b), BigInt(size), polyReversed, dim);
                    const expected = await crcOf(algo, Buffer.concat([a, b]));
    ...
    SPECS.forEach(({ algo, polyReversed, dim }) => {
        ...
        describe(`${label} (dim=${dim})`, () => {
            it('should return the part CRC unchanged for a single-part input', async () => {
                ...
                const got = combinePartCrcs([{ value: partCrc, length: buf.length }], polyReversed, dim);
            ...
            it('should match crc(concat) for two parts — base64 in, base64 out', async () => {
                ...
                const got = combinePartCrcs(parts, polyReversed, dim);

...
describe('objectLockHelpers: validateHeaders', () => {
    it('should fail if object lock is not enabled on the bucket', () => {
        ...
        const objectLockValidationError = validateHeaders(objLockDisabledBucketInfo, headers, log);
    ...
    it('should fail with invalid retention period header', () => {
        ...
        const objectLockValidationError = validateHeaders(bucketInfo, headers, log);
        const expectedError = errorInstances.InvalidArgument.customizeDescription('Unknown wormMode directive');
        assert.strictEqual(objectLockValidationError.is.InvalidArgument, true);
        assert.strictEqual(objectLockValidationError.description, expectedError.description);
    });
});
...
describe('objectLockHelpers: calculateRetainUntilDate', () => {
    it('should calculate retainUntilDate for config with days', () => {
        ...
        const date = moment();
        const expectedRetainUntilDate = date.add(mockConfigWithDays.days * 86400000, 'ms');
        const retainUntilDate = calculateRetainUntilDate(mockConfigWithDays);
        assert.strictEqual(retainUntilDate.slice(0, 16), expectedRetainUntilDate.toISOString().slice(0, 16));
    });
    ...
    it('should calculate retainUntilDate for config with years', () => {
        ...
        const date = moment();
        const expectedRetainUntilDate = date.add(mockConfigWithYears.years * 365 * 86400000, 'ms');
...
describe('objectLockHelpers: ObjectLockInfo', () => {
    ...
    describe('isExpired: ', () =>
        isExpiredTestCases.forEach(testCase => {
    ...
    describe('isLocked: ', () =>
        isLockedTestCases.forEach(testCase => {
    ...
    describe('canModifyPolicy: ', () =>
        policyChangeTestCases.forEach(testCase => {
            describe(testCase.desc, () => {
    ...
    describe('canModifyObject: ', () =>
        canModifyObjectTestCases.forEach(testCase => {
            describe(testCase.desc, () => {
                const objLockInfo = new ObjectLockInfo(testCase.policy);

...
describe('putObjectTagging API', () => {
    ...
    it("should update an object's metadata with tags resource and update originOp", done => {
        ...
        objectPutTagging(authInfo, testObjectPutTaggingRequest, log, err => {
            ...
            return metadata.getObjectMD(bucketName, objectName, {}, log, (err, objectMD) => {
                if (err) {
    ...
    describe('validateTagStructure ', () => {
        it('should return expected true if tag is valid false/undefined if not', done => {
            const tags = [
                ...
                { tagTest: { Key: ['foo', 'boo'], Value: ['bar'] }, isValid: false },
                { tagTest: { Key: ['foo'], Value: ['bar', 'boo'] }, isValid: false },
                { tagTest: { Key: ['foo', 'boo'], Value: ['bar', 'boo'] }, isValid: false },
        ...
        describe('validateXMLStructure ', () => {
            it('should return expected true if tag is valid false/undefined ' + 'if not', done => {
                const tags = [
                    ...
                    { tagging: { Tagging: { Tagset: [{ Tag: [] }], ExtraTagset: 'extratagset' } }, isValid: false },
                    { tagging: { Tagging: { Tagset: [{ Tag: [], ExtraTag: 'extratag' }] } }, isValid: false },
                    { tagging: { Tagging: { Tagset: [{ Tag: {} }] } }, isValid: false },
                ];

...
describe('putObjectRetention API', () => {
    ...
    describe('with Object Lock enabled on bucket', () => {
        ...
        it('should disallow update if the x-amz-bypass-governance-retention header is missing if GOVERNANCE mode is enabled', done => {
            objectPutRetention(authInfo, putObjRetRequestGovernance, log, err => {
                assert.ifError(err);
                return objectPutRetention(authInfo, putObjRetRequestGovernance, log, err => {
                    assert.deepStrictEqual(err, errors.AccessDenied);
        ...
        it('should allow update if the x-amz-bypass-governance-retention header is present if GOVERNANCE mode is enabled', done => {
            objectPutRetention(authInfo, putObjRetRequestGovernance, log, err => {
                assert.ifError(err);

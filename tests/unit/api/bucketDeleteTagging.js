...
describe('deleteBucketTagging API', () => {
    ...
    it('should delete tags resource', done => {
        ...
        bucketPutTagging(authInfo, testBucketPutTaggingRequest, log, err => {
            ...
            return bucketGetTagging(authInfo, testBucketGetTaggingRequest, log, (err, xml) => {
                ...
                const testBucketDeleteTaggingRequest = taggingUtil.createBucketTaggingRequest('DELETE', bucketName);
                return bucketDeleteTagging(authInfo, testBucketDeleteTaggingRequest, log, err => {
                    assert.ifError(err);
                    return bucketGetTagging(authInfo, testBucketGetTaggingRequest, log, err => {
    ...
    it('should return access denied if the authorization check fails', done => {
        ...
        bucketPutTagging(authInfo, testBucketPutTaggingRequest, log, err => {
            assert.ifError(err);
            const testBucketDeleteTaggingRequest = taggingUtil.createBucketTaggingRequest(
                'DELETE',
                bucketName,
                null,
                true,

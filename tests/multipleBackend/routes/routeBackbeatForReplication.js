...
describe('backbeat routes for replication', () => {
        ...
        async.series(
            {
                enableVersioningSource: next =>
                    srcS3.putBucketVersioning(
                        { Bucket: bucketSource, VersioningConfiguration: { Status: 'Enabled' } },
                        next
                    ),
                putObject: next =>
                    srcS3.putObject({ Bucket: bucketSource, Key: keyName, Body: new Buffer(testData) }, (err, data) => {
                        if (err) {
                            return next(err);
                        }
                ...
                enableVersioningDestination: next =>
                    dstS3.putBucketVersioning(
                        { Bucket: bucketDestination, VersioningConfiguration: { Status: 'Enabled' } },
                        next
                    ),
                getMetadata: next =>
                    makeBackbeatRequest(
                ...
                headObject: next =>
                    dstS3.headObject({ Bucket: bucketDestination, Key: keyName, VersionId: versionId }, next),
        ...
        async.series(
            {
                ...
                getObjectTagging: next =>
                    dstS3.getObjectTagging({ Bucket: bucketDestination, Key: keyName, VersionId: versionId }, next),
                listObjectVersions: next => dstS3.listObjectVersions({ Bucket: bucketDestination }, next),
            },
            (err, results) => {
                if (err) {
                ...
                getMetadata: next =>
                    makeBackbeatRequest(
                        ...
                        (err, data) => {
                            ...
                            objMD = objectMDFromRequestBody(data).getSerialized();
                            return next();
                        }
                    ),
                replicateMetadata: next =>
        ...
        async.series(
            {
                ...
                deleteObject: next =>
                    srcS3.deleteObject({ Bucket: bucketSource, Key: keyName }, (err, data) => {
                        if (err) {
                            return next(err);
                        }
    ...
    it('should successfully replicate a null version', done => {
        ...
        async.series(
            {
                putObject: next =>
                    srcS3.putObject({ Bucket: bucketSource, Key: keyName, Body: new Buffer(testData) }, next),
                enableVersioningSource: next =>
                    srcS3.putBucketVersioning(
                        { Bucket: bucketSource, VersioningConfiguration: { Status: 'Enabled' } },
                        next
                ...
                headObject: next =>
                    dstS3.headObject({ Bucket: bucketDestination, Key: keyName, VersionId: 'null' }, next),
                listObjectVersions: next => dstS3.listObjectVersions({ Bucket: bucketDestination }, next),
            },
            (err, results) => {
                if (err) {
        ...
        async.series(
            {
                suspendVersioningSource: next =>
                    srcS3.putBucketVersioning(
                        { Bucket: bucketSource, VersioningConfiguration: { Status: 'Suspended' } },
                        next
                    ),
                putObject: next =>
                    srcS3.putObject({ Bucket: bucketSource, Key: keyName, Body: new Buffer(testData) }, next),
    ...
    it('should successfully replicate a null version and update it', done => {
        ...
        async.series(
            {
                putObject: next =>
                    srcS3.putObject({ Bucket: bucketSource, Key: keyName, Body: Buffer.from(testData) }, next),
                enableVersioningSource: next =>
                    srcS3.putBucketVersioning(
                        { Bucket: bucketSource, VersioningConfiguration: { Status: 'Enabled' } },
                        next
        ...
        async.series(
            {
                ...
                putObjectDestination: next =>
                    dstS3.putObject(
                        { Bucket: bucketDestination, Key: keyName, Body: Buffer.from(testData) },
                        (err, data) => {
                            if (err) {
                                return next(err);
                            }
        ...
        async.series(
            {
                ...
                putObjectSource: next =>
                    srcS3.putObject(
                        { Bucket: bucketSource, Key: keyName, Body: Buffer.from(testData) },
                        (err, data) => {
                            if (err) {
                                return next(err);
                            }
                ...
                headObjectFirstVersion: next =>
                    dstS3.headObject({ Bucket: bucketDestination, Key: keyName, VersionId: firstVersionId }, next),
                headObjectSecondVersion: next =>
                    dstS3.headObject({ Bucket: bucketDestination, Key: keyName, VersionId: secondVersionId }, next),
                listObjectVersions: next => dstS3.listObjectVersions({ Bucket: bucketDestination }, next),
            },
            (err, results) => {
                if (err) {
        ...
        async.series(
            {
                putObjectDestinationInitial: next =>
                    dstS3.putObject({ Bucket: bucketDestination, Key: keyName, Body: Buffer.from(testData) }, next),
                enableVersioningDestination: next =>
                    dstS3.putBucketVersioning(
                        { Bucket: bucketDestination, VersioningConfiguration: { Status: 'Enabled' } },
                        next
        ...
        async.series(
            {
                suspendVersioningDestination: next =>
                    dstS3.putBucketVersioning(
                        { Bucket: bucketDestination, VersioningConfiguration: { Status: 'Suspended' } },
                        next
                    ),
                putObjectDestinationInitial: next =>
                    dstS3.putObject({ Bucket: bucketDestination, Key: keyName, Body: Buffer.from(testData) }, next),
    ...
    it.skip('should replicate/put metadata to a destination that has a suspended null version with internal version', done => {
        const tagSet = [
            {
                Key: 'key1',
                Value: 'value1',
        ...
        async.series(
            {
                ...
                putObjectTagging: next =>
                    dstS3.putObjectTagging(
                        { Bucket: bucketDestination, Key: keyName, Tagging: { TagSet: tagSet } },
                        next
                    ),
                enableVersioningDestination: next =>
                    dstS3.putBucketVersioning(
                ...
                getObjectTaggingNullVersion: next =>
                    dstS3.getObjectTagging({ Bucket: bucketDestination, Key: keyName, VersionId: 'null' }, next),
                listObjectVersions: next => dstS3.listObjectVersions({ Bucket: bucketDestination }, next),
            },
            (err, results) => {
                if (err) {
        ...
        async.series(
            {
                ...
                listObjectVersionsBeforeReplicate: next =>
                    dstS3.listObjectVersions({ Bucket: bucketDestination }, next),
        ...
        async.series(
            {
                ...
                simulateLifecycleNullVersion: next =>
                    makeBackbeatRequest(
                        {
                            method: 'GET',
                            resourceType: 'metadata',

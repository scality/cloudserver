...
const { hasLocation, describeSkipIfNotMultiple } = require('../../functional/aws-node-sdk/lib/utility/test-utils');
const {
    awsLocation,
    awsS3: awsClient,
    awsBucket,
...
describe('backbeat routes', () => {
    ...
    describe('null version', () => {
        ...
        beforeEach(() => {
            ...
            return bucketUtil.emptyIfExists(bucket).then(() => s3.send(new CreateBucketCommand({ Bucket: bucket })));
        });

        afterEach(() => bucketUtil.empty(bucket).then(() => s3.send(new DeleteBucketCommand({ Bucket: bucket }))));
    ...
    describe('backbeat authorization checks', () => {
        ...
        describe(`when api proxy is ${apiProxy ? '' : 'NOT '}configured`, () => {
            ...
            it(`GET /_/backbeat/api/... should respond with ${
                apiProxy ? 503 : 405
            } on authenticated requests (API server down)`, done => {
                const options = {
                    authCredentials: {
                        accessKey: accessKeyLisa,
                        secretKey: secretAccessKeyLisa,
            ...
            it(`GET /_/backbeat/api/... should respond with ${
                apiProxy ? 403 : 405
            } if the request is unauthenticated`, done => {
                const options = {
                    hostname: ipAddress,
                    port: 8000,
                    method: 'GET',
    ...
    describeIfLocationAws('backbeat multipart upload operations (external location)', function test() {
        ...
        it('should put tags if the source is Azure and tags are provided when completing the multipart upload', done => {
            const containerName = getAzureContainerName(azureLocation);
            const blob = uuidv4();
    ...
    describe('checksums', () => {
        ...
        describe('data route integrity check', () => {
            ...
            itIfLocationAws('should ignore a mismatching x-amz-checksum-sha256 header (multiplebackenddata)', done => {
                makeBackbeatRequest(
                    {
                        method: 'PUT',
                        resourceType: 'multiplebackenddata',

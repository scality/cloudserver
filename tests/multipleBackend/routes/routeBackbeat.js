...
const testDataMd5 = crypto.createHash('md5').update(testData, 'utf-8').digest('hex');
            ...
            [
                ...
                (UploadId, next) =>
                    awsClient.listMultipartUploads(
                        ...
                        (err, response) => {
                            assert.ifError(err);
                            const hasOngoingUpload = response.Uploads.some(upload => upload === UploadId);
...
describe('backbeat routes', () => {
    ...
    before(done => {
        ...
        s3 = bucketUtil.s3;
        bucketUtil
            .emptyManyIfExists([TEST_BUCKET, TEST_ENCRYPTED_BUCKET, NONVERSIONED_BUCKET, VERSION_SUSPENDED_BUCKET])
    ...
    after(() =>
        bucketUtil
            ...
            .then(() => s3.deleteBucket({ Bucket: VERSION_SUSPENDED_BUCKET }).promise())
    );
    ...
    describe('null version', () => {
        ...
        beforeEach(() =>
            bucketUtil
                .emptyIfExists(BUCKET_FOR_NULL_VERSION)
                .then(() => s3.createBucket({ Bucket: BUCKET_FOR_NULL_VERSION }).promise())
        );

        afterEach(() =>
            bucketUtil
                .empty(BUCKET_FOR_NULL_VERSION)
            ...
            return async.series(
                {
                    putObjectInitial: next =>
                        s3.putObject({ Bucket: bucket, Key: keyName, Body: new Buffer(testData) }, next),
                    ...
                    putObjectAgain: next =>
                        s3.putObject({ Bucket: bucket, Key: keyName, Body: new Buffer(testData) }, (err, data) => {
                            if (err) {
                                return next(err);
                            }
            ...
            return async.series(
                {
                    ...
                    putObject: next =>
                        s3.putObject({ Bucket: bucket, Key: keyName, Body: Buffer.from(testData) }, next),
                    enableVersioning: next =>
                        s3.putBucketVersioning(
                            { Bucket: bucket, VersioningConfiguration: { Status: 'Enabled' } },
                            next
        ...
        it('should create a new null version if versioning suspended and version has version id', done => {
            ...
            return async.series(
                ...
                (err, data) => {
                    ...
                    return setTimeout(
                        () => checkVersionData(s3, bucket, keyName, expectedVersionId, testData, done),
                        1000
                    );
                }
            );
    ...
    describe('backbeat PUT routes', () => {
        describe('PUT data + metadata should create a new complete object', () => {
            [
                ...
                },
                {
                    ...
                    key: testKeyUTF8,
                    encodedKey: encodeURI(testKeyUTF8),
                },
                {
                            ...
                            [
                                next => {
                                    ...
                                    makeBackbeatRequest(
                                        {
                                            method: 'PUT',
                                            bucket: testCase.encryption ? TEST_ENCRYPTED_BUCKET : TEST_BUCKET,
                                ...
                                (response, next) => {
                                    ...
                                    checkObjectData(
                                        s3,
                                        testCase.encryption ? TEST_ENCRYPTED_BUCKET : TEST_BUCKET,
                                        testCase.key,
                                        testData,
                                        next
                                    );
            ...
            async.waterfall(
                [
                    ...
                    (response, next) => {
                        assert.strictEqual(response.statusCode, 200);
                        // give some time for the async deletes to complete
                        setTimeout(() => checkObjectData(s3, TEST_BUCKET, testKey, testData, next), 1000);
                    },
                    next => {
            ...
            async.waterfall(
                [
                    ...
                    (response, next) => {
                        assert.strictEqual(response.statusCode, 200);
                        // give some time for the async deletes to complete
                        setTimeout(() => checkObjectData(s3, TEST_BUCKET, testKey, '', next), 1000);
                    },
                    next => {
                        // check that the object copy referencing the old data
                        // locations is unreadable, confirming that the old
        ...
        it('should not remove data locations on replayed metadata PUT', done => {
            let serializedNewMd;
            async.waterfall(
                [
                    next => {
    ...
    describe('backbeat authorization checks', () => {
        ...
        it(
            'GET  /_/backbeat/api/... should respond with ' + '503 on authenticated requests (API server down)',
            done => {
                const options = {
                    authCredentials: {
                        accessKey: 'accessKey2',
        ...
        it(
            'GET  /_/backbeat/api/... should respond with ' + '403 Forbidden if the request is unauthenticated',
            done => {
                const options = {
                    hostname: ipAddress,
                    port: 8000,
    ...
    describe('backbeat multipart upload operations', function test() {
        ...
        it(
            'should put tags if the source is Azure and tags are provided ' + 'when completing the multipart upload',
                ...
                const containerName = getAzureContainerName(azureLocation);
                const blob = uuidv4();
                const multipleBackendPath = `/_/backbeat/multiplebackenddata/${containerName}/${blob}`;
    ...
    describe('Batch Delete Route', function test() {
                ...
                [
                        ...
                        makeBackbeatRequest(
                            ...
                            (err, data) => {
                                assert.ifError(err);
                                assert.strictEqual(data.statusCode, 200);
                                const metadata = JSON.parse(JSON.parse(data.body).Body);
                        ...
                        const options = {
                            ...
                            requestBody: `{"Locations":${JSON.stringify(location)}}`,
                            jsonResponse: true,
                        };
                            ...
                            makeRequest(
                                {
                                    ...
                                    path: '/_/backbeat/batchdelete',
                                    headers: {
                                        'if-unmodified-since': 'Sun, 31 Mar 2019 00:00:00 GMT',
                                        'x-scal-storage-class': awsLocation,
                                        'x-scal-tags': JSON.stringify({
                                            'scal-delete-marker': 'true',
                                            'scal-delete-service': 'lifecycle-transition',
        ...
        itSkipCeph('should put tags if the source is not Azure and ' + 'if-unmodified-since condition is met', done => {
            const awsKey = uuidv4();
            let lastModified;
            async.series(
                [
                            ...
                            makeRequest(
                                {
                                    ...
                                    port: 8000,
                                    method: 'POST',
                                    path: `/_/backbeat/batchdelete/${containerName}/${blob}`,
                                    headers: {
                                        'if-unmodified-since': 'Sun, 31 Mar 2019 00:00:00 GMT',
                                        'x-scal-storage-class': azureLocation,
                                        'x-scal-tags': JSON.stringify({
        ...
        it('should delete the object if the source is Azure and ' + 'if-unmodified-since condition is met', done => {
            const blob = uuidv4();
            let lastModified;
            async.series(
                [

...
const clientCheck = require('../../lib/utilities/healthcheckHandler').clientCheck;
            ...
            clientCheck(false, log, (err, results) => {
                ...
                locConstraints.forEach(constraint => {
                    ...
                    if (Object.keys(results).indexOf(constraint) === -1) {
                        const locationType = config.locationConstraints[constraint].type;
                        assert(
                            Object.keys(results).some(
                                result => config.locationConstraints[result].type === locationType
                            )
                        );
                    }
                });
    ...
    describe.skip('Azure container creation', () => {
        ...
        it(
            "should not create an azure location's container even if it is " +
                'missing if the check is not a flightCheckOnStartUp',
            done => {
                clientCheck(false, log, err => {
                    assert.strictEqual(err, null, `got unexpected err in clientCheck: ${err}`);
                    return azureClient

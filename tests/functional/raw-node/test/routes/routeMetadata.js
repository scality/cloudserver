...
function makeMetadataRequest(params, callback) {
    const { method, headers, authCredentials, requestBody, queryObj, path } = params;
    const options = {
        authCredentials,
        hostname: ipAddress,
        port: 8000,
...
describe('metadata routes with metadata', () => {
    ...
    if (process.env.S3_END_TO_END) {
        ...
        before(done => {
            httpServer = http.createServer((req, res) => metadataMock.onRequest(req, res)).listen(9000, done);
        });
...
describe('checksum stored in object metadata after PutObject', () => {
    ...
    it('should store crc32 checksum in metadata when no explicit checksum header is provided (AWS SDK default)', done => {
        makeMetadataRequest(
            {
                method: 'GET',
                authCredentials: metadataAuthCredentials,

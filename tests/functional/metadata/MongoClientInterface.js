...
runIfMongo('MongoClientInterface', () => {
        ...
        collection
            ...
            .then(doc => {
                ...
                if (doc.value.tags) {
                    ...
                    doc.value.tags = unescapeJSON(doc.value.tags);
                }
    ...
    function checkVersionAndMasterMatch(versionId, cb) {
        async.parallel([next => getObject({}, next), next => getObject({ versionId }, next)], (err, res) => {
            if (err) {
                return cb(err);
            }
    ...
    function putObject(caseNum, params, objectValue, cb) {
        ...
        mongoClientInterface[method](collection, BUCKET_NAME, OBJECT_NAME, dupeObjVal, params, log, (err, res) => {
            if (err) {
                return cb(err);
            }
            let parsedRes;
    ...
    function checkNewPutObject(caseNum, params, cb) {
        ...
        async.series(
            [
                next => mongoClientInterface[method](collection, bucket, key, updatedObjVal, params, log, next),
                next => {
    ...
    describe('::putObjectVerCase1', () => {
        it('should put new metadata and update master', done => {
            async.waterfall(
                [
                    next => putObject(1, {}, null, (err, res) => next(err, res.versionId)),
        ...
        it('should set new version id for master', done => {
            async.waterfall(
                [
                    ...
                    (res, next) => checkVersionAndMasterMatch(res.versionId, err => next(err, res.versionId)),
                    // call ver case 2
                    (id, next) =>
                        putObject(2, {}, null, (err, res) => {
                            if (err) {
                                return next(err);
                            }
    ...
    describe('::putObjectVerCase3', () => {
        ...
        it('should put new metadata and not update master', done => {
            async.waterfall(
                [
                    ...
                    // call ver case 3
                    (id, next) =>
                        putObject(3, { versionId: VERSION_ID }, null, (err, res) => {
                            if (err) {
                                return next(err);
                            }
                    ...
                    next =>
                        getObject({ versionId: VERSION_ID }, (err, res) => {
                            if (err) {
                                return next(err);
                            }
                            assert(res);
        ...
        it('should put new metadata and update master if version id matches', done => {
            async.waterfall(
                [
                    // first create new ver and master
                    next => putObject(1, {}, null, next),
                    ...
                    (id, next) =>
                        mongoClientInterface.putObjectVerCase3(
                            collection,
                            BUCKET_NAME,
                            OBJECT_NAME,
                            updatedObjVal,
        ...
        function putAndCheckCase4(versionId, cb) {
            ...
            async.waterfall(
                [
                    // put object
                    next =>
                        putObject(4, { versionId }, objectValue, (err, res) => {
                            if (err) {
                                return next(err);
                            }
                            return next(null, res.versionId);

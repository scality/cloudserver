...
function createDanglingPHD(bucket, key, cb) {
    return putObject(bucket, key, (err, versionId) => (err ? cb(err) : deleteVersion(bucket, key, versionId, cb)));
}
        ...
        next => {
            ...
            return makeBackbeatRequest(
                ...
                (err, response) => {
                    ...
                    const nextKeyMarker = listType === 'orphan' ? data.NextMarker : data.NextKeyMarker;
                    ...
                    if (keyMarker !== undefined) {
                        if (nextKeyMarker < keyMarker) {
                            return next(new Error(`marker went backwards: ${keyMarker} -> ${nextKeyMarker}`));
                        }
...
function assertDesertWasScanned(pages, seededAt, label) {
    const markers = pages.map(page => page.NextMarker || page.NextKeyMarker).filter(Boolean);
...
function contentsKeys(pages) {
    return pages.reduce((acc, page) => acc.concat((page.Contents || []).map(entry => entry.Key)), []);
}
...
describePHD('listLifecycle over a dangling-PHD desert', () => {
    ...
    describe('noncurrent crosses the desert', () => {
        ...
        before(done =>
            async.series(
                [
                    ...
                    next => deleteVersion(bucket, 'www-survivors', survivorVersionIds[2], next),
        ...
        it('should list noncurrent versions on both sides of the desert and protect the PHD survivor', async () => {
            ...
            assert.deepStrictEqual(listed.map(entry => `${entry.Key}:${entry.VersionId}`).sort(), [

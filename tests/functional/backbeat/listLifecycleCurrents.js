                    ...
                    next =>
                        async.times(
                            ...
                            (n, cb) => {
                                ...
                                s3.putObject(
                                    { Bucket: testBucket, Key: keyName, Body: '123', Tagging: 'mykey=myvalue' },
                                    (err, data) => {
                                        if (err) {
                                            cb(err);
                                        }
    ...
    before(done => {
        ...
        return async.series(
            [
                ...
                next => {
                    s3.putObject(
                        { Bucket: testBucket, Key: keyName0, Body: '123', Tagging: 'mykey=myvalue' },
                        (err, data) => {
                            if (err) {
                                next(err);
                            }
                ...
                next =>
                    s3.putObject({ Bucket: testBucket, Key: keyName1, Body: '123', Tagging: 'mykey=myvalue' }, next),
                next => s3.deleteObject({ Bucket: testBucket, Key: keyName1 }, next),
                next =>
                    s3.putObject(
                        { Bucket: testBucket, Key: keyName2, Body: '123', Tagging: 'mykey=myvalue' },
                        (err, data) => {
                            if (err) {
                                next(err);
                            }
                    ...
                    s3.putObject(
                        ...
                        (err, data) => {
                            ...
                            return s3.deleteObject(
                                { Bucket: testBucket, Key: keyName2, VersionId: data.VersionId },
                                next
                            );
                        }
                    ),

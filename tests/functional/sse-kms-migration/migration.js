...
async function assertObjectSSE(
    ...
    const expectedKey = `${
        sseMD.SSEKMSKeyId && isScalityKmsArn(sseMD.SSEKMSKeyId) ? '' : arnPrefix
    }${sseMD.SSEKMSKeyId}`;
...
describe('SSE KMS migration', () => {
    ...
    before('setup', async () => {
        ...
        const { SSEAlgorithm, KMSMasterKeyID } =
            copySSE.ServerSideEncryptionConfiguration.Rules[0].ApplyServerSideEncryptionByDefault;
    ...
    scenarios.testCases.forEach(bktConf =>
        describe(`bucket enc-bkt-${bktConf.name}`, () => {
            ...
            if (!bktConf.algo) {
                ...
                if (!bktConf.deleteSSE) {
                    it('should have non mandatory SSE in bucket MD as test init put an object with AES256', async () =>
                        scenarios.tests.getBucketNonMandatorySSE(bkt.name, log, 'migration'));
                }
            } else {
            ...
            scenarios.testCasesObj.forEach(objConf =>
                it(`should have pre uploaded object with SSE ${objConf.name}`, async () => {
            ...
            scenarios.testCasesObj.forEach(objConf =>
                describe(`object enc-obj-${objConf.name}`, () => {
                    ...
                    const optionalSkip =
                        objConf.algo || bktConf.masterKeyId || (!bktConf.algo && !bktConf.deleteSSE) ? it.skip : it;
                    ...
                    [
                        ...
                        optionalSkip(`should migrate completed MPU ${name}`, async () => {
                            ...
                            await assertObjectSSE(assertion, { objConf, obj }, { bktConf, bkt }, fileArnPrefix);
                        })
                    );
                    ...
                    async function prepareMPUTest(mpuKey, expectedExistingParts) {
                        ...
                        const existingParts = await helpers.s3
                            .listParts({
                                Bucket: bkt.name,
                                Key: mpuKey,
                                UploadId: uploadId,
                        ...
                        assert.strictEqual(
                            existingParts.IsTruncated,
                            false,
                            'Too much parts, need to loop on pagination'
                        );
                        ...
                        return {
                            ...
                            mpuOverviewMDSSE,
                            partCount,
                            existingParts: existingParts.Parts || [],
                        };
                    }
                    ...
                    optionalSkip(
                        'should finish ongoing encrypted MPU with 2 parts by copy and upload part',
                        async () => {
                            const { mpuKey, uploadId, mpuOverviewMDSSE, existingParts, partCount } =
                    ...
                    optionalSkip('should finish ongoing encrypted MPU with 2 parts by 2 copy byte range', async () => {
                        const { mpuKey, uploadId, mpuOverviewMDSSE, existingParts, partCount } = await prepareMPUTest(
                            `${obj.name}-migration-mpucopyrange`,
                            2
                        );

...
describe('ensure MPU use good SSE', () => {
    ...
    // const mpuKmsBktKey = 'bkt-mpu-kms-key';
    let kmsKeympuKmsBkt;


    before(async () => {
        kmsKeympuKmsBkt = (await promisify(kms.createBucketKey)(bucketInfo, log)).masterKeyArn;
        void await promisify(metadata.setup.bind(metadata))();
        void await s3.createBucket({ Bucket: mpuKmsBkt }).promise();
        // void await s3.createBucket({ Bucket: mpuKmsBktKey }).promise();
        // void await s3.putBucketEncryption({
        //     Bucket: mpuKmsBktKey,
        //     ServerSideEncryptionConfiguration: hydrateSSEConfig({ algo: 'AES256' }) }).promise();
        // void await s3.putBucketEncryption({
        //     Bucket: mpuKmsBktKey,
        //     ServerSideEncryptionConfiguration:
        //         hydrateSSEConfig({ algo: 'aws:kms', masterKeyId: kmsKeympuKmsBkt }) }).promise();
        void await s3.putBucketEncryption({
            Bucket: mpuKmsBkt,
            ServerSideEncryptionConfiguration:
                hydrateSSEConfig({ algo: 'aws:kms', masterKeyId: kmsKeympuKmsBkt }) }).promise();
    ...
    after(async () => {
        void await bucketUtil.empty(mpuKmsBkt);
        void await s3.deleteBucket({ Bucket: mpuKmsBkt }).promise();
        // void await s3.deleteBucket({ Bucket: mpuKmsBktKey }).promise();
    });
    ...
    it('mpu should use encryption from createMPU', async () => {
        const key = 'mpuKey';
        const mpuKms = (await promisify(kms.createBucketKey)(bucketInfo, log)).masterKeyArn;
        const mpu = await s3.createMultipartUpload({
        ...
        void await assertObjectSSE(mpuKmsBkt, key,
            ...
            { algo: 'aws:kms', masterKeyId: true },
            { kmsKey: kmsKeympuKmsBkt, kmsKeyInfo: { masterKeyId: kmsKeympuKmsBkt, masterKeyArn: kmsKeympuKmsBkt } },
            null, 'Scality');
    });
});

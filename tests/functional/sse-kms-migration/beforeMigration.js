    ...
    scenarios.testCases.forEach(bktConf => describe(`bucket enc-bkt-${bktConf.name}`, () => {
        ...
        scenarios.testCasesObj.forEach(objConf => describe(`object enc-obj-${objConf.name}`, () => {
            ...
            optionalSkip('should encrypt MPU and copy an encrypted parts from encrypted bucket', async () => {
                ...
                console.log('{ objConf, obj }', { objConf, obj });
                console.log('{ bktConf, bkt }', { bktConf, bkt });
                await scenarios.assertObjectSSE(assertion, { objConf, obj }, { bktConf, bkt });
            });
            ...
            it(`should PutObject versioned with SSE ${obj.name}`, async () => {
                ...
                const data = await helpers.s3.listObjectVersions({ Bucket: bkt.vname });
                ...
                const data2 = await helpers.s3.listObjectVersions({ Bucket: bkt.vname });
                ...
                const data3 = await helpers.s3.listObjectVersions({ Bucket: bkt.vname });
                const versions3 = data3.Versions || [];
                assert.strictEqual(versions3.length, 3);
                ...
                const data4 = await helpers.s3.listObjectVersions({ Bucket: bkt.vname });
                const versions4 = data4.Versions || [];
                assert.strictEqual(versions4.length, 3);
                ...
                const data5 = await helpers.s3.listObjectVersions({ Bucket: bkt.vname });
                const versions5 = data5.Versions || [];
                assert.strictEqual(versions5.length, 3);
                ...
                await scenarios.assertObjectSSE(
                    { ...baseAssertion, VersionId: v2.VersionId, Body: bodyV2 }, { objConf, obj }, { bktConf, bkt });
                const data6 = await helpers.s3.listObjectVersions({ Bucket: bkt.vname });
                const versions6 = data6.Versions || [];
                assert.strictEqual(versions6.length, 3);
                ...
                await scenarios.assertObjectSSE(
                    { ...baseAssertion, Body: bodyFinal }, { objConf, obj }, { bktConf, bkt }, 'null');
                const data7 = await helpers.s3.listObjectVersions({ Bucket: bkt.vname });
                const versions7 = data7.Versions || [];
                assert.strictEqual(versions7.length, 3);
            });
        }));
    }));

...
Object.defineProperty(kms, 'arnPrefix', {
    get() {
        return '';
    },
});
            ...
            if (!bktConf.algo) {
                ...
                if (!bktConf.deleteSSE) {
                    it('should have non mandatory SSE in bucket MD as test init put an object with AES256', async () =>
                        await scenarios.tests.getBucketNonMandatorySSE(bkt.name, log, 'before'));
                }
            } else {
            ...
            scenarios.testCasesObj.forEach(objConf =>
                describe(`object enc-obj-${objConf.name}`, () => {
                    ...
                    it(`should PutObject ${obj.name} overriding bucket SSE`, async () =>
                        scenarios.tests.putObjectOverrideSSE({ objConf, obj }, { bktConf, bkt }));
                    ...
                    optionalSkip(
                        'should encrypt MPU and copy an encrypted range parts from encrypted bucket',
                        async () => {
                    ...
                    optionalSkip(
                        'should prepare encrypted MPU and put 2 encrypted parts without completion',
                        async () => {
                    ...
                    optionalSkip(
                        'should prepare encrypted MPU and copy an encrypted parts ' +
                            'from encrypted bucket without completion',
                        async () => {
                    ...
                    optionalSkip(
                        'should prepare encrypted MPU and copy an encrypted range parts ' +
                            'from encrypted bucket without completion',
                        async () => {
                    ...
                    it(`should PutObject versioned with SSE ${obj.name}`, async () => {
                        ...
                        await helpers.s3.putBucketVersioning({
                            Bucket: bkt.vname,

...
function methodRequest(params, callback) {
    const { method, bucket, objectKey, query, headers, code, headersResponse, headersOmitted, isWebsite } = params;
    ...
    const req = transport.request(options, res => {
        ...
        res.on('end', () => {
            ...
            if (code) {
                const message = Number.isNaN(parseInt(code, 10)) ? `<Code>${code}</Code>` : '';
                assert(total.indexOf(message) > -1, `Expected ${message}`);
                assert.deepEqual(res.statusCode, statusCode[code], `status code expected: ${statusCode[code]}`);
            }
            if (headersResponse) {
                Object.keys(headersResponse).forEach(key => {
                    assert.deepEqual(res.headers[key], headersResponse[key], `error header: ${key}`);
                });
            } else {
                ...
                [
                    ...
                    assert.strictEqual(res.headers[key], undefined, `Error: ${key} should not have value`);
                });
            }
            if (headersOmitted) {

...
function _makeGETRequest(path, callback) {
    const options = {
        hostname,
...
'that has x-amz-website-redirect-location header ::', () => {
    ...
    describe('when x-amz-website-redirect-location: /redirect.html', () => {
        ...
        it('should serve redirect file', done => {
    ...
    describe(`with redirect all requests to ${redirectEndpoint}`, () => {
        ...
        'x-amz-website-redirect-location header location', done => {
            browser.visit(endpoint, () => setTimeout(() => {
                WebsiteConfigTester.checkHTML(browser, '200',
                  redirectEndpoint);
                done();
    ...
    () => {
        ...
        it(`should redirect to ${redirectEndpoint}/about instead of ` +
        'about/ key x-amz-website-redirect-location header location', done => {
            _makeGETRequest('/about/', (err, res) => {
                if (err) {
                    done(err);
                }

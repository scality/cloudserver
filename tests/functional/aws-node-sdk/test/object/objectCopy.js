...
describe('Object Copy', () => {
    withV4(sigCfg => {
        ...
        afterEach(() => bucketUtil.empty(sourceBucketName).then(() => bucketUtil.empty(destBucketName)));
        ...
        it(
            'should copy an object from a source bucket to a different ' +
                ...
                s3.copyObject(
                    { Bucket: destBucketName, Key: destObjName, CopySource: `${sourceBucketName}/${sourceObjName}` },
                    (err, res) => successCopyCheck(err, res, originalMetadata, destBucketName, destObjName, done)
                );
            }
        );
        ...
        it(
            'should copy an object from a source to the same destination ' +
                ...
                s3.putObject({ Bucket: sourceBucketName, Key: 'emptyobject' }, err => {
                    checkNoError(err);
                    s3.copyObject(
                        {
                            Bucket: destBucketName,
        ...
        it('should copy an object from a source bucket to a different ' + 'key in the same bucket', done => {
            s3.copyObject(
                { Bucket: sourceBucketName, Key: destObjName, CopySource: `${sourceBucketName}/${sourceObjName}` },
                (err, res) => successCopyCheck(err, res, originalMetadata, sourceBucketName, destObjName, done)
            );
        });
        ...
        it('should copy a 0 byte object to different destination', done => {
            ...
            s3.putObject({ Bucket: sourceBucketName, Key: sourceObjName, Body: '', Metadata: originalMetadata }, () => {
                s3.copyObject(
                    { Bucket: destBucketName, Key: destObjName, CopySource: `${sourceBucketName}/${sourceObjName}` },
                    (err, res) => {
                        checkNoError(err);
                ...
                s3.putObject({ Bucket: sourceBucketName, Key: sourceObjName, Body: '' }, () => {
                    s3.copyObject(
                        ...
                        (err, res) => {
                            checkNoError(err);
                            assert.strictEqual(res.ETag, emptyFileETag);
                            s3.getObject({ Bucket: sourceBucketName, Key: sourceObjName }, (err, res) => {
...
describeSkipIfCeph('Object Copy with object lock enabled on both destination ' + 'bucket and source bucket', () => {
    withV4(sigCfg => {
        let bucketUtil;
        let s3;
        let versionId;

    ...
    withV4(sigCfg => {
        ...
        beforeEach(() =>
            s3
                ...
                .promise()
                .then(() => s3.headObject({ Bucket: bucketName, Key: objectName }).promise())
                .then(res => {
        ...
        it('should overwrite all user metadata and data on overwrite put', () =>
            s3
                ...
                .promise()
                .then(() => s3.getObject({ Bucket: bucketName, Key: objectName }).promise())
                .then(res => {
                    assert.deepStrictEqual(res.Metadata, secondPutMetadata);
                    assert.deepStrictEqual(res.Body.toString(), 'Much different');
                }));
    });
});

    ...
    withV4(sigCfg => {
        ...
        function _completeMpuAndCheckVid(uploadId, eTag, expectedVid, cb) {
            ...
            .then(data => {
                ...
                if (expectedVid) {
                    ...
                    assert.strictEqual(data.VersionId, data.VersionId);
                }
                cb();
            })
            .catch(err => cb(err));
        ...
        beforeEach(async () => {
            try {
                ...
                throw new Error(`Bucket creation failed for ${bucket}: ${err.message}`);
            }
        });
        ...
        afterEach(async () => {
            try {
                ...
                // Add context to the error instead of just re-throwing
                throw new Error(`Cleanup failed for bucket ${bucket}: ${err.message}`);
            }
        });

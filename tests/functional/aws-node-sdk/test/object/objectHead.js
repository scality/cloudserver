...
describe('HEAD object, conditions', () => {
    withV4(sigCfg => {
        ...
        before(() => {
            ...
            return bucketUtil
                .empty(bucketName)
                .then(() => bucketUtil.deleteOne(bucketName))
                .catch(err => {
        ...
        beforeEach(() =>
            s3
                ...
                .then(res => {
                    ...
                    return s3.send(new HeadObjectCommand({ Bucket: bucketName, Key: objectName }));
                })
                .then(res => {
        ...
        it('WebsiteRedirectLocation is set & it appears in response', done => {
            ...
            s3.send(new PutObjectCommand(redirBktwBody)).then(() => {
                s3.send(new HeadObjectCommand(redirBkt)).then(data => {
                    assert.strictEqual(data.WebsiteRedirectLocation, 'http://google.com');
                    done();
                });
            });
        });
        ...
        it('WebsiteRedirectLocation is not set & is absent', done => {
            requestHead({}, (err, data) => {
                checkNoError(err);
                assert.strictEqual(
                    'WebsiteRedirectLocation' in data,
                    false,
                    'WebsiteRedirectLocation header is present.',
                );
                done();
            });
        });
                ...
                [
                    ...
                    (data, next) => {
                        ...
                        s3.send(
                            ...
                            .then(data => next(null, uploadId, data.ETag))
                            .catch(next);
                    },
                    (uploadId, etagOne, next) =>
                        s3
                            .send(
                                new UploadPartCommand({
                                    Bucket: bucketName,
                            ...
                            .then(data => next(null, uploadId, etagOne, data.ETag))
                            .catch(next),

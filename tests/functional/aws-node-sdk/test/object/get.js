...
describe('GET object', () => {
    withV4(sigCfg => {
        ...
        it('should return NoSuchKey error when no such object', done => {
            s3.send(new GetObjectCommand({ Bucket: bucketName, Key: 'nope' }))
                .then(() => {
        ...
        it('should return NoSuchKey error when no such object even with key longer than 915 bytes', done => {
            s3.send(new GetObjectCommand({ Bucket: bucketName, Key: 'a'.repeat(2000) }))
                .then(() => {
                    assert.fail('Expected failure but got success');
                })
                .catch(err => {
...
describe('GET object checksum mode', () => {
    withV4(sigCfg => {
        ...
        before(async () => {
            ...
            bucketUtil = new BucketUtility('default', { ...sigCfg, responseChecksumValidation: 'WHEN_REQUIRED' });
            s3 = bucketUtil.s3;
            await s3.send(new CreateBucketCommand({ Bucket: checksumBucket }));

            for (const { internalName } of checksumAlgorithms) {
                expectedDigests[internalName] = await algorithms[internalName].digest(body);
            }
        });

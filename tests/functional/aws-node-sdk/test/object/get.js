    ...
    withV4(sigCfg => {
            ...
            () => {
                describe('if specified in put object request', () => {
                    ...
                    it('should return additional headers', done => {
                        s3.getObject({ Bucket: bucketName, Key: objectName }, (err, res) => {
                            if (err) {
                                return done(err);
                            }
                ...
                describe('if response content headers are set in query', () => {
                    before(done => {
                        s3.putObject({ Bucket: bucketName, Key: objectName }, err => done(err));
                    });
        ...
        describe('x-amz-tagging-count', () => {
            ...
            describe('tag associated with the object', () => {
                ...
                it(
                    'should return "x-amz-tagging-count" header that provides ' +
        ...
        describe('conditional headers', () => {
            ...
            it('If-Modified-Since: returns NotModified if Last modified ' + 'date is equal', done => {
                s3.headObject({ Bucket: bucketName, Key: objectName }, (err, data) => {
                    checkNoError(err);
            ...
            it('If-Unmodified-Since: returns no error when lastModified ' + 'date is equal', done => {
                s3.headObject({ Bucket: bucketName, Key: objectName }, (err, data) => {
                    checkNoError(err);
                    const lastModified = dateConvert(data.LastModified);
                    requestGet({ IfUnmodifiedSince: lastModified }, err => {
                        checkNoError(err);
                        done();
                    });
                });
            ...
            it(
                'If-None-Match & If-Modified-Since: returns NotModified when ' +
        ...
        describe('With PartNumber field', () => {
            ...
            orderedPartNumbers.forEach(num =>
                it(`should get the body of part ${num} when ordered MPU`, done =>
            ...
            orderedPartNumbers.forEach(num =>
                it(`should get the body of part ${num} when unordered MPU`, done =>
                    completeMPU(unOrderedPartNumbers, err => {
                        checkNoError(err);
                        return requestGet({ PartNumber: num }, (err, data) => {
                            checkNoError(err);
                            checkIntegerHeader(data.ContentLength, partSize);
                            const md5Hash = crypto.createHash('md5');
                            const md5HashExpected = crypto.createHash('md5');
                            const expected = Buffer.alloc(partSize).fill(unOrderedPartNumbers[num - 1]);
                            assert.strictEqual(
                                md5Hash.update(data.Body).digest('hex'),
                                md5HashExpected.update(expected).digest('hex')
                            );
            ...
            invalidPartNumbers.forEach(num =>
                it(`should not accept a partNumber that is not 1-10000: ${num}`, done =>
                    completeMPU(orderedPartNumbers, err => {
                        checkNoError(err);
            ...
            it('should accept a part number of 1 for regular put object', done =>
                s3.putObject(
                    {
                        Bucket: bucketName,
                        Key: objectName,
            ...
            it('should not accept a part number greater than 1 for regular ' + 'put object', done =>
                s3.putObject(
                    {
                        Bucket: bucketName,
                        Key: objectName,
            ...
            it('should not include PartsCount response header for regular ' + 'put object', done => {
                s3.putObject(
                    {
                        Bucket: bucketName,
                        Key: objectName,
            ...
            it('should include PartsCount response header for mpu object', done => {
                completeMPU(orderedPartNumbers, err => {
                    assert.ifError(err);
                    return requestGet({ PartNumber: 1 }, (err, data) => {
                        assert.ifError(err);
            ...
            describe('uploadPartCopy', () => {
                ...
                const bufs = orderedPartNumbers.map(n => Buffer.alloc(partSize, n));
                ...
                it('should retrieve a part copied from an MPU', done =>
                    checkGetObjectPart(copyPartKey, 1, partOneSize, partOneBody, done));

                it('should retrieve a part put after part copied from MPU', done =>
                    checkGetObjectPart(copyPartKey, 2, partSize, partTwoBody, done));
            });
...
describeSkipIfCeph('GET object with object lock', () => {
    withV4(sigCfg => {
        ...
        afterEach(() =>
            changeLockPromise([{ bucket, key, versionId }], '')
        ...
        it('should return object lock headers if set on the object', done => {
            s3.getObject({ Bucket: bucket, Key: key }, (err, res) => {
                ...
                const responseDate = formatDate(res.ObjectLockRetainUntilDate.toISOString());

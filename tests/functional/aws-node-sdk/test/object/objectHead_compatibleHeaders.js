...
describe(
    'HEAD object, compatibility headers [Cache-Control, ' + 'Content-Disposition, Content-Encoding, Expires]',
    () => {
        withV4(sigCfg => {
            let bucketUtil;
            let s3;
            ...
            after(() => {
                ...
                return bucketUtil.empty(bucketName).then(() => bucketUtil.deleteOne(bucketName));
            });

            it('should return additional headers if specified in objectPUT ' + 'request', done => {
                s3.headObject({ Bucket: bucketName, Key: objectName }, (err, res) => {
                    if (err) {
                        return done(err);
                    }
                    ...
                    assert.strictEqual(res.Expires.toGMTString(), expires.toGMTString());
                    return done();
                });
            });
        });

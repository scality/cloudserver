...
describe("Head object 'ReplicationStatus' value", () => {
    withV4(sigCfg => {
        ...
        it('should be `undefined` when there is no bucket replication config', done =>
            checkHeadObj(`${keyPrefix}-foobar`, undefined, done));
        ...
        describe('With bucket replication config', () => {
            ...
            it("should be 'PENDING' when object key prefix applies", done =>
                checkHeadObj(`${keyPrefix}-foobar`, 'PENDING', done));

            it('should be `undefined` when object key prefix does not apply', done =>
                checkHeadObj(`foobar-${keyPrefix}`, undefined, done));
        });
    });
});

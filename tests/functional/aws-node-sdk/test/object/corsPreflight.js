...
describe('Preflight CORS request on non-existing bucket', () => {
    it('should respond no such bucket if bucket does not exist', done => {
        ...
        methodRequest({ method: 'GET', bucket, headers, code: 'NoSuchBucket', headersResponse: null }, done);
    });
...
describe('Preflight CORS request with existing bucket', () => {
    ...
    describe('allow PUT, POST, DELETE, GET methods and allow only one origin', () => {
        ...
        methods.forEach(method => {
            it(
                'should respond with 200 and access control headers to ' +
    ...
    describe('CORS allows method GET and allows one origin', () => {
        ...
        ['PUT', 'POST', 'DELETE'].forEach(method => {
            it(
                'should respond AccessForbidden to OPTIONS request from ' +
    ...
    methods.forEach(allowedMethod => {
        ...
        describe(`CORS allows method "${allowedMethod}" and allows all origins`, () => {
            ...
            methods
                .filter(method => method !== allowedMethod)
                .forEach(method => {
                    it(
                        'should respond AccessForbidden to OPTIONS request from ' +
                            `allowed origin but not allowed method "${method}"`,
    ...
    originsWithWildcards.forEach(origin => {
        ...
        describe(`CORS allows method GET and origin "${origin}"`, () => {
            ...
            [originWithoutWildcard, originReplaceWildcard].forEach(acceptableOrigin => {
                it(
                    'should return 200 and CORS header to OPTIONS request ' +
    ...
    describe('CORS response access-control-allow-origin header value', () => {
        ...
        const corsParams = {
            ...
            CORSConfiguration: {
                CORSRules: [
                    {
                        AllowedMethods: ['GET'],
                        AllowedOrigins: [allowedOrigin, originContainingWildcard],
                    },
                    {
                        AllowedMethods: ['GET'],
                        AllowedOrigins: ['*'],
        ...
        it(
            'if OPTIONS request matches rule with multiple origins, response access-control-request-origin ' +
                'header value should be request Origin (not list of AllowedOrigins)',
            done => {
                const headers = {
                    Origin: allowedOrigin,
                    'Access-Control-Request-Method': 'GET',
        ...
        it(
            'if OPTIONS request matches rule with origin containing wildcard, response ' +
                'access-control-request-origin header value should be request Origin (not value containing wildcard)',
            done => {
    ...
    describe('CORS allows method GET, allows all origins and allows header Content-Type', () => {
        ...
        it(
            'should respond with 200 and access control headers to OPTIONS request from allowed ' +
                'origin and method, even without request Access-Control-Request-Headers header value',
            done => {
                const headers = {
                    Origin: allowedOrigin,
                    'Access-Control-Request-Method': 'GET',
        ...
        it(
            'should respond AccessForbidden to OPTIONS request from allowed origin and method ' +
                'but not allowed Access-Control-Request-Headers in addition to Content-Type',
            done => {
                const headers = {
                    Origin: allowedOrigin,
                    'Access-Control-Request-Method': 'GET',
    ...
    describe('CORS response Access-Control-Allow-Headers header value', () => {
                ...
                CORSRules: [
                    {
                        AllowedMethods: ['GET'],
                        AllowedOrigins: ['*'],
                        AllowedHeaders: ['Content-Type', 'amz-*', 'Expires'],
                    },
                    {
                        AllowedMethods: ['GET'],
                        AllowedOrigins: ['*'],
        ...
        it(
            'should return request access-control-request-headers value, not list of ' +
                'AllowedHeaders from rule or corresponding AllowedHeader value containing wildcard',
            done => {
        ...
        it(
            'should return lowercase version of request Access-Control-' +
                ...
                const headersResponse = {
                    'access-control-allow-origin': '*',
                    'access-control-allow-methods': 'GET',
                    'access-control-allow-headers': requestHeaderValue.toLowerCase(),
                    vary,
                };
                methodRequest({ method: 'OPTIONS', bucket, headers, code: 200, headersResponse }, done);
            },
        ...
        it(
            'should remove empty comma-separated values derived from request Access-Control-Request-Method ' +
                'header and separate values with spaces when responding with Access-Control-Allow-Headers header',
            done => {
        ...
        it(
            'should return request Access-Control-Request-Headers value even if rule ' +
                'allows all headers (e.g. "*"), unlike access-control-allow-origin value',
            done => {
    ...
    describe('CORS and OPTIONS request with object keys', () => {
        ...
        it(
            'should respond with 200 and access control headers to OPTIONS ' +
                ...
                methodRequest({ method: 'OPTIONS', objectKey, bucket, headers, code: 200, headersResponse }, done);
            },
        );
        it(
            'should respond with 200 and access control headers to OPTIONS request ' +
                ...
                methodRequest(
                    { method: 'OPTIONS', bucket, objectKey: 'anotherObjectKey', headers, code: 200, headersResponse },
                    done,
                );
            },
        );
    ...
    describe('CORS and OPTIONS request', () => {
        ...
        it(
            'with fake auth credentials: should respond with 200 and access ' +
        ...
        it(
            'with cookies: should send identical response as to request ' +
    ...
    describe('CORS exposes headers', () => {
        ...
        it(
            "if OPTIONS request matches CORS rule with ExposeHeader's, " +
    ...
    describe('CORS max age seconds', () => {
        ...
        it(
            'if OPTIONS request matches CORS rule with max age seconds, ' +

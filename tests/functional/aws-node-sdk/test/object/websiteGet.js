...
// v2-compat shim over v3 client so existing callbacks continue to work
const s3 = {
    createBucket: (params, cb) => s3Client
        .send(new CreateBucketCommand(params)).then(d => cb(null, d)).catch(cb),
    deleteBucket: (params, cb) => s3Client
        .send(new DeleteBucketCommand(params)).then(d => cb(null, d)).catch(cb),
    putBucketWebsite: (params, cb) => s3Client
        .send(new PutBucketWebsiteCommand(params)).then(d => cb(null, d)).catch(cb),
    putObject: (params, cb) => s3Client
        .send(new PutObjectCommand(params)).then(d => cb(null, d)).catch(cb),
    deleteObject: (params, cb) => s3Client
        .send(new DeleteObjectCommand(params)).then(d => cb(null, d)).catch(cb),
    putBucketPolicy: (params, cb) => s3Client
        .send(new PutBucketPolicyCommand(params)).then(d => cb(null, d)).catch(cb),
};

...
const s3 = {
    createBucket: (params, cb) => {
        s3Client
            .send(new CreateBucketCommand(params))
            .then(d => cb(null, d))
            .catch(cb);
    },
    deleteBucket: (params, cb) => {
        s3Client
            .send(new DeleteBucketCommand(params))
            .then(d => cb(null, d))
            .catch(cb);
    },
    putBucketWebsite: (params, cb) => {
        s3Client
            .send(new PutBucketWebsiteCommand(params))
            .then(d => cb(null, d))
            .catch(cb);
    },
    putObject: (params, cb) => {
    ...
    putBucketPolicy: (params, cb) => {
        s3Client
            .send(new PutBucketPolicyCommand(params))
            .then(d => cb(null, d))
            .catch(cb);
    },
};
        ...
        return s3.putObject(
            {
                ...
                Key: key,
                ACL: 'public-read',
                Body: fs.readFileSync(path.join(__dirname, '/websiteFiles/redirect.html')),
...
describe('User visits bucket website endpoint', () => {
    ...
    describe('with existing bucket', () => {
        ...
        describe('with custom error document', () => {
            ...
            it('should serve custom error document if an error occurred', done => {
                WebsiteConfigTester.checkHTML(
                    {
                        method: 'GET',
                        url: endpoint,
            ...
            it('should serve custom error document with redirect', done => {
                s3.putObject(
                    {
                        Bucket: bucket,
                        Key: 'error.html',
        ...
        describe('redirect to key using ReplaceKeyWith', () => {
            beforeEach(done => {
                ...
                putBucketWebsiteAndPutObjectRedirect(redirect, condition, 'redirect.html', done);
            });
            ...
            it('should serve redirect file if error 403 error occured', done => {
                WebsiteConfigTester.checkHTML(
                    {
                        method: 'GET',
                        url: endpoint,
        ...
        describe.skip('redirect requests with prefix /about to redirect/', () => {
            ...
            it('should serve redirect file if key prefix is equal to "about"', done => {
                WebsiteConfigTester.checkHTML(
                    {
                        method: 'GET',
                        url: `${endpoint}/about/`,
        ...
        describe('with bucket policy', () => {
                                    ...
                                    Policy: JSON.stringify({
                                        ...
                                        Statement: [
                                            ...
                                            },
                                            {
                                                ...
                                                Principal: '*',
                                                Action: ['s3:GetObject'],
                                                Resource: [`arn:aws:s3:::${bucket}/unrelated_obj.html`],
                                            },
                                        ],
                                    }),
                                },
            ...
            afterEach(done => {
                async.waterfall(
                    [
                        next => s3.deleteObject({ Bucket: bucket, Key: 'index.html' }, next),
                        (data, next) => s3.deleteObject({ Bucket: bucket, Key: 'error.html' }, next),
                    ],
                    err => {
                        assert.ifError(err);
                        done();

        ...
        beforeEach(done => {
            ...
            async.series(
                [
                    ...
                    next =>
                        s3.send(new CreateBucketCommand({ Bucket: bucketName })).then(() => {
                            next();
                        }),
                    next =>
                        s3
                            .send(new CreateBucketCommand({ Bucket: bucketNameMD, ObjectLockEnabledForBucket: true }))
                            .then(() => {
                                next();
                            }),
                ],
        ...
        describe('error handling validation (without cold storage location)', () => {
            it('should fail if version id is invalid', done => {
                ...
                async.series(
                    [
                        ...
                        next =>
                            putObjectVersion(s3, params, 'aJLWKz4Ko9IjBBgXKj5KQT.G9UHv0g7P', err => {
            ...
            it('should fail if key does not exist', done => {
                ...
                async.series(
                    [
                        next =>
                            putObjectVersion(s3, params, '', err => {
            ...
            it('should fail if trying to overwrite a delete marker', done => {
                ...
                async.series(
                    [
                        ...
                        next => s3.send(new PutObjectCommand(params)).then(() => next()),
                        next =>
                            s3.send(new DeleteObjectCommand(params)).then(res => {
                                vId = res.VersionId;
                                return next();
                            }),
                        next =>
                            putObjectVersion(s3, params, vId, err => {
        ...
        describeSkipNullMdV1('with cold storage location', () => {
            it('should overwrite an object', done => {
                ...
                async.series(
                    [
                        ...
                        next =>
                            getMetadata(bucketName, objectName, undefined, (err, objMD) => {
                                objMDBefore = objMD;
                                return next(err);
                            }),
                        next =>
                            metadata.listObject(bucketName, mdListingParams, log, (err, res) => {
            ...
            it('should overwrite a version', done => {
                ...
                async.series(
                    [
                        ...
                        next =>
                            getMetadata(bucketName, objectName, vId, (err, objMD) => {
                                objMDBefore = objMD;
                                return next(err);
                            }),
            ...
            it('should overwrite a non-current null version', done => {
                ...
                async.series(
                    [
                        ...
                        next =>
                            getMetadata(bucketName, objectName, 'null', (err, objMD) => {
                                objMDBefore = objMD;
                                return next(err);
                            }),
                        next =>
            ...
            it('should overwrite the current version after bucket version suspended', done => {
                ...
                async.series(
                    [
                        ...
                        next =>
                            s3
                                .send(new PutBucketVersioningCommand(sParams))
                                .then(() => next())
                                .catch(next),
                        next => putObjectVersion(s3, params, vId, next),
                        next =>
                ...
                it(`should update restore metadata while keeping storage class (${versioning})`, done => {
                    ...
                    async.series(
                        [
                            ...
                            next =>
                                s3
                                    .send(new ListObjectsCommand({ Bucket: bucketName }))
                                    .then(res => {
            ...
            it('should "copy" all but non data-related metadata (data encryption, data size...)', done => {
                ...
                async.series(
                    [
                        ...
                        next =>
                            getMetadata(bucketNameMD, objectName, undefined, (err, objMD) => {
                                if (err) {
                                    return next(err);
                                }
                                /* eslint-disable no-param-reassign */
            ...
            it('should keep x-amz-meta-scal-version-id when restoring on ingestion bucket', async () => {
                ...
                try {
                    ...
                    const restoredObjMD = await getMetadata(ingestionBucketName, objectName, putVersionId);

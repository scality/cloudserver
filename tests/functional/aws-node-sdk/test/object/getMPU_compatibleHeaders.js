...
describe('GET multipart upload object [Cache-Control, Content-Disposition, Content-Encoding, Expires headers]', () => {
    withV4(sigCfg => {
        let bucketUtil;
        let s3;
        let uploadId;
        ...
        it(
            'should return additional headers when get request is performed ' +
                'on MPU, when they are specified in creation of MPU',
            () => {
                const params = { Bucket: bucketName, Key: 'key', PartNumber: 1, UploadId: uploadId };
                return s3
                    ...
                    .then(res => {
                        ...
                        return s3.send(
                            new CompleteMultipartUploadCommand({
                                ...
                                UploadId: uploadId,
                                MultipartUpload: {
                                    Parts: [{ ETag: res.ETag, PartNumber: 1 }],
                                },
                            }),
                        );
                    })

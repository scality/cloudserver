    ...
    withV4(sigCfg => {
        ...
        it('should put an object and set the acl via query param', done => {
            ...
            provideRawOutput(['-verbose', '-X', 'PUT', url, '--upload-file', 'uploadFile'], httpCode => {
                ...
                s3.getObjectAcl({ Bucket: bucket, Key: 'key' }, (err, result) => {
        ...
        describe('Put object with tag set', () => {
            taggingTests.forEach(taggingTest => {
                it(taggingTest.it, done => {
                    ...
                    s3.putObject(params, err => {
                        ...
                        assert.equal(err, null, 'Expected success, ' + `got error ${JSON.stringify(err)}`);
                        return s3.getObjectTagging({ Bucket: bucket, Key: object }, (err, data) => {
                            assert.equal(err, null, 'Expected success, ' + `got error ${JSON.stringify(err)}`);
            ...
            it('should be able to put object with 10 tags', done => {
                ...
                s3.putObject({ Bucket: bucket, Key: object, Tagging: taggingConfig }, err => {
                    assert.equal(err, null, 'Expected success, ' + `got error ${JSON.stringify(err)}`);
                    done();
                });
            });
            ...
            it('should be able to put an empty Tag set', done => {
                s3.putObject({ Bucket: bucket, Key: object, Tagging: '' }, err => {
                    assert.equal(err, null, 'Expected success, ' + `got error ${JSON.stringify(err)}`);
                    done();
                });
            });
            ...
            it('should be able to put object with empty tags', done => {
                s3.putObject({ Bucket: bucket, Key: object, Tagging: '&&&&&&&&&&&&&&&&&key1=value1' }, err => {
                    assert.equal(err, null, 'Expected success, ' + `got error ${JSON.stringify(err)}`);
                    done();
                });
            });
            ...
            it('should allow putting 50 tags', done => {
                ...
                s3.putObject({ Bucket: bucket, Key: object, Tagging: taggingConfig }, done);
            });
            ...
            it('should return InvalidArgument if using the same key twice', done => {
                s3.putObject({ Bucket: bucket, Key: object, Tagging: 'key1=value1&key1=value2' }, err => {
            ...
            it('should return InvalidArgument if using the same key twice ' + 'and empty tags', done => {
                s3.putObject(
                    { Bucket: bucket, Key: object, Tagging: '&&&&&&&&&&&&&&&&&key1=value1&key1=value2' },
                    err => {
                        _checkError(err, 'InvalidArgument', 400);
                        done();
                    }
            ...
            it('should return InvalidArgument if tag with no key', done => {
                s3.putObject({ Bucket: bucket, Key: object, Tagging: '=value1' }, err => {
                    _checkError(err, 'InvalidArgument', 400);
                    done();
                });
            });
            ...
            it('should return InvalidArgument putting object tag with ' + 'invalid characters: %', done => {
                const value = 'value1%';
                s3.putObject({ Bucket: bucket, Key: object, Tagging: `key1=${value}` }, err => {
                    _checkError(err, 'InvalidArgument', 400);
                    done();
                });
            });
...
describeSkipIfCeph('PUT object with object lock', () => {
    withV4(sigCfg => {
        ...
        it('should error with invalid legal hold status', done => {
            ...
            s3.putObject(params, err => {
                assert.strictEqual(err.code, 'InvalidArgument');
                assert.strictEqual(err.message, 'Legal hold status must be one of "ON", "OFF"');
                done();
            });
        });

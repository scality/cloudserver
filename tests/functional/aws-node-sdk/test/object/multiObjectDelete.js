...
describe('Multi-Object Delete Success', function success() {
    ...
    it('should batch delete 1000 objects', done => {
        ...
        s3.send(
            ...
            .then(res => {
                if (this.httpResponse?.body?.toString().indexOf('<?xml version="1.0"') === -1) {
                    return done('S3C-2642: should have included xml declaration');
                }
                assert.strictEqual(res.Deleted.length, 1000);
                assert.deepStrictEqual(
                    sortList(res.Deleted.map(obj => obj.Key)),
                    sortList(objects.map(obj => obj.Key)),
                );
                return done();
            })
            .catch(err => done(err));
...
describe('Multi-Object Delete Error Responses', () => {
    withV4(sigCfg => {
        ...
        beforeEach(() => {
            ...
            s3 = bucketUtil.s3;
            return s3.send(new CreateBucketCommand({ Bucket: bucketName })).catch(err => {
    ...
    before(() => {
        ...
        return s3
            ...
            .then(() => {
                ...
                return Promise.all(createObjects).catch(err => {
                    process.stdout.write(`Error creating objects: ${err}\n`);
                    throw err;
                });
            });

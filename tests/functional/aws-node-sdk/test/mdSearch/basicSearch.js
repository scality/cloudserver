...
runIfMongo('Basic search', () => {
    ...
    before(done => {
        s3Client.createBucket({ Bucket: bucketName }, err => {
            ...
            return s3Client.putObject(
                { Bucket: bucketName, Key: objectKey, Metadata: userMetadata, Tagging: objectTagData },
                err => {
                    if (err) {
                        return done(err);
                    }
                    return s3Client.putObject({ Bucket: bucketName, Key: hiddenKey, Tagging: hiddenTagData }, done);
                }
            );
        });
    });
    ...
    after(done => {
        s3Client.deleteObjects(
            { Bucket: bucketName, Delete: { Objects: [{ Key: objectKey }, { Key: hiddenKey }] } },
            err => {
                if (err) {
                    return done(err);
                }
    ...
    it('should list object with regex searched for system metadata with flags', done => {
    ...
    it('should return empty when no object match regex', done => {
        ...
        return runAndCheckSearch(s3Client, bucketName, encodedSearch, false, null, done);
    });

    it('should list object with searched for user metadata', done => {
        const encodedSearch = encodeURIComponent(`x-amz-meta-food="${userMetadata.food}"`);
        return runAndCheckSearch(s3Client, bucketName, encodedSearch, false, objectKey, done);
    });

    it('should list object with searched for tag metadata', done => {
        const encodedSearch = encodeURIComponent('tags.item-type="main"');
        return runAndCheckSearch(s3Client, bucketName, encodedSearch, false, objectKey, done);
    });

    it('should return empty listing when no object has user md', done => {
        const encodedSearch = encodeURIComponent('x-amz-meta-food="nosuchfood"');
        return runAndCheckSearch(s3Client, bucketName, encodedSearch, false, null, done);
    });

    describe('search when overwrite object', () => {
        before(done => {
            s3Client.putObject({ Bucket: bucketName, Key: objectKey, Metadata: updatedUserMetadata }, done);
        });
...
runIfMongo('Invalid regular expression searches', () => {
    ...
    it('should return error if pattern is invalid', done => {
        ...
        return runAndCheckSearch(s3Client, bucketName, encodedSearch, false, testError, done);
    });
});

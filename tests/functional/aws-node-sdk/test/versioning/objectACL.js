    ...
    putAndGetAcl(cannedAcl, versionId, expected, cb) {
        ...
        this.putObjectAcl(params, (err, data) => {
            if (expected.error) {
                ...
                assert.strictEqual(expected.error.statusCode, err.statusCode);
            } else {
                _Utils.assertNoError(err, `putting object acl with version id: ${versionId}`);
                assert.strictEqual(
                    data.VersionId,
                    expected.versionId,
            ...
            this.getObjectAcl(params, (err, data) => {
                if (expected.error) {
                    ...
                    _Utils.assertNoError(err, `getting object acl with version id: ${versionId}`);
                    assert.strictEqual(
                        data.VersionId,
                        expected.versionId,
                        `expected version id '${expected.versionId}' in ` +
...
function _testBehaviorVersioningEnabledOrSuspended(utils, versionIds) {
    ...
    it(
        'should return 405 MethodNotAllowed putting acl without ' + 'version id if latest version is a delete marker',
            ...
            s3.deleteObject({ Bucket: bucket, Key: key }, (err, data) => {
                assert.strictEqual(err, null, `Unexpected err deleting object: ${err}`);
    ...
    it('should return 404 NoSuchKey getting acl without ' + 'version id if latest version is a delete marker', done => {
        const aclParams = {
            Bucket: bucket,
            Key: key,
        };
    ...
    it(
        'non-version specific put and get ACL should target latest ' +
            'version AND return version ID in response headers',
        done => {
            const latestVersion = versionIds[versionIds.length - 1];
    ...
    it('version specific put and get ACL should return version ID ' + 'in response headers', done => {
    ...
    it(
        'version specific put and get ACL (version id = "null") ' +
            'should return version ID ("null") in response headers',
        done => {
...
describe('versioned put and get object acl ::', () => {
    withV4(sigCfg => {
        ...
        describe('in bucket w/o versioning cfg :: ', () => {
            ...
            it('should not return version id for non-version specific ' + 'put and get ACL', done => {
                const expectedRes = { versionId: undefined };
                utils.putAndGetAcl('public-read', undefined, expectedRes, done);
            });

            it('should not return version id for version specific ' + 'put and get ACL (version id = "null")', done => {
                const expectedRes = { versionId: undefined };
                utils.putAndGetAcl('public-read', 'null', expectedRes, done);
            });

            it('should return NoSuchVersion if attempting to put or get acl ' + 'for non-existing version', done => {
                const error = { code: 'NoSuchVersion', statusCode: 404 };
                utils.putAndGetAcl('private', nonExistingId, { error }, done);
            });

            it('should return InvalidArgument if attempting to put/get acl ' + 'for invalid hex string', done => {
        ...
        describe('on a version-enabled bucket with non-versioned object :: ', () => {
            ...
            describe('before putting new versions :: ', () => {
                it(
                    'non-version specific put and get ACL should now ' +
                        'return version ID ("null") in response headers',
                    done => {
                        const expectedRes = { versionId: 'null' };
                        utils.putAndGetAcl('public-read', undefined, expectedRes, done);
                    }
        ...
        describe('on version-suspended bucket with non-versioned object :: ', () => {
            ...
            describe('before putting new versions :: ', () => {
                it(
                    'non-version specific put and get ACL should still ' +
                        'return version ID ("null") in response headers',
                    done => {
                        const expectedRes = { versionId: 'null' };
                        utils.putAndGetAcl('public-read', undefined, expectedRes, done);

...
function checkNoError(err) {
    assert.strictEqual(err, null, `Error occurred ${err}`);
}
...
describe('Multi-Object Versioning Delete Success', function success() {
    ...
    withV4(sigCfg => {
        ...
        afterEach(async () => {
            const err = await removeAllVersions({ Bucket: bucketName });
            if (err) {
                throw err;
            }
            try {
                ...
                throw new Error(`Error deleting bucket: ${error}`);
            }
        });
...
() => {
    withV4(sigCfg => {
        ...
        'to deleteVersionId', done => {
            async.waterfall([
                next => s3.send(new PutObjectCommand({ Bucket: bucketName, Key: key }),
                  err => next(err)),
      ...
      'stored for the object if trying to delete an object that does not exist',
            ...
            s3.send(new DeleteObjectsCommand({ Bucket: bucketName,
                ...
                assert.strictEqual(data.Deleted[0].DeleteMarker, true);
                return s3.send(new ListObjectVersionsCommand({ Bucket: bucketName }),
                  (err, data) => {
                      if (err) {
                          return done(err);
                      }
        ...
        'stored for the object if object exists but no version was specified',
            ...
            async.waterfall([
                ...
                (versionId, next) => s3.send(new DeleteObjectsCommand({ Bucket:
                  bucketName,
                    Delete: {
                        Objects: [
                            {

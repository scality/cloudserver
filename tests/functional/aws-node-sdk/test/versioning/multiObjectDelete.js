...
describe('Multi-Object Versioning Delete Success', function success() {
    ...
    withV4(sigCfg => {
        ...
        afterEach(done => {
            removeAllVersions({ Bucket: bucketName }, err => {
                ...
                return s3.deleteBucket({ Bucket: bucketName }, err => {
                    assert.strictEqual(err, null, `Error deleting bucket: ${err}`);
                    return done();
                });
            });
        });
        ...
        it('should return NoSuchVersion in errors if one versionId is ' + 'invalid', () => {
            const objects = objectsRes.slice(0, 1000).map(obj => ({ Key: obj.Key, VersionId: obj.VersionId }));
        ...
        it(
            'should not send back any error if a versionId does not exist ' +
            ...
            () => {
                ...
                return s3
                    ...
                    .then(res => {
                        assert.strictEqual(res.Deleted.length, 1000);
                        assert.strictEqual(res.Errors.length, 0);
                        const foundVersionId = res.Deleted.find(entry => entry.VersionId === nonExistingId);
...
describe('Multi-Object Versioning Delete - deleting delete marker', () => {
    withV4(sigCfg => {
        ...
        it('should send back VersionId and DeleteMarkerVersionId both equal ' + 'to deleteVersionId', done => {
            async.waterfall(
                [
                    next => s3.putObject({ Bucket: bucketName, Key: key }, err => next(err)),
                    next =>
                        s3.deleteObject({ Bucket: bucketName, Key: key }, (err, data) => {
                            const deleteVersionId = data.VersionId;
                            next(err, deleteVersionId);
                        }),
                    (deleteVersionId, next) =>
                        s3.deleteObjects(
                            {
                                Bucket: bucketName,
                                Delete: {
                            ...
                            (err, data) => {
                                ...
                                assert.strictEqual(data.Deleted[0].VersionId, deleteVersionId);
                                assert.strictEqual(data.Deleted[0].DeleteMarkerVersionId, deleteVersionId);
                                next(err);
                            }
                        ),
                ],
        ...
        it(
            'should send back a DeleteMarkerVersionId matching the versionId ' +
                ...
                s3.deleteObjects(
                    ...
                    (err, data) => {
                        ...
                        const versionIdFromDeleteObjects = data.Deleted[0].DeleteMarkerVersionId;
                        assert.strictEqual(data.Deleted[0].DeleteMarker, true);
                        return s3.listObjectVersions({ Bucket: bucketName }, (err, data) => {
                            if (err) {
                                return done(err);
                            }
                            const versionIdFromListObjectVersions = data.DeleteMarkers[0].VersionId;
        ...
        it(
            'should send back a DeleteMarkerVersionId matching the versionId ' +
                ...
                async.waterfall(
                    [
                        next =>
                            s3.putObject({ Bucket: bucketName, Key: key }, (err, data) => {
                        ...
                        (versionId, next) =>
                            s3.deleteObjects(
                                ...
                                (err, data) => {
                                    ...
                                    assert.strictEqual(data.Deleted[0].DeleteMarker, true);
                                    const deleteVersionId = data.Deleted[0].DeleteMarkerVersionId;
                                    assert.notEqual(deleteVersionId, versionId);
                                    return next(err, deleteVersionId, versionId);
                                }
                            ),
                        (deleteVersionId, versionId, next) =>
                            s3.listObjectVersions({ Bucket: bucketName }, (err, data) => {
                                if (err) {
                                    return next(err);
                                }
                                assert.strictEqual(deleteVersionId, data.DeleteMarkers[0].VersionId);
                                assert.strictEqual(versionId, data.Versions[0].VersionId);
                                return next();
                            }),
                    ],
                    err => done(err)

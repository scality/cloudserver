...
() => {
    withV4(sigCfg => {
        ...
        beforeEach(done => {
            async.waterfall([
                next => s3.send(new CreateBucketCommand({ Bucket: bucketName })).then(() => next()).catch(err => next(err)),
                next => s3.send(new PutBucketVersioningCommand({
                    Bucket: bucketName,
                    VersioningConfiguration: {
                        Status: 'Enabled',
        ...
        'to deleteVersionId', async () => {
            await async.waterfall([
                next => s3.send(new PutObjectCommand({ Bucket: bucketName, Key: key })).then(() => next()).catch(err => next(err)),

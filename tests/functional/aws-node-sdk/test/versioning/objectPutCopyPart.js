    ...
    withV4(sigCfg => {
        ...
        describe('on bucket without versioning', () => {
            ...
            beforeEach(done => {
                async.waterfall(
                    [
                        next => s3.putObject({ Bucket: sourceBucket, Key: sourceKey, Body: 'foobar' }, next),
                        (data, next) => {
                            eTags.push(data.ETag);
                            s3.createMultipartUpload({ Bucket: destBucket, Key: destKey }, next);
                        },
                    ],
                    (err, data) => {
                        if (err) {
            ...
            it(
                'should allow specific version "null" for copy source ' +
                    'and return version id "null" in response headers',
                done => {
                    s3.uploadPartCopy(
                        {
                            Bucket: destBucket,
                        ...
                        (err, data) => {
                            _assertNoError(err, 'using specific version "null" for copy source');
        ...
        describe('on bucket with versioning', () => {
                    ...
                    [
                        ...
                        next =>
                            async.timesSeries(
                                ...
                                (i, cb) =>
                                    s3.putObject(
                                        { Bucket: sourceBucket, Key: sourceKey, Body: `foo${i}` },
                                        (err, data) => {
                        ...
                        next => s3.createMultipartUpload({ Bucket: destBucket, Key: destKey }, next),
                    ],
                    (err, data) => {
                        if (err) {
                            return done(err);
            ...
            it(
                'copy part without specifying version should return NoSuchKey ' +
                    'if latest version has a delete marker',
                done => {
            ...
            it(
                'copy part with specific version id should return ' + 'InvalidRequest if that id is a delete marker',
                done => {
                    async.waterfall(
                        [
                            next =>
                            ...
                            (data, next) => {
                                ...
                                return s3.uploadPartCopy(
                                    {
                                        Bucket: destBucket,
                                        CopySource: `${sourceBucket}/${sourceKey}` + `?versionId=${deleteMarkerId}`,
                                        Key: destKey,
                                        PartNumber: 1,
                                        UploadId: uploadId,
                                    },
            ...
            it('copy part with specific version should return NoSuchVersion ' + 'if version does not exist', done => {
                ...
                s3.deleteObject({ Bucket: sourceBucket, Key: sourceKey, VersionId: versionId }, (err, data) => {
            ...
            it(
                'copy part with specific version "null" should return copy ' + 'source version id "null" if it exists',
                done => {
                    s3.uploadPartCopy(
                        {
                            Bucket: destBucket,
        ...
        describe('on bucket with versioning suspended', () => {
            ...
            it(
                'copy part with specific version "null" should still return ' +
                    'copy source version id "null" if it exists',
                done => {
                    s3.uploadPartCopy(
                        {
                            Bucket: destBucket,

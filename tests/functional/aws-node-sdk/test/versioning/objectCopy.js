    ...
    withV4(sigCfg => {
        ...
        beforeEach(() =>
            bucketUtil
                ...
                .then(() =>
                    s3.putObject({ Bucket: sourceBucketName, Key: sourceObjName, Body: secondContent }).promise()
                )
        );
        ...
        function successCopyCheck(error, response, copyVersionMetadata, destBucketName, destObjName, done) {
            checkNoError(error);
            ...
            s3.getObject({ Bucket: destBucketName, Key: destObjName }, (err, res) => {
                checkNoError(err);
        ...
        function checkSuccessTagging(key, value, cb) {
            s3.getObjectTagging({ Bucket: destBucketName, Key: destObjName }, (err, data) => {
                checkNoError(err);
        ...
        it(
            'should copy an object from a source bucket to a different ' +
                ...
                s3.copyObject({ Bucket: destBucketName, Key: destObjName, CopySource: copySource }, err => {
                    checkNoError(err);
        ...
        it(
            'should copy an object from a source bucket to a different ' +
                ...
                s3.copyObject(
                    { Bucket: destBucketName, Key: destObjName, CopySource: copySource, TaggingDirective: 'COPY' },
                    err => {
                        checkNoError(err);
                        checkSuccessTagging(originalTagKey, originalTagValue, done);
                    }
        ...
        it('should return InvalidArgument for a request with versionId query', done => {
        ...
        it('should return InvalidArgument for a request with empty string ' + 'versionId query', done => {
            const params = { Bucket: destBucketName, Key: destObjName, CopySource: copySource };
        ...
        it(
            'should copy a version from a source bucket to a different ' +
                ...
                s3.copyObject({ Bucket: destBucketName, Key: destObjName, CopySource: copySource }, (err, res) =>
                    successCopyCheck(err, res, originalMetadata, destBucketName, destObjName, done)
                );
            }
        );
        ...
        it('should copy an object from a source bucket to a different ' + 'key in the same bucket', done => {
            s3.copyObject({ Bucket: sourceBucketName, Key: destObjName, CopySource: copySource }, (err, res) =>
                successCopyCheck(err, res, originalMetadata, sourceBucketName, destObjName, done)
            );
        });

        it('should copy an object from a source to the same destination ' + '(update metadata)', done => {
            s3.copyObject(
                {
                    Bucket: sourceBucketName,
                    Key: sourceObjName,
                ...
                (err, res) => successCopyCheck(err, res, newMetadata, sourceBucketName, sourceObjName, done)
            );
        });
        ...
        it(
            'should copy an object and replace the metadata if replace ' + 'included as metadata directive header',
                ...
                s3.copyObject(
                    ...
                    (err, res) => successCopyCheck(err, res, newMetadata, destBucketName, destObjName, done)
                );
            }
        );
        ...
        it(
            'should copy an object and keep ContentType if replace ' +
                ...
                s3.copyObject(
                    { Bucket: destBucketName, Key: destObjName, CopySource: copySource, MetadataDirective: 'REPLACE' },
                    () => {
                        s3.getObject({ Bucket: destBucketName, Key: destObjName }, (err, res) => {
                            if (err) {
                                return done(err);
        ...
        it('should copy a 0 byte object to different destination', done => {
            ...
            s3.putObject(
                { Bucket: sourceBucketName, Key: sourceObjName, Body: '', Metadata: originalMetadata },
                (err, res) => {
                    checkNoError(err);
                    copySource = `${sourceBucketName}/${sourceObjName}` + `?versionId=${res.VersionId}`;
                    s3.copyObject({ Bucket: destBucketName, Key: destObjName, CopySource: copySource }, (err, res) => {
                        checkNoError(err);
        ...
        it(
            'should copy an object to a new bucket and overwrite an already ' +
                ...
                s3.putObject(
                    { Bucket: destBucketName, Key: destObjName, Body: 'overwrite me', Metadata: originalMetadata },
                    err => {
                        checkNoError(err);
                        s3.copyObject(
                            {
        ...
        it('should copy an object and set the acl on the new object', done => {
            s3.copyObject(
                { Bucket: destBucketName, Key: destObjName, CopySource: copySource, ACL: 'authenticated-read' },
                err => {
                    checkNoError(err);
                    s3.getObjectAcl({ Bucket: destBucketName, Key: destObjName }, (err, res) => {
        ...
        it(
            'should copy an object and default the acl on the new object ' +
                ...
                s3.putObjectAcl({ Bucket: sourceBucketName, Key: sourceObjName, ACL: 'authenticated-read' }, () => {
                    s3.copyObject({ Bucket: destBucketName, Key: destObjName, CopySource: copySource }, () => {
                        s3.getObjectAcl({ Bucket: destBucketName, Key: destObjName }, (err, res) => {
        ...
        it('should copy a version to same object name to restore ' + 'version of object', done => {
            s3.copyObject({ Bucket: sourceBucketName, Key: sourceObjName, CopySource: copySource }, (err, res) =>
                successCopyCheck(err, res, originalMetadata, sourceBucketName, sourceObjName, done)
            );
        });

        it('should return an error if attempt to copy from nonexistent bucket', done => {
            s3.copyObject(
                { Bucket: destBucketName, Key: destObjName, CopySource: `nobucket453234/${sourceObjName}` },
                err => {
        ...
        it('should return an error if attempt to copy to nonexistent bucket', done => {
            s3.copyObject(
                { Bucket: 'nobucket453234', Key: destObjName, CopySource: `${sourceBucketName}/${sourceObjName}` },
                err => {
                    checkError(err, 'NoSuchBucket');
                    done();
                }
        ...
        it('should return an error if attempt to copy nonexistent object', done => {
            s3.copyObject(
                { Bucket: destBucketName, Key: destObjName, CopySource: `${sourceBucketName}/nokey` },
                err => {
        ...
        it('should return NoSuchKey if attempt to copy version with ' + 'delete marker', done => {
            s3.deleteObject(
                {
                    Bucket: sourceBucketName,
                    Key: sourceObjName,
        ...
        it('should return InvalidRequest if attempt to copy specific ' + 'version that is a delete marker', done => {
            s3.deleteObject(
                {
                    Bucket: sourceBucketName,
                    Key: sourceObjName,
        ...
        it('should return an error if send invalid metadata directive header', done => {
            s3.copyObject(
                { Bucket: destBucketName, Key: destObjName, CopySource: copySource, MetadataDirective: 'copyHalf' },
                err => {
        ...
        describe('copying by another account', () => {
            ...
            it(
                'should not allow an account without read persmission on the ' + 'source object to copy the object',
                    ...
                    otherAccountS3.copyObject(
                        { Bucket: otherAccountBucket, Key: otherAccountKey, CopySource: copySource },
                        err => {
            ...
            it(
                'should allow an account with read permission on the ' +
                    ...
                    s3.putObjectAcl(
                        { Bucket: sourceBucketName, Key: sourceObjName, ACL: 'public-read', VersionId: versionId },
                        () => {
                            otherAccountS3.copyObject(
                                { Bucket: otherAccountBucket, Key: otherAccountKey, CopySource: copySource },
                                err => {
        ...
        it(
            'If-None-Match & If-Modified-Since: returns NotModified when Etag ' +

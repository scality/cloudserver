    ...
    withV4(sigCfg => {
        ...
        if (constants.validStorageClasses.includes('REDUCED_REDUNDANCY')) {
            it('should copy a 0 byte object to same destination', async () => {
                const emptyFileETag = '"d41d8cd98f00b204e9800998ecf8427e"';
                const putRes = await s3.send(
                    new PutObjectCommand({ Bucket: sourceBucketName, Key: sourceObjName, Body: '' }),
                );
                copySource = `${sourceBucketName}/${sourceObjName}?versionId=${putRes.VersionId}`;
                const copyRes = await s3.send(
                    new CopyObjectCommand({
                ...
                const getRes = await s3.send(new GetObjectCommand({ Bucket: sourceBucketName, Key: sourceObjName }));
        ...
        it('should copy an object and set the acl on the new object', async () => {
            ...
            const res = await s3.send(new GetObjectAclCommand({ Bucket: destBucketName, Key: destObjName }));
            ...
            assert.strictEqual(res.Grants[1].Grantee.URI, 'http://acs.amazonaws.com/groups/global/AuthenticatedUsers');
        });
        ...
        it('should return NoSuchKey if attempt to copy version with delete marker', async () => {
            const delRes = await s3.send(new DeleteObjectCommand({ Bucket: sourceBucketName, Key: sourceObjName }));
        ...
        describe('copying by another account', () => {
            ...
            it(
                ...
                async () => {
                    await otherAccountS3.send(
                        new PutObjectCommand({ Bucket: otherAccountBucket, Key: otherAccountKey, Body: '' }),
                    );
                    try {
                        await otherAccountS3.send(
                            new CopyObjectCommand({

...
const { versioningSuspended, versioningEnabled, removeAllVersions } = require('../../lib/utility/versioning-util.js');
...
describe('delete marker creation in bucket with null version', () => {
    withV4(sigCfg => {
                ...
                [
                    ...
                    callback =>
                        s3.listObjectVersions({ Bucket: bucket }, (err, data) => {
                            ...
                            assert.strictEqual(data.Versions[0].VersionId, 'null');
                            return callback();
                        }),
                    callback =>
                    ...
                    (deleteMarkerVerId, callback) =>
                        s3.listObjectVersions({ Bucket: bucket }, (err, data) => {
                            ...
                            assert.strictEqual(data.Versions.length, 1);
                            assert.strictEqual(data.Versions[0].VersionId, 'null');
                            assert.strictEqual(data.DeleteMarkers[0].VersionId, deleteMarkerVerId);
                            return callback();
                        }),
                ],
                done
        ...
        it('delete marker overwrites null version if versioning suspended', done => {
            async.waterfall(
                [
                    callback =>
                        s3.putBucketVersioning(
...
describe('aws-node-sdk test delete object', () => {
    withV4(sigCfg => {
        ...
        it('delete non existent object should not create a delete marker', done => {
            s3.deleteObject(
                {
                    Bucket: bucket,
                    Key: `${key}000`,
        ...
        it('delete in non-versioned bucket should not create delete marker', done => {
            s3.putObject(
                {
                    Bucket: bucket,
                    Key: key,
        ...
        it('should not send back error for non-existing key (specific version)', done => {
            s3.deleteObject(
                {
                    Bucket: bucket,
                    Key: `${key}3`,
        ...
        it('delete non existent version should not create delete marker', done => {
            s3.deleteObject(
                {
                    Bucket: bucket,
                    Key: key,
                ...
                (err, res) => {
                    ...
                    return s3.getObject(
                        ...
                        (err, res) => {
                            ...
                            assert.strictEqual(res.VersionId, versionIds[versionIds.length - 1]);
                            return done();
                        }
                    );
                }

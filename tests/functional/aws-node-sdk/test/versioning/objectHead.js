        ...
        afterEach(done => {
            removeAllVersions({ Bucket: bucket }, err => {
                ...
                return s3.deleteBucket({ Bucket: bucket }, deleteErr => {
                    ...
                    console.log('[DEBUG objectHead afterEach] deleteBucket failed:', deleteErr.code);
                    return setTimeout(() => {
                        s3.listObjectVersions({ Bucket: bucket }, (listErr, data) => {
                            if (listErr) {
                                // eslint-disable-next-line no-console
                                console.log('[DEBUG objectHead afterEach] listObjectVersions error:', listErr);
                            } else {
                                // eslint-disable-next-line no-console
                                console.log('[DEBUG objectHead afterEach] after 20s:', JSON.stringify({
                                    bucket,

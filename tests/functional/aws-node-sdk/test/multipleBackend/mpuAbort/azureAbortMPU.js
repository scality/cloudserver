...
describeSkipIfNotMultipleOrCeph('Abort MPU on Azure data backend', function describeF() {
    this.timeout(50000);
    withV4(sigCfg => {
        ...
        describe('with bucket location header', () => {
            ...
            afterEach(done => s3.deleteBucket({ Bucket: azureContainerName }, done));
            ...
            it('should abort an MPU with one empty part ', function itFn(done) {
                ...
                async.waterfall(
                    [
                        next => {
                            const partParams = Object.assign({ PartNumber: 1 }, params);
                        ...
                        next => azureCheck(azureContainerName, this.test.key, expected, next),
                    ],
                    done
                );
            });

            it('should abort MPU with one part bigger than max subpart', function itFn(done) {
                const expected = { error: true };
                const params = {
                    Bucket: azureContainerName,
                    Key: this.test.key,

...
describeSkipIfNotMultiple('AWS backend object delete tagging with versioning ', function testSuite() {
    this.timeout(120000);
    ...
    withV4(sigCfg => {
        ...
        it(
            'versioning not configured: should delete a tag set on the ' + 'latest version if no version is specified',
            done => {
                ...
                async.waterfall(
                    [
                        ...
                        (versionId, next) => delTaggingAndAssert(s3, { bucket, key, expectedVersionId: false }, next),
        ...
        it('versioning not configured: should delete a tag set on the ' + 'version if specified (null)', done => {
            ...
            async.waterfall(
                [
                    ...
                    (versionId, next) =>
                        delTaggingAndAssert(s3, { bucket, key, versionId: 'null', expectedVersionId: false }, next),
                    next => awsGetAssertTags({ key, expectedTags: {} }, next),
                ],
                done
            );
        ...
        it('versioning suspended: should delete a tag set on a specific ' + 'version (null)', done => {
            ...
            async.waterfall(
                [
                    next => putNullVersionsToAws(s3, bucket, key, [undefined], next),
                    (versionIds, next) =>
                        putTaggingAndAssert(
                            s3,
                            { bucket, key, tags, versionId: 'null', expectedVersionId: 'null' },
                            next
                        ),
                    (versionId, next) =>
                        delTaggingAndAssert(
                            s3,
                            { bucket, key, versionId: 'null', expectedTags: tags, expectedVersionId: 'null' },
                            next
                        ),
                    next => awsGetAssertTags({ key, expectedTags: {} }, next),
                ],
        ...
        it(
            'versioning enabled then suspended: should delete a tag set on ' +
                'a specific (non-null) version if specified',
            done => {
                const key = `somekey-${genUniqID()}`;
                async.waterfall(
                    [
                        ...
                        (s3Vid, awsVid, next) =>
                            putNullVersionsToAws(s3, bucket, key, [someBody], () => next(null, s3Vid, awsVid)),
                        (s3Vid, awsVid, next) =>
                            putTaggingAndAssert(
                                s3,
            ...
            async.waterfall(
                [
                    ...
                    (putData, next) =>
                        putTaggingAndAssert(
                            s3,
                            { bucket, key, tags, versionId: putData.VersionId, expectedVersionId: putData.VersionId },
                            next
                        ),
                    (versionId, next) =>
                        delTaggingAndAssert(s3, { bucket, key, versionId, expectedVersionId: versionId }, next),
                    next => awsGetAssertTags({ key, expectedTags: {} }, next),
                ],
                done
            );
            ...
            'versioning enabled: should delete a tag set on a specific ' + 'version that is not the latest version',
                ...
                async.waterfall(
                    [
                        ...
                        // put another version
                        (s3Vid, awsVid, next) =>
                            s3.putObject({ Bucket: bucket, Key: key, Body: someBody }, err => next(err, s3Vid, awsVid)),
                        (s3Vid, awsVid, next) =>
                            putTaggingAndAssert(
                                s3,
                                { bucket, key, tags, versionId: s3Vid, expectedVersionId: s3Vid },
            ...
            'versioning suspended then enabled: should delete a tag set on ' + 'a specific version (null) if specified',
                ...
                async.waterfall(
                    [
                        ...
                        (awsVid, next) => putVersionsToAws(s3, bucket, key, [someBody], () => next(null, awsVid)),
                        (awsVid, next) =>
                            putTaggingAndAssert(
                                s3,
                                { bucket, key, tags, versionId: 'null', expectedVersionId: 'null' },
                        ...
                        (awsVid, next) =>
                            delTaggingAndAssert(s3, { bucket, key, versionId: 'null', expectedVersionId: 'null' }, () =>
                                next(null, awsVid)
                            ),
                        (awsVid, next) => awsGetAssertTags({ key, versionId: awsVid, expectedTags: {} }, next),
                    ],
        ...
        it(
            'should return an ServiceUnavailable if trying to delete ' +
                ...
                async.waterfall(
                    [
                        ...
                        (awsVid, next) => awsS3.deleteObject({ Bucket: awsBucket, Key: key, VersionId: awsVid }, next),
                        (delData, next) =>
                            delTaggingAndAssert(s3, { bucket, key, expectedError: 'ServiceUnavailable' }, next),
                    ],
                    done
                );
            }
                ...
                async.waterfall(
                    [
                        ...
                        (s3Vid, next) =>
                            delTaggingAndAssert(
                                s3,
                                { bucket, key, versionId: s3Vid, expectedError: 'ServiceUnavailable' },
                                next
                            ),
                    ],
                    done

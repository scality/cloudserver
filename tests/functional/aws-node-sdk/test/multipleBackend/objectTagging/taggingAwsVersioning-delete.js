...
describeSkipIfNotMultiple('AWS backend object delete tagging with versioning ', function testSuite() {
    ...
    withV4(sigCfg => {
        ...
        it('versioning not configured: should delete a tag set on the latest version if no version is specified', done => {
        ...
        it('versioning suspended then enabled: should delete a tag set on a specific version (null) if specified', done => {
            const key = `somekey-${genUniqID()}`;
            async.waterfall(
                [
                    next => putNullVersionsToAws(s3, bucket, key, [undefined], () => next()),
            ...
            'should return an ServiceUnavailable if trying to delete ' +
                ...
                async.waterfall(
                    [
                        ...
                        (awsVid, next) =>
                            awsS3
                                .send(new DeleteObjectCommand({ Bucket: awsBucket, Key: key, VersionId: awsVid }))

...
const { putTaggingAndAssert, getTaggingAndAssert, delTaggingAndAssert, awsGetAssertTags } = tagging;
const bucket = `awsversioningtag${genUniqID()}`;
const someBody = 'teststring';

describeSkipIfNotMultiple('AWS backend object put/get tagging with versioning', function testSuite() {
    this.timeout(120000);
    ...
    withV4(sigCfg => {
        ...
        it(
            'versioning enabled then suspended: should put/get a tag set on ' +
            ...
            async.waterfall(
                [
                    ...
                    (versionId, next) =>
                        getTaggingAndAssert(
                            s3,
                            { bucket, key, versionId, expectedTags: tags, expectedVersionId: versionId },
                            next
                        ),
                    (versionId, next) => awsGetAssertTags({ key, expectedTags: tags }, next),
                ],
        ...
        it(
            'versioning suspended then enabled: should put/get a tag set on ' +
                'a specific version (null) if specified',
            done => {
                const key = `somekey-${genUniqID()}`;
                async.waterfall(
                    [
            ...
            'should return an ServiceUnavailable if trying to put ' +
                ...
                async.waterfall(
                    [
                        ...
                        (awsVid, next) => awsS3.deleteObject({ Bucket: awsBucket, Key: key, VersionId: awsVid }, next),
                        (delData, next) =>
                            putTaggingAndAssert(s3, { bucket, key, tags, expectedError: 'ServiceUnavailable' }, next),
                    ],
                    done
                );
            }
                ...
                async.waterfall(
                    [
                        ...
                        (s3Vid, next) =>
                            putTaggingAndAssert(
                                s3,
                                { bucket, key, tags, versionId: s3Vid, expectedError: 'ServiceUnavailable' },
                                next
                            ),
                    ],
                    done

...
const [nonVersionedDelete, newDeleteMarker, deleteVersion, deleteDeleteMarker] = Object.keys(_deleteResultSchema);
...
function _assertDeleteResult(result, resultType, requestVersionId) {
    ...
    const [expectVersionId, matchReqVersionId, expectDeleteMarker] = _deleteResultSchema[resultType];
...
function delAndAssertResult(s3, params, cb) {
    ...
    return s3.deleteObject({ Bucket: bucket, Key: key, VersionId: versionId }, (err, result) => {
        if (resultError) {
        ...
        assert.strictEqual(err, null, 'Expected success ' + `deleting object, got error ${err}`);
...
function _createDeleteMarkers(s3, bucket, key, count, cb) {
    return async.timesSeries(
        count,
        (i, next) => delAndAssertResult(s3, { bucket, key, resultType: newDeleteMarker }, next),
        cb
    );
}
...
function _deleteDeleteMarkers(s3, bucket, key, deleteMarkerVids, cb) {
    return async.mapSeries(
        deleteMarkerVids,
        (versionId, next) => {
            delAndAssertResult(s3, { bucket, key, versionId, resultType: deleteDeleteMarker }, next);
        },
        () => cb()
    );
}
...
function _getAssertDeleted(s3, params, cb) {
    ...
    return s3.getObject({ Bucket: bucket, Key: key, VersionId: versionId }, err => {
...
describeSkipIfNotMultiple(
    ...
    function testSuite() {
        ...
        withV4(sigCfg => {
            ...
            it(
                'versioning not configured: if specifying "null" version, should ' +
                    'delete specific version in AWS backend',
                done => {
                    const key = `somekey-${genUniqID()}`;
                    async.waterfall(
                        [
                            next => putToAwsBackend(s3, bucket, key, someBody, err => next(err)),
                            next => awsGetLatestVerId(key, someBody, next),
                            (awsVerId, next) =>
                                delAndAssertResult(
                                    s3,
                                    { bucket, key, versionId: 'null', resultType: deleteVersion },
                                    err => next(err, awsVerId)
                                ),
                            (awsVerId, next) => {
                                const wanted = isCEPH ? 'NoSuchKey' : 'NoSuchVersion';
                                _awsGetAssertDeleted({ key, versionId: awsVerId, errorCode: wanted }, next);
                            },
                        ],
                        done
                    );
            ...
            it(
                'versioning not configured: specifying any version id other ' +
                    'than null should not result in its deletion in AWS backend',
                done => {
                    const key = `somekey-${genUniqID()}`;
                    async.waterfall(
                        [
                            ...
                            (awsVerId, next) =>
                                delAndAssertResult(
                                    s3,
                                    {
                                        bucket,
                                        key,
                                        versionId: 'awsVerIdWhichIsLongerThan40BytesButNotLongEnough',
                                        resultError: 'InvalidArgument',
                                    },
                                    err => next(err, awsVerId)
                                ),
                            (awsVerId, next) =>
                                awsGetLatestVerId(key, someBody, (err, resultVid) => {
            ...
            it('versioning suspended: should delete a specific version in AWS ' + 'backend successfully', done => {
                ...
                async.waterfall(
                    [
                        next => putNullVersionsToAws(s3, bucket, key, [someBody], err => next(err)),
                        next => awsGetLatestVerId(key, someBody, next),
                        (awsVerId, next) =>
                            delAndAssertResult(s3, { bucket, key, versionId: 'null', resultType: deleteVersion }, err =>
                                next(err, awsVerId)
                            ),
                        (awsVerId, next) => {
                            const wanted = isCEPH ? 'NoSuchKey' : 'NoSuchVersion';
            ...
            it('versioning enabled: should delete a specific version in AWS ' + 'backend successfully', done => {
                ...
                async.waterfall(
                    [
                        ...
                        (s3VerId, awsVerId, next) =>
                            delAndAssertResult(
                                s3,
                                { bucket, key, versionId: s3VerId, resultType: deleteVersion },
                                err => next(err, awsVerId)
                            ),
                        (awsVerId, next) => {
                            const wanted = isCEPH ? 'NoSuchKey' : 'NoSuchVersion';
            ...
            it(
                'versioning not configured: deleting existing object should ' +
                    ...
                    async.waterfall(
                        [
                            next => putToAwsBackend(s3, bucket, key, someBody, err => next(err)),
                            next =>
                                delAndAssertResult(s3, { bucket, key, resultType: nonVersionedDelete }, err =>
                                    next(err)
                                ),
            ...
            it(
                'versioning suspended: should create a delete marker in s3 ' +
                    'and aws successfully when deleting existing object',
                done => {
                    const key = `somekey-${genUniqID()}`;
                    async.waterfall(
                        [
                            next => putNullVersionsToAws(s3, bucket, key, [someBody], err => next(err)),
                            next =>
                                delAndAssertResult(s3, { bucket, key, resultType: newDeleteMarker }, err => next(err)),
                            next => _getAssertDeleted(s3, { key, errorCode: 'NoSuchKey' }, next),
                            next => _awsGetAssertDeleted({ key, errorCode: 'NoSuchKey' }, next),
                        ],
                        done
            ...
            it(
                'versioning suspended: creating a delete marker will overwrite an ' +
                ...
                function itF(done) {
                    ...
                    async.waterfall(
                        [
                            ...
                            next => delAndAssertResult(s3, { bucket, key, resultType: newDeleteMarker }, next),
                            // delete delete marker
                            (dmVid, next) =>
                                delAndAssertResult(
                                    s3,
                                    { bucket, key, versionId: dmVid, resultType: deleteDeleteMarker },
                                    err => next(err)
                                ),
            ...
            it(
                ...
                function itF(done) {
                    ...
                    async.waterfall(
                        [
                            // put null version
                            next => putToAwsBackend(s3, bucket, key, data[0], err => next(err)),
                            ...
                            next =>
                                delAndAssertResult(
                                    s3,
                                    { bucket, key, versionId: this.test.s3vid, resultType: deleteVersion },
                                    err => next(err)
                                ),
            ...
            it(
                'versioning enabled: should create a delete marker in s3 and ' +
                    'aws successfully when deleting existing object',
                done => {
                    const key = `somekey-${genUniqID()}`;
                    async.waterfall(
                        [
            ...
            it('versioning enabled: should delete a delete marker in s3 and ' + 'aws successfully', done => {
                ...
                async.waterfall(
                    [
                        ...
                        (s3vid, next) =>
                            delAndAssertResult(s3, { bucket, key, resultType: newDeleteMarker }, (err, delMarkerVid) =>
                        ...
                        (s3vid, next) =>
                            getAndAssertResult(s3, { bucket, key, body: someBody, expectedVersionId: s3vid }, next),
            ...
            it(
                'multiple delete markers: should be able to get pre-existing ' +
                    'versions after creating and deleting several delete markers',
                done => {
                    const key = `somekey-${genUniqID()}`;
                    async.waterfall(
                        [
                            ...
                            (s3vid, next) =>
                                _createDeleteMarkers(s3, bucket, key, 3, (err, dmVids) => next(err, s3vid, dmVids)),
                            (s3vid, dmVids, next) =>
                                _deleteDeleteMarkers(s3, bucket, key, dmVids, () => next(null, s3vid)),
                            // should be able to get object originally put from s3
                            (s3vid, next) =>
                                getAndAssertResult(s3, { bucket, key, body: someBody, expectedVersionId: s3vid }, next),
                            // latest version in aws should now be object originally put
            ...
            it('should get the new latest version after deleting the latest' + 'specific version', done => {
                ...
                async.waterfall(
                    [
                        // put 3 null versions
                        next => mapToAwsPuts(s3, bucket, key, data.slice(0, 3), err => next(err)),
                        // put one version
                        next =>
                            putVersionsToAws(s3, bucket, key, [data[3]], (err, versionIds) => next(err, versionIds[0])),
                        // delete the latest version
                        (versionId, next) =>
                            delAndAssertResult(s3, { bucket, key, versionId, resultType: deleteVersion }, err =>
                                next(err)
                            ),
                        // should get the last null version
                        next => getAndAssertResult(s3, { bucket, key, body: data[2], expectedVersionId: 'null' }, next),
                        next => awsGetLatestVerId(key, data[2], err => next(err)),
                        ...
                        next => awsGetLatestVerId(key, data[1], err => next(err)),
                    ],
                    done
                );
            });
                ...
                'should delete the correct version even if other versions or ' + 'delete markers put directly on aws',
                    ...
                    async.waterfall(
                        [
                            ...
                            (s3vid, awsVid, next) =>
                                awsS3.putObject({ Bucket: awsBucket, Key: key }, err => next(err, s3vid, awsVid)),
                            // create a delete marker in AWS
                            (s3vid, awsVid, next) =>
                                awsS3.deleteObject({ Bucket: awsBucket, Key: key }, err => next(err, s3vid, awsVid)),
                ...
                'should not return an error deleting a version that was already ' + 'deleted directly from AWS backend',
                    ...
                    async.waterfall(
                        [
                            ...
                            (s3vid, awsVid, next) =>
                                awsS3.deleteObject({ Bucket: awsBucket, Key: key, VersionId: awsVid }, err =>
                                    next(err, s3vid)
                                ),
    ...
    function testSuite() {
        ...
        withV4(sigCfg => {
            ...
            beforeEach(() => {
                ...
                s3 = bucketUtil.s3;
                return s3
                    .createBucket(createBucketParams)
                    .promise()
                    .catch(err => {
                        process.stdout.write(`Error creating bucket: ${err}\n`);
                        throw err;
            ...
            it(
                'versioning not configured: deleting non-existing object should ' +
                    'not return version id or x-amz-delete-marker: true nor create a ' +
                    'delete marker in aws ',
                done => {
                    const key = `somekey-${genUniqID()}`;
                    async.waterfall(
                        [
            ...
            it(
                'versioning suspended: should create a delete marker in s3 ' +
                    'and aws successfully when deleting non-existing object',
                done => {
                    const key = `somekey-${genUniqID()}`;
                    async.waterfall(
                        [
            ...
            it(
                'versioning enabled: should create a delete marker in s3 and ' +
                    'aws successfully when deleting non-existing object',
                done => {
                    const key = `somekey-${genUniqID()}`;
                    async.waterfall(
                        [

    ...
    withV4(sigCfg => {
        ...
        describe('Basic test: ', () => {
            beforeEach(function beforeFn(done) {
                ...
                async.waterfall(
                    [
                        ...
                        next =>
                            s3.createMultipartUpload(paramsGcp, (err, res) => {
                                assert.ifError(
                                    err,
                                    'createMultipartUpload ' + `on gcp: Expected success, got error: ${err}`
                                );
            ...
            afterEach(function afterFn(done) {
                ...
                async.waterfall(
                    [
                        next => s3.abortMultipartUpload(paramsGcp, err => next(err)),
                    ...
                    [
                        next =>
                            s3.uploadPartCopy(params, (err, res) => {
                                assert.ifError(err, 'uploadPartCopy: Expected ' + `success, got error: ${err}`);
...
describeSkipIfNotMultipleOrCeph('Put Copy Part to GCP with complete MPU', function describeF() {
    this.timeout(800000);
    withV4(sigCfg => {
        beforeEach(() => {
            bucketUtil = new BucketUtility('default', sigCfg);

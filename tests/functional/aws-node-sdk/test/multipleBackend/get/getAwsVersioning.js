...
function getAndAssertVersions(s3, bucket, key, versionIds, expectedData, cb) {
    async.mapSeries(
        versionIds,
        (versionId, next) => {
            s3.send(new GetObjectCommand({ Bucket: bucket, Key: key, VersionId: versionId }))
                .then(async result => {
...
describeSkipIfNotMultiple('AWS backend get object with versioning', function testSuite() {
    this.timeout(30000);
    withV4(sigCfg => {
        let bucketUtil;
        let s3;
        ...
        it(
            'should not return version ids when versioning has not been ' +
                'configured via CloudServer, even when version id specified',
            done => {
                const key = `somekey-${genUniqID()}`;
                s3.send(
                    new PutObjectCommand({
                    ...
                    .then(data => {
                        ...
                        getAndAssertResult(
                            s3,
                            { bucket, key, body: someBody, versionId: 'null', expectedVersionId: false },
                            done,
                        );
                    })
                    .catch(err => {
        ...
        it('should overwrite the null version if putting object twice before versioning is configured', done => {
            ...
            async.waterfall(
                [
                    ...
                    // get latest version
                    next => getAndAssertResult(s3, { bucket, key, body: data[1], expectedVersionId: false }, next),
                    // get specific version
                    next =>
                        getAndAssertResult(
                            s3,
                            { bucket, key, body: data[1], versionId: 'null', expectedVersionId: false },
                            next,
                        ),
                ],
                done,
        ...
        it('should overwrite existing null version if putting object after suspending versioning', done => {
            ...
            async.waterfall(
                [
                    ...
                    // get latest version
                    next => getAndAssertResult(s3, { bucket, key, body: data[1], expectedVersionId: 'null' }, next),
                    // get specific version
                    next =>
                        getAndAssertResult(
                            s3,
                            { bucket, key, body: data[1], versionId: 'null', expectedVersionId: 'null' },
                            next,
                        ),
                ],
                done,
            ...
            'should overwrite null version if putting object when ' +
                ...
                async.waterfall(
                    [
                        ...
                        // get latest version
                        next => getAndAssertResult(s3, { bucket, key, body: data[3], expectedVersionId: 'null' }, next),
                        // get specific version (null)
                        next =>
                            getAndAssertResult(
                                s3,
                                { bucket, key, body: data[3], versionId: 'null', expectedVersionId: 'null' },
                                next,
                            ),
        ...
        it('should get correct data from aws backend using version IDs', done => {
            const key = `somekey-${genUniqID()}`;
            ...
            async.waterfall(
                [
                    ...
                    next => getAndAssertVersions(s3, bucket, key, versionIds, data, next),
                ],
                done,
            );
        });

        it('should get correct version when getting without version ID', done => {
            const key = `somekey-${genUniqID()}`;
            const data = [...Array(5).keys()].map(i => i.toString());
            const versionIds = ['null'];
            async.waterfall(
                [
                    ...
                    next =>
                        getAndAssertResult(s3, { bucket, key, body: data[4], expectedVersionId: versionIds[4] }, next),
                ],
                done,
            );
        });
        ...
        it(
            'should get correct data from aws backend using version IDs after putting null ' +
                'versions, putting versions, putting more null versions and then putting more versions',
            done => {
                const key = `somekey-${genUniqID()}`;
                ...
                const finalDataArr = firstFiveVersions.concat([lastNullVersion]).concat(secondFiveVersions);
                async.waterfall(
                    [
                        next => mapToAwsPuts(s3, bucket, key, firstThreeNullVersions, err => next(err)),
                        next => putVersionsToAws(s3, bucket, key, firstFiveVersions, next),
                        (ids, next) => {
                            versionIds.push(...ids);
                            next();
                        },
                        next => putNullVersionsToAws(s3, bucket, key, secondThreeNullVersions, err => next(err)),
                        next => putVersionsToAws(s3, bucket, key, secondFiveVersions, next),
                        (ids, next) => {
                        ...
                        // get versions by id
                        next => getAndAssertVersions(s3, bucket, key, versionIds, finalDataArr, next),
        ...
        it(
            'should return the correct data getting versioned object ' +
                    ...
                    [
                        ...
                        next =>
                            s3
                                ...
                                .then(res => next(null, res.VersionId))
                                .catch(err => {
                                    next(err);
                                }),
                        // create a delete marker in AWS
                        (versionId, next) =>
                            awsS3.deleteObject({ Bucket: awsBucket, Key: key }, err => next(err, versionId)),
                        (versionId, next) =>
                            getAndAssertResult(s3, { bucket, key, body: someBody, expectedVersionId: versionId }, next),
                    ],
                    done,
                );
            },
            ...
            'should return the correct data getting versioned object ' +
                ...
                async.waterfall(
                    [
                        ...
                        (versionId, next) =>
                            awsS3
                                .send(new PutObjectCommand({ Bucket: awsBucket, Key: key }))
        ...
        it(
            'should return a LocationNotFound if trying to get an object ' +
                ...
                async.waterfall(
                    [
                        ...
                        (s3vid, next) =>
                            awsS3
                                .send(new GetObjectCommand({ Bucket: awsBucket, Key: key }))
                                .then(res => next(null, s3vid, res.VersionId))
                                .catch(err => {
                                    next(err);
                                }),
        ...
        it(
            'should return a LocationNotFound if trying to get a version ' +
                'that was deleted in AWS but exists in s3 metadata',
            done => {
                const key = `somekey-${genUniqID()}`;
                async.waterfall(
                    [
                        ...
                        (s3VerId, next) =>
                            s3
                                .send(new GetObjectCommand({ Bucket: bucket, Key: key, VersionId: s3VerId }))
                                .then(() => {
                                    next();
                                })
                                .catch(err => {

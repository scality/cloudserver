...
function getAndAssertVersions(s3, bucket, key, versionIds, expectedData, cb) {
    async.mapSeries(
        versionIds,
        (versionId, next) => {
            s3.getObject({ Bucket: bucket, Key: key, VersionId: versionId }, next);
        },
        (err, results) => {
            assert.strictEqual(err, null, 'Expected success ' + `getting object, got error ${err}`);
            const resultIds = results.map(result => result.VersionId);
            const resultData = results.map(result => result.Body.toString());
...
describeSkipIfNotMultiple('AWS backend get object with versioning', function testSuite() {
    ...
    withV4(sigCfg => {
        ...
        it('should not return version ids when versioning has not been ' + 'configured via CloudServer', done => {
            ...
            s3.putObject(
                { Bucket: bucket, Key: key, Body: someBody, Metadata: { 'scal-location-constraint': awsLocation } },
                (err, data) => {
        ...
        it(
            'should get correct data from aws backend using version IDs ' +
            ...
            'should return the correct data getting versioned object ' +
                ...
                async.waterfall(
                    [
                        ...
                        (versionId, next) =>
                            awsS3.putObject({ Bucket: awsBucket, Key: key }, err => next(err, versionId)),
                        (versionId, next) =>
                            getAndAssertResult(s3, { bucket, key, body: someBody, expectedVersionId: versionId }, next),
                    ],
                    done
            ...
            'should return a LocationNotFound if trying to get a version ' +
                ...
                async.waterfall(
                    [
                        ...
                        (s3VerId, next) =>
                            s3.getObject({ Bucket: bucket, Key: key, VersionId: s3VerId }, err => {
                                assert.strictEqual(err.code, 'LocationNotFound');
                                assert.strictEqual(err.statusCode, 424);
                                next();
                            }),

...
describeSkipIfNotMultiple('Multiple backend get object from Azure', function testSuite() {
    this.timeout(30000);
    withV4(sigCfg => {
        let bucketUtil;
        let s3;
        ...
        keys.forEach(key => {
            describe(`${key.describe} size`, () => {
                ...
                it(`should get an ${key.describe} object from Azure`, done => {
                    s3.send(new GetObjectCommand({ Bucket: azureContainerName, Key: testKey }))
                        .then(res => {
            ...
            it('should get an object with body 012345 with "bytes=0-5"', done => {
                s3.send(
                    ...
                    .then(async res => {
                        ...
                        assert.strictEqual(body, '012345');
                        done();
                    })
                    .catch(err => {
                        assert.equal(err, null, 'Expected success but got ' + `error ${err}`);

    ...
    withV4(sigCfg => {
        ...
        afterEach(() => {
            ...
            return bucketUtil
                ...
                .then(() => {
                    process.stdout.write('Deleting bucket ' + `${awsServerSideEncryptionbucket}\n`);
        ...
        it('should copy an object from mem to AWS relying on ' + 'destination bucket location', done => {
        ...
        it('should copy an object from Azure to AWS relying on ' + 'destination bucket location', done => {
        ...
        it('should copy an object from AWS to mem relying on destination ' + 'bucket location', done => {
        ...
        itSkipCeph('should copy an object from mem to AWS with aws server ' + 'side encryption', done => {
            putSourceObj(memLocation, false, bucket, key => {
                const copyKey = `copyKey-${genUniqID()}`;
                const copyParams = {
                    Bucket: bucket,
        ...
        itSkipCeph('should copy an object on AWS with aws server side ' + 'encryption', done => {
            putSourceObj(awsLocation, false, bucket, key => {
                const copyKey = `copyKey-${genUniqID()}`;
                const copyParams = {
                    Bucket: bucket,
        ...
        itSkipCeph('should copy an object on AWS with aws server side ' + 'encrypted bucket', done => {
            putSourceObj(awsLocation, false, awsServerSideEncryptionbucket, key => {
                const copyKey = `copyKey-${genUniqID()}`;
                const copyParams = {
                    Bucket: awsServerSideEncryptionbucket,
                    Key: copyKey,
        ...
        it('should copy an object from AWS to mem with "COPY" ' + 'directive and aws location metadata', done => {
            putSourceObj(awsLocation, false, bucket, key => {
                const copyKey = `copyKey-${genUniqID()}`;
                const copyParams = {
                    Bucket: bucket,
        ...
        it('should copy an object on AWS to a different AWS location ' + 'with source object READ access', done => {
            ...
            async.waterfall(
                [
                    ...
                    next => putSourceObj(awsLocation, false, bucket, key => next(null, key)),
                    (key, next) =>
                        awsS3.putObjectAcl({ Bucket: awsBucket, Key: key, ACL: 'public-read' }, err => next(err, key)),
                    (key, next) => {
                        const copyParams = {
                            Bucket: bucket,
                            Key: copyKey,
        ...
        it('should return error if AWS source object has ' + 'been deleted', done => {
            putSourceObj(awsLocation, false, bucket, key => {
                const awsBucket = config.locationConstraints[awsLocation].details.bucketName;
                awsS3.deleteObject({ Bucket: awsBucket, Key: key }, err => {
                    assert.equal(err, null, 'Error deleting object from AWS: ' + `${err}`);
                    const copyKey = `copyKey-${genUniqID()}`;
                    const copyParams = {
                        Bucket: bucket,
                        Key: copyKey,

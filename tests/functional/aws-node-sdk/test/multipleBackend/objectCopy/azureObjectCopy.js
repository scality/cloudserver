...
function assertGetObjects(
    ...
    async.series(
        ...
        (err, results) => {
            ...
            if (mdDirective === 'COPY') {
                assert.strictEqual(sourceRes.Metadata['test-header'], destRes.Metadata['test-header']);
                assert.strictEqual(azureRes[0].metadata.test_header, destRes.Metadata['test-header']);
            }
...
describeSkipIfNotMultipleOrCeph('MultipleBackend object copy: Azure', function testSuite() {
    this.timeout(250000);
    withV4(sigCfg => {

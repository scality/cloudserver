    ...
    withV4(sigCfg => {
        ...
        describeSkipIfNotMultiple(
            ...
            function describe() {
                ...
                it('should put objects with same key to AWS then file, and object should only be present in file', async () => {
                    const key = `somekey-${genUniqID()}`;
                    const params = {
                        Bucket: bucket,
                        Key: key,
                ...
                it('should put objects with same key to file then AWS, and object should only be present on AWS', async () => {
                    const key = `somekey-${genUniqID()}`;
                    const params = {
                        Bucket: bucket,
                        Key: key,
...
describeSkipIfNotMultiple('MultipleBackend put object based on bucket location', () => {
    withV4(sigCfg => {
        ...
        it('should put an object to mem with no location header', async () => {
            process.stdout.write('Creating bucket\n');
            await s3.send(

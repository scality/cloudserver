...
function getAwsSuccess(key, awsMD5, location, cb) {
    return getAwsRetry({ key }, 0, (err, res) => {
        assert.strictEqual(err, null, 'Expected success, got error ' + `on direct AWS call: ${err}`);
...
function awsGetCheck(objectKey, s3MD5, awsMD5, location, cb) {
    ...
    s3.getObject({ Bucket: bucket, Key: objectKey }, function s3GetCallback(err, res) {
        ...
        assert.strictEqual(err, null, 'Expected success, got error ' + `on call to AWS through S3: ${err}`);
...
describe('MultipleBackend put object', function testSuite() {
    ...
    withV4(sigCfg => {
        beforeEach(() => {
            ...
            return s3.createBucketPromise({ Bucket: bucket }).catch(err => {
        ...
        it.skip('should return an error to put request without a valid ' + 'bucket name', done => {
        ...
        describeSkipIfNotMultiple(
            ...
            function describe() {
                ...
                it('should return an error to put request without a valid ' + 'location constraint', done => {
                    const key = `somekey-${genUniqID()}`;
                    const params = {
                        Bucket: bucket,
                        Key: key,
                ...
                it('should put an object to mem', done => {
                    ...
                    s3.putObject(params, err => {
                        ...
                        s3.getObject({ Bucket: bucket, Key: key }, (err, res) => {
                            assert.strictEqual(err, null, 'Expected success, ' + `got error ${err}`);
                ...
                it('should put a 0-byte object to mem', done => {
                    const key = `somekey-${genUniqID()}`;
                    const params = { Bucket: bucket, Key: key, Metadata: { 'scal-location-constraint': memLocation } };
                    s3.putObject(params, err => {
                        assert.equal(err, null, 'Expected success, ' + `got error ${err}`);
                        s3.getObject({ Bucket: bucket, Key: key }, (err, res) => {
                            assert.strictEqual(err, null, 'Expected success, ' + `got error ${err}`);
                ...
                it('should put actual object with body and mdonly header', done => {
                    ...
                    s3.putObject(params, err => {
                        ...
                        s3.getObject({ Bucket: bucket, Key: key }, (err, res) => {
                            assert.strictEqual(err, null, 'Expected success, ' + `got error ${err}`);
                            assert.strictEqual(res.ETag, `"${correctMD5}"`);
                            awsGetCheck(key, correctMD5, correctMD5, awsLocation, () => done());
                        });
                    });
                });
                ...
                it('should put 0-byte normally with mdonly header', done => {
                    ...
                    s3.putObject(params, err => {
                        assert.equal(err, null, 'Expected success, ' + `got error ${err}`);
                        awsGetCheck(key, emptyMD5, emptyMD5, awsLocation, () => done());
                    });
                });

                it('should put a 0-byte object to AWS', done => {
                    const key = `somekey-${genUniqID()}`;
                    const params = { Bucket: bucket, Key: key, Metadata: { 'scal-location-constraint': awsLocation } };
                    return s3.putObject(params, err => {
                        assert.equal(err, null, 'Expected success, ' + `got error ${err}`);
                        return awsGetCheck(key, emptyMD5, emptyMD5, awsLocation, () => done());
                    });
                });
                ...
                it('should put an object to AWS', done => {
                    ...
                    return s3.putObject(params, err => {
                        assert.equal(err, null, 'Expected success, ' + `got error ${err}`);
                        return awsGetCheck(key, correctMD5, correctMD5, awsLocation, () => done());
                    });
                });

                it('should encrypt body only if bucket encrypted putting ' + 'object to AWS', done => {
                    const key = `somekey-${genUniqID()}`;
                    const params = {
                        Bucket: bucket,
                        Key: key,
                    ...
                    return s3.putObject(params, err => {
                        assert.equal(err, null, 'Expected success, ' + `got error ${err}`);
                        return getAwsSuccess(key, correctMD5, awsLocation, () => done());
                    });
                });
                ...
                it('should put an object to AWS with encryption', done => {
                    ...
                    const params = {
                        ...
                        Key: key,
                        Body: body,
                        Metadata: { 'scal-location-constraint': awsLocationEncryption },
                    };
                    return s3.putObject(params, err => {
                        assert.equal(err, null, 'Expected success, ' + `got error ${err}`);
                        return awsGetCheck(key, correctMD5, correctMD5, awsLocationEncryption, () => done());
                    });
                });

                it('should return a version id putting object to ' + 'to AWS with versioning enabled', done => {
                    const key = `somekey-${genUniqID()}`;
                    const params = {
                        Bucket: bucket,
                        Key: key,
                    ...
                    async.waterfall(
                        [
                            ...
                            (eTag, next) => getAwsSuccess(key, correctMD5, awsLocation, () => next()),
                        ],
                        done
                    );
                });
                ...
                it('should put a large object to AWS', done => {
                    ...
                    return s3.putObject(params, err => {
                        assert.equal(err, null, 'Expected sucess, ' + `got error ${err}`);
                        return awsGetCheck(key, bigS3MD5, bigAWSMD5, awsLocation, () => done());
                    });
                });

                it(
                    'should put objects with same key to AWS ' + 'then file, and object should only be present in file',
                    done => {
                        const key = `somekey-${genUniqID()}`;
                        const params = {
                            Bucket: bucket,
                ...
                it(
                    'should put objects with same key to file ' + 'then AWS, and object should only be present on AWS',
                    done => {
                        const key = `somekey-${genUniqID()}`;
                        const params = {
                            Bucket: bucket,
                ...
                it('should put two objects to AWS with same ' + 'key, and newest object should be returned', done => {
                    const key = `somekey-${genUniqID()}`;
                    const params = {
                        Bucket: bucket,
                        Key: key,
                    ...
                    return s3.putObject(params, err => {
                        ...
                        return s3.putObject(params, err => {
                            assert.equal(err, null, 'Expected success, ' + `got error ${err}`);
                            return awsGetCheck(key, correctMD5, correctMD5, awsLocation, result => {
        ...
        it('should put an object to AWS with no location header', done => {
            ...
            return s3.createBucket(
                ...
                err => {
                    ...
                    return s3.putObject(params, err => {
                        assert.equal(err, null, `Expected success, got error ${err}`);
                        return awsGetCheck(key, correctMD5, correctMD5, undefined, () => done());
                    });
                }
            );
        });

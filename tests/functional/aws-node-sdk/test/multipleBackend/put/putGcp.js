...
const { gcpClient, gcpBucket, gcpLocation, fileLocation, genUniqID, describeSkipIfNotMultiple } = require('../utils');
    ...
    function attempt() {
        s3.send(new GetObjectCommand(params))
            .then(res => {
                ...
                const metadataLocation = res.Metadata && res.Metadata['scal-location-constraint'];
...
describeSkipIfNotMultiple('MultipleBackend put object to GCP', function describeFn() {
    this.timeout(250000);
    withV4(sigCfg => {
        beforeEach(() => {
            ...
            putTests.forEach(test => {
                ...
                it(test.msg, done => {
                    ...
                    return s3
                        ...
                        .then(() => {
                            gcpGetCheck(key, s3MD5, gcpMD5, location, () => done());
                        })
                        .catch(done);
                });
            });
        ...
        describe('with object rewrites', function describe() {
            ...
            it('should put two objects to GCP with same ' + 'key, and newest object should be returned', done => {
                ...
                return s3
                    ...
                    .then(() => {
                        gcpGetCheck(key, correctMD5, correctMD5, gcpLocation, result => {
    ...
    withV4(sigCfg => {
        ...
        it('should put an object to GCP with no location header', done => {
            ...
            return s3
                ...
                .then(() => {
                    gcpGetCheck(key, correctMD5, correctMD5, undefined, () => done());
                })
                .catch(done);
        });
    });

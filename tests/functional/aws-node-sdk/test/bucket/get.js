...
describe('GET Bucket - AWS.S3.listObjects', () => {
    describe('When user is unauthorized', () => {
        ...
        it('should return 403 and AccessDenied on a private bucket', done => {
            ...
            bucketUtil.s3.makeUnauthenticatedRequest('listObjects', params, error => {
                assert(error);
    ...
    withV4(sigCfg => {
        ...
        tests.forEach(test => {
            it(`v2 should ${test.name}`, done => {
                ...
                Promise.mapSeries(test.objectPutParams(Bucket), param => s3.putObject(param).promise())
                    .then(() => s3.listObjectsV2(test.listObjectParams(Bucket)).promise())
                    .then(data => {
        ...
        ['&amp', '"quot', "'apos", '<lt', '>gt'].forEach(k => {
            it(`should list objects with key ${k} as Prefix`, done => {
                ...
                Promise.mapSeries(objects, param => s3.putObject(param).promise())
        ...
        ['&amp', '"quot', "'apos", '<lt', '>gt'].forEach(k => {
            it(`should list objects with key ${k} as NextMarker`, done => {
                ...
                Promise.mapSeries(objects, param => s3.putObject(param).promise())
                    .then(() => s3.listObjects({ Bucket, MaxKeys: 1, Delimiter: 'foo' }).promise())
                    .then(data => {
                        const isValidResponse = tv4.validate(data, bucketSchema);
                        if (!isValidResponse) {
                            throw new Error(tv4.error);
        ...
        ['&amp', '"quot', "'apos", '<lt', '>gt'].forEach(k => {
            it(`should list objects with key ${k} as StartAfter`, done => {
                ...
                Promise.mapSeries(objects, param => s3.putObject(param).promise())
                    .then(() => s3.listObjectsV2({ Bucket, StartAfter: k }).promise())
                    .then(data => {
                        const isValidResponse = tv4.validate(data, bucketSchemaV2);
                        if (!isValidResponse) {
                            throw new Error(tv4.error);
        ...
        ['&amp', '"quot', "'apos", '<lt', '>gt'].forEach(k => {
            it(`should list objects with key ${k} as ContinuationToken`, done => {
                const s3 = bucketUtil.s3;
                const Bucket = bucketName;
                const objects = [{ Bucket, Key: k }];
        ...
        ['&amp', '"quot', "'apos", '<lt', '>gt'].forEach(k => {
            it(`should list objects with key ${k} as NextContinuationToken`, done => {
                const s3 = bucketUtil.s3;
                const Bucket = bucketName;
                const objects = [
                    { Bucket, Key: k },
                ...
                Promise.mapSeries(objects, param => s3.putObject(param).promise())
                    .then(() => s3.listObjectsV2({ Bucket, MaxKeys: 1, Delimiter: 'foo' }).promise())
                    .then(data => {
                        const isValidResponse = tv4.validate(data, bucketSchemaV2);
                        if (!isValidResponse) {
                            throw new Error(tv4.error);

...
describe('aws-sdk test get bucket policy', () => {
    ...
    it('should return NoSuchBucket error if bucket does not exist', done => {
        s3.getBucketPolicy({ Bucket: bucket }, err => assertError(err, 'NoSuchBucket', done));
    });
    ...
    describe('policy rules', () => {
        ...
        it('should return MethodNotAllowed if user is not bucket owner', done => {
            otherAccountS3.getBucketPolicy({ Bucket: bucket }, err => assertError(err, 'MethodNotAllowed', done));
        });

        it('should return NoSuchBucketPolicy error if no policy put to bucket', done => {
        ...
        it('should get bucket policy', done => {
            s3.putBucketPolicy(
                ...
                err => {
                    ...
                    s3.getBucketPolicy({ Bucket: bucket }, (err, res) => {
                        const parsedRes = JSON.parse(res.Policy);
                        assert.equal(err, null, 'Error getting bucket policy: ' + `${err}`);

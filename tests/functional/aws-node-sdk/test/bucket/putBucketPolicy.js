...
describe('aws-sdk test put bucket policy', () => {
    ...
    it('should return NoSuchBucket error if bucket does not exist', done => {
        ...
        s3.putBucketPolicy(params, err => assertError(err, 'NoSuchBucket', done));
    });
    ...
    describe('config rules', () => {
        ...
        it('should return MethodNotAllowed if user is not bucket owner', done => {
            const params = getPolicyParams();
            otherAccountS3.putBucketPolicy(params, err => assertError(err, 'MethodNotAllowed', done));
        });
        ...
        it('should not allow bucket policy with no Principal', done => {
            const params = getPolicyParams({ key: 'Principal', value: '' });
            s3.putBucketPolicy(params, err => assertError(err, 'MalformedPolicy', done));
        });

        it('should return MalformedPolicy because Id is not a string', done => {
            const params = getPolicyParamsWithId(null, 59);
            s3.putBucketPolicy(params, err => assertError(err, 'MalformedPolicy', done));
        });

        it('should put a bucket policy on bucket since Id is a string', done => {
